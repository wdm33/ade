...
> HEAD: `928c2be` (test(testkit): proposal_procedures canonical corpus + replay harness (PROPOSAL-PROCEDURES-DECODE PP-S2), 2026-05-25)
> 145 commits, 11,321 files changed, +179,314 / −7,233,597 lines
...
> **PROPOSAL-PROCEDURES-DECODE cluster close note (newest thread).**
> This regen is cut at working HEAD `928c2be`. Since the prior
> grounding-doc refresh commit `3af9e2b` (which committed the PHASE4-N-E
> cluster close + archive + post-N-E grounding ripple at HEAD `caa5ce8`),
> **two new commits have landed** — the **PROPOSAL-PROCEDURES-DECODE
> cluster** (PP-S1 + PP-S2) closing the last open governance-domain
> decode seam carried forward from ENACTMENT-COMMITTEE-WRITEBACK and
> from PHASE4-N-E: `70bc85b` (PP-S1, new BLUE module
> `ade_codec::conway::governance` ~580 LOC + new closed type
> `ade_types::conway::governance::ProposalProcedure` + body-codec rewire
> at key 20 + new CI gate `ci_check_proposal_procedures_closed.sh` +
> new rule `DC-LEDGER-11` introduced + `enforced`; 14 unit tests in the
> decoder module + 3 integration tests in
> `crates/ade_codec/tests/conway_tx_body_proposal_procedures.rs`) and
> `928c2be` (PP-S2, new GREEN module
> `ade_testkit::governance::proposal_procedures_replay` with canonical
> synthetic corpus + replay harness; 4 integration tests in
> `crates/ade_testkit/tests/proposal_procedures_corpus_replay.rs`).
> **One new BLUE module, one new GREEN module, one new closed type,
> one new registry rule (`DC-LEDGER-11`), one new CI script (the 32nd),
> 21 new tests. `DC-LEDGER-10.cross_ref += "DC-LEDGER-11"` (bidirectional).
> Type change at the boundary:** `ConwayTxBody.proposal_procedures`
> went from `Option<Vec<u8>>` (opaque pass-through at key 20) to
> `Option<Vec<ProposalProcedure>>` (typed through the closed decoder
> at the canonical entry slice). **Cluster status at HEAD: closed.**
> **Scope locks held by parent declaration**: `voting_procedures`
> stays opaque (OQ-1); `ParameterChange.update` stays opaque (OQ-2);
> `NewConstitution.raw` stays opaque (OQ-3); `proposal_procedure.return_addr`
> stays `Vec<u8>` (OQ-4); `Anchor` stays opaque struct. Cluster
> directory **archived** to `docs/clusters/completed/PROPOSAL-PROCEDURES-DECODE/`
> (3 files: `cluster.md` + `PP-S1.md` + `PP-S2.md`) via `git mv`
> staged in the in-flight grounding-refresh commit that this
> HEAD_DELTAS is part of.
...
> 6 slice docs + 2 procedure docs + 1 evidence log) by `3af9e2b`.
> Tier-1 authority closed; CE-N-E-6 live N2N evidence captured at
> `CE-N-E-6_2026-05-25.log` (real preprod relay, N2N handshake v15,
> tx-submission2 opened, 1 frame received, BLUE `tx_submission2_transition`
> drove correctly, 97s active session, `[bridge] tx_bytes=0` per
> honest-scope framing). CE-N-E-7 **deferred** to the future
> node-binary cluster as cross-cluster obligation `CE-NODE-N2C-LTX`
> (NOT claimed closed). The five implementation slices `32c1ee6` (S1),
> `2d0c918` (S2), `509d714` (S3), `ca3f23a` (S4), `43fcc31` (S5) +
> `d1068b3` (S6) + `caa5ce8` (S6 hardening) shipped the BLUE
> `mempool::ingress` chokepoint, the GREEN `mempool::canonicalize`
> ordering function, the GREEN `testkit::mempool::ingress_replay`
> harness, the two GREEN bridges (`ade_core_interop::tx_submission`,
> `ade_core_interop::local_tx_submission`), and the RED
> `live_tx_submission_session` operator-action probe binary.
> Two registry rules (`DC-MEM-03`, `DC-MEM-04`); two CI scripts
...
> (`ade_core_interop -> ade_ledger`). 39 new tests across S1–S5; the
> S6 `#[ignore]`-gated closure-gate test adds 1 more.
...
> **B5 / B4 / B3F / B3 / B2 / B1 / N-D / N-B / N-A / PHASE4-N-E cluster
> notes (carried forward).** All closed and archived at
> `docs/clusters/completed/<NAME>/`.

The delta now covers twenty-five threads of work. The newest thread —
the **PROPOSAL-PROCEDURES-DECODE cluster** (`70bc85b`, `928c2be`) —
sits on the post-N-E grounding refresh `3af9e2b`, which closed +
archived PHASE4-N-E. In rough proportion of the substantive change
budget:

0. **PROPOSAL-PROCEDURES-DECODE (last open governance-domain decode
   seam) — closed in 2 slices.** PP-S1 (`70bc85b`, **BLUE**) introduces
   the new closed decoder module
   `crates/ade_codec/src/conway/governance.rs` (~580 LOC: closed
   `decode_proposal_procedures` + `encode_proposal_procedures` + helpers
   for all 7 `GovAction` variants — `decode_proposal_procedure`,
   `decode_gov_action`, `decode_anchor`, `decode_gov_action_id_opt`,
   `decode_stake_credential`, `decode_unit_interval`,
   `decode_cold_credential_set`, `decode_cold_credential_epoch_map`,
   `decode_hash28_opt` + mirror encoders + `read_hash{28,32}` / `peek_null`
   helpers; registered in `crates/ade_codec/src/conway/mod.rs`),
   introduces the new closed canonical type
   `ade_types::conway::governance::ProposalProcedure` (4 fields:
   `deposit: Coin`, `return_addr: Vec<u8>`, `gov_action: GovAction`,
   `anchor: Anchor`), rewires `ConwayTxBody.proposal_procedures` from
   `Option<Vec<u8>>` to `Option<Vec<ProposalProcedure>>` at the
   canonical entry slice in `crates/ade_codec/src/conway/tx.rs` (key
   20 path: decoder calls `decode_proposal_procedures` over the
   captured key-20 CBOR item bytes; encoder calls
   `encode_proposal_procedures` over the typed field — no opaque
   pass-through). New rule `DC-LEDGER-11` introduced + `enforced` +
   bidirectional `cross_ref` from/to `DC-LEDGER-10`. New CI script
   `ci/ci_check_proposal_procedures_closed.sh` (the 32nd) with five
   mechanical guards: (1) `ade_types::conway::governance` defines
   `pub struct ProposalProcedure` with the 4 required fields;
   (2) `ConwayTxBody.proposal_procedures` is `Option<Vec<ProposalProcedure>>`
   and **not** `Option<Vec<u8>>`; (3) `ade_codec::conway::governance`
   exports both `decode_proposal_procedures` and
   `encode_proposal_procedures`; (4) the Conway tx-body codec at key
   20 calls the typed decoder/encoder and does not reintroduce the
   `data[start..end].to_vec()` opaque pass-through; (5) no
   `ProposalProcedure { ... }` struct-literal construction outside
   sanctioned sites (the decoder file itself, `ade_testkit/`,
   `crates/*/tests/`, `#[cfg(test)]` blocks). 14 inline unit tests in
   `governance.rs` (`roundtrip_info_action_proposal`,
   `roundtrip_hard_fork_initiation`, `roundtrip_no_confidence`,
   `roundtrip_treasury_withdrawals`, `roundtrip_parameter_change`,
   `roundtrip_new_constitution`, `roundtrip_update_committee`,
   `roundtrip_multi_procedure`, `rejects_unknown_gov_action_tag`,
   `rejects_empty_proposal_procedures_set`, `rejects_trailing_garbage`,
   `rejects_truncated_proposal_procedure`,
   `rejects_invalid_stake_credential_in_update_committee`,
   `update_committee_keeps_stake_credential_discriminant`) + 3
   ...
   `body_and_sub_decoder_agree_on_proposal_procedures`). PP-S2
   (`928c2be`, **GREEN**) adds the canonical synthetic replay harness:
   new submodule `crates/ade_testkit/src/governance/mod.rs` +
   `crates/ade_testkit/src/governance/proposal_procedures_replay.rs`
   (~232 LOC) exporting `CorpusEntry { label, bytes, expected }`,
   `ReplayOutcome::{ Ok, DecodeMismatch, EncodeMismatch, DecodeError(String) }`,
   `canonical_corpus() -> Vec<CorpusEntry>` (9 entries, one per
   `GovAction` variant + multi-procedure + DC-LEDGER-10 cross-check),
   `replay_canonical_corpus() -> Vec<(String, ReplayOutcome)>`;
   registered in `crates/ade_testkit/src/lib.rs`. 4 integration tests
   in `crates/ade_testkit/tests/proposal_procedures_corpus_replay.rs`
   ...
   the DC-LEDGER-10 cross-check confirming `KeyHash(h)` and
   `ScriptHash(h)` of equal 28 bytes remain distinct through round
   trip). **OQ-8 contracted scope adjustment (NOT a regression):**
   two test sites in `crates/ade_ledger/src/conway.rs` (lines 344
   and 519) previously set `proposal_procedures = Some(vec![0x80])`
   as a presence marker (not real proposal data); reset to `None`
   because the prior placeholder no longer decodes through the typed
   path. The affected tests still exercise the "governance fields
   present" path via the other fields (`voting_procedures` /
   `treasury_value` / `donation`). **No new dependency edge.** **No
   change to `ade_ledger`'s authoritative paths.** **Scope locks
   held per parent declaration**: `voting_procedures` (tx body key
   19) stays opaque (OQ-1) — natural sibling cluster, deliberately
   deferred; `ParameterChange.update` stays opaque (OQ-2);
   `NewConstitution.raw` stays opaque (OQ-3); `proposal_procedure.return_addr`
   stays `Vec<u8>` (OQ-4) — typed `RewardAccount` is a separable
   fidelity decision; `Anchor` stays opaque struct (existing
   `{ raw: Vec<u8> }`). Cluster directory **archived** to
   `docs/clusters/completed/PROPOSAL-PROCEDURES-DECODE/` via `git mv`
   staged in this grounding-refresh commit.
1. **PHASE4-N-E S6 (live N2N tx-submission2 evidence binary) — cluster
   ...
   (~80 LOC, `#[ignore]`-gated). **S6 hardening** (`caa5ce8`, RED)
   folded a retry-on-`io::ErrorKind::TimedOut` fix into the read loop
   and corrected the elapsed-time logging; same commit committed the
   live-evidence log
   `docs/clusters/completed/PHASE4-N-E/CE-N-E-6_2026-05-25.log`.
2. **PHASE4-N-E S1–S5 (wire-level mempool ingress, Tier 1) — code +
   ...
   N2C}` is metadata only. `crates/ade_ledger/src/mempool/ingress.rs`
   (S1, BLUE), `crates/ade_ledger/src/mempool/canonicalize.rs` (S3,
   GREEN), `crates/ade_testkit/src/mempool/ingress_replay.rs` (S2,
   GREEN), `crates/ade_core_interop/src/tx_submission.rs` (S4, GREEN
   N2N bridge), `crates/ade_core_interop/src/local_tx_submission.rs`
...
3. **Post-WRITEBACK testkit follow-ups (four commits, GREEN-scope) —
...
4. **ENACTMENT-COMMITTEE-WRITEBACK — closed.** Wires committee
...
12. **Phase 4 cluster B3 (Conway value-conservation accounting) —
    ...
    `DC-TXV-07`.
13. **Phase 4 cluster B2 (tx validity agreement) — closed.** New
...
14. **Phase 4 cluster B1 (full block validity agreement) — closed.**
...
15. **Phase 4 cluster N-A (network mini-protocols) — closed.** 10
    slices. New BLUE crate `ade_network`.
16. **Phase 4 cluster N-B (consensus runtime) — closed.** 10
    slices. New BLUE `ade_core::consensus` module.
17. **CE-N-B-6 follow-mode bridge.** RED `ade_core_interop::follow`
    + live preprod tip-agreement evidence.
18. **Phase 4 cluster N-D (ChainDB persistence) — closed.**
    Slices S-33 → S-37.
19. **Phase 2C close-out / CE-73 reclassification.** CE-73 split
    Tier-2 / Tier-4.
20. **IDD canonicalization.** `chore(idd)` commits.
21. **Grounding-doc generation + ripple.** Successive refreshes,
    including `52642e5` (which archived 7 closed cluster dirs),
    `350130e` (post-N-E-S5 grounding ripple), and `3af9e2b`
    (PHASE4-N-E close + archive + post-N-E grounding ripple).
22. **BLUE-list drift closure.** Six CI scripts extended to full
    BLUE scope.
23. **Corpus relayout.** Credentialed `*_registered_creds.txt`
...
|--------|-------|---------|---------------|--------------------------|
| `ade_codec::conway::governance` (new file in an existing BLUE crate) | BLUE | **Closed-grammar Conway proposal-procedures decoder + encoder.** `decode_proposal_procedures(&[u8]) -> Result<Vec<ProposalProcedure>, CodecError>` is the single sanctioned entry point invoked from the Conway tx-body codec at key 20 (no opaque pass-through). Mirrors the established `decode_conway_certs` pattern: unknown `gov_action` tag → deterministic `CodecError` reject; structural failures reject; empty proposal-procedures sets reject (CIP-1694: non-empty); trailing garbage rejects; truncated input rejects. Decodes all 7 `GovAction` variants (`info_action`, `no_confidence`, `hard_fork_initiation`, `parameter_change`, `treasury_withdrawals`, `new_constitution`, `update_committee`) via dedicated helpers (`decode_gov_action`, `decode_anchor`, `decode_gov_action_id_opt`, `decode_stake_credential`, `decode_unit_interval`, `decode_cold_credential_set`, `decode_cold_credential_epoch_map`, `decode_hash28_opt`) and mirror encoders + `read_hash{28,32}` / `peek_null` helpers. **Scope locks per OQ resolution**: `voting_procedures` not decoded here (OQ-1, sibling cluster); `ParameterChange.update` stays opaque `Vec<u8>` (OQ-2); `NewConstitution.raw` stays opaque (OQ-3); `proposal_procedure.return_addr` stays raw reward-account `Vec<u8>` (OQ-4); `Anchor` stays opaque struct. The `update_committee` variant preserves the DC-LEDGER-10 `StakeCredential` discriminant end-to-end. Enforced by `ci_check_proposal_procedures_closed.sh` and `DC-LEDGER-11`. | `conway/governance.rs` (~856 lines incl. 14 inline unit tests); registered in `conway/mod.rs` | PROPOSAL-PROCEDURES-DECODE / PP-S1 (`70bc85b`) |
| `ade_testkit::governance::proposal_procedures_replay` (new submodule + file in an existing crate) | GREEN | **Canonical synthetic replay harness for the closed `proposal_procedures` decoder.** Per OQ-5, the corpus is synthetic-canonical (no real-chain Conway txs carrying proposal_procedures at PP-S1 HEAD); real-chain extraction is a separable strengthening when such txs become available. Exports `CorpusEntry { label, bytes, expected }`, `ReplayOutcome::{ Ok, DecodeMismatch, EncodeMismatch, DecodeError(String) }`, `canonical_corpus() -> Vec<CorpusEntry>` (9 entries: one per `GovAction` variant + multi-procedure + a DC-LEDGER-10 cross-check `update_committee_keyhash_scripthash_same_bytes`), `replay_canonical_corpus() -> Vec<(String, ReplayOutcome)>`. Corpus bytes are generated by the BLUE `encode_proposal_procedures` (canonical by construction); replay decodes, asserts typed shape match, and re-encodes to confirm byte-identicality. New submodule `governance/mod.rs` registered in `crates/ade_testkit/src/lib.rs`. | `governance/mod.rs`, `governance/proposal_procedures_replay.rs` (~232 lines) | PROPOSAL-PROCEDURES-DECODE / PP-S2 (`928c2be`) |
| `ade_core_interop` bin `live_tx_submission_session` (new RED binary in an existing RED crate) | RED | **Sustained-window N2N tx-submission2 live-evidence probe.** Modeled on the existing `live_consensus_session` binary. `main()` hermetic default prints a readiness banner and exits 0 (so the `#[ignore]`-gated closure-gate test stays offline). `--connect --network <net> --max-seconds N --max-frames M` dispatches to `run_live`, which connects to a real cardano-node peer, negotiates the N2N handshake, opens tx-submission2 (id 4), sends `Init`, and drives the BLUE `tx_submission2_transition` state machine against every peer-originated message until the deadline or `max_frames`. Replies empty for non-blocking `RequestTxIds`/`RequestTxs`; holds open on blocking `RequestTxIds`. Funnels any opportunistic `InventoryEvent::TxsDelivered` bytes through the S4 `event_to_ingress` / `PeerAccumulator` bridge. Writes `docs/clusters/PHASE4-N-E/CE-N-E-6_<date>.log`. Hardening (`caa5ce8`): retries on `io::ErrorKind::TimedOut` so the read loop survives inner socket timeouts. **Honest scope: evidences handshake + mux + codec + state-machine grammar conformance on real wire bytes; bulk tx_bytes delivery in the outbound-client direction is opportunistic — full N2C tx-delivery evidence is the cross-cluster `CE-NODE-N2C-LTX` deferral.** | `src/bin/live_tx_submission_session.rs` (~552 LOC); `tests/live_tx_submission_session.rs` (~80 LOC, `#[ignore]`-gated closure-gate test); `[[bin]]` entry in `crates/ade_core_interop/Cargo.toml`; operator procedure at `docs/clusters/completed/PHASE4-N-E/CE-N-E-6_PROCEDURE.md`; live-evidence log at `docs/clusters/completed/PHASE4-N-E/CE-N-E-6_2026-05-25.log` | PHASE4-N-E / S6 (`d1068b3` + `caa5ce8`) |
...
| `ade_core_interop::tx_submission` (new file in an existing RED crate) | GREEN | **N2N tx-submission2 → `mempool_ingress` bridge.** `event_to_ingress(&InventoryEvent, IngressSource)` maps `TxsDelivered.tx_bytes → Vec<IngressEvent>`; `PeerAccumulator` accumulates per-peer; `ingest_n2n_events(base, per_peer)` orchestrates over `replay_ingress_trace`. Pure, no I/O. | `src/tx_submission.rs`; `tests/tx_submission_ingress.rs` (7 integration tests) | PHASE4-N-E / S4 (`ca3f23a`) |
| `ade_core_interop::local_tx_submission` (new file in an existing RED crate) | GREEN | **N2C local-tx-submission → `mempool_ingress` bridge.** N2C mirror of S4. Cross-bridge agreement at the wire-event layer is the load-bearing CE-N-E-7 mechanical evidence (`n2n_and_n2c_bridges_produce_identical_outcomes`); the live-log half is **deferred** to `CE-NODE-N2C-LTX`. | `src/local_tx_submission.rs`; `tests/local_tx_submission_ingress.rs` (8 integration tests) | PHASE4-N-E / S5 (`43fcc31`) |
...
Both are RED-or-mixed. **PROPOSAL-PROCEDURES-DECODE added no new
crate** — PP-S1 added one new BLUE source file under `ade_codec`
(`conway/governance.rs`) and PP-S2 added one new GREEN submodule
under `ade_testkit` (`governance/{mod.rs, proposal_procedures_replay.rs}`).
**PHASE4-N-E S6 added no new crate either**; **PHASE4-N-E S1–S5
collectively added no new crate either.** **None of B3, B3F, B4,
B5, OQ5, FIDELITY, WRITEBACK, or the testkit follow-up thread added
a new crate either.**

Crate dependency shape at HEAD: **PHASE4-N-E S4 added one new dep edge
(carried forward)** — `ade_core_interop` now depends directly on
`ade_ledger` (was transitive only). **PROPOSAL-PROCEDURES-DECODE
added no new dep edge** — the new BLUE decoder uses only `ade_types`,
`minicbor`, and existing intra-crate items; the new GREEN harness
uses only `ade_codec`, `ade_types`, and existing intra-crate items.
**PHASE4-N-E S6 added no new dep edge.** `ade_network`, `ade_runtime`,
`ade_testkit` edges are unchanged. No edge from a BLUE crate to a
RED crate was introduced.
...
**PROPOSAL-PROCEDURES-DECODE added no external corpus** — the only
new test-data is the in-code synthetic-canonical 9-entry corpus
generated by `encode_proposal_procedures` itself, per OQ-5. Real-chain
extraction is a separable strengthening when proposal-bearing Conway
txs become available. **PHASE4-N-E S6 added no new corpus** — the
only new external-data artifact is the live-evidence log
`CE-N-E-6_2026-05-25.log`. **PHASE4-N-E S1–S5 reuse the existing
PHASE4-B2 B-track adversarial corpus verbatim** (per OQ-3).

Cross-reference: **The `ade-CODEMAP.md` regenerated in parallel with
this HEAD_DELTAS will record the new BLUE module
`ade_codec::conway::governance` as a row under the `ade_codec` crate's
BLUE listing, and the new GREEN submodule
`ade_testkit::governance::proposal_procedures_replay` under the
`ade_testkit` crate**; the prior CODEMAP at `3af9e2b` does NOT yet
contain either row. SEAMS will pick up the new closed-grammar surface
(`decode_proposal_procedures` as the sanctioned chokepoint at the
Conway tx-body proposal_procedures field) + the `ProposalProcedure`
closed type as a new canonical-type seam under the existing closed-type
registry; the prior SEAMS at `3af9e2b` does NOT contain either.
TRACEABILITY will pick up `DC-LEDGER-11` with its `ci_script ↔ rule`
edge (`ci_check_proposal_procedures_closed.sh → DC-LEDGER-11`) + the
21 test-method names + the cross-ref edge to `DC-LEDGER-10`; the
prior TRACEABILITY at `3af9e2b` does NOT contain it. All three
rewrites are in flight in the same grounding-refresh commit; the
...
|--------|-------|-------------|
| `ade_codec` | +13 source/test files (B3 + B3F + B4 + OQ5 + **PROPOSAL-PROCEDURES-DECODE PP-S1**). | **B3:** new `conway::cert` decoder + `conway::withdrawals` decoder; `error.rs` `UnknownCertTag` / `DuplicateMapKey`. **B3F-S2:** trailing-byte reject + bounded preallocation. **B4-S1:** owner-complete `decode_conway_certs`; new `decode_drep`. **OQ5-S1:** both era `decode_stake_credential` preserve the tag. **PROPOSAL-PROCEDURES-DECODE PP-S1 (`70bc85b`):** new BLUE module `conway::governance` (~856 lines incl. 14 inline unit tests; closed `decode_proposal_procedures` + `encode_proposal_procedures` + helpers for all 7 `GovAction` variants; reuses existing `decode_stake_credential` preserving the DC-LEDGER-10 discriminant); registered in `conway/mod.rs`. `conway/tx.rs` (+14/-4) rewires the tx-body codec at key 20 to invoke the typed decoder/encoder over the captured key-20 CBOR item bytes, replacing the prior `data[start..end].to_vec()` opaque pass-through; the `skip_item` bound is preserved as the canonical entry slice. 3 new integration tests in `crates/ade_codec/tests/conway_tx_body_proposal_procedures.rs` (~145 lines) — `body_with_proposal_procedures_round_trips_through_typed_field`, `body_none_skips_proposal_procedures_key_20`, `body_and_sub_decoder_agree_on_proposal_procedures`. |
| `ade_types` | +3 files (B3) + 2 files (B4) + governance ripples through the FIDELITY clusters + **PROPOSAL-PROCEDURES-DECODE PP-S1**. | **B3:** closed `ConwayCert` enum + classification types; `RewardAccount`. **B4-S1:** owner-complete `ConwayCert`; new `DRep` enum; `PoolRegistrationCert.owners`. **OQ5-S1:** `StakeCredential` tuple struct → closed enum + `hash()`. **COMMITTEE-CRED / DREP-VOTE / WRITEBACK:** governance-type ripples (committee_votes, drep_votes, structured `UpdateCommittee`). **PROPOSAL-PROCEDURES-DECODE PP-S1 (`70bc85b`):** new closed canonical type `ProposalProcedure { pub deposit: Coin, pub return_addr: Vec<u8>, pub gov_action: GovAction, pub anchor: Anchor }` in `conway/governance.rs` (+21); reuses the existing closed 7-variant `GovAction` enum (no shape change) and the existing opaque `Anchor` struct. `conway/tx.rs` (+3/-1) re-types `ConwayTxBody.proposal_procedures` from `Option<Vec<u8>>` to `Option<Vec<ProposalProcedure>>` — typed pin at the body boundary. |
| `ade_ledger` | +57 source/test files over the prior delta; **PROPOSAL-PROCEDURES-DECODE PP-S1 touched 1 file (`conway.rs`, +13/-2) as an OQ-8 test-fixture scrub — NOT a regression.** | **PHASE4-N-E:** new BLUE `mempool::ingress` chokepoint + closed `IngressSource::{N2N, N2C}` + `IngressEvent` + `mempool_ingress` pass-through; new GREEN `mempool::canonicalize` (`PeerId`, `PeerSubmissionQueue`, `canonicalize_peer_streams`, `source_byte`); re-exports from `mempool/mod.rs`. **B3:** the closed cert-deposit classifier `cert_classify.rs` and the full Conway value-conservation accounting in `conway.rs` (cert/withdrawal early-out **REMOVED**); new error variants; `ConwayOnlyDepositParams` + `conway_deposit_view()` in `pparams.rs`/`state.rs`. **B2:** `tx_validity/` + `mempool/` submodules + B2 integration tests; B2-S4 first cut of `check_conway_coin_conservation`. **B1:** `block_validity/`, `consensus_view.rs`, `consensus_input_extract.rs`, the `ade_core` dep edge. **B3F:** no source change (CI grep-gate added). **B4:** `delegation.rs` (+385) native owner-tagged apply model; `rules.rs` (+212) fail-closed `accumulate_tx_certs`; `cert_classify.rs` (+100) re-pointed at owner-complete `ConwayCert`. **B5:** new BLUE module `gov_cert.rs` (+366); `state.rs` (+56) `GovCertEnv` + `gov_cert_env()`; `pparams.rs` (+8) `drep_activity`; `error.rs` (+16) two new variants; `fingerprint.rs` (+14) tag 2→3 array; `rules.rs` (+161/−42). **OQ5:** `state.rs` re-key `Hash28` → `StakeCredential`; `fingerprint.rs` (+78) `write_stake_credential`; ripples across `gov_cert.rs`/`governance.rs`/`cert_classify.rs`/`rules.rs`. **COMMITTEE-CRED-FIDELITY:** re-key committee; `governance.rs` (+76) full-credential-equality ratification. **DREP-VOTE-FIDELITY:** `governance.rs` (+57) exact-variant DRep resolution. **ENACTMENT-COMMITTEE-FIDELITY:** `governance.rs` (+30) `EnactmentEffects.committee_changes` re-typed. **ENACTMENT-COMMITTEE-WRITEBACK:** `governance.rs` (+~189) live `enact_proposals` + `apply_committee_enactment`; `rules.rs` (+~53) epoch-boundary call site; `fingerprint.rs` (+~88) structured `write_gov_action`. **PROPOSAL-PROCEDURES-DECODE PP-S1 (`70bc85b`, OQ-8):** `conway.rs:344` and `conway.rs:519` previously set `proposal_procedures = Some(vec![0x80])` as a presence marker (not real proposal data); reset to `None` because the prior placeholder no longer decodes through the typed path. Affected tests still exercise the "governance fields present" path via the other fields (`voting_procedures` / `treasury_value` / `donation`). This is a contracted scope adjustment, not a regression. **No `ade_ledger` change for PP-S2.** |
| `ade_testkit` | +28 files across the prior delta; **PROPOSAL-PROCEDURES-DECODE PP-S2 added 4 files**; PHASE4-N-E S2/S3 added 5; S6 added 0. | **N-B:** `consensus/` harness. **B1:** `validity/` harness. **B2:** `tx_validity/` submodule. **B3:** extended `harness/snapshot_loader.rs` (intra-corpus resolution), `tx_validity` extensions. **OQ5 → WRITEBACK:** progressive snapshot-loader extensions (key/script tag preservation, fail-closed cold-credential parsing, structured `UpdateCommittee` decode). **Post-3d94c22 thread:** real-chain committee oracle, corpus alignment, `reward_provenance` generator, snapshot-loader follow-ups (tip slot + Conway UMElem). **PHASE4-N-E S2 (`2d0c918`):** new GREEN `mempool::ingress_replay` harness; 5 integration tests. **PHASE4-N-E S3 (`509d714`):** 2 integration tests in `tests/mempool_ingress_canonicalize.rs`. **PROPOSAL-PROCEDURES-DECODE PP-S2 (`928c2be`):** new GREEN submodule `governance/mod.rs` (+14) + `governance/proposal_procedures_replay.rs` (+232) exporting `CorpusEntry { label, bytes, expected }`, `ReplayOutcome::{ Ok, DecodeMismatch, EncodeMismatch, DecodeError(String) }`, `canonical_corpus() -> Vec<CorpusEntry>` (9 entries: one per `GovAction` variant + multi-procedure + `update_committee_keyhash_scripthash_same_bytes` DC-LEDGER-10 cross-check), `replay_canonical_corpus() -> Vec<(String, ReplayOutcome)>`; registered in `lib.rs` (+1). 4 new integration tests in `tests/proposal_procedures_corpus_replay.rs` (+129) — `canonical_corpus_decodes_to_expected_shape`, `canonical_corpus_round_trips_byte_identical`, `canonical_corpus_covers_all_gov_action_variants`, `canonical_corpus_includes_update_committee_discriminant_case`. |
| `ade_core_interop` | +1,546 across 6 files (B1/CE-N-B-6) + 4 files for N-E S4/S5 + 3 files for N-E S6. **No PROPOSAL-PROCEDURES-DECODE change.** | **CE-N-B-6:** follow-bridge (`e5f1f64`) + pin retarget (`807bcb6`). **PHASE4-N-E S4 (`ca3f23a`):** N2N `tx_submission` bridge module; 7 integration tests. **PHASE4-N-E S5 (`43fcc31`):** N2C `local_tx_submission` bridge module mirroring S4; 8 integration tests. Cargo edge added (`ade_core_interop -> ade_ledger`, was transitive only). **PHASE4-N-E S6 (`d1068b3` + `caa5ce8`):** new RED binary `live_tx_submission_session` (~552 LOC). Hardening (`caa5ce8`) folded a retry-on-`io::ErrorKind::TimedOut` fix into the read loop and corrected the elapsed-time logging. New `[[bin]]` manifest entry; new closure-gate test `cardano_node_tx_submission2_sustained_window` (`#[ignore]`-gated). |
...
| `ade_network` | 100 files, +17,861 lines (full N-A). **No PHASE4-N-E or PROPOSAL-PROCEDURES-DECODE change.** | DoS hardening of 6 codecs (`744ef34`, post-N-A close). The N-E bridges live in `ade_core_interop`, not in `ade_network`; the N-E S6 live binary *uses* `ade_network`'s tx-submission2 state machine + handshake codec + mux frame codec via the existing crate edge. |
| `ade_runtime` | +18 files, +3,440 lines (N-B `consensus/` + N-D `chaindb`/`recovery`; B1 one small touch). **No PHASE4-N-E or PROPOSAL-PROCEDURES-DECODE change.** | **N-B:** new `consensus/` submodule. **B1:** one small touch. N-D `chaindb`/`recovery` are §2 New Modules. |

No other crate had non-trivial source changes since baseline.
`ade_plutus` and `ade_node` were untouched by code commits.
**PROPOSAL-PROCEDURES-DECODE touched `ade_codec` (3 files: 1 new
module + `conway/mod.rs` registration + `conway/tx.rs` body rewire),
`ade_types` (2 files: new closed type + `ConwayTxBody` field
retype), `ade_ledger` (1 file: OQ-8 test-fixture scrub at
`conway.rs:344` and `:519`), `ade_testkit` (3 files: new GREEN
submodule + `lib.rs` registration), plus the 2 in-crate test files
under `crates/ade_codec/tests/` and `crates/ade_testkit/tests/`,
plus the new CI script, plus the registry update, plus the cluster
plan + invariants + cluster.md + PP-S1.md + PP-S2.md.** No
`.idd-config.json` change. **No BLUE authority-path semantics
changed apart from the new typed pin** — the body codec's other
key paths are untouched; the closed decoder strictly reduces
opaque surface to typed surface.
...
introduced through the PHASE4-N-E chokepoint, canonicalizer, and the
PROPOSAL-PROCEDURES-DECODE typed body pin).
...
At HEAD there are **32 scripts plus one git hook**
...
OQ5 added one (the 29th), PHASE4-N-E S1/S2 added two (the 30th and
31st), and **PROPOSAL-PROCEDURES-DECODE PP-S1 added one (the 32nd):
`ci_check_proposal_procedures_closed.sh`** (`DC-LEDGER-11`, 5
mechanical guards). **PHASE4-N-E S6, B1, B2, B4, COMMITTEE-CRED-FIDELITY,
DREP-VOTE-FIDELITY, ENACTMENT-COMMITTEE-FIDELITY,
ENACTMENT-COMMITTEE-WRITEBACK, the post-3d94c22 testkit thread, and
PROPOSAL-PROCEDURES-DECODE PP-S2 added no new CI script.** Grouped
by cluster.
...
| same | **Unmodified by PROPOSAL-PROCEDURES-DECODE** | The credential-discriminant gate stays the **29th** script. PP-S1's typed `update_committee` decoder reuses the existing `decode_stake_credential` and preserves the discriminant; the gate's existing clauses already cover the surface. |

### PHASE4-N-E wire-level mempool ingress closure (`32c1ee6`, `2d0c918`, `509d714`)

| Check | Status | What it checks |
|-------|--------|----------------|
| `ci/ci_check_mempool_ingress_closure.sh` | **New** (`32c1ee6`, S1) — the **30th** script | Enforces `DC-MEM-03` via 5 mechanical guards: (1) `mempool/ingress.rs` defines `IngressEvent`/`IngressSource`/`mempool_ingress` and is re-exported from `mempool/mod.rs`; (2) `IngressSource` is a closed 2-variant enum with no `#[non_exhaustive]`; (3) `MempoolState.accumulating` is field-written only inside `mempool/admit.rs`; (4) `admit()` is called only from `mempool/admit.rs` and `mempool/ingress.rs`; (5) `mempool_ingress` body must not reference `source`. |
| `ci/ci_check_mempool_ingress_replay.sh` | **New** (`2d0c918`, S2); **Modified** (`509d714`, S3, +2 clauses) — the **31st** script | Enforces `DC-MEM-04` via 6 mechanical guards: (1) `mempool/ingress_replay.rs` exists with required exports; (2) `replay_ingress_trace` body calls `mempool_ingress`, not `admit`; (3) 5 registry-pinned test functions exist; (4) no batching helpers (single-step per OQ-6); (5, S3) `canonicalize.rs` exists with required items; (6, S3) `canonicalize.rs` body contains no `HashMap`/`HashSet`/`tokio`/`async`/`SystemTime`/`Instant`/`rand`/`thread_rng`/`RwLock`/`Mutex`. |

### PHASE4-N-E S6 live-evidence binary

**No new CI script.** The S6 deliverables are a RED binary
`live_tx_submission_session` whose live-evidence pass is operator
action and an `#[ignore]`-gated closure-gate test that runs the
binary without `--connect` to assert it builds and prints its
readiness banner. The deterministic mechanical CI for the cluster
is the synthetic-event adapter test surface from S4/S5 plus the
two ingress CI scripts from S1/S2/S3. CI does not run the S6
network test.
...
| Check | Status | What it checks |
|-------|--------|----------------|
| `ci/ci_check_proposal_procedures_closed.sh` | **New** (`70bc85b`, PP-S1) — the **32nd** script | Enforces `DC-LEDGER-11` via 5 mechanical guards: (1) `ade_types::conway::governance` defines `pub struct ProposalProcedure` with the 4 required fields (`pub deposit:`, `pub return_addr:`, `pub gov_action:`, `pub anchor:`); (2) `ConwayTxBody.proposal_procedures` is `Option<Vec<ProposalProcedure>>` and is **not** `Option<Vec<u8>>` (the prior opaque form); (3) `ade_codec::conway::governance` exists and exports both `pub fn decode_proposal_procedures` and `pub fn encode_proposal_procedures`; (4) the Conway tx-body codec at key 20 calls `decode_proposal_procedures(` and `encode_proposal_procedures(` and does **not** reintroduce the `proposal_procedures = Some(data[...]` opaque pass-through; (5) no `ProposalProcedure { ... }` struct-literal construction outside sanctioned sites — the decoder file itself, `ade_testkit/`, `crates/*/tests/`, `benches/`, and `#[cfg(test)]` inline test blocks are exempt; all production callers must go through `decode_proposal_procedures`. PP-S2 (the GREEN harness) reuses the sanctioned synthesis sites and does not require gate extension. |
...
re-traced via `ci/ci_check_constitution_coverage.sh`. **PROPOSAL-PROCEDURES-DECODE
PP-S1 added one new `ci_script ↔ rule` edge**:
`ci_check_proposal_procedures_closed.sh → DC-LEDGER-11`. **PP-S2
added no new edge** — the canonical-corpus replay tests appear in
`DC-LEDGER-11.tests` (the rule's `tests` array gained 4 PP-S2 test
names: `canonical_corpus_decodes_to_expected_shape`,
`canonical_corpus_round_trips_byte_identical`,
`canonical_corpus_covers_all_gov_action_variants`,
`canonical_corpus_includes_update_committee_discriminant_case`), and
the rule's mechanical-enforcement edge is
`ci_check_proposal_procedures_closed.sh` from PP-S1.
...
= 381 (was 380 at prior refresh; +1 — `ade_types::conway::governance::ProposalProcedure`
added by PROPOSAL-PROCEDURES-DECODE PP-S1, `70bc85b`).** The
`ProposalProcedure` struct is a closed 4-field record; the existing
closed 7-variant `GovAction` enum and the opaque `Anchor` struct are
reused unchanged.
...
- Rules at prior refresh (`3af9e2b:docs/ade-invariant-registry.toml`): **175**
- Rules at HEAD (`928c2be:docs/ade-invariant-registry.toml`): **176**
- Net additions vs baseline: **+29** (PHASE4-N-A: 2; PHASE4-N-B: 8;
  ...
  **PROPOSAL-PROCEDURES-DECODE: 1** — `DC-LEDGER-11`, introduced by
  PP-S1 + `enforced` via the new CI script + 17 test names at PP-S1
  close expanding to 21 with PP-S2).
- Net additions vs prior refresh: **+1** (`DC-LEDGER-11`).
- Removals: **0** (expected under append-only discipline; clean).

- Strengthenings at HEAD:
  - **`DC-LEDGER-10`** (PROPOSAL-PROCEDURES-DECODE PP-S1, `70bc85b`):
    `cross_ref += "DC-LEDGER-11"` — bidirectional pairing with the
    new proposal-procedures decoder rule (the typed `update_committee`
    variant preserves the existing `StakeCredential` discriminant
    end-to-end, and the PP-S1 closure cannot regress without
    re-collapsing the credential).
  - **`DC-MEM-01`** (PHASE4-N-E S2/S3, carried forward).
  ...
  - **All earlier strengthenings carried forward unchanged**:
    ...
    → WRITEBACK → post-3d94c22 oracle → **PROPOSAL-PROCEDURES-DECODE
    cross_ref**; 20 tests at HEAD); `DC-LEDGER-08` (B5, via
    `cross_ref`); `T-DET-01` / `T-ENC-03` (OQ5); `DC-TXV-06` (B3F:
    `partial` → `enforced`); `DC-VAL-06` (B3F + B4); `T-CONSERV-01`
    / `CN-LEDGER-07` (B3); `DC-MEM-01,02` (B2, `declared` →
    `enforced`); `DC-EPOCH-02` (CE-73 reclassification); the N-D
    bundle; the N-A real-capture bundle; `T-CORE-02` (S-B1).

Family counts at HEAD: registry total **176** (= 175 + 1 from
`DC-LEDGER-11`); the `DC` family in particular grew by one. Per the
constitution coverage gate, `ci_check_constitution_coverage.sh`
will PASS after the parallel TRACEABILITY regen lists
`DC-LEDGER-11` with its `ci_script`.
...
- **OQ-8 contracted scope adjustment (PROPOSAL-PROCEDURES-DECODE
  PP-S1, `70bc85b`) — NOT A REGRESSION.** Two sites in
  `crates/ade_ledger/src/conway.rs` (lines 344 and 519) previously
  set `proposal_procedures = Some(vec![0x80])` as a presence marker
  (not real proposal data). The PP-S1 typed pin requires the field
  to deserialize through the closed decoder, so the placeholder no
  longer round-trips. PP-S1 resets both sites to `None`; the
  affected tests still exercise the "governance fields present"
  path via the other body fields (`voting_procedures` /
  `treasury_value` / `donation`). Recorded in PP-S1's commit message
  under "Cascading test-file updates (OQ-8)" and in the cluster
  doc; this is a deliberate scope adjustment under the cluster's
  declared scope, not a silent regression.
- **Pre-existing `boundary_fingerprint_matches_pins` failure on
  `byron_pre_hfc` predates this cluster.** Verified by the user via
  `git stash` + retest at HEAD `3af9e2b` (the prior refresh).
  Out-of-scope for PROPOSAL-PROCEDURES-DECODE; not introduced by
  PP-S1 or PP-S2. Will be tracked under a separate cluster.
- **PROPOSAL-PROCEDURES-DECODE cluster directory archived in this
  regen round.** `docs/clusters/PROPOSAL-PROCEDURES-DECODE/` was
  moved via `git mv` (3 renames) to
  `docs/clusters/completed/PROPOSAL-PROCEDURES-DECODE/` as part of
  the grounding-refresh commit that this HEAD_DELTAS is part of:
  `cluster.md`, `PP-S1.md`, `PP-S2.md`. (The planning artifacts
  `proposal-procedures-decode-cluster-slice-plan.md` and
  `proposal-procedures-decode-invariants.md` were committed under
  the cluster planning location per project convention.) Active
...
- **CODEMAP / SEAMS / TRACEABILITY are being regenerated in
  ...
  exact moment of this regen.** Prior CODEMAP (`3af9e2b`) does NOT
  yet contain the new BLUE module `ade_codec::conway::governance`
  or the new GREEN submodule
  `ade_testkit::governance::proposal_procedures_replay`. Prior
  SEAMS does NOT yet contain `decode_proposal_procedures` as a
  closed-grammar chokepoint or `ProposalProcedure` as a closed
  canonical type. Prior TRACEABILITY does NOT yet contain
  `DC-LEDGER-11`. All three rewrites are in flight in the same
  regen round; the four docs will be self-consistent at the next
  grounding-doc commit.
- **First use of in-tree synthetic-canonical corpus per OQ-5
  (PROPOSAL-PROCEDURES-DECODE PP-S2).** The existing in-tree
  Conway corpus contains no real-chain txs carrying proposal
  procedures at PP-S1 HEAD (verified by parent). The corpus
  generator produces canonical CBOR via the BLUE
  `encode_proposal_procedures` (canonical by construction); replay
  decodes, asserts typed shape, and re-encodes to confirm
  byte-identicality. Real-chain extraction is a separable
  strengthening slice when proposal-bearing Conway txs become
  available — recorded as the natural next strengthening on
  `DC-LEDGER-11`. The pattern is parallel to B4/B5's
  README-only synthetic notes; PP-S2 takes it one step further by
  shipping the corpus in code.
- **Scope locks held by parent declaration (PROPOSAL-PROCEDURES-DECODE).**
  Five distinct opacities remain open as **deliberate non-goals
  in this cluster**: (OQ-1) `voting_procedures` (tx body key 19)
  is the natural sibling cluster — same opaque-bytes shape, same
  pressure, deliberately deferred; (OQ-2) `ParameterChange.update`
  stays opaque `Vec<u8>` — closing it requires a full pparams
  delta sub-grammar, future strengthening; (OQ-3)
  `NewConstitution.raw` stays opaque — closing it requires a
  Constitution sub-grammar; (OQ-4) `proposal_procedure.return_addr`
  stays raw reward-account `Vec<u8>` — typed `RewardAccount` is a
  separable fidelity decision; `Anchor` stays opaque struct
  (existing `{ raw: Vec<u8> }`). These five are recorded in
  `DC-LEDGER-11.open_obligation` as future strengthening
  candidates and in the cluster doc's "Scope locks" section.
- **`ade_core_interop -> ade_ledger` new dependency edge
  (PHASE4-N-E S4, carried forward).** RED → BLUE direction is
  allowed by `ci_check_dependency_boundary.sh`. **PROPOSAL-PROCEDURES-DECODE
  added no new dep edge.**
- **All commit subjects in this regen carry a conventional-commits
  prefix.** The 2 PROPOSAL-PROCEDURES-DECODE commits are
  `feat(codec)` and `test(testkit)`. **All 2 commits in the
  `3af9e2b..928c2be` span carry the repo-required `Co-Authored-By`
...
- **`CE-NODE-N2C-LTX` cross-cluster obligation framing (PHASE4-N-E
  S6, carried forward).** Deliberate deferral, not a discipline
  gap. The CE-N-E-7 procedure spec is retained as the canonical
  spec for the future closure. Bounty / N2C certification remains
  blocked on the future cluster's discharge.
- **PHASE4-N-E source-invariance is the load-bearing wire-level
  no-false-accept property — carried forward.** `IngressSource` is
  metadata only. Mechanically enforced by CI guard #5 of
  `ci_check_mempool_ingress_closure.sh`.
- **Post-3d94c22 testkit thread (carried forward unchanged).** Four
  GREEN-scope commits with one registry-level effect: in-place
  strengthening of `DC-EPOCH-01.tests` and `DC-LEDGER-10.tests`
  plus the `authority_surface` / `open_obligation` text rewrites
  on both rules. `DC-LEDGER-10` at 20 tests at HEAD.
- **Cluster docs archived as of this HEAD.** Sixteen cluster
  ...
  PHASE4-N-D, PHASE4-N-E, **PROPOSAL-PROCEDURES-DECODE (newly
  archived in this regen round)**. `docs/clusters/` at HEAD
  contains only `completed/` and the carried-forward `PHASE4-N-B/`
  stray log directory.
- **ENACTMENT-COMMITTEE-WRITEBACK fingerprint change (carried
  forward).** `write_gov_action` emits the structured
  `UpdateCommittee` shape. **PROPOSAL-PROCEDURES-DECODE does NOT
  change the fingerprint surface** — the typed pin is at the
  decoder/encoder boundary; the fingerprint writer continues to
  serialize the structured `UpdateCommittee` shape from
  `EnactmentEffects`, which already feeds the structured form
  introduced in WRITEBACK.
- **WRITEBACK carry-forward follow-ups (narrowed, unchanged).**
  FIDELITY follow-up **(d)** RESOLVED + WIRED. **(e)** narrowed to
  the older `mk_credential` helper. The pre-OQ5 **(b)** Shelley
  unknown-cert zero-hash placeholder remains a WARN LOW non-goal.
- **B5 / B4 / B3F / B3 / B2 / B1 / N-D / N-B / N-A / PHASE4-N-E
  closures — carried forward unchanged.** All cluster docs at
  `docs/clusters/completed/<NAME>/`.
...
- **`strengthened_in` records the introducing cluster on
  freshly-created rules (carried forward).** `DC-LEDGER-11`
  records `strengthened_in = []` (no strengthenings yet) and
  `introduced_in = "PROPOSAL-PROCEDURES-DECODE"`. Harmless.
...
- **Adversarial corpora are derived, not committed (carried
  ...
  adversarial bytes committed for N-E. **PROPOSAL-PROCEDURES-DECODE
  PP-S2 ships its corpus in code** (canonical synthetic per OQ-5),
  not as committed bytes.
...
- **No removed canonical types** (n/a — no separate registry;
  canonical types at HEAD = 381, +1 from `ProposalProcedure`).
- **No removed registry rules** (expected: 0; actual: 0).
  **PROPOSAL-PROCEDURES-DECODE PP-S1 added one new registry rule
  (`DC-LEDGER-11`);** PP-S2 added no rule. Registry total: **176**
  at HEAD (was 175 at prior refresh).
...
`928c2be` (PROPOSAL-PROCEDURES-DECODE PP-S2). The prior regen
narrated HEAD `caa5ce8` (PHASE4-N-E S6 hardening); the new span is
`3af9e2b..928c2be` — 2 commits (`70bc85b` PP-S1 typed decoder +
body rewire + new BLUE module + new closed type + new rule + new
CI script; `928c2be` PP-S2 GREEN canonical synthetic corpus +
replay harness).
