...
## 9. Design Summary
On a Participant `Competing` block (`classify_receive` → `NeedsForkChoice`): **(proof center)** resolve the fork anchor by `ChainDb::get_block_by_hash(decoded.prev_hash)` → bind the **durable stored** `(slot, hash, block_no)` (never peer-supplied; `DC-NODE-29` discipline); obtain `anchor_chain_dep` by a **read-only `materialize_rolled_back_state`** at that durable anchor (no commit; passes the recovered eta0, `T-REC-06`). Accumulate the peer's header inputs (`BTreeMap`-keyed by `peer` from S1); call S2 `build_candidate_fragment(anchor, anchor_chain_dep, current_tip_block_no, headers, …)` + `assemble_candidate_set`. Derive the live `ChainSelectorState` from **durable authority (Option A)**: `current_tip` = `ChainDb::tip()`; `current_tip_block_no` = `chain_dep.last_block_no`; **`current_tiebreaker`** = a deterministic projection from Ade's **already-admitted durable tip block bytes** (local durable authority — NOT peer-derived, NOT revalidated, NOT minted); `security_param` = **k** from durable local configuration / `ForgeActivation.security_param`, never from peer input — a release/venue configuration authority, **not** a Cardano ledger protocol parameter (the authoritative depth bound); and a **conservative immutable floor** (the recovered anchor / genesis — a lower-bound guard, **NOT** an "immutable tip") in place of an O(chain) slot-walk to the exact k-deep block. **The eligibility authority is the block-number depth bound `rollback_depth ≤ k`** (`rollback_depth = current_tip_block_no − anchor_block_no`); the conservative floor never permits a rollback — it is selector-state input only. Then call **BLUE `select_best_chain`**. Dispatch the result: `Rejected{TiebreakerLossKeepCurrent}` or any ineligible reject (incl. `ExceededRollback` for depth > k) → **no-op** (current chain untouched, no `PendingForkSwitch`, S4 not invoked); `ChainSelected{…}` → set `PendingForkSwitch` + the `DC-NODE-28` forge fence (`pending_reselection`), **emit toward S4, apply nothing**. A `prev_hash` that is neither durable nor a known peer-candidate parent → **fail closed**. **S4's `materialize` remains the final, independent k-authority** (fail-closed `RollbackTooDeep` before any mutation).
...
- [ ] **Negative (depth)** `fork_anchor_older_than_k_refused_no_mutation` — a candidate whose fork anchor is older than k (`rollback_depth > k`) → `select_best_chain` returns `ExceededRollback` (ineligible) → **S3 emits no selection, sets no `PendingForkSwitch`, makes no durable mutation, and does NOT invoke S4.** (S4 keeps its own independent `RollbackTooDeep` negative test before any mutation.)
...
- **A conservative immutable floor may never be used to permit durable rollback.** It is selector-state input only. The authoritative rollback-depth enforcement remains `rollback_depth ≤ k` (block-number depth) **plus** S4's independent `materialize`/apply fail-closed (`RollbackTooDeep`) behavior. *(S4 keeps its own independent too-deep-rollback negative test before any mutation.)*
- **No O(chain) durable iteration in S3** — the conservative floor avoids walking slots to the exact k-deep block; the depth bound + S4 are the real safety boundary.
