...
- **Fail-closed:** the `SeedConsensusProvenanceMissing` variant exists for a future production warm-start that is in `RequiredFromRecoveredProvenance` mode yet finds the warm-start branch cannot proceed (e.g. a wiring that loses the view); A3b proves the variant + the five chain failures via tests. Cold-start branch: unaffected.
...
- [ ] the four `warm_start_fails_closed_on_{missing_sidecar,hash_mismatch,anchor_mismatch,epoch_mismatch}` tests pass.
- [ ] `warm_start_required_provenance_missing_view_fails_closed`, `cold_start_ignores_seed_epoch_source`, `warm_start_not_required_is_unchanged`, `warm_start_never_falls_back_to_consensus_inputs_path` pass.
