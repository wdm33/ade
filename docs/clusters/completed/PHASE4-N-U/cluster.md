...
### §13a Post-close correction (2026-06-05) — DC-NODE-06 was N-U-stranded
The "0 S3-introduced regressions" line above was **incomplete**. A follow-up gate-drift triage found that N-U S3 **stranded `ci_check_served_chain_handoff_fence.sh` (DC-NODE-06)**: that gate fences the `--mode node` spine `SelfAcceptedHandoff → push_atomic` handoff — exactly the mechanism S3 retired for serve-as-projection (DC-NODE-13). The close's **set-based** gate-diff masked it (the gate was *also* failing at baseline for an unrelated same-line technicality, so it appeared in both failing sets and was mis-classified pre-existing). Corrected in a dedicated follow-up commit: `ci_check_served_chain_handoff_fence.sh` **repointed** to fence DC-NODE-06's evolved durable-provenance serve (node-spine serve sources only `ServedChainSource::DurableChainDb`; no retired non-durable ingress), GREEN; `DC-NODE-06.strengthened_in += PHASE4-N-U` (mechanism superseded by DC-NODE-13; deeper invariant "only validated/admitted bytes served" preserved + strengthened via the CN-CONS-07 durable-provenance restatement; survives restart). Mirrors the DC-NODE-11 treatment. This was the **only** N-U-introduced gate item; the other 11 pre-existing failures are genuine gate-vs-code drift (separate gate-hygiene pass).
