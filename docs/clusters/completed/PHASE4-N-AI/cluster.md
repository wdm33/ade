...
- **AI-S4b-i — Participant venue declaration (RED, inert).** Add an explicit, closed venue declaration (`--participant-venue` → `VenueRole::Participant`); `Unknown`/absent stays the conservative non-fork-choice path; no silent inference either direction. **Truly inert** — recognizes the mode value only: it does NOT route to fork-choice, does NOT change any forge decision, does NOT alter `SingleProducer` behavior. Merges with no live behavior flip. Strengthens OQ-5 (venue is explicit + closed).
- **AI-S4b-ii — live rollback-follow routing + forge gate (RED, the flip).** The live receive loop classifies every block (S2 detector + venue resolver). **RollBack path (Participant):** `RollBackward(point)` → verify `point` is in the durable ChainDb → compute `to_block_no`+`depth` from the durable chain → construct `ChainEvent::RolledBack` → `apply_chain_event` (S3: materialize+lockstep+`WalEntry::RollBack`, replay-equivalent). A `point` not in the durable chain / beyond-k / crossing the immutable tip **fails closed** (no fabricated block_no; `apply_chain_event`'s materialize is the within-k authority). This is **NOT `process_stream_input`** — the orchestrator's in-memory rollback ring is header-arrival-populated and empty on the live block/pump follow path; the loop never calls `select_best_chain` (DC-CONS-03 honored). **Block path:** `AlreadyHave`→drop, `LinearExtend`→`pump_block`, `Competing`→**fail closed (all venues)** — a bare competing block (non-linear RollForward without a prior RollBackward) has no safe fork point. `SingleProducer`/`Unknown` keep the `DC-NODE-20` / existing fail-closed; `RollBackward`→refuse. **Forge gate (`DC-NODE-28`, lands together — irreducible):** while a rollback/apply is pending, forging refuses (typed `ForgeRefused`) — never forges on the stale pre-resolution tip. **Honesty:** this proves single-best-peer rollback *following* (replay-equivalent peer-branch adoption), NOT multi-candidate live `select_best_chain` selection (CN-CONS-03 full claim = CE-AI-6/AI-S5 + a later multi-peer slice). The go-live behavior flip.
...
- **CE-AI-3 (`DC-NODE-25`+`DC-NODE-26` apply + reconcile) [S3+S4b]:** hermetic — a `ChainSelected` requiring rollback applies via materialize+lockstep+`WalEntry::RollBack`+`pump_block`; no header-only tip advance; `selector.current_tip == ChainDb::tip` after apply; the live Participant path **follows a peer's `RollBackward` reorg end-to-end** (single-best-peer: durable-point lookup → `ChainEvent::RolledBack` → `apply_chain_event`; bare competing blocks fail closed — not multi-candidate selection). New gate `ci/ci_check_live_fork_choice_apply.sh` green; `ci_check_rollback_materialize_closure.sh` + `ci_check_receive_reducer_closure.sh` + `ci_check_forged_durable_admit_via_pump.sh` stay green.
...
- **AI-S4b-i** Participant venue declaration (inert) — OQ-5 (CE-AI-2 venue precursor). **RED** (`cli`/`node_lifecycle`); no live behavior flip.
- **AI-S4b-ii** live rollback-follow routing + forge gate (the flip) — CE-AI-3 (live) + CE-AI-4 (+ CE-AI-2 live). **RED** (`node_lifecycle`/`node_sync`). Proves single-best-peer rollback following, NOT multi-candidate live selection.
