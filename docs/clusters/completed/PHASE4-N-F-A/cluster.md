...
> **Status: CLOSED (2026-05-30) as the recovered seed-epoch consensus-input CAPABILITY cluster — NOT production wiring.** Slices A1–A4 merged (A1 `c13c2e9`, A2 `f6bf50f`, A3a `c507159`, A3b `104982d`, A4 `8b60524`). A5 is a SCOPING doc (not a slice) handing the production lifecycle to a successor cluster.
> **What this cluster proves (capability):** bootstrap-time seed-epoch consensus inputs → anchor-bound sidecar (A2) → WAL provenance (A3a) → warm-start verification *capability* (A3b, authority surface) → projection to `PoolDistrView`/`ExpectedVrfInput` (A4). **What it does NOT do:** it does not wire the producer, does not satisfy BA-02 by itself, makes no claim that `produce_mode` has stopped using `--consensus-inputs-path`, and makes no claim that `Mode::Admission` / `run_node_until_shutdown` / `recover_node_state` is the final production lifecycle. It claims no BA-01/BA-03/BA-04/BA-09 completion.
> **Deferred to successor cluster `PHASE4-N-F-C — Producer Recovered-State Lifecycle`** (see `A5-SCOPING-producer-recovered-state-lifecycle.md`): all production wiring, incl. **CE-A-4b** (bounty-primary producer consumes the recovered surface). The core safety line — recovered state must come from verified bootstrap + persist + WAL + warm-start, never a forge-time shape-swap of `--consensus-inputs-path` — is binding on that successor.
...
## Primary invariant
The seed-epoch consensus inputs (eta0, PoolDistr, ASC, pool_vrf_keyhashes) established **during verified bootstrap** become an **anchor-bound, canonical, version-gated, persisted, recovered** Ade state surface, such that `bootstrap → persist → recover` restores them **byte-identically**. **[Capability — proven by A1–A4 at the authority surface.]** — and the bounty-primary leader/forge path consumes them **only** from that recovered surface, never from a forge-time bundle. **[Production-wiring half — DEFERRED to `PHASE4-N-F-C`; NOT proven by this cluster. `produce_mode` still cold-starts from `--consensus-inputs-path`.]** *(Candidate `CN-CINPUT-01`; strengthens `CN-ANCHOR-01`/`DC-ANCHOR-01`, `CN-NODE-01`, `T-REC-01`/`T-REC-02`.)*

This cluster **does not forge** and **does not wire the producer**. It establishes and proves the recovered-state *capability* that a successor lifecycle cluster will thread into production to make BA-02 safe.
...
- **CE-A-1** ✅ (CLOSED) — `SeedEpochConsensusInputs` closed type + sole codec; round-trip test `seed_epoch_consensus_inputs_round_trips_byte_identical` + version-gated `seed_cinput_decode_rejects_unknown_version` pass.
- **CE-A-2** ✅ (CLOSED) — bootstrap populates it; `ci_check_consensus_input_provenance.sh` passes (bootstrap-only population + forge-time bundle fenced + anchor-bound).
- **CE-A-3** ✅ (CLOSED — **authority-surface** byte-identity, reworded from the original "production warm-start"): test `warm_start_restores_seed_epoch_consensus_inputs_byte_identical` proves `bootstrap_initial_state` warm-start, given a persisted keyed-SnapshotStore sidecar + its A3a WAL provenance, restores the same eta0/PoolDistr/ASC/pool_vrf_keyhashes byte-identically, fail-closed (A3b). It exercises the warm-start branch directly, **NOT** a `main.rs` production flow and **NOT** a test-only `recover_node_state` capability. *(The original "production warm-start" wording overclaimed: no production mode is wired to this path — see the A3b doc scope-honesty note. The production-path proof is deferred to `PHASE4-N-F-C` C3.)*
- **CE-A-4a** ✅ (CLOSED): projection test `recovered_surface_projects_pooldistrview_and_expected_vrf_input` shows the A4 projection of the recovered `SeedEpochConsensusInputs` equals the prior `pool_distr_view_from_consensus_inputs` output for the seed epoch (plus eta0 → `leader_vrf_input` equivalence).
- **CE-A-4b** ⏸ (DEFERRED to `PHASE4-N-F-C` C4 — NOT satisfied by this cluster): the bounty-primary produce call site (`produce_mode.rs:197`) consumes the recovered surface. Today the producer cold-starts from `--consensus-inputs-path`; wiring it to consume recovered state is the successor lifecycle cluster's job (and must not be a forge-time shape-swap).
