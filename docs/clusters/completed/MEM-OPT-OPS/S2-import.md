...
- **Unique-outref enforcement (M1 + the per-cluster security review):** distinct JSON key strings can collide on one canonical `TxIn` (uppercase vs lowercase hex; `#0` vs `#00`) — and the whole-buffer (String-sorted) and streaming (textual-order) paths would otherwise pick *different* survivors → different fingerprint. **The streaming PRODUCTION path now rejects ANY duplicate `TxIn` fail-closed** (`JsonSeedError::DuplicateTxIn`) — both the canonical-collision case and the exact-duplicate JSON string-key case — so production is **byte-identical-or-rejected**, never an order-dependent survivor. **Honest asymmetry (an evidence-scope correction, NOT a production bug — production already fails closed):** the whole-buffer *oracle* (test-only) is **not authoritative for exact-duplicate JSON keys** — serde map ingestion collapses them last-wins (`Ok`), so on that one input the two test paths diverge (streaming `Err`, oracle `Ok`). **Equivalence is therefore claimed only for valid unique-key seeds AND for malformed inputs where both paths are expected to agree**; the exact-duplicate-string-key case is a pinned, documented oracle asymmetry (`streaming_rejects_exact_duplicate_string_key_but_oracle_collapses`), never a silent assurance. A UTxO dump has unique outrefs by construction, so this is a no-op on the canonical cardano-cli seed (verified live: fp == S1's).
