...
- **AI-S4b-ii — live fork-choice routing + forge gate (RED, the flip).** The live receive loop consumes the surfaced rollback signal + classifies (S2) and routes `NeedsForkChoice` (Participant) → orchestrator → apply driver (S3); `SingleProducer` keeps the `DC-NODE-20` fail-closed. **The routing + the pending-reselection state + the forge gate (`DC-NODE-28`) land together** (irreducible — splitting them would leave live fork-choice without the producer-race fence): while a re-selection is pending, forging refuses (typed `ForgeRefused`) — never forges on the stale pre-resolution tip. The rollback point comes from the peer's chain-sync `RollBackward`, surfaced by AI-S4a (single-best-peer). The go-live behavior flip.
...
- **AI-S4b-ii** live fork-choice routing + forge gate (the flip) — CE-AI-3 (live) + CE-AI-4 (+ CE-AI-2 live). **RED** (`node_lifecycle`/`node_sync`).
