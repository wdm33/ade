...
> 11 crates, 452 canonical types, 2070 tests, 105 CI checks at HEAD (`67d1ccc`, post-PHASE4-N-Z close).
...
5. The invariant registry `docs/ade-invariant-registry.toml` (299 rules) — each rule's `code_locus` + `statement` is the authoritative source for the **MUST NOT** rows below.

### Active cluster at HEAD

**PHASE4-N-Z — CLOSED (Mithril production-bootstrap wiring + seed-point independence).** HEAD `67d1ccc` is the close commit; the cluster doc + S1 slice doc are archived under `docs/clusters/completed/PHASE4-N-Z/`. No cluster directory lives outside `completed/` at this HEAD. The cluster's **primary invariant** (DC-MITHRIL-02) is seed-point independence: for a Mithril bootstrap the `BootstrapAnchor` `seed_point` MUST originate from the operator-provided independent seed-point inputs, **never** from the Mithril manifest — so that `verify_mithril_binding` compares two structurally independent origins and can actually fail closed on a forged/wrong-point snapshot. It is a one-slice cluster (S1) that ships the wired RED production composition `bootstrap_from_mithril_snapshot`, mirroring `genesis_bootstrap` in shape: a composition-only shell with a closed error surface and no new authority, routing initial state through the single `bootstrap_initial_state` authority (CN-NODE-01) and running the BLUE `verify_mithril_binding` cross-check **before** any storage initializes.

The cluster introduces / extends:
- **RED** `ade_runtime::mithril_bootstrap` (NEW) — `bootstrap_from_mithril_snapshot<D, S>` mints the anchor `seed_point` from the operator-provided `MithrilSeedPointInputs` (origin independent of the manifest), populates `seed_provenance` from the imported manifest, runs `verify_mithril_binding` fail-closed **before** `bootstrap_initial_state`, and returns `MithrilBootstrapOutput`. Closed error surface `MithrilBootstrapError` = `Import(MithrilManifestError)` / `Binding(MithrilImportError)` / `Bootstrap(BootstrapError)`. These are RED composition types — not BLUE canonical types — so the canonical-type count is unchanged.
- **NEW CI gate** `ci_check_mithril_seed_point_independence.sh` (DC-MITHRIL-02) — a data-flow-resistant containment gate: the anchor `seed_point` may never be sourced from the manifest, and the import's point-bearing fields may never be drilled into or laundered (via a local binding or mutate-before-mint) into the anchor; the manifest import may be referenced only as whole values (`import.provenance → seed_provenance`; `&import.report →` the verify call). Hardened against laundering in `bccec39` per the N-Z security review BLOCK.
- **Registry:** 298 → **299**. NEW `DC-MITHRIL-02` (enforced). `CN-MITHRIL-01` `strengthened_in += "PHASE4-N-Z"` (its `code_locus` now also cites `mithril_bootstrap.rs` as the production wiring site). `RO-MITHRIL-IMPORT-01` `strengthened_in += "PHASE4-N-Z"` — item (b) (the wired production composition) CLOSED; status stays `partial` for the remaining honest-scope items (a) seed-bytes-from-Mithril decode and (c) a committed reproducible Mithril fixture + CI/release evidence (`blocked_until_mithril_seed_bytes_and_fixture`).

**Carried-forward (PHASE4-N-Y, still load-bearing below).** The prior cluster shipped, all still inventoried in the module entries: **BLUE** `ade_ledger::bootstrap_anchor` (the `SeedProvenance` enum + `ANCHOR_SCHEMA_VERSION = 2` + the `bootstrap_anchor::binding` `verify_mithril_binding` predicate / `MithrilImportError` / `MithrilManifestReport`), **BLUE** `ade_ledger::genesis_source` (`genesis_initial_state` + `GenesisSourceError::NonConwayEra`), **GREEN** `ade_runtime::forward_sync::reducer` + **RED** `ade_runtime::forward_sync::pump`, **RED** `ade_runtime::{mithril_import, genesis_bootstrap, recovery::restart}`, and **GREEN** `ade_testkit::harness::sync_diff`. The N-Z `mithril_bootstrap` composition consumes N-Y's BLUE `verify_mithril_binding` + RED `mithril_import` importer.

**Governance note.** N-Z preserves the load-bearing structural decisions: the **single `bootstrap_initial_state` authority** is upheld — `mithril_bootstrap` (like `genesis_bootstrap`) routes through it, never a parallel storage-init path (CN-NODE-01); the Mithril binding decision stays in the BLUE `verify_mithril_binding` predicate (the RED composition decides nothing semantic); and the two-origin independence of the binding comparison is now mechanically protected by the data-flow-resistant containment gate (DC-MITHRIL-02).

**Cluster-doc location.** Every closed cluster doc is archived under `docs/clusters/completed/` — including the entire **PHASE4-N-Q / N-R-\* / N-S-\*** set, the **N-M-\*** (admission/seed/WAL/anchor) sub-trees, **N-O**, **N-P**, **N-T**, **N-V**, **N-W**, **N-X**, **N-Y**, and now **N-Z**. There is no cluster directory living outside `completed/` at this HEAD.

This regeneration is a **full inventory at HEAD `67d1ccc`**. The N-M-\* admission/seed/WAL/anchor sub-trees, the N-Q/N-R/N-S producer/network/node sub-trees, the N-W Praos-VRF authority, the N-X tag-24 wire-envelope authority, the N-Y Mithril-bootstrap / forward-sync / recovery surfaces, and the N-Z Mithril production-bootstrap composition are all inventoried below.
...
| Canonical types | **452** | `grep -rhE "^(pub )?(struct\|enum) " --include='*.rs'` over the 6 BLUE crate `src/` trees + the 9 BLUE `ade_network` submodule paths, summed. Breakdown: `ade_codec` 11, `ade_types` 81, `ade_crypto` 21, `ade_core` 49, `ade_ledger` **173**, `ade_plutus` 8 (= 343) + `ade_network` BLUE submodules 109 (`mux/frame.rs` 5, `codec/` 38, `handshake/` 9, `chain_sync/` 11, `block_fetch/` 10, `tx_submission/` 5, `keep_alive/` 5, `peer_sharing/` 5, `n2c/` 21). `canonical_type_registry: null`, so the structural grep is authoritative. **Δ vs prior CODEMAP (452): 0** — N-Z added the RED `mithril_bootstrap` types (`MithrilSeedPointInputs`, `MithrilBootstrapError`, `MithrilBootstrapOutput`), which live in the RED `ade_runtime` shell and are not canonical-counted. |
| Tests | **2070** | `grep -rEc "#\[test\]\|#\[tokio::test\]" --include='*.rs' crates/`, summed (`awk` over the `path:count` lines). Approximate per the template fallback rule (count of attributes, not a runner enumeration). **Δ vs prior (2067): +3** — the three N-Z `mithril_bootstrap` tests (`mithril_bootstrap_fails_closed_on_seed_point_mismatch`, `mithril_bootstrap_verifies_before_storage_init`, `mithril_bootstrap_succeeds_when_seed_point_matches`). |
| CI checks | **105** | `ls ci/ci_check_*.sh \| wc -l`. CI scripts live under `ci/` (`ci_dirs = ["ci"]`). A `.github/workflows/` dir exists (`notify-atlas.yml`) but it is a grounding-doc→atlas-rebuild dispatch workflow, **not** an Ade invariant gate, so it is not counted as a CI check. **Δ vs prior (104): +1** — `ci_check_mithril_seed_point_independence.sh` (N-Z, DC-MITHRIL-02): a data-flow-resistant containment gate — the anchor `seed_point` is never sourced from the Mithril manifest; the import's point-bearing fields may never be drilled into or laundered into the anchor. |
| Registry rules | **299** | `grep -cE "^id = " docs/ade-invariant-registry.toml`. Reference only — not a header count. **Δ vs prior (298): +1** — NEW `DC-MITHRIL-02` (enforced); `CN-MITHRIL-01` + `RO-MITHRIL-IMPORT-01` `strengthened_in += "PHASE4-N-Z"` (RO-MITHRIL-IMPORT-01 item (b) closed, status stays `partial`). |
...
| Attribute | Value |
|-----------|-------|
| **Purpose** | The imperative shell — moves bytes, owns sockets/files/clocks/keys, drives tokio tasks. Hosts: producer-mode key custody + shell composition (`producer/`), the N2N network drivers (`network/`), the node orchestrator runners (`orchestrator/`), the receive/rollback/admission shells, the seed-import/WAL/consensus-inputs importers, ChainDB + recovery shells, the N-Y bootstrap/forward-sync surfaces: `forward_sync::pump` (RED driver), `mithril_import` (RED manifest shell), `genesis_bootstrap` (RED Conway-genesis entry), `recovery/restart` (RED crash-recovery wiring); and the N-Z `mithril_bootstrap` (RED Mithril production-bootstrap composition — composition-only, no new authority). |
| **Creates (RED-only)** | `KesSecret`, `VrfSigningKey`, `ColdSigningKey` custody wrappers; `KeyLoadError`; `MuxTransportHandle` consumers; `OutboundCommand` (closed enum, N-S-B); `PerPeerOutbound` map; `DispatchError`; `ServedChainHandle`; `GenesisAnchor`. **N-Y:** `forward_sync::pump::{PumpError (incl. TipBeforeDurable), PumpTip, SnapshotSink trait, NoCheckpointSink}`; `mithril_import::{RawMithrilManifest, RawCertifiedPoint, RawImmutableRange, MithrilManifestError, MithrilProvenanceImport}`; `recovery::restart::{NodeRecoveryError (incl. WalTailFingerprintMismatch), RecoveredNode}`. **N-Z:** `mithril_bootstrap::{MithrilSeedPointInputs, MithrilBootstrapOutput, MithrilBootstrapError (closed: Import / Binding / Bootstrap)}`. Never semantic/canonical types. |
...
> Earlier-cluster scripts (N-A through N-Y) are present and counted in the 105 total. The per-script enforce/scope detail is in the registry's `ci_script` fields per rule.

---

## Generation notes

- Regenerated full at HEAD `67d1ccc` (`git rev-parse --short HEAD`). PHASE4-N-Z (Mithril production-bootstrap wiring + seed-point independence) is CLOSED at commit `67d1ccc`; the cluster doc + S1 slice doc are archived under `docs/clusters/completed/PHASE4-N-Z/`. The N-Z close span over the prior CODEMAP HEAD `5db9aae`: `588a554` (re-reproduced grounding docs at `5db9aae`), `9b4177f` (scope), `f2b1562` (S1 implementation + gate), `bccec39` (gate hardening per review BLOCK), `c876022` (registry close), `67d1ccc` (close commit).
...
- All mechanical counts recomputed fresh from the tree (not copied from the prior CODEMAP): 11 crates (Δ 0), 452 canonical types (Δ 0 — N-Z added only RED `mithril_bootstrap` types, which are not canonical-counted), 2070 tests (**Δ +3** — the three N-Z `mithril_bootstrap` tests), 105 CI checks (**Δ +1**: `ci_check_mithril_seed_point_independence.sh`), 299 registry rules (**Δ +1**: NEW `DC-MITHRIL-02`; `CN-MITHRIL-01` + `RO-MITHRIL-IMPORT-01` strengthened).
...
- `.idd-config.json` `_ci_dirs_doc` still reads "No .github/workflows in this repo yet" — that note is **stale**; the workflow that exists (`notify-atlas.yml`) is a non-gate atlas-rebuild dispatch. The config note should be updated (or `.github/workflows` added to `ci_dirs`) only if/when an actual invariant gate is added there; the 105 CI-check count is unaffected. (`.idd-config.json` `_invariant_registry_doc` also still reads "298 entries" — likewise now stale at 299.)
- TCB color for every module was verified against the on-disk `// Core Contract:` / `//!` banner: the N-Z `mithril_bootstrap` opens `//! RED Mithril-snapshot bootstrap entry` and lives under the RED `crates/ade_runtime/` — a composition-only RED shell, not a canonical-type author. Carried-forward N-Y colors unchanged: `bootstrap_anchor::binding` + `genesis_source` BLUE under `crates/ade_ledger/`; `forward_sync::reducer` GREEN-by-content / `forward_sync::pump` RED (both under `crates/ade_runtime/`); `mithril_import`, `genesis_bootstrap`, `recovery::restart` RED; `harness::sync_diff` GREEN (testkit).
