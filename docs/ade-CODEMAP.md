...
> 11 crates, 445 canonical types, 2016 tests, 98 CI checks at HEAD (`01e7e08`, PHASE4-N-W closing).
...
**PHASE4-N-W — closing now.** This is the producer-side Praos VRF authority migration: the producer's leader-eligibility VRF construction is unified with the Conway/Praos *validator* authority through a single era→construction surface. It introduces the closed `ExpectedVrfInput` enum + the `leader_vrf_input(era, slot, eta0)` / `leader_value_for(input, output)` authorities in `ade_core::consensus::vrf_cert`, retypes `LeaderScheduleAnswer.expected_vrf_input` from a bare `[u8; VRF_INPUT_LEN]` to `ExpectedVrfInput`, adds an `era` parameter (with era-correct threshold) to `leader_check::verify_and_evaluate_leader`, adds `CardanoEra::is_praos()`, and fail-closes the RED `produce_mode` forge on a non-Praos era (`ForgeFailureReason::UnsupportedProducerEra`). It flips registry rule **CN-FORGE-04** declared→enforced and ships one new CI gate (`ci_check_producer_praos_vrf.sh`).

**Cluster-doc location (corrected).** As of the registry/doc hygiene pass, every closed cluster doc is archived under `docs/clusters/completed/` — including the entire **PHASE4-N-Q / N-R-A / N-R-B / N-R-C / N-S-A / N-S-B / N-S-C** set, plus the **N-M-\*** (admission/seed/WAL/anchor) sub-trees, **N-O**, **N-P**, **N-T**, and **N-V**. The **only** cluster directory living outside `completed/` is the now-closing **PHASE4-N-W**. (The prior CODEMAP's claim that the N-Q/N-R/N-S docs "remain under `docs/clusters/` (not yet archived to `completed/`)" is stale and has been corrected.)

This regeneration is a **full inventory at HEAD `01e7e08`**. The N-M-* admission/seed/WAL/anchor sub-trees, the N-Q/N-R/N-S producer/network/node sub-trees, and the N-W Praos-VRF authority surface are all inventoried below.
...
| Canonical types | **445** | `grep -rhE "^(pub )?(struct\|enum) " --include='*.rs'` over the 6 BLUE crate `src/` trees + the 9 BLUE `ade_network` submodule paths. Breakdown: `ade_codec` 10, `ade_types` 81, `ade_crypto` 21, `ade_core` 49, `ade_ledger` 167, `ade_plutus` 8 (= 336) + `ade_network` BLUE submodules 109 (`mux/frame.rs` 5, `codec/` 38, `handshake/` 9, `chain_sync/` 11, `block_fetch/` 10, `tx_submission/` 5, `keep_alive/` 5, `peer_sharing/` 5, `n2c/` 21). `canonical_type_registry: null`, so the structural grep is authoritative. **Δ vs prior CODEMAP (444): +1 in `ade_core` (48→49) from the new closed `ExpectedVrfInput` enum (N-W).** |
| Tests | **2016** | `grep -rEc "#\[test\]\|#\[tokio::test\]" --include='*.rs' crates/`, summed. Approximate per the template fallback rule (count of attributes, not a runner enumeration). Per-crate: `ade_codec` 164, `ade_types` 24, `ade_crypto` 86, `ade_core` 135, `ade_ledger` 600, `ade_plutus` 28, `ade_testkit` 312, `ade_runtime` 330, `ade_network` 237, `ade_node` 73, `ade_core_interop` 27. **Δ vs prior (2013): +3 (ade_types 23→24, ade_core 134→135, ade_node 72→73) — the N-W is_praos / VRF-authority / era-guard tests.** |
| CI checks | **98** | `ls ci/ci_check_*.sh \| wc -l`. No `.github/workflows/` in this repo; `ci_dirs = ["ci"]`. **Δ vs prior (97): +1 — `ci_check_producer_praos_vrf.sh` (N-W, CN-FORGE-04).** |
| Registry rules | **291** | `grep -cE "^id = " docs/ade-invariant-registry.toml`. Reference only — not a header count. Unchanged from prior CODEMAP; N-W flipped **CN-FORGE-04** declared→enforced (no new rule id). |
...
| Attribute | Value |
|-----------|-------|
| **Purpose** | Owns Cardano-canonical CBOR ingress — the only place in the workspace that turns raw bytes into typed semantic values, with wire-byte preservation for every hash-bearing structure. Owns the standalone opcert byte authority and the canonical Conway-tx preserved-byte splitter. Owns the canonical block-envelope **encoder** `cbor::envelope::encode_block_envelope` (N-V), symmetric to the long-standing `decode_block_envelope`. |
...
| **Entry points** | `use ade_core::consensus::{…}` aggregator; top-level transitions `validate_and_apply_header`, `select_best_chain`, `apply_rollback`, `apply_nonce_input`, `apply_op_cert`, `query_leader_schedule`, `verify_vrf_cert`, `tiebreaker_prefer`; `ade_core::consensus::opcert_validate::opcert_validate`; `ade_core::consensus::leader_check::{verify_and_evaluate_leader, LeaderCheckVerdict, is_leader_for_vrf_output}`; **NEW** `ade_core::consensus::vrf_cert::{ExpectedVrfInput, leader_vrf_input, leader_value_for, praos_vrf_input, praos_leader_value}`. |
...
| `producer::producer_log` *(N-Q, extended N-W)* | Closed-vocabulary producer evidence log — `ProducerLogEvent` is the closed enum emitted on every observable producer-mode transition; the closed `ForgeFailureReason` enum now carries the **NEW (N-W)** `UnsupportedProducerEra` variant for a non-Praos forge attempt. | Add an open/wildcard event variant; emit `BlockServed` for a block not in the served snapshot (CN-PROD-04). | `ci_check_operator_evidence_manifest_schema.sh` (evidence schema) |
...
Closed semantic surfaces detected at HEAD: `AcceptedMiniProtocol` (mini-protocol registry, `ade_network::session`), `ExpectedVrfInput` (2-variant era-discriminated leader-VRF-input authority, `ade_core::consensus::vrf_cert`, N-W), `LeaderCheckVerdict` (2-variant, `ade_core::consensus::leader_check`), `OutboundCommand` (typed relay, `ade_runtime::network::outbound_command`), `DispatchError`, `ProducerLogEvent` + `ForgeFailureReason` (closed evidence vocabulary — `ForgeFailureReason` gained `UnsupportedProducerEra` in N-W), `ChainEvolutionError`, `ServedChainAdmitError`, `KesError` / `KesParseError`, the admission/wire-only/live-log JSONL event vocabularies, the operator-evidence manifest TOML schema, the closed `CardanoEra` (with `is_praos()` classifier) / Conway cert + governance enums.

### CI enforcement (98 scripts under `ci/`)

The full list is mechanically obtainable via `ls ci/ci_check_*.sh` (98 at HEAD). New / load-bearing since the prior CODEMAP HEAD `d6f3399` (PHASE4-N-Q → N-W) — for the upcoming SEAMS / HEAD_DELTAS / TRACEABILITY regens:

| Script | Enforces | Cluster |
|---|---|---|
| `ci_check_producer_praos_vrf.sh` | **NEW** CN-FORGE-04 — single era→leader-VRF-input authority (`leader_vrf_input` defined exactly once); no TPraos/Praos fallback for one era; producer alpha = validator alpha for Praos; closed `ExpectedVrfInput`. | N-W |
...
> Earlier-cluster scripts (N-A through N-P, the N-M-* admission/seed/WAL/anchor set, the N-L wire-session set) are present and counted in the 98 total. The per-script enforce/scope detail for those is in the registry's `ci_script` fields per rule.

---

## Generation notes

- Regenerated full at HEAD `01e7e08` (`git rev-parse --short HEAD`). PHASE4-N-W (producer Praos VRF authority migration) is the cluster closing now.
- **Cluster-doc location was corrected this regen:** after the registry/doc hygiene pass, the N-Q/N-R/N-S set (and N-M-* / N-O / N-P / N-T / N-V) are all under `docs/clusters/completed/`; the only cluster directory outside `completed/` is the closing `PHASE4-N-W`. The prior CODEMAP's "N-Q/N-R/N-S docs remain under `docs/clusters/`" note was stale.
- All mechanical counts recomputed fresh from the tree (not copied): 11 crates, 445 canonical types (+1 from `ExpectedVrfInput`), 2016 tests (+3), 98 CI checks (+`ci_check_producer_praos_vrf.sh`), 291 registry rules (unchanged; CN-FORGE-04 flipped declared→enforced).
...
- TCB color for every new/changed module was verified against the on-disk `// Core Contract:` / `//!` banner, not inferred from the cluster doc alone. The new `ExpectedVrfInput` / `leader_vrf_input` / `leader_value_for` surface resolves BLUE under the `crates/ade_core/` `core_paths` prefix.
