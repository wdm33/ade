...
>
> **HEAD:** `67d1ccc` · **Generated:** 2026-05-29 · **Baseline:** `3b78008` (`.idd-config.json` `head_deltas_baseline`)
...
- **[not found on disk — drift]** (tests) / **[not on disk — drift]** (CI) flags a registry-named test/CI that could not be located in the working tree at HEAD `67d1ccc` — a registry↔code coherence gap to reconcile (rename the registry entry or restore the artifact). Test drift is resolved against the **leaf** function name (the segment after the last `::`).
...
By tier: constraint 1 · derived 182 · operational 16 · release 39 · true 61.

By family: T 29 · DC 139 · CN 107 · RO 15 · OP 9.
...
_tier: release · status: enforced · strengthened: PHASE4-N-M-A, PHASE4-N-Y_
...
_tier: release · status: enforced · strengthened: PHASE4-N-K, PHASE4-N-M-B, PHASE4-N-T, PHASE4-N-Y_
...
Regenerated from the registry (`docs/ade-invariant-registry.toml`, 299 rules) as primary source; tests and CI scripts cross-checked against the working tree at HEAD `67d1ccc` using **grep/ls only** — no `cargo build`/`cargo test`/`cargo check` was run. Test existence is verified against the leaf function name (segment after the last `::`) over all `fn` definitions under `crates/**.rs`; CI existence is verified by basename against `ci/ci_check_*.sh`. Both the `ci_script` (comma-joined string) and `ci_scripts` (array) registry fields are read for the **CI** row. The registry is parsed with a TOML parser, so multi-line array and triple-quoted-string fields are read correctly (the prior pass fixed a multi-line-array parsing bug; this pass keeps that — every `tests = [` array, single- or multi-line, resolves).

HEAD `67d1ccc` is the **PHASE4-N-Z close** (Mithril production-bootstrap wiring + seed-point independence). Delta vs the `5db9aae` pass: **+1 rule** (now 299), the new **enforced** DC-MITHRIL-02 (Mithril-bootstrap seed-point independence; Code `mithril_bootstrap.rs` + the containment gate; 3 tests; CI `ci_check_mithril_seed_point_independence.sh`). CN-MITHRIL-01 is `strengthened_in += PHASE4-N-Z` (now also cites `mithril_bootstrap.rs` + the new gate). RO-MITHRIL-IMPORT-01 is `strengthened_in += PHASE4-N-Z` (open-obligation item (b) closed; stays `partial` — items (a) seed-bytes and (c) fixture/live evidence remain). Status breakdown moves from 166/19/113 to **167 enforced / 19 partial / 113 declared / 0 deprecated**. One new CI gate on disk (`ci_check_mithril_seed_point_independence.sh`) raises the `ci_check_*.sh` count to **105**.

At this HEAD: **no test drift and no CI drift** — every registry-named test (across all `tests` arrays) resolves to an on-disk `fn`, and every registry-named `ci_script`/`ci_scripts` entry resolves to an on-disk gate. The `_(… — gap)_` cells and the **Honest enforcement gaps** section are the actionable output — reconcile them in the registry or the codebase; do not delete them. Regenerate after every cluster close.
