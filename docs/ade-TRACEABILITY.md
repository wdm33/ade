...
>
> **HEAD:** `999199f8` (post-N-U-close gate-hygiene span — `fix(ci): repair 10 pre-existing gate-vs-code drifts (gate hygiene; 0 invariants weakened)`) · **Generated:** 2026-06-05 · **Prior pass:** `4e358e92` (PHASE4-N-U close, 333 rules)
...
The canonical source is the **invariant registry** at `docs/ade-invariant-registry.toml` (`.idd-config.json` `invariant_registry`), **333 rules** at HEAD `4e358e92` (`+5` this span: the five NEW PHASE4-N-U rules `DC-NODE-12`, `DC-CONS-23`, `DC-WAL-04` (S1), `T-REC-05` (S2), `DC-NODE-13` (S3), all introduced `enforced`; `+2` `strengthened_in += "PHASE4-N-U"` appends — `CN-CONS-07` (serve-provenance clause) and `DC-NODE-11` (serve mechanism superseded by serve-as-projection); `0` rules removed or deprecated). **This delta-refresh (`4e358e92` → `999199f8`, post-N-U-close gate-hygiene span: commits `60deecf3`, `e92b40b7`, `999199f8`) is a near-no-op on the rule set: `0` rules added, `0` removed, the registry count is UNCHANGED at 333 (201 enforced). The ONE material rule delta is `DC-NODE-06`, which `60deecf3` reconciled** (its handoff-fence gate was STRANDED by the N-U close — see its entry) — re-traced from scratch this pass (`strengthened_in += PHASE4-N-U`; Source → Code → Tests → CI brought to the registry). The current in-tree `DC-NODE-06` row had drifted: it cited the renamed test `relay_loop_containment_semantics_unchanged_with_serve_sibling` (now `…_after_serve_sibling_retired` on disk) and omitted the durable-projection serve. That drift is now reconciled. This doc is a join: registry entries × codebase introspection. Rule IDs, families, names, sources, and the registry-recorded `tests` / `ci_script` / `code_locus` fields come from the registry; each test name and CI script is cross-checked for existence in the working tree at `4e358e92`, and divergences are flagged inline as **drift**. (Verification this pass is grep/ls only — no build or test execution. This is a **fast delta-refresh** — only the seven PHASE4-N-U rules (five new + two strengthened) were re-traced from scratch; every other rule's trace row is preserved verbatim from the prior pass. The PHASE4-N-U leaves were all located on disk: all **12 unique cited test leaf-fns** across the five new rules resolve (`fn <name>` present) — `forge_tick_durable_admit_advances_tip` / `forge_successor_builds_block_1_from_durable_tip` / `forged_admit_bytes_byte_identical_to_self_accept` / `stale_tip_forge_fails_closed` / `forged_admit_wal_prior_fp_chains` in `ade_node/tests/forge_succeeds.rs`; `warm_start_drops_orphan_block_above_wal_tail` + `forge_kill_then_warm_start_recovers_same_tip_via_forward_replay` inline-tested in `ade_node/src/{node_lifecycle.rs,node_sync.rs}`; `served_view_projects_durable_chain` / `follower_fetches_coherent_history_incl_ingested_predecessor` / `served_view_retires_accumulator` in `ade_node/tests/node_spine_serve_loopback.rs`. All cited CI gates are present in `ci/` — `ci_check_forged_durable_admit_via_pump.sh` (S1, NEW), `ci_check_node_run_loop_containment.sh` (extended in place), `ci_check_served_chain_projection.sh` (S3, NEW) — and the RETIRED G-R gate `ci_check_served_chain_stability.sh` is confirmed **absent** (mechanism superseded). `T-REC-05` carries `ci_script = ""` by design — **test-enforced, NOT an enforcement gap** (see its entry). HONEST DRIFT recorded: the PHASE4-N-U `cluster.md §8` named a CE-5 gate `ci_check_forged_tip_recovery.sh` and a CE-6 test `forge_two_clean_runs_byte_identical` that were **not created literally**; S2 enforced replay-equivalence via the test-only `T-REC-05` path — see Generation notes.)
...
By tier: constraint 4 · constraint_network 2 · derived 205 · operational 17 · release 41 · true 64.

By family: T 32 · DC 158 · CN 118 · RO 16 · OP 9.
...
_cross-refs (carried unchanged — the cap is an additive bound on the reassembly contract; no existing rule weakened or re-joined): `CN-SESS-04`, `DC-SESS-06`, `DC-SYNC-01`, `DC-SYNC-02`, `DC-NODE-06`, `CN-NODE-02`._
