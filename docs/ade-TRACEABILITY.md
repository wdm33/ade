...
>
> **HEAD:** `0887b2ad` (slice `TXSUB2-CODEC-REALWIRE` — the tx-submission2 real-wire codec close). **THE HEADLINE: `DC-PROTO-02` (transcript-equivalent mini-protocol behavior with the Haskell node) FLIPPED `declared → enforced`, and a NEW rule `DC-PROTO-11` (tx-submission2 real-wire codec fidelity) was added `enforced`.** A live server-side capture had the docker public-preprod `cardano-node 11.0.1` dial Ade as a `localRoots` peer and complete the full tx-submission2 exchange (`Init → RequestTxIds → ReplyTxIds → RequestTxs → ReplyTxs`); the node-originated `MsgReplyTxIds` (era-tagged Conway txid `[6, hash32]` inside a CBOR **indefinite-length** array) and `MsgReplyTxs` (era-wrapped tx body) had previously been **false-rejected** by Ade's codec ("indefinite-length array not allowed") — a real Cardano incompatibility the synthetic round-trip tests missed. `DC-PROTO-11`'s codec fix (accept definite *and* indefinite sequences; re-emit the indefinite form so a captured frame re-encodes byte-identically; preserve the era tag) closes the last mini-protocol surface so **all 11 N2N+N2C mini-protocol surfaces now have real-capture byte-identical round-trips**, which is what backs the `DC-PROTO-02` flip. Both rules are gated by `ci/ci_check_tx_submission2_real_capture.sh`. · **This is a delta-refresh** over the prior pass at `862cd2cb` (PHASE4-N-AO close): every rule's four-cell row was regenerated from the registry joined with on-disk introspection (`git grep` / `ls` only; no build or test execution), so the in-span declared→enforced flips that also landed before this HEAD are reflected verbatim from the registry: `DC-CORE-01`, `CN-WIRE-07`, `CN-CONS-04`, `DC-PROTO-01`, `DC-PROTO-03`, `DC-PROTO-04`, `DC-PROTO-06`, `CN-PLUTUS-04`, `CN-PLUTUS-01` (10 declared→enforced flips total across the span per `HEAD_DELTAS` §7, plus the `+1` new `DC-PROTO-11`). · **Generated:** by `/traceability` at HEAD `0887b2ad` · **Registry:** `docs/ade-invariant-registry.toml` @ **373 rules** (250 enforced / 1 enforced_scaffolding / 19 partial / 103 declared / 0 deprecated) · **CODEMAP:** `docs/ade-CODEMAP.md` is **pinned one window back at `862cd2cb`** (462 canonical types · 173 CI · 372 rules) — it predates this slice and does NOT yet carry `DC-PROTO-11` or the tx-submission2 gate; this is a known, intentional one-window lag (informational, NOT a TRACEABILITY drift — see Cross-references) · **HEAD_DELTAS:** `docs/ade-HEAD_DELTAS.md` is **current at `0887b2ad`** (§0 HEAD `0887b2ad`; §5 CI **173 → 183**, +10 new; §7 rules **372 → 373**, +1 rule + 10 declared→enforced flips) and catalogs the tx-submission2 gate + `DC-PROTO-11`.
...
The canonical source is the **invariant registry** at `docs/ade-invariant-registry.toml` (`.idd-config.json` `invariant_registry`), **373 rules** at HEAD `0887b2ad` (250 enforced / 1 enforced_scaffolding / 19 partial / 103 declared / 0 deprecated). TRACEABILITY is a join: every registry `[[rules]]` entry becomes one rule sub-section; **Source** / **Requirement** are lifted from the registry `source` / `statement`; **Code** / **Tests** / **CI** are populated from the registry `code_locus` / `tests` / `ci_script`(`s`) and then **verified against the working tree** (every named test function and CI script is checked for on-disk existence; misses are flagged inline as drift). The registry↔TRACEABILITY join is **373 ↔ 373** (1:1, verified by rule-heading count == registry `[[rules]]` count, family-for-family: T 33 · DC 196 · CN 119 · RO 16 · OP 9). **No MUST/SHALL candidate rules were found in the `.idd-config.json` `normative_docs`** (the gate-statement / contract docs carry no numbered normative MUST clauses; all rules live in the registry) — so there are no un-registered candidates to surface this pass.

This pass's substantive delta (`862cd2cb` → `0887b2ad`):
...
- **`_[not found on disk — drift]_`** (tests) / **`_[not on disk — drift]_`** (CI) flags a registry-named test/CI that could not be located in the working tree at `0887b2ad`. **This pass: exactly ONE such test-name drift** — `DC-NODE-18` names `extend_own_spine_promotion_requires_adoption_certificate`, which has no `fn` on disk (the adoption-certificate mechanism `DC-NODE-18` introduced was superseded by `DC-NODE-20` and the cert removed by `DC-NODE-21`; the other 5 tests in `DC-NODE-18`'s array resolve). Recorded honestly in that row, not hidden. **Zero CI drift** — every CI script any rule names exists on disk (175 distinct rule-referenced gates, all present).
...
By tier: true 65 · derived 244 · operational 17 · release 41 · constraint 4 · constraint_network 2.

By family: T 33 · DC 196 · CN 119 · RO 16 · OP 9. _(registry-exact; the doc body has exactly 373 rule headings matching the registry family-for-family.)_

_Delta (`862cd2cb` → `0887b2ad`, slice `TXSUB2-CODEC-REALWIRE`): `+1` rule (372 → 373, `DC-PROTO-11`, `enforced`). **`DC-PROTO-02` FLIPPED `declared → enforced`** (the headline) plus 9 further in-span declared→enforced flips reflected from the registry (10 flips total per `HEAD_DELTAS` §7). CI: `173 → 183` on disk (+10). Registry↔TRACEABILITY join: 373 ↔ 373 (1:1). `0` rules removed / deprecated / weakened. `0` `RO-*` flipped._
...
### `0887b2ad` pass (slice `TXSUB2-CODEC-REALWIRE`) — current-pass coherence
