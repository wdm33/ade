...
>
> **HEAD:** `3b78008` · **Generated:** 2026-05-29 · **Baseline:** `273c887` (`.idd-config.json` `head_deltas_baseline`)
...
- **[not found on disk — drift]** (tests) / **[not on disk — drift]** (CI) flags a registry-named test/CI that could not be located in the working tree at HEAD `3b78008` — a registry↔code coherence gap to reconcile (rename the registry entry or restore the artifact). Test drift is resolved against the **leaf** function name (the segment after the last `::`).
...
By tier: true 61 · derived 181 · release 39 · operational 16.
...
CODEMAP was regenerated at this doc's HEAD `3b78008` (the PHASE4-N-Y close): 11 crates, 452 canonical types, 2067 tests, 103 CI checks. Every crate named in a **Code** row is present in CODEMAP: `ade_codec`, `ade_core`, `ade_core_interop`, `ade_crypto`, `ade_ledger`, `ade_network`, `ade_node`, `ade_plutus`, `ade_runtime`, `ade_testkit`, `ade_types`. No crate-level CODEMAP mismatch detected. The PHASE4-N-Y modules referenced by the new **Code** rows — `crates/ade_ledger/src/bootstrap_anchor/binding.rs` (CN-MITHRIL-01 / DC-MITHRIL-01 / RO-MITHRIL-IMPORT-01), `crates/ade_runtime/src/mithril_import/` (CN-MITHRIL-01 / RO-MITHRIL-IMPORT-01), `crates/ade_runtime/src/forward_sync/{reducer,pump}.rs` + `crates/ade_runtime/src/recovery/restart.rs` (DC-SYNC-01), `crates/ade_ledger/src/genesis_source.rs` + `crates/ade_runtime/src/genesis_bootstrap.rs` (DC-GENESIS-SRC-01), and `crates/ade_testkit/src/harness/sync_diff.rs` (DC-COMPAT-01 / RO-SYNC-EVIDENCE-01) — all live inside already-mapped crates and are catalogued at the N-Y CODEMAP regeneration. No staleness gap this pass.

### HEAD_DELTAS (`docs/ade-HEAD_DELTAS.md` §5 @ `3b78008`)

HEAD_DELTAS was regenerated at HEAD `3b78008` over the `273c887 → 3b78008` span (the PHASE4-N-Y close). All four PHASE4-N-Y CI gates that appear in **CI** rows above are catalogued in HEAD_DELTAS §5:

- `ci_check_mithril_uses_bootstrap_initial_state.sh` — bound to CN-MITHRIL-01, DC-MITHRIL-01, DC-GENESIS-SRC-01, RO-MITHRIL-IMPORT-01.
- `ci_check_forward_sync_chokepoint_only.sh` — bound to DC-SYNC-01.
- `ci_check_no_haskell_fingerprint_equality.sh` — bound to DC-COMPAT-01.
- `ci_check_sync_evidence_manifest_schema.sh` — bound to RO-SYNC-EVIDENCE-01.

**CI gates on disk not bound to any rule CI row** (surfaced, not fixed this pass — pre-existing, NOT introduced by N-Y): `ci_check_module_headers.sh`, `ci_check_no_secrets.sh`, and `ci_check_pallas_quarantine.sh` appear in no registry `ci_script`/`ci_scripts` field — real green gates with no covering rule entry, candidates for a future rule binding. (The prior pass also listed `ci_check_ingress_chokepoints.sh` here; it is now bound to a rule and no longer unreferenced. 100 of 103 on-disk `ci_check_*.sh` gates are now rule-bound.)
...
## Honest enforcement gaps (PHASE4-N-Y and standing)

The four-cell contract is filled for every PHASE4-N-Y `enforced` rule (Code + Tests + CI all present and verified on disk at HEAD `3b78008`). The remaining gaps are recorded here rather than hidden:

- **RO-MITHRIL-IMPORT-01 — `partial` (provenance-only).** What N-Y enforces is *provenance binding*: a Mithril manifest's attested `{network_magic, genesis_hash, certified_point, certificate_hash}` is cross-checked against the independently `--json-seed`-minted `BootstrapAnchor` and fails closed on mismatch (CN-MITHRIL-01 / DC-MITHRIL-01). What it does **not** yet enforce: (a) seed-*bytes*-from-Mithril decode (needs a Mithril artifact-type spike + forward-replay); (b) a wired production composition site with a CI gate asserting the bound anchor's `seed_point` originates from the `--json-seed` UTxO extraction and **not** the Mithril manifest — the **re-tautologization hazard** flagged in the S7 security re-review (if the two compared origins ever collapse to one, the binding compares a value to itself and the check becomes vacuous); (c) a committed reproducible Mithril fixture + release evidence. Status `partial`, `blocked_until_mithril_import_wiring_slice`.

- **RO-SYNC-EVIDENCE-01 — `partial` (vacuous-until-committed).** The schema + sha256 gate (`ci_check_sync_evidence_manifest_schema.sh`) is enforced but **vacuously satisfied** until an operator commits a manifest (it mirrors the CN-OPERATOR-EVIDENCE-01 pattern). The two-Haskell-node private-Conway-testnet snapshot→tip live capture is **operator-witnessed**: `blocked_until_operator_pass_executed` (CE-Y-16). It flips to enforcing content the moment a CE-Y-16 manifest lands; the gate then verifies schema + sha256 against the committed fixture.

- **CE-Y-16 (operator-witnessed live leg).** The snapshot→tip differential sync against two real Haskell nodes is the live evidence backing DC-COMPAT-01 and RO-SYNC-EVIDENCE-01. DC-COMPAT-01 is `enforced` for the *mechanism* (the differential harness `sync_differential_snapshot_to_tip` and the forbidden-fingerprint-equality gate run in CI), but the *witnessed live capture* is an operator pass not yet executed — the same class of gate as the CN-CONS-06 / RO-LIVE-01 operator-pass leg tracked under the operator-pass C1 follow-on.
...
- **Governance note — CE-not-law (mechanism vs. authority).** Several PHASE4-N-Y enforcement points are **cluster-mechanical-acceptance (CE) gates**, not constitutional-law (true-tier) enforcement, and the distinction is load-bearing:
  - `bootstrap_initial_state` is enforced as the *single* genesis→initial-state authority by `ci_check_mithril_uses_bootstrap_initial_state.sh` (a text/structure gate) plus `genesis_path_fp_equals_snapshot_path_fp` (an Ade-vs-Ade fingerprint-equality test). This proves the two Ade construction paths agree; it does **not** prove agreement with cardano-node serialized state — and DC-COMPAT-01 explicitly **forbids** asserting Ade-fingerprint == Haskell-state-hash. The compatibility claim rests on observable surfaces (verdicts, tip hash, query-utxo, transcripts), not internal fingerprint equality.
  - The forward-sync durability ordering (DC-SYNC-01: bytes + WAL durable *before* AdvanceTip) is structurally enforced by a private `AdmitPlan` with a single `durable()` emit site and a RED `TipBeforeDurable` fail-close, backed by the **two-driver** replay test (`forward_sync_replay_two_runs_byte_identical`) and the chokepoint gate. This is a strong mechanical guarantee for the modelled effect sequence; it is *not* a proof that every future call site routes through the reducer — that breadth is a CI text-gate (`ci_check_forward_sync_chokepoint_only.sh`), which catches added bypasses but is a grep, not a type-level impossibility.
  - The `WalEntry` recovery-reconciliation invariant (DC-SYNC-01 S6: a torn `put_block`/wal-append crash cannot incorporate an un-WAL'd orphan) is proven by a single targeted crash-injection test (`recovery_torn_put_block_before_wal_append_drops_orphan`) over the modelled torn-write point. It is a CE-level guarantee at that injection point, not an exhaustive proof across all crash interleavings.

  None of these are regressions or hidden weaknesses — they are the honest boundary between *mechanism enforced now* and *authority claimed*. They are recorded so the next cluster can decide whether to promote any of them from CE-gate to true-tier law.

---

## Generation notes

Regenerated from the registry (`docs/ade-invariant-registry.toml`, 298 rules) as primary source; tests and CI scripts cross-checked against the working tree at HEAD `3b78008` (PHASE4-N-Y close — Mithril-anchored bootstrap, network forward-sync & WAL recovery). The five new PHASE4-N-Y `enforced`/`partial` rules (CN-MITHRIL-01, DC-MITHRIL-01, DC-SYNC-01, DC-GENESIS-SRC-01, DC-COMPAT-01, RO-SYNC-EVIDENCE-01) and RO-MITHRIL-IMPORT-01 (`declared → partial`) were threaded with their Source → Code → Tests → CI cells; all 14 of their associated test functions and all 4 CI gates were verified present on disk. The 14 carry-forward strengthenings (CN-ANCHOR-01, DC-ANCHOR-01, CN-SEED-01, DC-CONS-20, DC-STORE-01/02/03/05, DC-WAL-01/02/03, CN-GENESIS-01, CN-NODE-01, T-DET-01) now carry `PHASE4-N-Y` in their `strengthened:` line. Test drift is resolved against the leaf function name (segment after the last `::`); both the `ci_script` (string) and `ci_scripts` (array) registry fields are read for the CI row. No test or CI drift was detected at this HEAD. Regenerate after every cluster close. The `_(… — gap)_` cells and the **Honest enforcement gaps** section are the actionable output — reconcile them in the registry or the codebase, do not delete them.
