...
>
> **HEAD:** `90791691` (working tree, PHASE4-N-F-G-C close — registry close-pass edits applied) · **Generated:** 2026-06-02 · **Prior pass:** `339cccb1` (PHASE4-N-F-G-B close)
...
The canonical source is the **invariant registry** at `docs/ade-invariant-registry.toml` (`.idd-config.json` `invariant_registry`), **313 rules** at this working tree (unchanged this close — no new rule). This doc is a join: registry entries × codebase introspection. Rule IDs, families, names, sources, and the registry-recorded `tests` / `ci_script` / `ci_scripts` / `code_locus` fields come from the registry; each test name and CI script is cross-checked for existence in the working tree at `90791691`, and divergences are flagged inline as **drift**. (Verification this pass is grep/ls only — no build or test execution; the three rule-bound G-C gates were additionally executed green via `bash`.)
...
- **[not found on disk — drift]** (tests) / **[not on disk — drift]** (CI) flags a registry-named test/CI that could not be located in the working tree at `90791691` — a registry↔code coherence gap to reconcile. This pass: **0** such drifts (every cited test leaf-fn and every cited CI script resolves on disk; the 6 G-C test/CI/loci additions all resolve).
...
### CODEMAP (`docs/ade-CODEMAP.md` @ `90791691`, PHASE4-N-F-G-C close)

CODEMAP was regenerated for the PHASE4-N-F-G-C close at **`90791691`** (working tree, alongside this doc). It records **118** `ci_check_*.sh` gates, **456** canonical types, **2230** tests — the CI count matches the on-disk count at this working tree (`ls ci/ci_check_*.sh | wc -l` = 118). Every crate named in a **Code** row is present in CODEMAP: `ade_codec`, `ade_core`, `ade_core_interop`, `ade_crypto`, `ade_ledger`, `ade_network`, `ade_node`, `ade_plutus`, `ade_runtime`, `ade_testkit`, `ade_types`. No crate-level mismatch. Every `crates/**.rs` path cited in any `code_locus` resolves on disk (the `ci_check_registry_code_locus_exists.sh` invariant) — including the NEW G-C locus `crates/ade_node/src/ba02_pass.rs`.

- **No BLUE crate changed this cluster.** PHASE4-N-F-G-C wires the **MECHANICAL live-feed + evidence scaffolding ONLY** on the `--mode node` spine. All changes land in the RED binary/driver (`ade_node`), CI, and docs. CODEMAP confirms the new/changed loci — **S1** the RED `ade_node::node_lifecycle::spawn_live_wire_pump_source(peer_addrs, network_magic, recovered_tip) -> NodeBlockSource` that builds a LIVE `NodeBlockSource::WirePump` by reusing the closed `ade_runtime::admission::{dial_for_admission, run_admission_wire_pump}` verbatim (`NodeBlockSource` stays the closed 2-variant `{WirePump, InMemory}` — a fill, not a new variant); **S2** the NEW RED module `ade_node::ba02_pass` (`correlate_peer_log_file` + `write_ba02_manifest`, accepting ONLY a `Ba02Manifest`) — the GREEN `ade_node::ba02_evidence` is unchanged (`correlate` stays the sole `Ba02Manifest` ctor). The canonical-type total holds at **456** (no BLUE change; `ba02_pass` is RED I/O over existing GREEN/BLUE types).
- **CI count is aligned:** CODEMAP's 118 matches the on-disk `ci/ci_check_*.sh` count (Δ +1 vs the N-F-G-B close's 117: the one new G-C gate `ci_check_ba02_evidence_manifest_schema.sh`; the handoff fence `ci_check_served_chain_handoff_fence.sh` was **broadened in place** — same path, 40+/20-, NOT a new file; the containment gate `ci_check_node_run_loop_containment.sh` is byte-unchanged between `339cccb1` and `90791691`).

### HEAD_DELTAS (`docs/ade-HEAD_DELTAS.md` §5)

HEAD_DELTAS is **one cluster behind this pass** — it is still at the **G-B** close (HEAD `339cccb1`, baseline `80dac1f7`) and renders `DC-NODE-06` as `declared` / **117** CI gates. It does **not yet narrate the G-C span** (`339cccb1 → 90791691`) or catalogue the new G-C gate. **This is a recorded coherence note, not soften'd:** the registry, CODEMAP, and *this* TRACEABILITY all agree at `90791691` (118 gates, 4 `strengthened_in += "PHASE4-N-F-G-C"` bumps, the new gate + the broadened fence); HEAD_DELTAS lags at `339cccb1` and is the one of the four docs to refresh next (`/head-deltas 339cccb1`). Its load-bearing registry note (anticipating the G-B `DC-NODE-06 declared→enforced` close-pass state) was satisfied at the G-B close — the working tree this pass reads has that flip already committed.

The G-C gate(s) and their rule-binding status (these will appear in HEAD_DELTAS §5 once it is refreshed to `90791691`):

- `ci_check_ba02_evidence_manifest_schema.sh` (CE-G-C-LIVE, S2) — **NEW gate, rule-bound to `RO-LIVE-06` + `CN-OPERATOR-EVIDENCE-01`** (appears on both rules' **CI** rows above; resolves on disk; exit 0). It is **vacuous-until-committed**: it scans `docs/clusters/PHASE4-N-F-G-C/` for `CE-G-C-LIVE_*.toml`; **0** such manifests exist at HEAD (verified: cluster dir holds only `cluster.md` + `S1` + `S2` docs), so the gate passes vacuously (`PASS (no manifest committed; vacuous — live ACCEPT is operator-gated)`). When a manifest is committed it verifies the 8-field schema + the `peer_log_file_sha256` cross-check — the no-synthetic-manifest enforcer.
- `ci_check_served_chain_handoff_fence.sh` (CE-G-B-3, **broadened in place** by G-C S1) — still **rule-bound to `DC-NODE-06`** (serve-ingress clause). Broadening: owner set `node_lifecycle.rs` → `{node_lifecycle.rs, node_sync.rs}`; guard-3 went from a 3-name **deny-list** (`<Vec<u8>>` / `<ForgedBlockArtifact>` / `<bool>`) to an **allow-list** (every node-spine unbounded handoff channel MUST carry `SelfAcceptedHandoff`, and at least one `UnboundedSender<SelfAcceptedHandoff>` must be present). Same path, exit 0 — this addresses the carried IDD-review WARN (see Honest gaps).
- `ci_check_node_run_loop_containment.sh` (pre-existing, CN-NODE-02 / DC-SYNC-02 / DC-NODE-05) — **co-bound to `DC-NODE-06`** (containment-unchanged clause). **Byte-identical** between `339cccb1` and `90791691` (verified `git diff --stat` empty); exit 0.
- `ci_check_ba02_evidence_closed.sh` (pre-existing, `DC-EVIDENCE-01` + now `RO-LIVE-06`) — unchanged; exit 0 (`single Ba02Manifest constructor inside correlate; no self-evidence token … no synthetic committed manifest`).

**CI gates on disk not bound to any rule CI row (3 of 118):** unchanged from the prior pass — the project-wide hygiene/meta gates `ci_check_module_headers.sh`, `ci_check_no_secrets.sh`, `ci_check_pallas_quarantine.sh` (expected-unbound; no single invariant to attribute them to). The one new G-C gate `ci_check_ba02_evidence_manifest_schema.sh` is rule-bound (`RO-LIVE-06` + `CN-OPERATOR-EVIDENCE-01`) on landing — no new unbound gate. The other **115** on-disk gates are rule-bound.

### SEAMS

Closed registries / closed grammars referenced by rule sources (mini-protocol ID registry, closed message taxonomies, closed log/event vocabularies, `LeaderCheckVerdict`, `ExpectedVrfInput`, `Mode`, `IngressSource`, the PHASE4-N-Y closed sets, the PHASE4-N-Z `MithrilBootstrapError`, the PHASE4-N-F-C BA-02 evidence surface, the N-F-D loop-planner lifecycle vocabulary, the N-F-E forge-tick `Due | NotDue` planner input, the N-F-F two-variant `ForgeIntent { On(ForgePaths), Off }`, the N-F-G-A closed `SlotAlignmentError` + `ForgeEpochAdmission`, the N-F-G-B constructor-fenced `SelfAcceptedHandoff`) are catalogued in `docs/ade-SEAMS.md` §3. PHASE4-N-F-G-C adds **no new closed error vocabulary and no new plugin seam**: the NEW RED `ade_node::ba02_pass` is pure operator-pass I/O (`correlate_peer_log_file` + `write_ba02_manifest`) that funnels through the existing GREEN `correlate` — the sole `Ba02Manifest` constructor — and the live-feed wire reuses the closed `ade_runtime::admission::{dial_for_admission, run_admission_wire_pump}` verbatim. `NodeBlockSource` stays the **closed 2-variant** `{WirePump, InMemory}` (a fill of the existing `WirePump` arm, **not a new variant**); **no new `CoordinatorEvent` variant**.

---

## Honest enforcement gaps (standing)

The four-cell contract is filled for every `enforced` rule (Code + Tests + CI all present and verified in the working tree via grep/ls, with the three rule-bound G-C gates additionally executed green this pass: `ci_check_ba02_evidence_manifest_schema.sh`, the broadened `ci_check_served_chain_handoff_fence.sh`, and the co-bound `ci_check_node_run_loop_containment.sh` — plus the carried `ci_check_ba02_evidence_closed.sh`). The standing gaps below are recorded rather than hidden.

- **Honest scope of this close (load-bearing — do not soften).** PHASE4-N-F-G-C closes the **MECHANICAL live-feed + evidence scaffolding ONLY**. `RO-LIVE-01` stays `partial` / `blocked_until_operator_stake_available`; `RO-LIVE-06` enforces **schema + correlation mechanics only — NO live BA-02 claim**; peer ACCEPT is proven **ONLY** by the operator-captured peer log through `ba02_evidence::correlate`, **never** by Ade self-accept / `ForgeSucceeded` / a served block / any wire-success signal; `correlate` is the **sole** `Ba02Manifest` constructor (`write_ba02_manifest` accepts only a `Ba02Manifest`, verified in `ba02_pass.rs`); **no synthetic manifest is committed** (0 `CE-G-C-LIVE_*.toml` on disk; the new gate is vacuous); the **bounty acceptance criterion is NOT satisfied** by this cluster.

- **`declared` rules (112) are open enforcement gaps by design.** Each carries `_(no enforcing code located — gap)_` / `_(no tests named in registry — gap)_` / `_(no CI script — gap)_` cells (or a subset). These are named-but-not-yet-mechanically-enforced invariants; the gap is the actionable backlog, not a defect to paper over. (Unchanged at 112 — no status flip this close; the four G-C-touched rules were already `enforced`/`partial`.)

- **`RO-LIVE-01` (PHASE4-N-G, `partial`; strengthened N-Q + N-F-G-C) — producer-side live leg, MECHANICAL half wired.** Four-cell contract has Code + Tests + CI, but **status stays `partial`**. G-C added two mechanical-half tests (`live_wire_pump_feed_reaches_forge_tick` in `node_sync.rs` proves a live `WirePump` feed makes `LoopStep::ForgeTick` reachable; `live_feed_forge_serve_loopback_returns_forged_block` in `forge_succeeds.rs` proves the forge-derived self-accepted block is served byte-identically) plus the operator-pass runbook + BA-02 wiring (via `RO-LIVE-06`). The **LIVE half is UNCHANGED**: peer ACCEPT stays `blocked_until_operator_stake_available`, proven only by the operator-captured peer log. This is an operator-gated release leg, carried.

- **`RO-LIVE-06` (PHASE4-N-F-C, `enforced`; strengthened N-F-G-C) — BA-02 evidence, schema + correlation mechanics ONLY.** Four-cell contract filled (Code: `ba02_evidence.rs` GREEN correlator + the NEW G-C RED `ba02_pass.rs` operator-pass I/O; Tests: 18 carried + 2 G-C — `correlate_wired_to_operator_peer_log` + `correlate_from_operator_log_file_is_deterministic`, both in `node_operator_pass_ba02.rs`; CI: `ci_check_ba02_evidence_closed.sh` + the new `ci_check_ba02_evidence_manifest_schema.sh`, both exit 0). **What is enforced:** the closed versioned `Ba02Manifest`, the pure/total/deterministic HASH-PRIMARY `correlate` as the SOLE constructor, the allow-list parser, and (G-C) the operator-pass file I/O whose write path accepts only a `correlate`-produced `Ba02Manifest`. **What is NOT claimed:** that BA-02 was achieved live — `RO-LIVE-06` is schema + mechanics; a real BA-02 needs an Ade-forged block hash + real Haskell-peer accept evidence naming that exact hash + a reviewed manifest over the captured log. The live capture is operator-gated.

- **`CN-OPERATOR-EVIDENCE-01` (PHASE4-N-S-C, `enforced`; strengthened N-F-G-C) — operator-pass manifest schema; standing Tests gap.** Code row gains the G-C runbook (`docs/evidence/phase4-n-f-g-c-operator-pass-README.md`) + the new gate; CI row gains `ci_check_ba02_evidence_manifest_schema.sh` (exit 0, vacuous); `cross_ref` gains `RO-LIVE-06`. **Tests row remains an explicit gap** — the rule's `tests` array is `[]` in the registry; it is **gate-enforced** (the two manifest-schema gates fire only when a `CE-*-LIVE_*.toml` is committed, and **0** are committed at HEAD, so both are vacuously satisfied). Carry as a standing Tests gap; both schema gates are vacuous-until-committed by design.

- **`DC-NODE-06` (PHASE4-N-F-G-B, `enforced`; strengthened N-F-G-C) — self-accept → serve handoff, mechanism-only.** The four-cell contract is now filled (Code: `node_lifecycle.rs` sibling admit task + `node_sync.rs` handoff wrap + `served_chain.rs` + `served_chain_handle.rs`; Tests: 10 G-B handoff/serve/replay/loopback tests, all on disk — `handoff_carrier_*` + `forge_surfaces_*` + `serve_ingress_*` + `sibling_serve_*` + `serve_sibling_*` + `relay_loop_containment_*` + `block_fetch_*`; CI: `ci_check_served_chain_handoff_fence.sh` + `ci_check_node_run_loop_containment.sh`, both exit 0). What is enforced: only a BLUE self-accepted artifact (typed `SelfAcceptedHandoff`, sole provenance `from_self_accepted`) reaches the single `ServedChainHandle::push_atomic`, fed only by `into_accepted()`; the relay-loop body performs no serve/admit/`push_atomic`/durable-tip mutation (containment byte-unchanged). What is NOT claimed: there is **no live feed** (no `WirePump` / `n2n_dialer` / session wiring into the binary — that is G-C), **no peer-acceptance / BA-02 / RO-LIVE acceptance** (peer ACCEPT stays operator-gated; `RO-LIVE-01` partial), and on the empty binary source the loop still halts before any `ForgeTick` (forge-CAPABLE, not observable). This is the serve *mechanism* enforced at the lifecycle surface, not a live serve-into-the-network claim. **G-C addition:** the Tests row gains `live_feed_forge_serve_loopback_returns_forged_block` (the forge-derived self-accepted block is served byte-identically), and the serve-ingress gate `ci_check_served_chain_handoff_fence.sh` was **broadened in place** (owner set `{node_lifecycle.rs, node_sync.rs}`; guard-3 deny-list→allow-list) — same path, hardened; the co-bound `ci_check_node_run_loop_containment.sh` is byte-unchanged.

- **(IDD-review WARN — PARTLY ADDRESSED by G-C) `ci_check_served_chain_handoff_fence.sh` scope + guard-3.** The carried G-B WARN was that the gate was file-scoped to `node_lifecycle.rs` with a non-exhaustive 3-name **deny-list** (guard 3). **G-C S1 broadened it in place:** the owner set is now `{node_lifecycle.rs, node_sync.rs}` (every current node-spine serve owner), and guard 3 went from a deny-list to an **allow-list** (every node-spine unbounded handoff channel MUST carry `SelfAcceptedHandoff`, and at least one `UnboundedSender<SelfAcceptedHandoff>` must be present) — closing the non-exhaustiveness. The gate still strips `#[cfg(test)]` + comments and greps production bodies of the named owners; it is **not yet a whole-`ade_node`/`ade_runtime` surface scan** (a residual partiality), but it is no longer single-file and no longer deny-list-bypassable. Carried as a soft residual: extend to a surface-wide scan if the serve path grows beyond the two named owners.
...
- **Forge is now LIVE-FEED-WIREABLE (G-C); LIVE acceptance still not claimed.** With **no** `--peer`, the empty source (`NodeBlockSource::in_memory(Vec::new())`) still halts on iteration 1 before any `ForgeTick` (forge-CAPABLE, not observable). **G-C** makes the `On` arm **live-feed-wireable**: when `--peer` is supplied, `node_lifecycle::spawn_live_wire_pump_source` builds a LIVE `NodeBlockSource::WirePump` (reusing the closed admission dial + pump), so with a live feed the forge is observable when the feed is Continuing and a due leader slot is reached. **Peer ACCEPT is still NOT claimed** — it is operator-gated (`RO-LIVE-01` partial / `blocked_until_operator_stake_available`), proven only by the operator-captured peer log through `ba02_evidence::correlate`. The L6 `ba02_evidence` correlator is now reached by the RED `ba02_pass` operator-pass I/O, but **BA-02 is satisfied nowhere in-repo** (no synthetic manifest; the schema gate is vacuous). Ade self-accept / `ForgeSucceeded` / served-block / wire success **≠ peer acceptance**.

- **Operator-gated release legs unchanged this close (none flipped):** `RO-LIVE-01` (producer-side live leg against a real peer — MECHANICAL half wired by G-C, LIVE half operator-gated), `RO-LIVE-06` (BA-02 evidence — schema/correlation mechanics only; peer ACCEPT proven only by the peer's validation log, never by Ade's self-accept / `ForgeSucceeded` / served-block / any wire-success signal), `RO-SYNC-EVIDENCE-01` (vacuous-until-committed manifest), `CE-Y-16` (two-Haskell-node witnessed live capture). PHASE4-N-F-G-C does **not** flip any of these — it adds mechanical-half tests + evidence scaffolding only.

---

## Generation notes

INCREMENTAL refresh — **PHASE4-N-F-G-C close** at HEAD `90791691` (working tree). The rule set is **unchanged at 313** (no new rule); this pass re-joins the four `strengthened_in += "PHASE4-N-F-G-C"` rows to their Source / Code / Tests / CI from the working-tree registry (the close-pass edits already applied) and re-runs the mechanical counts + the on-disk existence checks (grep/ls), plus a green execution of the three rule-bound G-C gates. The prior pass was the PHASE4-N-F-G-B close (HEAD `339cccb1`, also 313 rules, inventory 181 / 20 / 112).

The substantive deltas since the prior pass (all working-tree registry close-pass edits) — **four `strengthened_in` bumps, NO status flip**:

- **`RO-LIVE-01` — STAYS `partial`** (NOT flipped; live half `blocked_until_operator_stake_available`). `strengthened_in = ["PHASE4-N-Q", "PHASE4-N-F-G-C"]`. **Tests** row gains `live_wire_pump_feed_reaches_forge_tick` (in `crates/ade_node/src/node_sync.rs`) + `live_feed_forge_serve_loopback_returns_forged_block` (in `crates/ade_node/tests/forge_succeeds.rs`). `open_obligation` now records that G-C wired the MECHANICAL half (live WirePump feed → ForgeTick reachable; forge-derived self-accepted block served byte-identically; operator-pass runbook + BA-02 wiring) while the LIVE half is unchanged.
- **`RO-LIVE-06` — STAYS `enforced`** (schema + correlation mechanics ONLY; LIVE BA-02 still NOT claimed). `strengthened_in = ["PHASE4-N-F-G-C"]`. **Tests** row gains `correlate_wired_to_operator_peer_log` + `correlate_from_operator_log_file_is_deterministic` (both in `crates/ade_node/tests/node_operator_pass_ba02.rs`). **CI** row gains `ci/ci_check_ba02_evidence_manifest_schema.sh`. **Code** row (`code_locus`) gains `crates/ade_node/src/ba02_pass.rs`.
- **`CN-OPERATOR-EVIDENCE-01` — STAYS `enforced`.** `strengthened_in = ["PHASE4-N-F-G-C"]`. **CI** row gains `ci/ci_check_ba02_evidence_manifest_schema.sh`. **Code** row gains the runbook `docs/evidence/phase4-n-f-g-c-operator-pass-README.md` + the new gate. `cross_ref` gains `RO-LIVE-06`. Its `tests` array is `[]` (unchanged) — gate-enforced, an explicit standing Tests gap carried.
- **`DC-NODE-06` — STAYS `enforced`.** `strengthened_in = ["PHASE4-N-F-G-C"]`. **Tests** row gains `live_feed_forge_serve_loopback_returns_forged_block`. Its serve-ingress gate `ci/ci_check_served_chain_handoff_fence.sh` was **BROADENED in place** (owner set `{node_lifecycle.rs, node_sync.rs}` + guard-3 deny-list→allow-list) — same path, hardened; the co-bound `ci/ci_check_node_run_loop_containment.sh` is byte-unchanged.

No rule was added, removed, weakened, or deprecated. No BLUE crate / canonical type changed (**456** holds). One CI script added on disk (117 → 118: `ci_check_ba02_evidence_manifest_schema.sh`), rule-bound on landing to `RO-LIVE-06` + `CN-OPERATOR-EVIDENCE-01`; the handoff fence `ci_check_served_chain_handoff_fence.sh` was broadened in place (same path, NOT a new file); the containment gate `ci_check_node_run_loop_containment.sh` is byte-unchanged.

Mechanical counts recomputed fresh from the registry at `90791691` (`grep -cE '^id = '` = **313**; no duplicate IDs):

- Status: **181 enforced** / **20 partial** / **112 declared** / 0 deprecated. (**Unchanged from the prior pass** — four `strengthened_in` bumps move no status. Cross-checked against the registry: 181 / 20 / 112 `status =` lines.)
- By tier: constraint 4 · constraint_network 2 · derived **189** · operational 16 · release 40 · true 62. (Unchanged — a metadata bump moves no tier.)
- By family: T 30 · DC **146** · CN 112 · RO 16 · OP 9. (Unchanged — a metadata bump moves no family.)

**Drift check (this pass, the changed rows): 0 drifts.** All six G-C test/CI/code additions resolve on disk: tests `live_wire_pump_feed_reaches_forge_tick` (`node_sync.rs:2037`), `live_feed_forge_serve_loopback_returns_forged_block` (`forge_succeeds.rs:519`), `correlate_wired_to_operator_peer_log` (`node_operator_pass_ba02.rs:74`), `correlate_from_operator_log_file_is_deterministic` (`node_operator_pass_ba02.rs:131`); code locus `crates/ade_node/src/ba02_pass.rs`; runbook `docs/evidence/phase4-n-f-g-c-operator-pass-README.md`; CI gate `ci/ci_check_ba02_evidence_manifest_schema.sh`. The three rule-bound G-C gates executed green (the new BA-02 manifest-schema gate = vacuous PASS, 0 `CE-G-C-LIVE_*.toml` committed; the broadened handoff fence = OK; the byte-unchanged containment gate = OK), plus the carried `ci_check_ba02_evidence_closed.sh` = OK. Across the full 313 rules the prior pass recorded 0 unresolved references; this close adds only resolved references, so the **0-drift** property holds.

**CODEMAP** (regenerated this close at `90791691`, **118** CI checks, **456** canonical types, **2230** tests) is coherent with this pass: it records 118 gates (matching on-disk), no BLUE change, and the G-C loci (`ba02_pass.rs` RED I/O, `node_lifecycle::spawn_live_wire_pump_source`, the broadened fence + new gate). **HEAD_DELTAS is one cluster behind** — it is still at the **G-B** close (baseline `80dac1f7`, HEAD `339cccb1`, **117** CI, `DC-NODE-06` rendered `declared`) and does not yet narrate the `339cccb1 → 90791691` G-C span. This is recorded as a coherence note (not soften'd): registry + CODEMAP + this TRACEABILITY agree at `90791691`; HEAD_DELTAS lags and is the doc to refresh next (`/head-deltas 339cccb1`). The remaining `_(… — gap)_` cells (the 112 `declared` rules; the standing `CN-PROD-04` CI gap; the `CN-OPERATOR-EVIDENCE-01` standing Tests gap; the project-wide hygiene/meta unbound gates) and the **Honest enforcement gaps** section are the actionable output; reconcile them in the registry or the codebase, do not delete them. Regenerate after every cluster close.
