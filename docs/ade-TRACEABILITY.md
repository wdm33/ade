...
>
> **HEAD:** `273c887` · **Generated:** 2026-05-29 · **Baseline:** `01e7e08` (`.idd-config.json` `head_deltas_baseline`)
...
- **[not found on disk — drift]** (tests) / **[not on disk — drift]** (CI) flags a registry-named test/CI that could not be located in the working tree at HEAD `273c887` — a registry↔code coherence gap to reconcile (rename the registry entry or restore the artifact). Test drift is resolved against the **leaf** function name (the segment after the last `::`).
...
By tier: true 61 · derived 177 · release 38 · operational 16.

By family: T 29 · DC 134 · CN 106 · RO 14 · OP 9.
...
Derived invariants — Cardano-compatibility constraints the authoritative core must satisfy (ledger rules, tx-validity, crypto vectors, epoch/HFC translation, consensus state, storage/WAL, admission).
...
_tier: derived · status: enforced · introduced: PHASE4-N-T · strengthened: PHASE4-N-W_
...
Constraint-network invariants from the classification table — adversarial-rationale-backed constraints on the wire, ledger, consensus, storage, session, and producer surfaces.
...
### `CN-WIRE-08`: N2N tag-24 CBOR-in-CBOR wire-envelope authority — one shared BLUE wrap/unwrap, fixture-pinned per-protocol composition, fail-closed unwrap

_tier: derived · status: enforced · introduced: PHASE4-N-X_
...
| **Code** | crates/ade_codec/src/cbor/tag24.rs (wrap_tag24 / unwrap_tag24 + TagEnvelopeError — the single shared BLUE tag-24 byte authority); crates/ade_codec/src/cbor/mod.rs (read_bytes / read_text / skip_item length-arg overflow guard — fail-closed, no panic on a huge declared length); crates/ade_network/src/codec/block_fetch.rs + crates/ade_network/src/codec/chain_sync.rs (per-protocol compose/decompose over the shared authority); crates/ade_network/src/block_fetch/server.rs + crates/ade_network/src/chain_sync/server.rs (serve emits the composed wrapped bytes); crates/ade_node/src/admission/runner.rs + crates/ade_core_interop/src/follow.rs (RED unwraps migrated onto the shared authority — no hand-rolled parse) |
...
_tier: derived · status: enforced · introduced: PHASE4-N-R-A · strengthened: PHASE4-N-S-A, PHASE4-N-T, PHASE4-N-W_
...
_tier: derived · status: enforced · introduced: PHASE4-N-V · strengthened: PHASE4-N-X_
...
_tier: derived · status: enforced · introduced: PHASE4-N-W_
...
Release obligations — properties that must hold before the project ships a release (live-pass evidence, genesis/Mithril import, close-gate discipline, test-corpus presence).
...
### CODEMAP (`docs/ade-CODEMAP.md` @ `01e7e08`)

Every crate named in a **Code** row is present in CODEMAP: `ade_codec`, `ade_core`, `ade_core_interop`, `ade_crypto`, `ade_ledger`, `ade_network`, `ade_node`, `ade_plutus`, `ade_runtime`, `ade_testkit`, `ade_types`. No crate-level CODEMAP mismatch detected. **Staleness note:** CODEMAP was regenerated at `01e7e08` (the N-W close), NOT at this doc's HEAD `273c887`. The PHASE4-N-X module `crates/ade_codec/src/cbor/tag24.rs` (new, referenced by CN-WIRE-08's **Code** row) is therefore not yet catalogued in CODEMAP — regenerate `/codemap` at `273c887` to close the gap. No crate-level mismatch (tag24.rs lives inside the already-mapped `ade_codec` crate).

### HEAD_DELTAS (`docs/ade-HEAD_DELTAS.md` §5 @ `01e7e08`)

The PHASE4-N-X close adds `ci/ci_check_tag24_wire_authority.sh` (bound to CN-WIRE-08) — the N2N tag-24 wire-envelope gate that fails the build if a serve path emits a bare (un-wrapped) BlockFetch payload or bare ChainSync RollForward header, if a second hand-rolled tag-24 parser appears in RED, or if `unwrap_tag24` stops failing closed. This script appears as the **CI** row of CN-WIRE-08 above. **Staleness note:** HEAD_DELTAS §5 was last regenerated at `01e7e08` (the N-W close) and does NOT yet list `ci_check_tag24_wire_authority.sh` — regenerate `/head-deltas` at `273c887` to catalogue the N-X CI gate. The prior-window `ci_check_producer_praos_vrf.sh` (N-W, bound to CN-FORGE-04) and earlier scripts (N-T / N-V: `ci_check_produce_mode_uses_bootstrap_initial_state.sh`, `ci_check_node_binary_uses_single_bootstrap.sh`, `ci_check_forge_decode_round_trip.sh`) remain catalogued in the respective HEAD_DELTAS baselines.

**CI gates on disk not bound to any rule CI row** (surfaced, not fixed this pass — pre-existing, NOT introduced by N-X): `ci_check_module_headers.sh`, `ci_check_no_secrets.sh`, `ci_check_pallas_quarantine.sh`, and `ci_check_ingress_chokepoints.sh` appear in no registry `ci_script`/`ci_scripts` field — real green gates with no covering rule entry, candidates for a future rule binding. Two of these encode live enforcement findings worth recording: `ci_check_module_headers.sh` currently flags `crates/ade_ledger/src/block_validity/mod.rs` (missing/non-conforming module header), and `ci_check_ingress_chokepoints.sh` flags a pre-existing `impl From<minicbor::decode::Error>` in the `encoding.rs` codec files (an ingress conversion outside the designated chokepoint). Both are pre-existing conditions, NOT regressions from PHASE4-N-X; they are surfaced here as standing enforcement gaps to reconcile (bind the gate to a rule, or fix the flagged source).

### SEAMS

Closed registries referenced by rule sources (mini-protocol ID registry, closed message taxonomies, closed log/event vocabularies, `LeaderCheckVerdict`, `ExpectedVrfInput`, `Mode`, `IngressSource`) are catalogued in `docs/ade-SEAMS.md` §3.

---

## Generation notes

Regenerated from the registry as primary source; tests/CI cross-checked against the working tree at HEAD `273c887` (PHASE4-N-X close — N2N Tag-24 Wire Envelope Authority). Test drift is resolved against the leaf function name (segment after the last `::`). Both the `ci_script` (string) and `ci_scripts` (array) registry fields are read for the CI row. Regenerate after every cluster close. The **drift** flags and `_(… — gap)_` cells are the actionable output — reconcile them in the registry or the codebase, do not delete them.
