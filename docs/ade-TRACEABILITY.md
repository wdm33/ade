...
>
> **HEAD:** `550eec3a` (PHASE4-N-F-G-J close — S5 impl `feat(node): C1 genesis-successor rehearsal harness`) · **Generated:** 2026-06-03 · **Prior pass:** `f59b1eaa` (PHASE4-N-F-G-H close)
...
The canonical source is the **invariant registry** at `docs/ade-invariant-registry.toml` (`.idd-config.json` `invariant_registry`), **319 rules** at HEAD `550eec3a` (`+3` this close: the three NEW PHASE4-N-F-G-J rules `CN-WIRE-09`, `DC-NODE-08`, `CN-NODE-04`, all introduced `enforced`; `CN-REHEARSAL-FIDELITY-01` is `strengthened_in += PHASE4-N-F-G-J` — no new ID). This doc is a join: registry entries × codebase introspection. Rule IDs, families, names, sources, and the registry-recorded `tests` / `ci_script` / `code_locus` fields come from the registry; each test name and CI script is cross-checked for existence in the working tree at `550eec3a`, and divergences are flagged inline as **drift**. (Verification this pass is grep/ls only — no build or test execution. This is a **fast delta-refresh** — only the four G-J rules were re-traced from scratch; every other rule's trace row is preserved verbatim from the prior pass. The G-J leaves were located on disk: all 20 `CN-WIRE-09` tests across `ade_codec/src/shelley/block.rs`, `ade_ledger/src/block_validity/{header_position,header_input,unsigned_header_pre_image}.rs` + `ade_ledger/src/producer/forge.rs` + `ade_runtime/src/producer/chain_evolution.rs`; all 9 `DC-NODE-08` tests across `ade_node/src/{node_sync,node_lifecycle}.rs`; both `CN-NODE-04` tests in `ade_node/src/{node_sync.rs,live_log/sched_event.rs}`; the 3 G-J `CN-REHEARSAL-FIDELITY-01` tests in `ade_node/tests/{forge_succeeds.rs,node_c1_genesis_rehearsal.rs}`; and all five G-J CI gates `ci/ci_check_{prevhash_single_wire_authority,genesis_successor_reachability,node_sched_events_emit_only,node_path_fidelity,rehearsal_manifest_schema}.sh` all present.)
...
- **[not found on disk — drift]** (tests) / **[not on disk — drift]** (CI) flags a registry-named test/CI that could not be located in the working tree at `550eec3a` — a registry↔code coherence gap to reconcile. This pass: **0** such drifts among the four re-traced G-J rules (every one of the 34 cited G-J test leaf-fns and all five G-J CI scripts resolves on disk). Rows outside G-J were not re-verified this delta-refresh — their drift status is carried from the prior pass.
...
By tier: constraint 4 · constraint_network 2 · derived 193 · operational 17 · release 41 · true 62.

By family: T 30 · DC 149 · CN 115 · RO 16 · OP 9.

_G-J close delta: `+3` rules (`CN-WIRE-09` + `DC-NODE-08` derived/CN+DC; `CN-NODE-04` operational/CN), all `enforced`; `CN-REHEARSAL-FIDELITY-01` strengthened (no ID/status change)._
...
### CODEMAP (`docs/ade-CODEMAP.md` @ `550eec3a`, PHASE4-N-F-G-J close)

CODEMAP was regenerated for the PHASE4-N-F-G-J close at **`550eec3a`** (alongside this doc). Every crate named in a **Code** row is present in CODEMAP: `ade_codec`, `ade_core`, `ade_core_interop`, `ade_crypto`, `ade_ledger`, `ade_network`, `ade_node`, `ade_plutus`, `ade_runtime`, `ade_testkit`, `ade_types`. No crate-level mismatch. Every `crates/**.rs` path cited in a re-traced G-J **Code** row resolves on disk (spot-checked this pass): the `CN-WIRE-09` loci — `crates/ade_types/src/shelley/block.rs`, `crates/ade_codec/src/shelley/block.rs`, `crates/ade_ledger/src/block_validity/{header_position,header_input,verdict,unsigned_header_pre_image}.rs`, `crates/ade_ledger/src/producer/forge.rs`, `crates/ade_runtime/src/producer/chain_evolution.rs`; the `DC-NODE-08` + `CN-NODE-04` loci — `crates/ade_node/src/{node_sync.rs,node_lifecycle.rs,forge_intent.rs,live_log/sched_event.rs,live_log/sched_writer.rs}`.

- **The G-J wire/codec touch is a BLUE change** (unlike the prior G-H/G-D harness closes): `CN-WIRE-09` migrates the Shelley `header_body.prev_hash` field from a flat `Hash32` to the closed sum `PrevHash = Genesis | Block(Hash32)` in `ade_types` (BLUE) + its codec in `ade_codec` (BLUE) + the position-aware `check_header_position` in `ade_ledger` (BLUE). CODEMAP at `550eec3a` should reflect the `PrevHash` sum on the `ade_types` / `ade_ledger` BLUE inventory; the canonical-type count is expected to move accordingly (re-derive from the refreshed CODEMAP rather than carrying the prior `456`). `DC-NODE-08` + `CN-NODE-04` are GREEN/operational (`ade_node` cold-start decision + closed sched-event vocabulary) — no BLUE authority.
- **CI count:** the on-disk `ci/ci_check_*.sh` count is **126** at `550eec3a` (Δ +5 vs the G-H close's 121: the five NEW G-J gates `ci_check_prevhash_single_wire_authority.sh`, `ci_check_genesis_successor_reachability.sh`, `ci_check_node_sched_events_emit_only.sh`, plus `ci_check_node_path_fidelity.sh` + `ci_check_rehearsal_manifest_schema.sh` carried from G-D and now joined by the genesis-rehearsal home coverage). CODEMAP's recorded gate count should match **126**; if it still reads 121, CODEMAP lags this close.

### HEAD_DELTAS (`docs/ade-HEAD_DELTAS.md` §5)

HEAD_DELTAS is **behind this pass** (carried, expected) — at the prior pass its window was the **G-E** close (`90791691 → 6f848825`, 119 CI gates) and it did not yet narrate G-D / G-H / G-J. The five NEW G-J CI gates (`ci_check_prevhash_single_wire_authority.sh`, `ci_check_genesis_successor_reachability.sh`, `ci_check_node_sched_events_emit_only.sh`, `ci_check_node_path_fidelity.sh`, `ci_check_rehearsal_manifest_schema.sh`) are **not yet listed** in HEAD_DELTAS §5. This is a recorded coherence note, not soften'd: the registry + this TRACEABILITY agree at `550eec3a` (126 gates on disk, 319 rules, the four G-J rules `enforced` / strengthened); HEAD_DELTAS lags and is the doc to refresh next (`/head-deltas`).

**CI gates on disk not bound to any rule CI row (carried — 3 expected-unbound):** the project-wide hygiene/meta gates `ci_check_module_headers.sh`, `ci_check_no_secrets.sh`, `ci_check_pallas_quarantine.sh` (no single invariant to attribute them to). All five NEW G-J gates **are rule-bound** (`ci_check_prevhash_single_wire_authority.sh` → `CN-WIRE-09`; `ci_check_genesis_successor_reachability.sh` → `DC-NODE-08` (+ `CN-REHEARSAL-FIDELITY-01` S5 reuse); `ci_check_node_sched_events_emit_only.sh` → `CN-NODE-04`; `ci_check_node_path_fidelity.sh` + `ci_check_rehearsal_manifest_schema.sh` → `CN-REHEARSAL-FIDELITY-01`), so the unbound set stays **3**, not 8.

### SEAMS

SEAMS (`docs/ade-SEAMS.md`) catalogues the closed registries / closed grammars referenced by rule sources (mini-protocol ID registry, closed message taxonomies, closed log/event vocabularies, `LeaderCheckVerdict`, the PHASE4-N-Y/N-Z closed sets, the BA-02 evidence surface, the loop-planner lifecycle vocabulary, the `ForgeIntent` two-variant, the `SelfAcceptedHandoff` constructor-fence, the rehearsal-evidence non-promotable surface) in §3. PHASE4-N-F-G-J adds **two new closed grammars + one closed event vocabulary**: the `CN-WIRE-09` closed wire sum `PrevHash = Genesis | Block(Hash32)` (`$hash32 / null`; position-blind codec + the single position-aware `check_header_position` authority), and the `CN-NODE-04` closed feed/forge sched-event vocabulary (`SchedEvent` + closed reason/outcome enums; a new variant is a compile error at the exhaustive JSONL encoder; emit-only). `DC-NODE-08` adds the closed cold-start convention (`forge_header_position`: `None ⇒ block 0 + Genesis`, `Some ⇒ n+1 + Block`) but no new plugin seam. SEAMS should be re-derived at `550eec3a` to record the `PrevHash` sum and the `SchedEvent` vocabulary; if it still reads the prior 121-gate / pre-`PrevHash` inventory, it lags this close.

---
## Honest enforcement gaps (standing)

The four-cell contract is filled for every `enforced` rule re-traced this pass (the four G-J rules: Code + Tests + CI all present and verified on disk via grep/ls — all 20 `CN-WIRE-09` tests, all 9 `DC-NODE-08` tests, both `CN-NODE-04` tests, the 3 G-J `CN-REHEARSAL-FIDELITY-01` S5 tests, and all five G-J CI gates resolve). The standing gaps below are recorded rather than hidden.

- **Honest scope of this close (load-bearing — do NOT soften / do NOT broaden).** PHASE4-N-F-G-J ships the **genesis-successor (first-block) PrevHash authority + the node-spine cold-start reachability to forge it**, plus a closed feed/forge sched-event vocabulary and a genesis-successor extension of the C1 rehearsal harness — and nothing more. The precise enforced claims: `CN-WIRE-09` (the `prev_hash` wire field is the closed `$hash32 / null` sum; block 0 ⇔ `Genesis`/null, block >0 ⇔ `Block`/hash32, through one shared position-blind codec + one position-aware check); `DC-NODE-08` (`--mode node` MAY forge the FIRST block from the recovered base when both tips are `None`, gated by recovered lineage + `ForgeIntent::On` + a forge-eligible feed, carrying `PrevHash::Genesis`, flowing through self-accept → handoff → served-view, firing EXACTLY ONCE); `CN-NODE-04` (the closed emit-only feed/forge diagnostic event vocabulary). **What is NOT claimed:** that any block was accepted by a real peer; that the durable tip advances from forge scheduling (the forge engine takes no `ChainDb` handle, so block-1+ durable progression is the N-U durability slice); any private-only / C1-only forge path. There is **NO `RO-LIVE-01` / `RO-LIVE-06` flip** (verified: the literal `PHASE4-N-F-G-J` appears in **no** `RO-*` `strengthened_in` array). **Hard limit (do not soften):** forge + self-accept + serve ≠ peer acceptance — peer ACCEPT is proven only by the peer's validation log through `ba02_evidence::correlate`.

- **`CN-WIRE-09` (PHASE4-N-F-G-J, `enforced`) — genesis-successor PrevHash wire authority. No gap on this rule.** Four-cell contract filled and verified on disk (Code: BLUE `PrevHash` sum in `ade_types` + position-blind codec in `ade_codec` + position-aware `check_header_position` in `ade_ledger` + forge/pre-image/chain-evolution migration; Tests: **20**, all on disk, decomposed across codec round-trip / position-aware accept-reject / decode-path / forge+KES-pre-image / producer chain-evolution; CI: `ci_check_prevhash_single_wire_authority.sh`, NEW). `tier = derived`. Its `cross_ref` (`CN-WIRE-08`, `DC-FORGE-01`, `CN-FORGE-01`, `DC-NODE-08`, `RO-LIVE-01`) names the carried wire/forge invariants and the genesis forge it underpins — it does **NOT** advance `RO-LIVE-01`.

- **`DC-NODE-08` (PHASE4-N-F-G-J, `enforced`) — node-spine cold-start genesis-successor reachability. No gap on this rule.** Four-cell contract filled and verified on disk (Code: GREEN `forge_header_position` single cold-start convention + `may_cold_start_forge` permission gate + the `forge_one_from_recovered(Option<&ChainTip>)` route into the same `run_real_forge`; Tests: **9**, all on disk, decomposing the two GREEN decisions + the both-`None` end-to-end path + the fail-closed `RecoveredTipMissingBlockNo` edge; CI: `ci_check_genesis_successor_reachability.sh`, NEW). `tier = derived`. Its `cross_ref` (`DC-NODE-05`, `DC-NODE-06`, `DC-NODE-07`, `DC-EPOCH-03`, `CN-CINPUT-03`, `DC-CINPUT-02b`, `DC-FORGE-01`, `CN-FORGE-01`, `CN-NODE-04`, `CN-WIRE-09`, `RO-LIVE-01`) names the carried lineage/forge/serve invariants — it does **NOT** advance `RO-LIVE-01`. The exactly-once first-block firing is scoped to the hermetic cold-start execution (NO `genesis_forged` latch); durable block-1+ progression is explicitly deferred to N-U.

- **`CN-NODE-04` (PHASE4-N-F-G-J, `enforced`) — closed emit-only feed/forge sched-event vocabulary. No gap on this rule.** Four-cell contract filled and verified on disk (Code: closed `SchedEvent` + closed reason/outcome enums + exhaustive JSONL encoder in `live_log/sched_event.rs` + emit-only writer in `live_log/sched_writer.rs` + the planner EMIT sites; Tests: **2**, both on disk — the closed-vocabulary emission + the allow-list closedness rejection; CI: `ci_check_node_sched_events_emit_only.sh`, NEW — closedness + emit-only one-directionality). `tier = operational`. Its `cross_ref` (`DC-NODE-08`, `DC-NODE-05`, `RO-LIVE-01`) ties the forge-eligible reason split to the `DC-NODE-08` allowance — diagnostic only; emitting the events changes no forge scheduling, base, or authority, and advances no `RO-*`.

- **`CN-REHEARSAL-FIDELITY-01` (PHASE4-N-F-G-D, `enforced`; `strengthened_in += PHASE4-N-F-G-J` this close) — bounty dry-run path-fidelity + evidence-non-promotability, now covering the genesis-successor path. No gap on this rule.** The G-J S5 strengthening adds genesis-successor rehearsal coverage: +3 tests (`genesis_rehearsal_manifest_binds_block_zero_genesis`, `genesis_rehearsal_no_evidence_writes_nothing` in `crates/ade_node/tests/forge_succeeds.rs`; `node_c1_genesis_rehearsal_live` env-gated in `crates/ade_node/tests/node_c1_genesis_rehearsal.rs`) + the `ci_check_genesis_successor_reachability.sh` reuse, with the genesis-rehearsal home `docs/evidence/phase4-n-f-g-j-genesis-rehearsal-*.toml` covered by the same `ci_check_rehearsal_manifest_schema.sh` schema/non-promotability gate. **OPEN obligation (load-bearing — recorded, NOT claimed enforced):** the actual C1 genesis-successor live execution stays `blocked_until_operator_c1_genesis_successor_rehearsal` — the env-gated `node_c1_genesis_rehearsal_live` harness is skipped in CI, the schema gate is vacuous-until-a-real-manifest-is-committed (**0** committed), and `RO-LIVE-01` **stays `partial`** (NOT flipped). **No `strengthened_in` bump on any `RO-*` sibling** — the literal `PHASE4-N-F-G-J` appears in **no** `RO-*` `strengthened_in` array (verified by grep).

- **`declared` rules (112) are open enforcement gaps by design.** Each carries `_(no enforcing code located — gap)_` / `_(no tests named in registry — gap)_` / `_(no CI script — gap)_` cells (or a subset). These are named-but-not-yet-mechanically-enforced invariants; the gap is the actionable backlog, not a defect to paper over. (Unchanged this close — none of the three NEW G-J rules was `declared`; all three were introduced `enforced`, and `CN-REHEARSAL-FIDELITY-01` was already `enforced`.)
...
- **`RO-LIVE-01` (PHASE4-N-G, `partial`; strengthened N-Q + N-F-G-C) — producer-side live leg; NOT advanced by G-J (carried, OPEN).** Four-cell contract has Code + Tests + CI, but **status stays `partial`** (verified: registry `status = "partial"`, `strengthened_in = ["PHASE4-N-Q", "PHASE4-N-F-G-C"]` — **no `PHASE4-N-F-G-J` token**). The LIVE half is operator-gated: peer ACCEPT stays `blocked_until_operator_stake_available` / `blocked_until_operator_c1_genesis_successor_rehearsal`. G-J enforces the genesis-successor forge MECHANISM (`DC-NODE-08`) + the wire authority (`CN-WIRE-09`); the live peer-ACCEPT stays operator-gated. Forge + self-accept ≠ peer acceptance. Carried unchanged.

- **`RO-LIVE-06` (PHASE4-N-F-C, `enforced`; strengthened N-F-G-C) — BA-02 evidence, schema + correlation mechanics ONLY; NOT advanced by G-J (carried).** What is enforced: the closed versioned `Ba02Manifest`, the pure/total/deterministic HASH-PRIMARY `correlate` as the SOLE constructor, the allow-list parser, the operator-pass file I/O whose write path accepts only a `correlate`-produced `Ba02Manifest`. **What is NOT claimed:** that BA-02 was achieved live. Verified: registry `status = "enforced"`, `strengthened_in = ["PHASE4-N-F-G-C"]` — **no `PHASE4-N-F-G-J` token**. Carried unchanged.
...
- **Operator-gated release legs unchanged this close (none flipped):** `RO-LIVE-01` (producer-side live leg — MECHANICAL half wired, LIVE peer-ACCEPT operator-gated), `RO-LIVE-06` (BA-02 evidence — schema/correlation mechanics only), `RO-SYNC-EVIDENCE-01` (vacuous-until-committed manifest), `CE-Y-16` (two-Haskell-node witnessed live capture). PHASE4-N-F-G-J does **not** flip any of these — it enforces the genesis-successor forge mechanism + wire authority while the live peer-ACCEPT stays operator-gated (`blocked_until_operator_c1_genesis_successor_rehearsal`). The literal `PHASE4-N-F-G-J` appears in **no** `RO-*` `strengthened_in` array — verified by grep over the registry.

---

## Generation notes

FAST DELTA-REFRESH — **PHASE4-N-F-G-J close** at HEAD `550eec3a` (the S5 impl). Only the four G-J rules were re-traced from scratch (`CN-WIRE-09`, `DC-NODE-08`, `CN-NODE-04`, `CN-REHEARSAL-FIDELITY-01`); every other rule's four-cell trace row is **preserved verbatim** from the prior PHASE4-N-F-G-H pass (HEAD `f59b1eaa`). The rule set is **319** (`+3` vs the G-H `316`). This pass re-runs the mechanical counts + on-disk existence checks for the G-J leaves only (grep/ls); no build or test execution, and no re-verification of the ~315 non-G-J rows.

The substantive delta since the prior pass — **three rules introduced `enforced` + one strengthening; NO existing rule weakened / re-joined / removed / deprecated; NO `RO-*` advanced:**

- **`CN-WIRE-09` — NEW, `enforced`.** `tier = derived`, `introduced_in = "PHASE4-N-F-G-J"`, `strengthened_in = []`. **Source:** `docs/planning/phase4-n-f-g-j-genesis-successor-prevhash-invariants.md`. The closed `prev_hash` wire grammar `$hash32 / null` → `PrevHash = Genesis | Block(Hash32)`; one shared position-blind `ade_codec` authority + one position-aware `ade_ledger::block_validity::header_position::check_header_position`. 20 tests, 1 NEW gate `ci_check_prevhash_single_wire_authority.sh`. This is the lone BLUE change this close (the `ade_types` / `ade_codec` / `ade_ledger` `PrevHash` migration).
- **`DC-NODE-08` — NEW, `enforced`.** `tier = derived`, `introduced_in = "PHASE4-N-F-G-J"`, `strengthened_in = []`. **Source:** `docs/planning/phase4-n-f-g-j-genesis-successor-prevhash-invariants.md`. Node-spine cold-start first-block reachability (both tips `None` ⇒ forge block 0 + `PrevHash::Genesis` through the same `run_real_forge` → self-accept → handoff, gated by recovered lineage + `ForgeIntent::On` + forge-eligible feed; exactly once; no durable-tip advance). 9 tests, 1 NEW gate `ci_check_genesis_successor_reachability.sh`. GREEN/operational decision wiring — no BLUE change.
- **`CN-NODE-04` — NEW, `enforced` (S1).** `tier = operational`, `introduced_in = "PHASE4-N-F-G-J"`, `strengthened_in = []`. **Source:** `docs/planning/phase4-n-f-g-j-invariants.md`. The closed emit-only feed/forge diagnostic sched-event vocabulary (`SchedEvent` + closed reason/outcome enums; exhaustive JSONL encoder; planner emits but never consumes). 2 tests, 1 NEW gate `ci_check_node_sched_events_emit_only.sh`.
- **`CN-REHEARSAL-FIDELITY-01` — strengthened, `strengthened_in += "PHASE4-N-F-G-J"` (S5).** Stays `enforced`, `tier = release`. The C1 rehearsal harness now covers the genesis-successor path: +3 tests + the `ci_check_genesis_successor_reachability.sh` reuse + the genesis-rehearsal home covered by `ci_check_rehearsal_manifest_schema.sh`. **OPEN:** the live half stays `blocked_until_operator_c1_genesis_successor_rehearsal`; `RO-LIVE-01` stays `partial` (NOT flipped).

No other rule was added, removed, weakened, re-joined, or deprecated. **G-J advances no bounty / RO deliverable:** the literal `PHASE4-N-F-G-J` appears in **no** `RO-*` `strengthened_in` array — `RO-LIVE-01` stays `partial` (`["PHASE4-N-Q", "PHASE4-N-F-G-C"]`), `RO-LIVE-06` stays `enforced` (`["PHASE4-N-F-G-C"]`). The genesis-successor forge MECHANISM + the `PrevHash` wire authority are enforced; the live peer-ACCEPT stays operator-gated. Five CI gates are on disk for G-J (the on-disk `ci/ci_check_*.sh` count is **126** at `550eec3a`, Δ +5 vs the G-H `121`): three NEW (`ci_check_prevhash_single_wire_authority.sh`, `ci_check_genesis_successor_reachability.sh`, `ci_check_node_sched_events_emit_only.sh`) + the two G-D rehearsal gates now also covering the genesis-rehearsal home.

Mechanical counts recomputed fresh from the registry working tree (`grep -cE '^\[\[rules\]\]'` = **319**; `grep -cE '^status = '` = **319**; no duplicate IDs):

- Status: **187 enforced** / **20 partial** / **112 declared** / 0 deprecated. (**+3 enforced** vs the prior pass — the three NEW G-J rules introduced `enforced`; `partial` and `declared` unchanged. Cross-checked against the anchored registry `^status =` lines: 187 / 20 / 112; sum 319.)
- By tier: constraint 4 · constraint_network 2 · derived **193** · operational **17** · release 41 · true 62. (`derived` +2: `CN-WIRE-09` + `DC-NODE-08`; `operational` +1: `CN-NODE-04`.)
- By family: T 30 · DC **149** · CN **115** · RO 16 · OP 9. (`DC` +1: `DC-NODE-08`; `CN` +2: `CN-WIRE-09` + `CN-NODE-04`. The new rows are inserted in ID order — `DC-NODE-08` after `DC-NODE-07`; `CN-NODE-04` after `CN-NODE-03`; `CN-WIRE-09` after `CN-WIRE-08`.)

**Drift check (this pass): 0 drifts among the four re-traced G-J rules.** All 20 `CN-WIRE-09` test leaf-fns resolve (`ade_codec/src/shelley/block.rs`, `ade_ledger/src/block_validity/{header_position,unsigned_header_pre_image}.rs`, `ade_ledger/src/producer/forge.rs`, `ade_runtime/src/producer/chain_evolution.rs`); all 9 `DC-NODE-08` leaf-fns resolve (`ade_node/src/{node_sync.rs,node_lifecycle.rs}`); both `CN-NODE-04` leaf-fns resolve (`ade_node/src/{node_sync.rs,live_log/sched_event.rs}`); the 3 G-J `CN-REHEARSAL-FIDELITY-01` S5 leaf-fns resolve (`ade_node/tests/{forge_succeeds.rs,node_c1_genesis_rehearsal.rs}`). All five G-J CI scripts resolve on disk. All cited G-J `code_locus` paths resolve. Rows **outside** the four G-J rules were not re-verified this delta-refresh — their 0-drift status is carried from the prior pass.

**Cross-doc coherence (carried, expected lag):** CODEMAP / SEAMS / HEAD_DELTAS were last regenerated at or before the G-D/G-H span and do **not** yet reflect the G-J `PrevHash` BLUE migration, the `SchedEvent` vocabulary, or the five G-J gates / 126-gate on-disk count. The registry + this TRACEABILITY agree at `550eec3a`; the other three grounding docs lag and should be regenerated this close (`/codemap`, `/seams`, `/head-deltas`). The actionable output — the `_(… — gap)_` cells (the 112 `declared` rules; the standing `CN-PROD-04` CI gap; the `CN-OPERATOR-EVIDENCE-01` standing Tests gap; the carried `ci_check_ba02_evidence_manifest_schema.sh` stale-active-home follow-up; the 3 expected-unbound hygiene/meta gates) and the OPEN operator-gated live legs (`RO-LIVE-01` `partial`, the `blocked_until_operator_c1_genesis_successor_rehearsal` obligation on `CN-REHEARSAL-FIDELITY-01`) — should be reconciled in the registry or the codebase, not deleted. Regenerate after every cluster close.
