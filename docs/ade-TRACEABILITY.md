...
>
> **HEAD:** `1d54abb4` (PHASE4-N-AC close — KES signing evolves the operator key to the current period before signing, `DC-CRYPTO-10`) · **Generated:** 2026-06-06 · **Prior pass:** `c6e7fafe` (PHASE4-N-AB close, 335 rules) · **Registry:** `docs/ade-invariant-registry.toml` @ 336 rules · **CODEMAP:** `docs/ade-CODEMAP.md` @ `1d54abb4` (458 canonical types · 138 CI · 336 rules)
...
The canonical source is the **invariant registry** at `docs/ade-invariant-registry.toml` (`.idd-config.json` `invariant_registry`), **336 rules** at HEAD `1d54abb4` (204 enforced / 20 partial / 112 declared / 0 deprecated). **This pass is the PHASE4-N-AC close delta-refresh (`c6e7fafe` → `1d54abb4`):** `+1` NEW rule — `DC-CRYPTO-10` (derived/DC, `introduced_in = "PHASE4-N-AC"`, declared at scoping → `enforced` at the S1 close) — the RED signing shell EVOLVES the operator KES signing key forward to the requested KES period BEFORE the single real KES sign, fail-closed backwards / beyond-lifetime; `+1` `strengthened_in += "PHASE4-N-AC"` append — `CN-KES-HEADER-01` (the KES-signs-the-real-unsigned-header-pre-image invariant now ALSO covers period evolution — the signed pre-image is at the EVOLVED current period, not a stale minted-at-0 period); `0` rules removed or deprecated. CI: `+1` NEW gate `ci/ci_check_kes_evolution_before_sign.sh` → 137 → **138** (aligned with CODEMAP at `1d54abb4`). The registry↔TRACEABILITY join is **336 ↔ 336** (1:1). This doc is a join: registry entries × codebase introspection. Rule IDs, families, names, sources, and the registry-recorded `tests` / `ci_script` / `code_locus` fields come from the registry; each cited test name and CI script is cross-checked for existence in the working tree at `1d54abb4`, and divergences are flagged inline as **drift**. This is a **fast delta-refresh** — only the two PHASE4-N-AC rules (one new + one strengthened) were re-traced from scratch against disk; every other rule's four-cell trace row is preserved verbatim from the prior pass (HEAD `c6e7fafe`). Verification this pass is grep / ls only — no build or test execution. **DC-CRYPTO-10 disk-verification:** all **4** registry-cited test leaf-fns resolve (`fn <name>` present, exactly one each) in `crates/ade_runtime/src/producer/producer_shell.rs::tests::` (`shell_kes_sign_header_advancing_evolves_then_signs`, `shell_kes_sign_header_advancing_at_current_period_signs`, `shell_kes_sign_header_advancing_backwards_fails_closed`, `shell_kes_sign_header_advancing_beyond_lifetime_fails_closed`); the Code loci resolve — `kes_sign_header_advancing` (= `kes_advance_to(period)` then `kes_sign_header`) + `kes_advance_to` (→ `kes_update`, fail-closed `EvolutionBackwards` / `EvolutionExhausted`) in `crates/ade_runtime/src/producer/producer_shell.rs`, and the forge's single real KES sign at `crates/ade_node/src/produce_mode.rs` calls `kes_sign_header_advancing`; the gate `ci/ci_check_kes_evolution_before_sign.sh` is present (executable). **0 drifts; 0 registry reconciliations** — the registry `tests`/`ci_script`/`code_locus` for `DC-CRYPTO-10` are all accurate on disk.
...
By tier: constraint 4 · constraint_network 2 · derived 208 · operational 17 · release 41 · true 64.

By family: T 32 · DC 160 · CN 119 · RO 16 · OP 9.
...
_tier: true · status: enforced_ · _introduced_in: PHASE4-N-U_
...
_tier: release · status: enforced_ · _introduced_in: PHASE4-N-C · strengthened_in: PHASE4-N-G, PHASE4-N-H, PHASE4-N-F-G-B, PHASE4-N-U_
