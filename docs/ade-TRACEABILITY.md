...
>
> **HEAD:** `f59b1eaa` (PHASE4-N-F-G-H close — S3 close commit `f59b1eaa`; registry close-pass `DC-NODE-07` flip staged in the working tree) · **Generated:** 2026-06-03 · **Prior pass:** `6bd60c80` (PHASE4-N-F-G-D close)
...
The canonical source is the **invariant registry** at `docs/ade-invariant-registry.toml` (`.idd-config.json` `invariant_registry`), **316 rules** at HEAD `f59b1eaa` (`+0` this close on count — `DC-NODE-07` was already present as a `declared` row from the G-H invariant sketch and is flipped `declared → enforced` at this close; `+1` vs the G-D-baseline `315` reflects that sketch addition). This doc is a join: registry entries × codebase introspection. Rule IDs, families, names, sources, and the registry-recorded `tests` / `ci_script` / `code_locus` fields come from the registry; each test name and CI script is cross-checked for existence in the working tree at `6bd60c80`, and divergences are flagged inline as **drift**. (Verification this pass is grep/ls only — no build or test execution. The four `DC-NODE-07` tests and its two new CI gates were located on disk: `node_spine_serve_loopback_follower_fetches_self_accepted_block` + `node_serve_start_failure_is_surfaced_not_silent` in `crates/ade_node/tests/node_spine_serve_loopback.rs`, `n2n_supported_for_magic_produces_configured_magic` in `crates/ade_network/src/handshake/version_table.rs`, `node_c1_serve_live` in `crates/ade_node/tests/node_c1_serve_live.rs`; `ci/ci_check_single_serve_dispatch_authority.sh` + `ci/ci_check_serve_listener_magic_aware.sh` both present.)
...
- **[not found on disk — drift]** (tests) / **[not on disk — drift]** (CI) flags a registry-named test/CI that could not be located in the working tree at `f59b1eaa` — a registry↔code coherence gap to reconcile. This pass: **0** such drifts (every cited test leaf-fn and every cited CI script resolves on disk; the 4 `DC-NODE-07` tests + its 2 new CI gates all resolve).
...
| `enforced` | 184 |
| `partial` | 20 |
| `declared` | 112 |  <!-- DC-NODE-07 left declared at G-D; flipped enforced this close -->
| `deprecated` | 0 |

By tier: constraint 4 · constraint_network 2 · derived 191 · operational 16 · release 41 · true 62.

By family: T 30 · DC 148 · CN 113 · RO 16 · OP 9.
...
| Aspect | Location |
|--------|----------|
| **Source** | `docs/planning/phase4-n-f-g-d-invariants.md`; cluster docs `docs/clusters/PHASE4-N-F-G-D/` (`cluster.md`, `S1-path-fidelity-proof-fence.md`, `S2-rehearsal-evidence-surface-gate.md`, `S3-c1-dry-run-runbook-operator-scaffold.md`, `S4-rehearsal-leak-gate-archived-home-hardening.md`) |
...
### CODEMAP (`docs/ade-CODEMAP.md` @ `6bd60c80`, PHASE4-N-F-G-D close)

CODEMAP was regenerated for the PHASE4-N-F-G-D close at **`6bd60c80`** (alongside this doc). It records **121** `ci_check_*.sh` gates, **456** canonical types, **2241** tests, **315** rules — the CI count matches the on-disk count at this working tree (`ls ci/ci_check_*.sh | wc -l` = 121). Every crate named in a **Code** row is present in CODEMAP: `ade_codec`, `ade_core`, `ade_core_interop`, `ade_crypto`, `ade_ledger`, `ade_network`, `ade_node`, `ade_plutus`, `ade_runtime`, `ade_testkit`, `ade_types`. No crate-level mismatch. Every `crates/**.rs` path cited in any `code_locus` resolves on disk (the `ci_check_registry_code_locus_exists.sh` invariant) — including the four loci on the NEW `CN-REHEARSAL-FIDELITY-01` **Code** row: `crates/ade_node/src/rehearsal_evidence.rs` + `crates/ade_node/src/rehearsal_pass.rs` (both NEW this cluster), `crates/ade_node/src/ba02_evidence.rs` (reused), and `crates/ade_runtime/src/consensus_inputs/canonical.rs` (the shared `import_live_consensus_inputs` at `canonical.rs:112`, pre-existing).

- **No BLUE crate changed this cluster.** PHASE4-N-F-G-D is a four-slice bounty-DRY-RUN-harness cluster: it adds a path-fidelity proof + fence (S1), a non-promotable rehearsal-evidence surface + gate (S2), a C1 dry-run runbook + operator-gated scaffold (S3), and a close-surfaced rehearsal-leak-gate archived-home hardening (S4). Independently verified two ways: (a) `git diff --name-only da205bff..6bd60c80 --` over every `.idd-config.json` `core_paths` (BLUE) entry is **empty** — every changed `crates/` file is in RED `ade_node` (`src/{lib.rs, rehearsal_evidence.rs, rehearsal_pass.rs}`, `tests/{node_path_fidelity.rs, node_c1_dry_run_rehearsal.rs, rehearsal_gate_archived_home.rs}`); (b) CODEMAP re-derives every BLUE crate's struct/enum count byte-stable at BOTH `6f848825` and `6bd60c80`: `ade_codec` 11, `ade_types` 81, `ade_crypto` 21, `ade_core` 49, `ade_ledger` 177, `ade_plutus` 8 (= 347) + `ade_network` BLUE submodules 109 = **456**. The NEW `ade_node::rehearsal_evidence` is **GREEN-by-content** (closed-enum `match` + `format!`; no `HashMap` / float / clock / rand / I/O) and the NEW `ade_node::rehearsal_pass` is **RED** (`fs::read`/`fs::write` glue reusing the GREEN `ba02_pass::correlate_peer_log_file` verbatim) — neither lives on a BLUE `core_paths` path, so neither is canonical-counted. The canonical-type total holds at **456**.
- **CI count is aligned:** CODEMAP's 121 matches the on-disk `ci/ci_check_*.sh` count (Δ +2 vs the N-F-G-E close's 119: the two new G-D gates `ci_check_node_path_fidelity.sh` + `ci_check_rehearsal_manifest_schema.sh`). The three serve/containment/memory fences `ci_check_node_run_loop_containment.sh` + `ci_check_served_chain_handoff_fence.sh` + `ci_check_live_feed_memory_bounds.sh` are **byte-unchanged** across the G-D span (`git diff --stat da205bff 6bd60c80 --` over all three paths is empty). The bounty BA-02 gate `ci_check_ba02_evidence_manifest_schema.sh` is likewise **byte-unchanged** vs the G-C slice-span HEAD `90791691` (verified `git diff --stat 90791691 6bd60c80 --` empty) — G-D deliberately does not touch it.

### HEAD_DELTAS (`docs/ade-HEAD_DELTAS.md` §5)

HEAD_DELTAS is **one cluster behind this pass** — its window is the **G-E** close (baseline `90791691`, HEAD `6f848825`) and it renders **314** rules / **119** CI gates with `DC-LIVEMEM-01` as the newest entry. It does **not yet narrate** the G-D span (`6f848825 → 6bd60c80`) and does not catalogue the two new G-D gates (`ci_check_node_path_fidelity.sh`, `ci_check_rehearsal_manifest_schema.sh`) or the NEW `CN-REHEARSAL-FIDELITY-01`. **This is a recorded coherence note, not soften'd:** the registry, CODEMAP, SEAMS, and *this* TRACEABILITY all agree at `6bd60c80` (121 gates, 315 rules, `CN-REHEARSAL-FIDELITY-01` enforced); HEAD_DELTAS lags at `6f848825` and is the one of the four docs to refresh next (`/head-deltas 6f848825`). (It has advanced since the prior TRACEABILITY pass, which found it two clusters behind at `339cccb1`.)

The two G-D gates and their rule-binding status (these will appear in HEAD_DELTAS §5 once it is refreshed):

- `ci_check_node_path_fidelity.sh` (CE-G-D, S1) — **NEW gate, rule-bound to `CN-REHEARSAL-FIDELITY-01`** clause 1 (appears on that rule's **CI** row above; resolves on disk; **3** registry hits). Guard (a): the `cli.rs` argv flag set equals the pinned 28-flag closed allow-list (G-D adds none — a private-only / venue flag trips it). Guard (b): no from-genesis consensus-inputs constructor exists AND `node_lifecycle.rs` sources consensus inputs only via the shared `import_live_consensus_inputs`. Executed this pass: `OK (node path fidelity): cli.rs flag set matches the pinned closed allow-list (28 flags); no from-genesis consensus-inputs constructor; node path sources consensus inputs via the shared import_live_consensus_inputs (CN-REHEARSAL-FIDELITY-01 clause 1).` (exit 0).
- `ci_check_rehearsal_manifest_schema.sh` (CE-G-D, S2; hardened S4) — **NEW gate, rule-bound to `CN-REHEARSAL-FIDELITY-01`** clause 2 (on the same **CI** row; resolves on disk; **4** registry hits). Vacuous-until-committed; closed rehearsal schema + `venue`/private-testnet + `is_rehearsal`/`not_bounty_evidence` markers + `peer_log_file_sha256` binding; the THREE non-promotability barriers, with the S4 hardening that the bounty-home cross-check scans BOTH the active `docs/clusters/PHASE4-N-F-G-C/` AND the archived `docs/clusters/completed/PHASE4-N-F-G-C/` homes (the durable regression `rehearsal_gate_fails_on_archived_home_leak` pins this). Executed this pass: `[ci_check_rehearsal_manifest_schema] PASS (no rehearsal manifest committed; vacuous — the C1 dry-run is operator-gated)` (exit 0).
- `ci_check_ba02_evidence_manifest_schema.sh` (the bounty BA-02 gate, G-C S2) — **byte-unchanged** across the G-D span; re-run green this pass: `[ci_check_ba02_evidence_manifest_schema] PASS (no manifest committed; vacuous — live ACCEPT is operator-gated)` (exit 0). **Carried follow-up (a real gap, not a G-D regression):** its bounty-home glob is still the single-home `find docs/clusters/PHASE4-N-F-G-C -name "CE-G-C-LIVE_*.toml"` — it does **NOT** scan the archived `docs/clusters/completed/PHASE4-N-F-G-C/` home. The S4 hardening fixed exactly this stale-active-home class **on the rehearsal gate**; the identical fix on the BA-02 bounty gate is a named, deliberately-deferred follow-up (recorded in the registry S4 evidence note: "NO change to the BA-02 bounty gate (its same stale active-home glob is a separate follow-up)"). It is benign **today** (0 `CE-G-C-LIVE_*.toml` committed under either home, so the BA-02 gate is vacuously green either way), but it should be hardened the same way before any live BA-02 manifest is committed.

**CI gates on disk not bound to any rule CI row (3 of 121):** unchanged from the prior pass — the project-wide hygiene/meta gates `ci_check_module_headers.sh`, `ci_check_no_secrets.sh`, `ci_check_pallas_quarantine.sh` (expected-unbound; no single invariant to attribute them to; verified **0** registry hits each). Both new G-D gates **are rule-bound** (`ci_check_node_path_fidelity.sh` 3 hits, `ci_check_rehearsal_manifest_schema.sh` 4 hits) — so neither is a new unbound gate; the unbound set stays **3**, not 5. The other **118** on-disk gates are rule-bound.

### SEAMS

SEAMS (`docs/ade-SEAMS.md`) was **regenerated at this same HEAD `6bd60c80`** (456 canonical types, 121 CI checks, 315 rules) and is current. Closed registries / closed grammars referenced by rule sources (mini-protocol ID registry, closed message taxonomies, closed log/event vocabularies, `LeaderCheckVerdict`, `ExpectedVrfInput`, `Mode`, `IngressSource`, the PHASE4-N-Y closed sets, the PHASE4-N-Z `MithrilBootstrapError`, the PHASE4-N-F-C BA-02 evidence surface, the N-F-D loop-planner lifecycle vocabulary, the N-F-E forge-tick `Due | NotDue` planner input, the N-F-F two-variant `ForgeIntent { On(ForgePaths), Off }`, the N-F-G-A closed `SlotAlignmentError` + `ForgeEpochAdmission`, the N-F-G-B constructor-fenced `SelfAcceptedHandoff`, the N-F-G-E additive `SessionError::ReassemblyBufferOverflow`) are catalogued in `docs/ade-SEAMS.md` §3. PHASE4-N-F-G-D adds **one new non-promotable closed surface and no new plugin seam**: the GREEN `ade_node::rehearsal_evidence` surface — `PrivateRehearsalManifest` WRAPS a `correlate`-produced `Ba02Manifest` in a structurally distinct, non-promotable envelope (closed 1-variant `RehearsalVenue { PrivateTestnetC1 }`; sole ctor `from_correlate_outcome` returns `None` on `NoEvidence`; `to_canonical_toml` always emits `is_rehearsal = true` + `not_bounty_evidence = true` as literals). It introduces **no new BLUE authority, no new canonical type, no new `NodeBlockSource` variant, no new `CoordinatorEvent` variant, no new `--mode node` argv flag, and no from-genesis consensus-inputs constructor** — the litmus the S1 fence enforces (every G-D element must transfer verbatim to preview/preprod).

---
## Honest enforcement gaps (standing)

The four-cell contract is filled for every `enforced` rule (Code + Tests + CI all present and verified in the working tree at `f59b1eaa` via grep/ls — the four `DC-NODE-07` tests and its two new CI gates `ci_check_single_serve_dispatch_authority.sh` + `ci_check_serve_listener_magic_aware.sh` all resolve on disk). The standing gaps below are recorded rather than hidden.

- **Honest scope of this close (load-bearing — do NOT soften / do NOT broaden).** PHASE4-N-F-G-H wires `--mode node` to **serve the G-B self-accepted `ServedChainView` READ-ONLY to real peers** through the single shared ChainSync + BlockFetch serve path, on the configured network magic, via a **sibling** listener/serve-dispatch task spawned **OUTSIDE** `run_relay_loop` — and nothing more. The precise enforced claim (`DC-NODE-07`): *`--mode node` can serve the G-B self-accepted `ServedChainView` to real peers through the single shared ChainSync + BlockFetch serve path, using the configured network magic, outside `run_relay_loop`*. A follower can now **discover Ade's served tip** (ChainSync header advertisement, `DC-CONS-18`) and **fetch the served block by hash** (BlockFetch body, `DC-CONS-17`). It is **NOT** a peer-ACCEPTANCE claim, **NOT** BA-02 / live-evidence, **NOT** an `advance_tip` / durable-tip / gossip change, and **NOT** a `--mode produce` switch. The serve is **request-driven only** (no proactive `advance_tip` / `changed()` reactor). `run_relay_loop` containment is **byte-/semantically unchanged** (`CN-NODE-02`); there is **NO** second `ServedChain` authority, **NO** parallel tag-24 serializer (the single `CN-WIRE-08` authority is reused), **NO** new `--mode node` flag (the existing `--listen` is reused), and **NO** new BLUE authority / canonical type / variant (the lone BLUE touch is the additive *function* `n2n_supported_for_magic`). There is **NO RO-LIVE flip**: the live C1 serve execution stays `blocked_until_operator_c1_serve_executed` (env-gated `node_c1_serve_live` harness only). **Hard limit (do not soften):** wire-serve success ≠ peer acceptance — peer ACCEPT is proven only by the peer's validation log through `ba02_evidence::correlate`.

- **`DC-NODE-07` (PHASE4-N-F-G-H, `declared → enforced` this close) — node-spine live serve-to-peer. No gap on this rule.** Four-cell contract filled and verified on disk (Code: NEW RED `ade_runtime::network::serve_dispatch` single shared core [S1, OQ1 resolved — extracted, not duplicated] + NEW RED `ade_node::node_lifecycle::run_node_serve_task` sibling serve task outside `run_relay_loop` [S2] + the additive BLUE `ade_network::handshake::version_table::n2n_supported_for_magic` configured-magic serve table [S2b] + the reused-unchanged BLUE `producer_chain_sync_serve` / `producer_block_fetch_serve` + the reused GREEN `ServedChainView`; Tests: **4**, all on disk — `node_spine_serve_loopback_follower_fetches_self_accepted_block`, `node_serve_start_failure_is_surfaced_not_silent`, `n2n_supported_for_magic_produces_configured_magic`, `node_c1_serve_live`; CI: `ci_check_single_serve_dispatch_authority.sh` + `ci_check_serve_listener_magic_aware.sh`, both NEW). `tier = derived`. Its `cross_ref` (`DC-NODE-06`, `CN-NODE-02`, `DC-CONS-17`, `DC-CONS-18`, `CN-PROTO-06`, `DC-PROTO-07`, `DC-PROTO-08`, `CN-WIRE-08`, `CN-FORGE-01`, `RO-LIVE-01`, `RO-LIVE-06`, `CN-REHEARSAL-FIDELITY-01`) names the carried invariants and the live ACCEPT it unblocks — it does **NOT** advance `RO-LIVE-01` / `RO-LIVE-06` (verified: neither contains `PHASE4-N-F-G-H` in `strengthened_in`).

- **Five carry-forward strengthenings (`DC-CONS-17`, `DC-CONS-18`, `DC-PROTO-07`, `DC-PROTO-08`, `CN-PROTO-06`) — each gained `strengthened_in += "PHASE4-N-F-G-H"`; no four-cell change.** S2's hermetic `node_spine_serve_loopback` test is a NEW end-to-end serve-direction enforcement site for each — the BLUE serve reducers (and the closed server-agency reply surface) driven over a **real socket** from the `--mode node` spine through the extracted single `serve_dispatch` core. The reducers are **reused unchanged** (no BLUE change), so each rule's Source / Code / Tests / CI rows are unchanged; the registry records the added enforcement site via `strengthened_in`, surfaced as a per-rule note above the table. Verified in the registry: each of the five carries `PHASE4-N-F-G-H` in `strengthened_in`.

- **Honest scope of this close (load-bearing — do NOT soften / do NOT broaden).** PHASE4-N-F-G-D ships a **path-faithful, non-promotable bounty-acceptance DRY-RUN harness** on the `--mode node` accepted-block spine — and nothing more. It enforces that the C1 private dry-run uses the **SAME** accepted-block path as preview/preprod (clause 1) and that any C1 rehearsal manifest is **structurally non-promotable** to bounty evidence (clause 2). It does **NOT** enforce that a C1 run has succeeded — the rehearsal-schema gate is **vacuous until a real operator-produced manifest is committed** (only the README is committed under the rehearsal home; **0** `.toml` manifests). There is **NO RO-LIVE flip**, **NO bounty / preview / preprod completion claim**, and **NO BLUE change**; the live C1 execution stays `blocked_until_operator_c1_net_executed`. **Hard limit (do not soften):** private C1 acceptance ≠ bounty completion — preview/preprod acceptance is the single bounty deliverable, captured separately. The serve / forge / live-feed / containment surfaces (`ci_check_node_run_loop_containment.sh`, `ci_check_served_chain_handoff_fence.sh`, `ci_check_live_feed_memory_bounds.sh`) and the BA-02 bounty gate (`ci_check_ba02_evidence_manifest_schema.sh`) are all **byte-unchanged**.

- **`CN-REHEARSAL-FIDELITY-01` (PHASE4-N-F-G-D, `enforced`) — NEW; bounty dry-run path-fidelity + evidence-non-promotability. No gap on this rule.** Four-cell contract filled and verified (Code: GREEN `ade_node::rehearsal_evidence` `PrivateRehearsalManifest` + RED `ade_node::rehearsal_pass` I/O + the reused GREEN `ba02_evidence::correlate` + the shared `ade_runtime::consensus_inputs::canonical::import_live_consensus_inputs` the S1 fence pins; Tests: **7**, all on disk — `node_accepted_block_consensus_inputs_via_shared_import`, the 3 `rehearsal_*` unit tests in `rehearsal_pass.rs`, `c1_dry_run_correlate_to_rehearsal_envelope`, `node_c1_dry_run_rehearsal_live`, `rehearsal_gate_fails_on_archived_home_leak`; CI: `ci_check_node_path_fidelity.sh` + `ci_check_rehearsal_manifest_schema.sh`, both NEW, both exit 0). `tier = release`. **No `strengthened_in` bump on any sibling** — its `cross_ref` (`RO-LIVE-01`, `RO-LIVE-06`, `CN-OPERATOR-EVIDENCE-01`, `DC-NODE-06`, `DC-EPOCH-03`, `CN-NODE-02`, `CN-CINPUT-03`, `DC-CINPUT-02b`) references the bounty path it rehearses but does **not** advance it; verified that the literal `PHASE4-N-F-G-D` appears in **no** `strengthened_in` array in the registry.

- **`ci_check_ba02_evidence_manifest_schema.sh` stale active-home glob — a REAL carried follow-up, not a G-D regression (surfaced, not hidden).** The bounty BA-02 gate scans only the single active home (`find docs/clusters/PHASE4-N-F-G-C -name "CE-G-C-LIVE_*.toml"`); it does **NOT** scan the archived `docs/clusters/completed/PHASE4-N-F-G-C/` home. The S4 close-surfaced fix hardened exactly this stale-active-home class **on the rehearsal gate** (`ci_check_rehearsal_manifest_schema.sh` now scans both homes, pinned by the durable regression `rehearsal_gate_fails_on_archived_home_leak`) but **deliberately did not** apply the identical fix to the BA-02 bounty gate — recorded verbatim in the registry S4 evidence note ("NO change to the BA-02 bounty gate (its same stale active-home glob is a separate follow-up)"). It is **benign today** (0 `CE-G-C-LIVE_*.toml` committed under either home → the gate is vacuously green either way), and the gate is **byte-unchanged** vs `90791691`, so this is a **carried** gap, not a G-D regression. **Action:** harden the BA-02 bounty gate's glob the same way (scan active + archived bounty homes, fail-closed) before any live BA-02 manifest is committed.

- **`DC-LIVEMEM-01` (PHASE4-N-F-G-E, `enforced`) — live-feed bounded memory, operational-hardening (carried).** Four-cell contract filled and verified at the prior pass and unchanged here (Code: the GREEN `session::core` reassembly-tail cap + additive `SessionError::ReassemblyBufferOverflow`, the RED `node_sync` WirePump lookahead cap, the RED `mux_pump` overflow→drop-peer mapping; Tests: 4; CI: `ci_check_live_feed_memory_bounds.sh`). `tier = derived` — explicitly **NOT BLUE consensus law**. **G-H does not touch `DC-LIVEMEM-01`** nor the gate that enforces it (byte-unchanged). Carried unchanged.

- **`declared` rules (112) are open enforcement gaps by design.** Each carries `_(no enforcing code located — gap)_` / `_(no tests named in registry — gap)_` / `_(no CI script — gap)_` cells (or a subset). These are named-but-not-yet-mechanically-enforced invariants; the gap is the actionable backlog, not a defect to paper over. (113 → **112** this close — `DC-NODE-07` flipped `declared → enforced`; no other `declared` row moved.)

- **`CN-PROD-04` (PHASE4-N-T, `enforced`; strengthened in N-F-G-B) — produce_mode broadcast bridge; standing CI gap (carried).** The four-cell contract is filled except CI (Code: `produce_mode.rs` `BroadcastBlock` arm → `push_atomic` + `BroadcastPushError`; Tests: 3; CI: `_(no CI script — gap)_`). The rule's `ci_script` is empty in the registry; its enforcement is test-only (the N-T loopback + rejection tests). Carried unchanged from the prior pass.

- **`CN-OPERATOR-EVIDENCE-01` (PHASE4-N-S-C, `enforced`; strengthened N-F-G-C) — operator-pass manifest schema; standing Tests gap (carried).** The rule's `tests` array is `[]` in the registry; it is **gate-enforced** (the manifest-schema gates fire only when a `CE-*-LIVE_*.toml` is committed, and **0** are committed at HEAD, so they are vacuously satisfied). **G-H does not bump it** (no `strengthened_in += "PHASE4-N-F-G-H"`). Carried unchanged as a standing Tests gap.

- **`RO-LIVE-01` (PHASE4-N-G, `partial`; strengthened N-Q + N-F-G-C) — producer-side live leg, MECHANICAL half wired; NOT advanced by G-H (carried).** Four-cell contract has Code + Tests + CI, but **status stays `partial`** (verified: registry `status = "partial"`, `strengthened_in = ["PHASE4-N-Q", "PHASE4-N-F-G-C"]` — **no `PHASE4-N-F-G-H` token**). The LIVE half is operator-gated: peer ACCEPT stays `blocked_until_operator_stake_available`. **G-H does NOT touch `RO-LIVE-01`'s status or `strengthened_in`** — `DC-NODE-07` only `cross_ref`s it. G-H enforces the serve MECHANISM (`DC-NODE-07`); the live peer-ACCEPT stays operator-gated (`blocked_until_operator_c1_serve_executed`). Wire-serve success ≠ peer acceptance. Carried unchanged.

- **`RO-LIVE-06` (PHASE4-N-F-C, `enforced`; strengthened N-F-G-C) — BA-02 evidence, schema + correlation mechanics ONLY; NOT advanced by G-H (carried).** What is enforced: the closed versioned `Ba02Manifest`, the pure/total/deterministic HASH-PRIMARY `correlate` as the SOLE constructor, the allow-list parser, and the operator-pass file I/O whose write path accepts only a `correlate`-produced `Ba02Manifest`. **What is NOT claimed:** that BA-02 was achieved live. Verified: registry `status = "enforced"`, `strengthened_in = ["PHASE4-N-F-G-C"]` — **no `PHASE4-N-F-G-H` token**. **G-H does NOT touch `RO-LIVE-06`'s status or `strengthened_in`** — `DC-NODE-07` only `cross_ref`s it; wiring the serve produces no BA-02 / acceptance claim. Carried unchanged.

- **`DC-NODE-06` (PHASE4-N-F-G-B, `enforced`; strengthened N-F-G-C) — self-accept → serve handoff, mechanism-only (carried).** What is enforced: only a BLUE self-accepted artifact (typed `SelfAcceptedHandoff`, sole provenance `from_self_accepted`) reaches the single `ServedChainHandle::push_atomic`, fed only by `into_accepted()`; the relay-loop body performs no serve/admit/`push_atomic`/durable-tip mutation. **G-H consumes — but does not modify — this fence:** `DC-NODE-07`'s serve reads the `ServedChainView` (the read side of the single `ServedChainHandle` `DC-NODE-06` feeds), keeping the single-served-source invariant; the handoff fence is byte-unchanged and gains no `strengthened_in` token. Carried unchanged.

- **Operator-gated release legs unchanged this close (none flipped):** `RO-LIVE-01` (producer-side live leg against a real peer — MECHANICAL half wired by G-C, LIVE peer-ACCEPT operator-gated), `RO-LIVE-06` (BA-02 evidence — schema/correlation mechanics only; peer ACCEPT proven only by the peer's validation log, never by Ade's self-accept / `ForgeSucceeded` / served-block / any wire-success signal), `RO-SYNC-EVIDENCE-01` (vacuous-until-committed manifest), `CE-Y-16` (two-Haskell-node witnessed live capture). PHASE4-N-F-G-H does **not** flip any of these — it enforces the serve MECHANISM (`DC-NODE-07`, `declared → enforced`) on the `--mode node` spine while the live peer-ACCEPT stays operator-gated (`blocked_until_operator_c1_serve_executed`); `DC-NODE-07` only `cross_ref`s `RO-LIVE-01` / `RO-LIVE-06` and advances neither (the decoupling). The literal `PHASE4-N-F-G-H` appears in **no** `RO-*` `strengthened_in` array — verified by grep over the registry.

---

## Generation notes

INCREMENTAL refresh — **PHASE4-N-F-G-H close** at HEAD `f59b1eaa` (the S3 close commit; the registry `DC-NODE-07` `declared → enforced` flip + the five `strengthened_in` bumps are staged in the working tree alongside the refreshed `docs/ade-CODEMAP.md`). The rule set is **316** (count unchanged this close): `DC-NODE-07` (`tier = derived`, `introduced_in = "PHASE4-N-F-G-H"`) was already present as a `declared` row from the G-H invariant sketch and is flipped to `status = enforced` here, joining its Source / Code / Tests / CI from the registry + on-disk introspection. This pass re-runs the mechanical counts + on-disk existence checks (grep/ls); no build or test execution. The prior pass was the PHASE4-N-F-G-D close (HEAD `6bd60c80`, 315 rules, inventory 183 / 20 / 112).

The substantive delta since the prior pass — **one rule flipped `declared → enforced` + five carry-forward `strengthened_in` bumps; NO existing rule weakened / re-joined / removed / deprecated; NO `RO-*` advanced:**

- **`DC-NODE-07` — flipped `declared → enforced`.** `introduced_in = "PHASE4-N-F-G-H"`, `strengthened_in = []`, `tier = derived`. **Source:** `docs/planning/phase4-n-f-g-h-invariants.md`. **Enforced claim (NARROW):** `--mode node` can serve the G-B self-accepted `ServedChainView` to real peers through the single shared ChainSync + BlockFetch serve path, on the configured network magic, **outside** `run_relay_loop`. **Code:** NEW RED `ade_runtime::network::serve_dispatch` (S1 — single shared serve-dispatch core, `dispatch_server_frame_event_to_outbound` extracted verbatim from `produce_mode`; OQ1 resolved → no second serve authority) + NEW RED `ade_node::node_lifecycle::run_node_serve_task` (S2 — sibling serve task spawned outside `run_relay_loop`, retains the `ServedChainView`, closed `ServeStartError`) + additive BLUE `ade_network::handshake::version_table::n2n_supported_for_magic` (S2b — configured-magic serve table; the lone BLUE *function* touch, no struct/enum) + reused-unchanged BLUE `producer_chain_sync_serve` / `producer_block_fetch_serve` + reused GREEN `ServedChainView`. **Tests:** the 4 named below. **CI:** `ci/ci_check_single_serve_dispatch_authority.sh` + `ci/ci_check_serve_listener_magic_aware.sh` (both NEW). **cross_ref:** `DC-NODE-06`, `CN-NODE-02`, `DC-CONS-17`, `DC-CONS-18`, `CN-PROTO-06`, `DC-PROTO-07`, `DC-PROTO-08`, `CN-WIRE-08`, `CN-FORGE-01`, `RO-LIVE-01`, `RO-LIVE-06`, `CN-REHEARSAL-FIDELITY-01` — names the live ACCEPT it unblocks (`RO-LIVE-01`) but does **NOT** advance it.

- **Five carry-forward strengthenings — `DC-CONS-17`, `DC-CONS-18`, `DC-PROTO-07`, `DC-PROTO-08`, `CN-PROTO-06` each gained `strengthened_in += "PHASE4-N-F-G-H"`.** S2's hermetic `node_spine_serve_loopback` test (`node_spine_serve_loopback_follower_fetches_self_accepted_block`) is a NEW end-to-end serve-direction enforcement site: the BLUE serve reducers (and the closed server-agency reply surface `CN-PROTO-06`) driven over a **real socket** from the `--mode node` spine through the extracted single `serve_dispatch` core. The reducers are **reused unchanged (no BLUE change)** — so each rule's Source / Code / Tests / CI rows are unchanged; only `strengthened_in` records the added site (surfaced as a per-rule note above each table). Verified: each of the five carries `PHASE4-N-F-G-H` in `strengthened_in`.

No other rule was added, removed, weakened, re-joined, or deprecated. **Crucially, G-H advances no bounty / RO deliverable:** the literal `PHASE4-N-F-G-H` appears in **no** `RO-*` `strengthened_in` array in the registry — `RO-LIVE-01` stays `partial` (`["PHASE4-N-Q", "PHASE4-N-F-G-C"]`), `RO-LIVE-06` stays `enforced` (`["PHASE4-N-F-G-C"]`) — verified by grep. `DC-NODE-07` only `cross_ref`s `RO-LIVE-01` / `RO-LIVE-06`: the serve MECHANISM is enforced; the live peer-ACCEPT stays operator-gated (`blocked_until_operator_c1_serve_executed`). No BLUE canonical type changed (**456** holds per the refreshed CODEMAP — the lone BLUE touch is the additive *function* `n2n_supported_for_magic`, not a struct/enum). Two CI scripts added on disk (121 → 123: `ci_check_single_serve_dispatch_authority.sh` + `ci_check_serve_listener_magic_aware.sh`), both rule-bound to `DC-NODE-07`.

Mechanical counts recomputed fresh from the registry working tree (`grep -cE '^\[\[rules\]\]'` = **316**; `grep -cE '^id = '` = **316**; no duplicate IDs; `grep -cE '^status = '` = **316**):

- Status: **184 enforced** / **20 partial** / **112 declared** / 0 deprecated. (**+1 enforced** vs the prior pass — `DC-NODE-07` flipped `declared → enforced`; `declared` correspondingly **−1** (113 → 112). Cross-checked against the anchored registry `^status =` lines: 184 / 20 / 112; sum 316.)
- By tier: constraint 4 · constraint_network 2 · derived **191** · operational 16 · release 41 · true 62. (`derived` +1 vs the G-D-baseline — `DC-NODE-07` is `tier = derived` (the G-H-sketch addition); no tier moved at the close flip.)
- By family: T 30 · DC **148** · CN 113 · RO 16 · OP 9. (`DC` +1 vs the G-D-baseline — `DC-NODE-07`'s `DC-` ID prefix; inserted in ID order between `DC-NODE-06` and `DC-OPCERT-01`.)

**Drift check (this pass): 0 drifts.** All 4 `DC-NODE-07` tests resolve on disk: `node_spine_serve_loopback_follower_fetches_self_accepted_block` + `node_serve_start_failure_is_surfaced_not_silent` (`crates/ade_node/tests/node_spine_serve_loopback.rs`); `n2n_supported_for_magic_produces_configured_magic` (`crates/ade_network/src/handshake/version_table.rs:108`); `node_c1_serve_live` (`crates/ade_node/tests/node_c1_serve_live.rs`). All code loci resolve on disk: `serve_dispatch` module (`crates/ade_runtime/src/network/serve_dispatch.rs`, declared in `network/mod.rs`); `run_node_serve_task` (`crates/ade_node/src/node_lifecycle.rs:726`); `n2n_supported_for_magic` (`crates/ade_network/src/handshake/version_table.rs:69`); the reused BLUE `producer_chain_sync_serve` / `producer_block_fetch_serve` (`crates/ade_network/src/{chain_sync,block_fetch}/server.rs`); the reused GREEN `ServedChainHandle` / `ServedChainView` (`crates/ade_runtime/src/producer/served_chain_handle.rs`). The source doc `docs/planning/phase4-n-f-g-h-invariants.md` and the cluster/slice docs under `docs/clusters/PHASE4-N-F-G-H/` resolve. Both CI gates resolve on disk: `ci/ci_check_single_serve_dispatch_authority.sh` + `ci/ci_check_serve_listener_magic_aware.sh`. The five strengthened rows' tests/CI were unchanged and were already on-disk at the prior pass. **Cross-doc coherence:** the three G-H code loci are all listed in the refreshed `docs/ade-CODEMAP.md` (PHASE4-N-F-G-H delta block). **HEAD_DELTAS note (carried, expected):** `docs/ade-HEAD_DELTAS.md` is still at the G-D baseline (`6bd60c80`) and does **not** yet list the two new G-H CI gates in §5 — a separate doc refresh, not a TRACEABILITY drift. Across the full 316 rules the prior pass recorded 0 unresolved references; this close adds only resolved references, so the **0-drift** property holds.

**CODEMAP** (regenerated this close at `6bd60c80`, **121** CI checks, **456** canonical types, **2241** tests, **315** rules) is coherent with this pass: it records 121 gates (matching on-disk `ls ci/ci_check_*.sh | wc -l` = 121), no BLUE change (456 canonical types unchanged), and the `CN-REHEARSAL-FIDELITY-01` loci (GREEN `rehearsal_evidence`, RED `rehearsal_pass`, the reused `ba02_evidence::correlate`, the shared `import_live_consensus_inputs`). **SEAMS** was regenerated at the same HEAD (`6bd60c80`; 456 / 121 / 315) and catalogues the new non-promotable rehearsal-evidence surface in §3. **HEAD_DELTAS is one cluster behind** — its window is the **G-E** close (baseline `90791691`, HEAD `6f848825`, **119** CI, 314 rules, `DC-LIVEMEM-01` the newest entry) and it narrates neither the `6f848825 → 6bd60c80` G-D span nor the two new G-D gates. This is recorded as a coherence note (not soften'd): registry + CODEMAP + SEAMS + this TRACEABILITY agree at `6bd60c80`; HEAD_DELTAS lags and is the doc to refresh next (`/head-deltas 6f848825`). The remaining actionable output — the `_(… — gap)_` cells (the 112 `declared` rules; the standing `CN-PROD-04` CI gap; the `CN-OPERATOR-EVIDENCE-01` standing Tests gap; the project-wide hygiene/meta unbound gates, 3 of 121) and the **carried `ci_check_ba02_evidence_manifest_schema.sh` stale-active-home follow-up** flagged in the **Honest enforcement gaps** section — should be reconciled in the registry or the codebase, not deleted. Regenerate after every cluster close.
