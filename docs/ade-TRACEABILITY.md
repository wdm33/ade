...
>
> **HEAD:** `cd2484f` (working tree, PHASE4-N-F-E cluster close) · **Generated:** 2026-05-31 · **Baseline:** `51c9fbf` (`.idd-config.json` `head_deltas_baseline`, PHASE4-N-F-C close)
...
The canonical source is the **invariant registry** at `docs/ade-invariant-registry.toml` (`.idd-config.json` `invariant_registry`), **310 rules** at this working tree. This doc is a join: registry entries × codebase introspection. Rule IDs, families, names, sources, and the registry-recorded `tests` / `ci_script` / `ci_scripts` / `code_locus` fields come from the registry; each test name and CI script is cross-checked for existence in the working tree at `cd2484f`, and divergences are flagged inline as **drift**. (Verification this pass is grep/ls only — no build or test execution.)
...
- **[not found on disk — drift]** (tests) / **[not on disk — drift]** (CI) flags a registry-named test/CI that could not be located in the working tree at `cd2484f` — a registry↔code coherence gap to reconcile (rename the registry entry or restore the artifact). Test drift is resolved against the **leaf** function name (the segment after the last `::`).
...
By tier: constraint 4 · constraint_network 1 · derived 187 · operational 16 · release 40 · true 62.

By family: T 30 · DC 144 · CN 111 · RO 16 · OP 9.
...
### CODEMAP (`docs/ade-CODEMAP.md` @ `cd2484f`, PHASE4-N-F-E close)

CODEMAP was regenerated for the PHASE4-N-F-E close at `cd2484f`. It records **110** `ci_check_*.sh` gates (matching the on-disk count at this working tree). Every crate named in a **Code** row is present in CODEMAP: `ade_codec`, `ade_core`, `ade_core_interop`, `ade_crypto`, `ade_ledger`, `ade_network`, `ade_node`, `ade_plutus`, `ade_runtime`, `ade_testkit`, `ade_types`. No crate-level mismatch.

- **No BLUE crate changed this cluster.** PHASE4-N-F-E is GREEN/RED relay-loop forge-tick wiring inside `ade_node` (`node_lifecycle.rs`, `run_loop_planner.rs`) on top of the N-F-D spine; leadership eligibility stays in BLUE `forge_one_from_recovered`. The DC-NODE-05 code loci are in the already-mapped `ade_node` crate. Enforcement is **mechanical at the library surface** (named tests + the two per-rule gates), not via the binary's live run path — RO-LIVE-01 is unchanged / operator-gated.
- **CI count is aligned:** CODEMAP's 110 matches the on-disk `ci/ci_check_*.sh` count at this working tree.

### HEAD_DELTAS (`docs/ade-HEAD_DELTAS.md` §5)

HEAD_DELTAS' baseline is `51c9fbf` (the PHASE4-N-F-C close). The PHASE4-N-F-D run-loop gates and the PHASE4-N-F-E forge-tick gates are **rule-bound**; reconcile their cataloguing on the next HEAD_DELTAS regeneration (bump baseline to the N-F-E close commit `cd2484f`):

- `ci_check_node_run_loop_containment.sh` — bound to **CN-NODE-02, DC-SYNC-02, T-REC-03, DC-NODE-05** (N-F-D relay-loop containment, extended by N-F-E forge-tick subordination).
- `ci_check_loop_planner_closed.sh` — bound to **CN-NODE-02, DC-NODE-05** (closed GREEN loop-planner lifecycle vocabulary + forge-precedence table).
- `ci_check_node_sync_via_pump.sh` — bound to **DC-SYNC-01, DC-SYNC-02**.

No HEAD_DELTAS §5 gate is referenced by zero rules across this delta.

**CI gates on disk not bound to any rule CI row** (surfaced, not fixed this pass): **3 of 110** on-disk `ci_check_*.sh` gates are unbound — these are project-wide hygiene/meta gates that cannot be attributed to a single invariant. Full unbound list: `ci_check_module_headers.sh`, `ci_check_no_secrets.sh`, `ci_check_pallas_quarantine.sh`. The remaining 107 on-disk gates are rule-bound.

### SEAMS

Closed registries / closed grammars referenced by rule sources (mini-protocol ID registry, closed message taxonomies, closed log/event vocabularies, `LeaderCheckVerdict`, `ExpectedVrfInput`, `Mode`, `IngressSource`, the PHASE4-N-Y closed sets, the PHASE4-N-Z `MithrilBootstrapError`, the PHASE4-N-F-C BA-02 evidence surface) are catalogued in `docs/ade-SEAMS.md` §3. PHASE4-N-F-D adds the closed GREEN loop-planner lifecycle vocabulary `{ SyncOnce, Idle, HaltCleanly }`; PHASE4-N-F-E extends the planner forge input to a closed content-blind `Due | NotDue` and the planner output with a forge-tick precedence table — no new plugin seam: the relay loop consumes closed types and never opens an extension point.
...
- **CN-NODE-02 test drift — RESOLVED this pass.** The prior (N-F-C-baseline) TRACEABILITY flagged CN-NODE-02's `tests` array as naming `plan_loop_step_decision_table_is_total`, which had been renamed by the N-F-E planner extension and was not on disk. The registry `tests` array has since been corrected to `plan_loop_step_forge_precedence_table_is_total` (the total-coverage successor in `crates/ade_node/src/run_loop_planner.rs`), and the old name is absent from both the registry and the codebase at `cd2484f`. All five CN-NODE-02 tests now resolve on disk; the rule carries no drift flag.

- **RO-LIVE-01 — remains `partial` / operator-gated (unchanged this close).** The producer-side block-fetch live leg against a real Haskell peer is still an operator-action pass not yet executed. PHASE4-N-F-E wires the forge tick into the relay run-loop but the live unbounded-peer leg stays RO-LIVE-01 follow-on — DC-NODE-05's forge tick advances NO durable tip and is a local self-accept artifact only.

- **DC-NODE-05 (PHASE4-N-F-E, enforced) — forge tick is subordinate to the sync spine.** Enforced by `ci/ci_check_loop_planner_closed.sh` + `ci/ci_check_node_run_loop_containment.sh` + 10 forge planner/guard/wiring/replay/fail-closed tests in `ade_node` (all resolve on disk). The forge tick advances no durable tip, admits/serves/gossips nothing, and single-epoch-fails-closed on an unsupported slot (cluster-scope containment, not permanent behaviour). This is the **mechanism** enforced at the library surface; it is NOT a live forge-into-the-network claim.

- **Operator-gated release legs unchanged this close:** RO-LIVE-06 (BA-02 evidence — schema/correlation mechanics only), RO-SYNC-EVIDENCE-01 (vacuous-until-committed manifest), CE-Y-16 (two-Haskell-node witnessed live capture). PHASE4-N-F-E does not touch these.

---

## Generation notes

FULL regeneration for the **PHASE4-N-F-E cluster-close** at HEAD `cd2484f` (working tree). Primary source: the working-tree invariant registry (`docs/ade-invariant-registry.toml`, **310 rules**). Every rule was re-joined to its Source / Code / Tests / CI from the registry and every registry-named test (resolved to its leaf `fn`) and CI script was cross-checked against the working tree using **grep/ls only** — no `cargo build`/`cargo test`/`cargo check` was run.

This is a **re-run to clear a single test-drift entry** carried by the prior pass. Since the prior generation, CN-NODE-02's `tests` array was corrected: the renamed N-F-E test `plan_loop_step_forge_precedence_table_is_total` (which exists in `crates/ade_node/src/run_loop_planner.rs`) replaced the stale `plan_loop_step_decision_table_is_total`. Everything else is as in the prior pass (catch-up through PHASE4-N-F-D + PHASE4-N-F-E was completed there).

N-F surface (unchanged structurally; all resolve on disk this pass):

- PHASE4-N-F-D introduced **CN-NODE-02** (`enforced`), **DC-SYNC-02** (`enforced`), **T-REC-03** (`enforced`). PHASE4-N-F-E introduced **DC-NODE-05** (`enforced`, forge-slot discipline on the relay run-loop).
- **`strengthened_in += PHASE4-N-F-E`:** CN-NODE-02, DC-SYNC-02, T-REC-03, DC-NODE-03, CN-PROD-02, DC-CINPUT-02b.
- **T-REC-03 code locus** points at `crates/ade_node/src/node_sync.rs` (where its evidence test `relay_loop_two_clean_runs_byte_identical` lives) alongside `node_lifecycle.rs` (the `run_relay_loop` driver). Both reflected in the row.

Status breakdown at this close: **178 enforced / 20 partial / 112 declared / 0 deprecated**. By tier: constraint 4 · constraint_network 1 · derived 187 · operational 16 · release 40 · true 62. By family: T 30 · DC 144 · CN 111 · RO 16 · OP 9.

**Drift check (this pass):** **0** registry-named tests and **0** registry-named CI scripts fail to resolve on disk at `cd2484f`, across all 310 rules. The CN-NODE-02 drift flagged in the prior pass is **CLEARED** — its `tests` array now names `plan_loop_step_forge_precedence_table_is_total`, present on disk; the old name `plan_loop_step_decision_table_is_total` is absent from both registry and codebase. All DC-NODE-05 / CN-NODE-02 / DC-SYNC-02 / T-REC-03 CI scripts (`ci_check_loop_planner_closed.sh`, `ci_check_node_run_loop_containment.sh`, `ci_check_node_sync_via_pump.sh`) resolve on disk.

**CODEMAP** (regenerated at `cd2484f`, 110 CI checks) maps every code locus inside the already-mapped `ade_node` crate; the on-disk `ci/ci_check_*.sh` count (110) matches CODEMAP. **HEAD_DELTAS** baseline is one+ cluster behind (`51c9fbf`, N-F-C close) and does not yet catalogue the N-F-D/N-F-E gates — reconcile on the next HEAD_DELTAS regeneration. The `_(… — gap)_` cells and the **Honest enforcement gaps** section are the actionable output — reconcile them in the registry or the codebase; do not delete them. Regenerate after every cluster close.
