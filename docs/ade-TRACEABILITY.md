...
>
> **HEAD:** `65954fa3` (PHASE4-N-F-G-K…G-R + C1 evidence catch-up — close-pass `evidence(c1): run-2 genesis-rehearsal reproduction`) · **Generated:** 2026-06-05 · **Prior pass:** `550eec3a` (PHASE4-N-F-G-J close)
...
The canonical source is the **invariant registry** at `docs/ade-invariant-registry.toml` (`.idd-config.json` `invariant_registry`), **328 rules** at HEAD `65954fa3` (`+9` this span: the nine NEW PHASE4-N-F-G-K…G-R rules `DC-NODE-09` (G-K), `CN-WIRE-10` (G-L), `CN-WIRE-11` (G-M), `T-REC-04` + `DC-CINPUT-03` (G-N), `CN-WIRE-12` (G-O), `DC-CINPUT-04` (G-P), `DC-NODE-10` (G-Q), `DC-NODE-11` (G-R), all introduced `enforced`; **0** `strengthened_in` appends across the G-K…G-R span — re-confirmed by grep). This doc is a join: registry entries × codebase introspection. Rule IDs, families, names, sources, and the registry-recorded `tests` / `ci_script` / `code_locus` fields come from the registry; each test name and CI script is cross-checked for existence in the working tree at `65954fa3`, and divergences are flagged inline as **drift**. (Verification this pass is grep/ls only — no build or test execution. This is a **fast delta-refresh** — only the nine G-K…G-R rules were re-traced from scratch; every other rule's trace row is preserved verbatim from the prior pass. The G-K…G-R leaves were all located on disk: every one of the **29 unique cited test leaf-fns** across the nine rules resolves (`fn <name>` present) — `serve_task_*` / `node_spine_serve_loopback_*` / `node_serve_start_failure_*` in `ade_node/tests/node_spine_serve_loopback.rs`; `responder_v15_*` in `ade_network/tests/handshake_responder_cardano_node_fixture.rs`; `real_cardano_node_findintersect_*` in `ade_network/tests/serve_chainsync_findintersect_cardano_node_fixture.rs`; `producer_chain_sync_serve_find_intersect_*` + `decode_array_header_still_rejects_indefinite` + `two_form_accepts_definite_and_indefinite` in `ade_network/src/{chain_sync/server.rs,codec/primitives.rs}`; `warm_start_*` in `ade_runtime/src/bootstrap.rs`; `seed_cinput_decode_rejects_unknown_version` + `seed_epoch_consensus_inputs_round_trips_byte_identical` in `ade_ledger/src/seed_consensus_inputs.rs`; `pinning_*` in `ade_testkit/src/consensus/genesis_pinning.rs`; `recovered_surface_projects_*` in `ade_runtime/src/seed_consensus_merge.rs`; `block_fetch_*` in `ade_runtime/src/admission/wire_pump.rs`; `feed_*` + `serve_gate_keeps_*` in `ade_node/tests/forge_succeeds.rs`; `forge_successor_*` + `forge_one_from_recovered_cold_start_*` + `serve_gate_admits_*` in `ade_node/src/{node_sync.rs,node_lifecycle.rs}`. All **8 distinct CI gates** are present in `ci/` (G-N's `T-REC-04` + `DC-CINPUT-03` share `ci_check_warmstart_eta0_overlay.sh`), and all 8 source cluster docs resolve (`PHASE4-N-F-G-{K,L}` under `docs/clusters/completed/`, `G-{M,N,O,P,Q,R}` under `docs/clusters/`).)
...
- **[not found on disk — drift]** (tests) / **[not on disk — drift]** (CI) flags a registry-named test/CI that could not be located in the working tree at `65954fa3` — a registry↔code coherence gap to reconcile. This pass: **0** such drifts among the nine re-traced G-K…G-R rules (every one of the 29 unique cited test leaf-fns and all 8 distinct CI scripts resolves on disk). Rows outside the G-K…G-R span were not re-verified this delta-refresh — their drift status is carried from the prior pass.
...
By tier: constraint 4 · constraint_network 2 · derived 201 · operational 17 · release 41 · true 63.

By family: T 31 · DC 154 · CN 118 · RO 16 · OP 9.

_G-K…G-R catch-up delta: `+9` rules — `T-REC-04` (true/T), `DC-CINPUT-03` + `DC-CINPUT-04` + `DC-NODE-09` + `DC-NODE-10` + `DC-NODE-11` (derived/DC), `CN-WIRE-10` + `CN-WIRE-11` + `CN-WIRE-12` (derived/CN) — all introduced `enforced`; `+8` distinct CI gates (G-N's two rules share one gate); **0** `strengthened_in` appends in the span. (Section counts below are now reconciled to the live per-section `###` rule-heading counts — the prior pass's DC/CN section labels under-counted by 5/2 respectively; they now equal the by-family ID-prefix counts.)_
...
### `DC-NODE-11`: Stable served block 0 via a monotone node-level serve gate — the served view never re-serves an already-served height…

_tier: derived · status: enforced_ · _introduced_in: PHASE4-N-F-G-R_

| Aspect | Location |
|--------|----------|
| **Source** | `docs/clusters/PHASE4-N-F-G-R/cluster.md` |
| **Requirement** | Once `--mode node` has self-accepted and SERVED a genesis-successor block at `block_no` 0, it MUST NOT add/replace the served view (`ServedChainView`) with another `block_no`-0 block during the same recovered NO-TIP episode (durable ChainDb tip + recovered tip both `None`). The node-level serve gate admits a self-accepted forge handoff to the `ServedChainView` ONLY when its `block_no` STRICTLY EXCEEDS the highest already-served `block_no` (`serve_gate_admits`) — so the FIRST block 0 wins the served view and the hermetic forge's subsequent block-0 re-forges (`DC-NODE-05`, no own-tip advance) are NOT re-served; a follower then sees a STABLE block 0 to fetch + adopt. NARROW: no durable own-tip advance (the own-tip-adoption path is a separate cluster); no forged block 1+ claim; no synthetic numbering; the served block is still self-accepted (no bypass of `self_accept`, no serve of unvalidated bytes); the forge + `served_chain_admit` + durable tip are UNCHANGED (`DC-NODE-05` intact). |
| **Code** | `crates/ade_node/src/node_lifecycle.rs` (`serve_gate_admits` — the pure monotone-`block_no` gate; the serve sibling tracks `highest_served_block_no` + `push_atomic`s a handoff ONLY when `serve_gate_admits` is true); `crates/ade_ledger/src/producer/served_chain.rs` (`ServedChainSnapshot` append-only `BTreeMap` + `served_chain_admit` — UNCHANGED; the node-level gate decides what is pushed) |
| **Tests** | `crates/ade_node/src/node_lifecycle.rs`: `serve_gate_admits_first_block_zero_then_skips_reforged_block_zero`. `crates/ade_node/tests/forge_succeeds.rs`: `serve_gate_keeps_first_block_zero_skips_reforge`. (2 named; all resolve on disk.) |
| **CI** | `ci/ci_check_served_chain_stability.sh` (NEW — pins the monotone serve gate: the served tip never regresses; an already-served height is not re-served with a divergent hash) |
