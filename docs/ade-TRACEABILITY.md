...
>
> **HEAD:** `d313a5a` · **Generated:** 2026-05-28 · **Baseline:** `22eef90` (`.idd-config.json` `head_deltas_baseline`)
...
### CODEMAP (`docs/ade-CODEMAP.md` @ `d313a5a`)

Every crate named in a **Code** row is present in CODEMAP: `ade_codec`, `ade_core`, `ade_core_interop`, `ade_crypto`, `ade_ledger`, `ade_network`, `ade_node`, `ade_plutus`, `ade_runtime`, `ade_testkit`, `ade_types`. No crate-level CODEMAP mismatch detected. **Stale narrative (not fixed this pass):** CODEMAP/SEAMS prose lines stating the closed N-Q / N-R / N-S / N-M-* / N-O cluster docs "remain under `docs/clusters/`" are stale — those docs were moved to `docs/clusters/completed/` this pass. Reconcile at the next cluster close.

### HEAD_DELTAS (`docs/ade-HEAD_DELTAS.md` §5 @ `d313a5a`)

HEAD_DELTAS §5 (baseline window N-T, N-V) catalogs `ci_check_produce_mode_uses_bootstrap_initial_state.sh` (CN-PROD-03 / strengthens CN-NODE-01 + CN-PROD-02), `ci_check_node_binary_uses_single_bootstrap.sh` (CN-NODE-01), `ci_check_forge_decode_round_trip.sh` (CN-FORGE-03) — all three appear as **CI** rows above. Earlier-window CI scripts are catalogued in prior HEAD_DELTAS baselines.

**CI gates on disk not bound to any rule CI row** (surfaced, not fixed this pass): `ci_check_module_headers.sh`, `ci_check_no_secrets.sh`, `ci_check_pallas_quarantine.sh` appear in no registry `ci_script`/`ci_scripts` field — three real green gates with no covering rule entry, candidates for a future rule binding. `ci_check_node_mode_closure.sh` is **now bound** to CN-NODE-01 (registry `ci_script` field, this pass) and no longer appears in the unbound list.
...
Regenerated from the registry (working tree, with the registry hygiene-pass edits applied) as primary source; tests/CI cross-checked against the working tree at HEAD `d313a5a`. Test drift is resolved against the leaf function name (segment after the last `::`). Both the `ci_script` (string) and `ci_scripts` (array) registry fields are read for the CI row. Regenerate after every cluster close. The **drift** flags and `_(… — gap)_` cells are the actionable output — reconcile them in the registry or the codebase, do not delete them.
