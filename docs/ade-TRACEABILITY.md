...
>
> **HEAD:** `62cb8718` (working tree, PHASE4-N-F-G-A close follow-up — registry CI bindings) · **Generated:** 2026-06-01 · **Prior pass:** `80dac1f7` (PHASE4-N-F-G-A close)
...
The canonical source is the **invariant registry** at `docs/ade-invariant-registry.toml` (`.idd-config.json` `invariant_registry`), **313 rules** at this working tree. This doc is a join: registry entries × codebase introspection. Rule IDs, families, names, sources, and the registry-recorded `tests` / `ci_script` / `ci_scripts` / `code_locus` fields come from the registry; each test name and CI script is cross-checked for existence in the working tree at `62cb8718`, and divergences are flagged inline as **drift**. (Verification this pass is grep/ls only — no build or test execution.)
...
- **[not found on disk — drift]** (tests) / **[not on disk — drift]** (CI) flags a registry-named test/CI that could not be located in the working tree at `62cb8718` — a registry↔code coherence gap to reconcile. This pass: **0** such drifts (every cited test leaf-fn and every cited CI script resolves on disk).
...
### CODEMAP (`docs/ade-CODEMAP.md` @ `80dac1f7`, PHASE4-N-F-G-A close)

CODEMAP was regenerated for the PHASE4-N-F-G-A close at `80dac1f7`; this follow-up (`62cb8718`) changed only registry `ci_script` bindings (no code, no CI script added/removed), so CODEMAP remains current. It records **116** `ci_check_*.sh` gates — matching the on-disk count at this working tree (`ls ci/ci_check_*.sh` = 116). Every crate named in a **Code** row is present in CODEMAP: `ade_codec`, `ade_core`, `ade_core_interop`, `ade_crypto`, `ade_ledger`, `ade_network`, `ade_node`, `ade_plutus`, `ade_runtime`, `ade_testkit`, `ade_types`. No crate-level mismatch. Every `crates/**.rs` path cited in any `code_locus` resolves on disk (the `ci_check_registry_code_locus_exists.sh` invariant).

- **No BLUE crate changed this cluster.** PHASE4-N-F-G-A is forge-fidelity hardening on the `--mode node` relay spine: real config ingress + current pparams + two new fail-closed boundaries. All changes land in the RED shell (`ade_runtime`), the RED binary/driver (`ade_node`), and the GREEN `#[cfg(test)]` test crate (`ade_testkit`). CODEMAP confirms the new/changed loci — `ade_runtime::consensus_inputs::protocol_params` (`//! GREEN`), `ade_runtime::clock::checked_millis_to_slot` (GREEN-by-content clock seam), `ade_testkit::consensus::genesis_pinning` (`// GREEN` `#[cfg(test)]`), `ade_node::node_sync::forge_epoch_admission` (`GREEN — pure`), `ade_node::operator_forge` (`//! RED`), `ade_node::node_lifecycle` (`//! RED`) — and the canonical-type total holds at **456** (no BLUE change).
- **CI count is aligned:** CODEMAP's 116 matches the on-disk `ci/ci_check_*.sh` count at this working tree (Δ +4 vs the N-F-F close's 112: the four new G-A gates `ci_check_genesis_consistency_fixture_present.sh`, `ci_check_recovered_ledger_pparams_sourced.sh`, `ci_check_node_forge_real_cli_ingress.sh`, `ci_check_node_forge_single_epoch_fail_closed.sh`).

### HEAD_DELTAS (`docs/ade-HEAD_DELTAS.md` §5)

HEAD_DELTAS is **at this same HEAD** (`80dac1f7`, baseline `4eb7610` — the N-F-F close). It narrates the `4eb7610 → 80dac1f7` G-A span and its §5 catalogues the four new G-A gates. HEAD_DELTAS carries a load-bearing **registry note**: it was generated while the committed registry was in *slice-span* state (DC-EPOCH-03 still `declared`, no strengthenings), and it explicitly states that the **close-pass commit** flips `DC-EPOCH-03` `declared → enforced` and records the 7 cross-slice strengthenings. **This TRACEABILITY pass reads the working tree with those close-pass edits already applied** — DC-EPOCH-03 is `enforced` with its 6 tests + 1 gate, and all 7 `strengthened_in += "PHASE4-N-F-G-A"` tokens are present. Registry ↔ HEAD_DELTAS are therefore coherent (HEAD_DELTAS anticipates exactly this state).

The four G-A gates and their rule-binding status:

- `ci_check_node_forge_single_epoch_fail_closed.sh` (CE-G-A-4) — **rule-bound to `DC-EPOCH-03`.** Off-epoch forge fails closed before leadership / KES signing; the forge path drives no `NonceInput::EpochBoundary` / `CandidateFreeze` nonce promotion.
- `ci_check_node_forge_real_cli_ingress.sh` (CE-G-A-2) — **now rule-bound to `CN-OPCERT-01` + `CN-GENESIS-01`** (registry `ci_script` fields; bound in this follow-up). Enforces that the `--mode node` operator-forge ingress uses the REAL `parse_opcert_envelope` + `parse_shelley_genesis` (retiring the `parse_simple_*` stubs on the node path).
- `ci_check_recovered_ledger_pparams_sourced.sh` (CE-G-A-2a) — **now rule-bound to `DC-CINPUT-02b`** (registry `ci_script` field; bound in this follow-up). Enforces that the recovered ledger's `protocol_params` are sourced from the operator consensus-inputs bundle's oracle preimage, never `ProtocolParameters::default()`.
- `ci_check_genesis_consistency_fixture_present.sh` (CE-G-A-1) — **now rule-bound to `CN-GENESIS-01` + `DC-NODE-05`** (registry `ci_script` fields; bound in this follow-up). Proves the S1 private-net Ade-as-leader reference fixture + pinning harness are committed, well-formed, and leak no secret key material.

**CI gates on disk not bound to any rule CI row (3 of 116):** all four new G-A gates are now rule-bound (`ci_check_node_forge_single_epoch_fail_closed.sh` → `DC-EPOCH-03`; `ci_check_node_forge_real_cli_ingress.sh` → `CN-OPCERT-01` + `CN-GENESIS-01`; `ci_check_recovered_ledger_pparams_sourced.sh` → `DC-CINPUT-02b`; `ci_check_genesis_consistency_fixture_present.sh` → `CN-GENESIS-01` + `DC-NODE-05`) — the prior pass's three-unbound-G-A-gate traceability gap is **closed**. The remaining **3** unbound gates are the standing project-wide hygiene/meta gates that cannot be attributed to a single invariant: `ci_check_module_headers.sh`, `ci_check_no_secrets.sh`, `ci_check_pallas_quarantine.sh` (expected-unbound). The other **113** on-disk gates are rule-bound.

### SEAMS

Closed registries / closed grammars referenced by rule sources (mini-protocol ID registry, closed message taxonomies, closed log/event vocabularies, `LeaderCheckVerdict`, `ExpectedVrfInput`, `Mode`, `IngressSource`, the PHASE4-N-Y closed sets, the PHASE4-N-Z `MithrilBootstrapError`, the PHASE4-N-F-C BA-02 evidence surface, the N-F-D loop-planner lifecycle vocabulary, the N-F-E forge-tick `Due | NotDue` planner input, the N-F-F two-variant `ForgeIntent { On(ForgePaths), Off }`) are catalogued in `docs/ade-SEAMS.md` §3. PHASE4-N-F-G-A adds two new closed error vocabularies on the forge-fidelity path: the GREEN clock-seam `SlotAlignmentError` (`checked_millis_to_slot` fails closed before-anchor) and the GREEN epoch-admission `ForgeEpochAdmission` (`forge_epoch_admission` fails closed off-epoch) — both closed sum types, no new plugin seam.
...
- **`declared` rules (113) are open enforcement gaps by design.** Each carries `_(no enforcing code located — gap)_` / `_(no tests named in registry — gap)_` / `_(no CI script — gap)_` cells (or a subset). These are named-but-not-yet-mechanically-enforced invariants; the gap is the actionable backlog, not a defect to paper over.

- **`DC-NODE-06` (`declared`, introduced in PHASE4-N-F-G-B) is a forward sketch, NOT a G-A enforcement.** Self-accept → serve handoff on the relay spine (shape B). Its `code_locus` names the *intended* loci (`ade_node::node_lifecycle`, `ade_node::node_sync`, `ade_ledger::producer::served_chain`, `ade_runtime::producer::served_chain_handle` — all on disk), but **Tests and CI are empty** — it is the next sub-cluster's (PHASE4-N-F-G-B) deliverable, surfaced here so the handoff contract is visible before it is built. Do not read its populated Code row as enforcement; the empty Tests/CI cells and the `declared` status are the truth.

- **(RESOLVED this follow-up) The three new G-A CI gates are now rule-bound — the prior pass's traceability gap is closed.** `ci_check_node_forge_real_cli_ingress.sh` (CE-G-A-2) is now in the `ci_script` field of `CN-OPCERT-01` and `CN-GENESIS-01`; `ci_check_recovered_ledger_pparams_sourced.sh` (CE-G-A-2a) in `DC-CINPUT-02b`; `ci_check_genesis_consistency_fixture_present.sh` (CE-G-A-1) in `CN-GENESIS-01` and `DC-NODE-05` — exactly the append the prior pass recommended. All three appear on the **CI** rows of those rules above, and all three resolve on disk. The only remaining unbound `ci_check_*.sh` gates (3) are the project-wide hygiene/meta gates `ci_check_module_headers.sh` / `ci_check_no_secrets.sh` / `ci_check_pallas_quarantine.sh`, which are expected-unbound (no single invariant to attribute them to).

- **`DC-EPOCH-03` (PHASE4-N-F-G-A, `enforced`) — single-epoch forge fail-closed, mechanism-only.** The four-cell contract is filled (Code: `node_sync.rs` `forge_epoch_admission` + `node_lifecycle.rs` + `run_loop_planner.rs` + `ade_core::consensus::nonce`; Tests: 6 admission/fail-closed/no-promotion tests, all on disk; CI: `ci_check_node_forge_single_epoch_fail_closed.sh`, on disk). What is enforced is that an off-epoch (or unlocatable) forge slot **fails closed before leadership / KES signing** and drives no nonce promotion. What is NOT claimed: cross-epoch production (nonce roll + epoch transition) is a *separate* future cluster; the off-epoch guard is single-epoch cluster-scope containment, not permanent behaviour. This is the mechanism enforced at the library surface, not a live forge-into-the-network claim.

- **`DC-NODE-05` (PHASE4-N-F-E, `enforced`; strengthened in N-F-G-A) — forge tick is subordinate + self-accept-only.** Enforced by `ci_check_loop_planner_closed.sh` + `ci_check_node_run_loop_containment.sh` + 10 forge planner/guard/wiring/replay/fail-closed tests in `ade_node` (all resolve on disk). G-A hardens it: the slot is now derived through the checked clock seam over the real genesis anchor (S3, DC-NODE-03) and the off-epoch guard sits inside the fenced `forge_one_from_recovered` before leadership (S4, DC-EPOCH-03). The N-F-E containment gate is **byte-unchanged**; the forge advances no durable tip and admits/serves/gossips nothing.

- **Forge is CAPABLE but not OBSERVABLE (carried).** On the empty binary source (`NodeBlockSource::in_memory(Vec::new())`) the loop halts on iteration 1 before any `ForgeTick`. The `On` arm is forge-CAPABLE with real operator keys + real config + current pparams, but the forge is **not observable** on the binary path; there is no live forge / serve / gossip / peer-acceptance / BA-02 / durable-tip claim. Observable forge against a real peer is the **RO-LIVE-01 follow-on**. **BA-02 is satisfied nowhere** — the L6 `ba02_evidence` correlator remains a library surface reached by no binary arm.

- **Operator-gated release legs unchanged this close:** `RO-LIVE-01` (producer-side live leg against a real peer), `RO-LIVE-06` (BA-02 evidence — schema/correlation mechanics only), `RO-SYNC-EVIDENCE-01` (vacuous-until-committed manifest), `CE-Y-16` (two-Haskell-node witnessed live capture). PHASE4-N-F-G-A does not touch these. The S1 genesis-consistency reference fixture is **evidence input, never runtime authority**.

---

## Generation notes

INCREMENTAL refresh — **PHASE4-N-F-G-A close follow-up** at HEAD `62cb8718` (working tree). The rule set is **unchanged at 313**; this pass re-joins every rule to its Source / Code / Tests / CI from the working-tree registry and re-runs the mechanical counts and the on-disk existence checks (grep/ls only — no `cargo build` / `cargo test` / `cargo check`). The prior pass was the PHASE4-N-F-G-A close (HEAD `80dac1f7`, also 313 rules).

The single substantive delta since the prior pass: the **three previously-unbound PHASE4-N-F-G-A CI gates are now bound into the `ci_script` fields of the rules they enforce** — closing the prior pass's documented traceability gap. No rule was added, removed, weakened, or deprecated; no code locus or test changed; no CI script was added or removed on disk (count holds at 116).

- `ci/ci_check_node_forge_real_cli_ingress.sh` (CE-G-A-2) → appended to `CN-OPCERT-01` and `CN-GENESIS-01`. Both CI rows flipped from `_(no CI script — gap)_` to the bound gate (CN-GENESIS-01 also carries the genesis-consistency gate).
- `ci/ci_check_recovered_ledger_pparams_sourced.sh` (CE-G-A-2a) → appended to `DC-CINPUT-02b` (alongside the pre-existing `ci_check_consensus_input_provenance.sh`).
- `ci/ci_check_genesis_consistency_fixture_present.sh` (CE-G-A-1) → appended to `CN-GENESIS-01` and `DC-NODE-05` (the latter alongside its two pre-existing loop-planner / run-loop-containment gates).

State carried forward unchanged from the PHASE4-N-F-G-A close (re-verified this pass against the working tree):

- **`DC-EPOCH-03`** (`tier = derived`, `status = enforced`, `introduced_in = PHASE4-N-F-G-A`): single-epoch forge fail-closed on the `--mode node` spine. Code in `ade_node::node_sync` (`forge_epoch_admission` + closed `ForgeEpochAdmission`), `node_lifecycle`, `run_loop_planner`, `ade_core::consensus::nonce`; all **6** registry-named tests and its **1** CI gate (`ci_check_node_forge_single_epoch_fail_closed.sh`) resolve on disk.
- **`DC-NODE-06`** (`tier = derived`, `status = declared`, `introduced_in = PHASE4-N-F-G-B`): self-accept → serve handoff (shape B) — a **forward sketch for the next sub-cluster, NOT enforced by G-A.** Empty `tests` / `ci_script`; rendered with `declared` status and gap Tests/CI cells.
- **`strengthened_in` carries `PHASE4-N-F-G-A` on seven rules:** `CN-OPCERT-01`, `CN-GENESIS-01` (S2 real cardano-cli text-envelope ingress on the node path), `DC-LEDGER-10` (credential fidelity through the recovered current ledger view), `CN-NODE-01`, `DC-CINPUT-02b` (S2a oracle-current `ProtocolParameters`, not `::default()`), `DC-NODE-05` (S1/S4 genesis-consistency-proven + epoch-hardened), `DC-NODE-03` (S3 clock seam over the real genesis anchor via checked `checked_millis_to_slot`).

Mechanical counts recomputed fresh from the registry at `62cb8718` (`grep -c '^\[\[rules\]\]'` = **313**; no duplicate IDs) — **identical to the prior pass** (only `ci_script` strings changed):

- Status: **180 enforced** / **20 partial** / **113 declared** / 0 deprecated.
- By tier: constraint 4 · constraint_network 2 · derived **189** · operational 16 · release 40 · true 62.
- By family: T 30 · DC **146** · CN 112 · RO 16 · OP 9.

**Drift check (this pass, all rows):** across all 313 rules, **0** cited test references (1047 distinct leaf `fn` names) and **0** cited CI scripts (113 distinct, after path-prefix normalization) fail to resolve on disk at `62cb8718`. Every `crates/**.rs` code locus and every `ci/**.sh` gate cited in any row is present. The registry's enforcement pointers are clean.

**CODEMAP** (regenerated at the `80dac1f7` close, 116 CI checks) is unaffected by this binding-only follow-up; every G-A code locus stays inside the already-mapped `ade_node` / `ade_runtime` / `ade_testkit` crates and the on-disk `ci/ci_check_*.sh` count (116) still matches. **HEAD_DELTAS** (baseline `4eb7610`, HEAD `80dac1f7`) already catalogues all four G-A gates in its §5; this follow-up only moves them into registry `ci_script` fields, so HEAD_DELTAS remains coherent. The remaining `_(… — gap)_` cells (the 113 `declared` rules + `DC-NODE-06`'s empty Tests/CI) and the **Honest enforcement gaps** section are the actionable output — reconcile them in the registry or the codebase; do not delete them. Regenerate after every cluster close.
