...
>
> **HEAD:** `5db9aae` · **Generated:** 2026-05-29 · **Baseline:** `273c887` (`.idd-config.json` `head_deltas_baseline`)
...
- **[not found on disk — drift]** (tests) / **[not on disk — drift]** (CI) flags a registry-named test/CI that could not be located in the working tree at HEAD `5db9aae` — a registry↔code coherence gap to reconcile (rename the registry entry or restore the artifact). Test drift is resolved against the **leaf** function name (the segment after the last `::`).
...
By tier: constraint 1 · derived 181 · operational 16 · release 39 · true 61.

By family: T 29 · DC 138 · CN 107 · RO 15 · OP 9.
...
| **Tests** | `full_corpus_round_trip::all_42_blocks_round_trip_byte_identical`, `full_corpus_round_trip::all_42_blocks_fields_match_reference`, `codec::handshake::tests::roundtrip_every_variant`, `codec::n2c_handshake::tests::roundtrip_every_variant`, `codec::chain_sync::tests::roundtrip_every_variant`, `codec::block_fetch::tests::roundtrip_every_variant`, `codec::tx_submission::tests::roundtrip_every_variant`, `codec::keep_alive::tests::roundtrip_every_variant`, `codec::peer_sharing::tests::roundtrip_every_variant`, `codec::local_chain_sync::tests::roundtrip_every_variant`, `codec::local_tx_submission::tests::roundtrip_every_variant`, `codec::local_state_query::tests::roundtrip_every_variant`, … (+7 more) |
...
### `DC-CONS-20`: ChainDb-ledger-chain_dep lockstep: a successful receive-side admission updates ChainDb, LedgerState, and PraosChainDepState as one structural transition. A successful RollBackward rolls back all three to the same slot. No path leaves them out of sync; no partial admission; no partial rollback.
...
### `DC-PROTO-09`: Receive-side transcript determinism: given canonical inputs (initial_ledger, initial_chain_dep, initial_chaindb, event_sequence), the bridge reducer's output state (ledger', chain_dep', chaindb') is byte-identical across replays. The reducer is a pure, total transition.
...
### `DC-STORE-07`: Snapshot cadence determinism: the decision to take a snapshot at slot S is a pure function of (slot, block_no, cadence_params, last_snapshot). Same canonical input chain history produces the same set of snapshot slot keys. Cadence is BLUE-structural; not operator-tunable in this cluster (operator-tunable cadence is out of scope until represented as anchored, replay-derivable runtime data).
...
### `DC-STORE-08`: Snapshot encoder canonicality: encode_snapshot(s) is byte-identical across runs. Encoder uses BTreeMap iteration only; no HashMap, no wall-clock, no floats, no rand. Definite-length CBOR containers.
...
### `DC-STORE-09`: Snapshot bytes carry a closed u32 version tag (initial == 1) and the source state's blake2b-256 fingerprint. Decoder reads the version tag first and rejects unknown versions before decoding the ledger or chain-dep payload; decoder recomputes the fingerprint on the decoded state and rejects on mismatch.
...
### `DC-NODE-01`: Per-peer session isolation: one peer session's failure (decode error, validity reject, rollback-too-deep, protocol violation) halts only that peer's session. The orchestrator continues serving other peers and producing blocks. No cross-peer state sharing; each peer session owns its own working memory.
...
### `DC-NODE-03`: Clock-injection seam + replay equivalence: the orchestrator depends on a Clock trait yielding now() and tick_stream(). No SystemTime::now() or tokio::time::Instant::now() reachable from the orchestrator core. Replaying the orchestrator core with a deterministic Clock and a recorded OrchestratorEvent stream against a frozen snapshot store + chaindb produces byte-identical final (LedgerState fingerprint, PraosChainDepState, ChainDb tip) across runs.
...
### `DC-SESS-01`: Handshake-before-traffic: no mini-protocol frame reaches the orchestrator inbox until the handshake state machine has emitted Accepted. Type-state: a `MuxSession` in `Handshaking` cannot emit `SessionEffect::DeliverPeerFrame`; only `Connected` can. Compile- time guarantee + runtime test.
...
### `DC-SESS-02`: Closed mini-protocol id registry: the dispatch table over `MiniProtocolId` is a closed `match` on a closed `AcceptedMiniProtocol` enum. Unknown ids return `SessionError::UnknownMiniProtocolId { id }` (peer-fatal). No runtime extension; no silent acceptance.
...
### `DC-SESS-03`: Per-mini-protocol ordering + session replay equivalence: replaying the same byte chunks through `session::core::step` yields byte-identical outbound frames and an identical sequence of `SessionEffect::DeliverPeerFrame` values. Within a single (peer, mini_protocol_id) pair, frame ordering is preserved.
...
### `DC-SESS-04`: Backpressure discipline: every per-peer + per-mini-protocol channel is bounded; queue overflow is fail-fast `TransportError::BackpressureExceeded` rather than silent drop. No `mpsc::unbounded_channel` in any wire-session / mux-pump / dialer / keep-alive-session file.
...
### `DC-SESS-05`: Wire-layer clock injection: the session reducer + dispatch table contain no SystemTime / Instant::now / tokio::time reads. Keep-alive is driven by the PHASE4-N-K Clock seam (ade_runtime::clock::Clock). Mux frame timestamps enter through caller-supplied parameters at the RED runner boundary, not the GREEN core.
...
### `DC-SEED-01`: Canonical UtxoFingerprint determinism: the imported UTxOState uses BTreeMap<TxIn, TxOut> iteration order; UtxoFingerprint is Blake2b-256 over canonical CBOR map(N) [TxIn → TxOut] in that order. Re-importing the same JSON seed yields the same UtxoFingerprint byte-identically across runs.
...
### `DC-ANCHOR-01`: BootstrapAnchor canonical CBOR round-trip: encode + decode preserves all 6 fields byte-identically. SCHEMA_VERSION = 1 in the encoded bytes; unknown version on decode is fail-fast.
...
### `DC-WAL-01`: WAL is append-only by type: the WalStore trait carries no method named truncate / rewrite / replace / delete / clear. CI grep enforces across the workspace (no impl adds such methods out-of-trait).
...
### `DC-WAL-02`: WAL fingerprint-chain integrity: every WalEntry::AdmitBlock has prior_fp == previous entry's post_fp (or anchor's initial_ledger_fingerprint for the first entry). WalStore::verify_chain walks the WAL + asserts the chain holds. Verify failure is authority-fatal at the binary boundary.
...
### `DC-ADMIT-02`: Verdict emitted exactly once per admit-attempt: every successful admit path produces exactly one agreement_verdict JSONL event. Never twice for the same block_hash; never zero for a successful admit. Test pins the property via DeterministicClock replay of a recorded admission run.
...
### `DC-ADMIT-05`: Per-admit WAL append: every successful admit appends exactly one WalEntry::AdmitBlock to the configured WalStore. The entry's prior_fp chains to the previous entry's post_fp (or the anchor's initial_ledger_fingerprint for the first entry). Failure to append is authority-fatal. CN-WAL-01 single-authority preserved.
...
### `DC-ADMIT-06`: Verdict reducer is pure: verdict::derive(admit_outcome, peer_tip) is a pure function over closed input enums → closed output enum. No I/O, no clock, no state. The reducer is the GREEN-evidence boundary per [[feedback-evidence-reducers-are-green-not-authority]] — it compares authoritative outputs; it never decides authority.
...
### `DC-PUMP-01`: Wire pump emits AdmissionPeerEvent::{Block, TipUpdate, Disconnected} only. It MUST NOT synthesize AgreementVerdict values or any validity claim. The verdict remains the GREEN reducer's sole output (memory [[feedback-evidence-reducers-are-green-not-authority]] + ¬P-C3 no RED-derived verdicts).
...
### `DC-PUMP-02`: TipUpdate emitted on every chain-sync reply carrying a Tip (IntersectFound, IntersectNotFound, RollForward, RollBackward). The runner's next verdict::derive call sees the freshest peer comparison input.
...
### `DC-EVIDENCE-02`: Adversarial false-accept rejection across 4 mandatory mutation classes: 1. Body byte flip preserving envelope shape 2. Header body-hash mismatch 3. KES / signature corruption 4. VRF proof or output tamper Each mutation produces either: (a) BlockAdmitted NOT emitted + AgreementVerdict::Diverged + exit code 30, OR (b) AdmissionHalted { reason: PeerSentUndecodableBytes } when corruption breaks decode before admit-attempt. In NO case may a mutation produce BlockAdmitted (false accept), Agreed, or InputNotFound. False-accept is release-blocking (memory [[feedback-fail-closed-validation]]).
...
### `DC-EVIDENCE-01`: Operator-pass live evidence: the C5 live operator pass against the local docker cardano-node-preprod peer produces a JSONL transcript containing AT LEAST: - 1 AdmissionStarted with consensus_inputs_fingerprint, - 1 BootstrapComplete with the fingerprint, - >= 1 BlockAdmitted with the fingerprint, - >= 1 AgreementVerdict { kind: "agreed" }. And AT MOST: - 0 AgreementVerdict { kind: "diverged" } (would mean divergence vs. live preprod — release-blocking), - 0 BlockAdmitted for any block whose hash differs from a block the live peer announces at the same slot. The Lagging count is unconstrained (Lagging is evidence-only; DC-ADMIT-08).
...
### `DC-GENESIS-SRC-01`: A controlled genesis enters initial state ONLY through the single closed bootstrap_initial_state authority (genesis_initial); the genesis->initial-state transform is a pure deterministic BLUE function; a non-Conway genesis fails closed (GenesisSourceError::NonConwayEra) in this cluster — no Byron->Conway historical replay path is invoked. No GenesisAnchor/MithrilAnchor trait or plugin seam.
...
### `CN-STORE-08`: Single encoder authority: encode_ledger_state + decode_ledger_state + encode_chain_dep + decode_chain_dep + encode_snapshot + decode_snapshot are the SOLE pub fn pairs in the project encoding or decoding LedgerState / PraosChainDepState / (LedgerState, PraosChainDepState) to/from bytes. No parallel canonical encoders. Type-level + CI grep enforcement, mirroring CN-STORE-07.
...
### `CN-SESS-01`: Single mux frame authority: ade_network::mux::frame::{encode_frame, decode_frame} is the SOLE pub fn pair encoding/decoding `MuxFrame` to/from bytes in the workspace. No parallel mux codecs. Type-level + CI grep enforcement (mirrors CN-STORE-08).
...
### `CN-SESS-02`: Single handshake authority: ade_network::handshake::n2n_transition is the SOLE pub fn driving the N2N handshake state machine. ade_network::handshake::n2c_transition is the SOLE pub fn driving the N2C handshake state machine. No parallel handshake reducers. Type-level + CI grep enforcement.
...
### `CN-SESS-03`: Single session step authority: ade_network::session::core::step is the SOLE pub fn reducing (SessionState, ByteChunkIn) -> (SessionState, Vec<SessionEffect>). No parallel session reducers. Mirrors CN-NODE-01 (single bootstrap) and CN-STORE-07 (single materialize) for the wire-session layer.
...
### `CN-SEED-01`: Single JSON seed-importer authority: exactly one pub fn in ade_runtime::seed_import::import_cardano_cli_json_utxo converts a cardano-cli `query utxo --whole-utxo` JSON dump into the Ade canonical (UTxOState, UtxoFingerprint) pair. No second importer; no per-call fallback; no silent partial seed.
...
### `CN-WAL-01`: Single WAL append authority: WalStore::append is the SOLE mutation method on any WalStore impl. No truncate/rewrite/replace method exists on the trait or any impl. Append-only by type, not by convention.
...
### `CN-ADMIT-01`: Single admission-mode entry authority: exactly one pub fn in ade_node::admission::runner::run_admission enters the admission tokio runner. No second entry point; no per-call fallback. The runner composes N-M-A seed-import + bootstrap_anchor::mint + seed_to_snapshot + bootstrap_initial_state warm-start + N-L n2n_dialer + per-AdmittedBlock loop + WalStore::append + verdict::derive + admission_log emit.
...
### `CN-ADMIT-02`: Single seed-to-snapshot bridge authority: exactly one pub fn in ade_node::admission::seed_to_snapshot converts the imported (UTxOState, ledger_fingerprint, seed_point) into a persisted snapshot via PersistentSnapshotCache::capture. The bridge does NOT bypass bootstrap_initial_state — it persists a snapshot at seed_point.slot so the warm-start branch picks it up. CN-NODE-01 preserved.
...
| **Code** | crates/ade_ledger/src/bootstrap_anchor/binding.rs (verify_mithril_binding + MithrilManifestReport + closed MithrilImportError); crates/ade_runtime/src/mithril_import/ (RED manifest importer) |
| **Tests** | `mithril_binding_rejects_certified_point_other_than_seed_point`, `mithril_anchor_rejects_field_mismatch`, `mithril_import_fail_closed_blocks_storage_init` |
| **CI** | `ci/ci_check_mithril_uses_bootstrap_initial_state.sh` |
...
### `RO-LIVE-02`: A cardano-node peer's RollForward + BlockDelivered stream, consumed by the receive bridge, produces a ChainDb tip equal to the peer's announced tip at every step over a captured follow window. Live evidence captured against a private cardano-node peer; underlying invariants are CN-CONS-08, DC-CONS-19, DC-CONS-20, DC-PROTO-09, and the existing block_validity (B1) authority.
...
### `RO-MITHRIL-IMPORT-01`: Ade imports a Mithril-authenticated snapshot as an alternative to the cardano-cli JSON seed. Provides cryptographic provenance for the seed artifact (over and above the Blake2b-256 seed_artifact_hash that CN-ANCHOR-01 records). Lower priority than RO-LIVE-05; resolves after PHASE4-N-M-C closes.

_tier: release · status: partial · strengthened: PHASE4-N-Y_
...
| **Code** | crates/ade_ledger/src/bootstrap_anchor/binding.rs (verify_mithril_binding + MithrilManifestReport); crates/ade_runtime/src/mithril_import/ (manifest importer) |
| **Tests** | `mithril_binding_rejects_certified_point_other_than_seed_point`, `mithril_anchor_rejects_field_mismatch`, `mithril_import_fail_closed_blocks_storage_init` |
| **CI** | `ci/ci_check_mithril_uses_bootstrap_initial_state.sh` |
...
CODEMAP was last regenerated at `3b78008` (the PHASE4-N-Y close): 11 crates, 452 canonical types, 2067 tests, 103 CI checks. HEAD `5db9aae` is three commits later and ships **no new modules, types, or rules** — only the registry `code_locus` drift repair (`recovery.rs` → `recovery/mod.rs`) and one new meta-gate. Every crate named in a **Code** row is present in CODEMAP: `ade_codec`, `ade_core`, `ade_core_interop`, `ade_crypto`, `ade_ledger`, `ade_network`, `ade_node`, `ade_plutus`, `ade_runtime`, `ade_testkit`, `ade_types`. No crate-level mismatch.

- The recovery-module rename is **already reflected** in CODEMAP (its crate map records `ade_runtime::recovery` "promoted to a dir (`recovery/restart.rs` — `recover_node_state`)"). The `5db9aae` registry repair points DC-STORE-05, T-REC-01, and T-REC-02 at `crates/ade_runtime/src/recovery/mod.rs` + `recovery/restart.rs` (T-REC-01 additionally keeps `chaindb/crash_safety.rs`); all three paths exist on disk at HEAD. No CODEMAP staleness from the rename.
- **One count delta:** CODEMAP stamps **103** `ci_check_*.sh` gates at `3b78008`; at HEAD `5db9aae` there are **104** — the +1 is `ci_check_registry_code_locus_exists.sh`, the new registry-integrity meta-gate added by `5db9aae`. CODEMAP's CI count is therefore one behind HEAD by exactly this gate. Reconcile on the next CODEMAP regeneration (no rule is mis-traced as a result — the meta-gate is intentionally not bound to a single rule; see below).
...
**CI gates on disk not bound to any rule CI row** (surfaced, not fixed this pass): `ci_check_module_headers.sh`, `ci_check_no_secrets.sh`, `ci_check_pallas_quarantine.sh` (pre-existing, project-wide hygiene gates with no single covering rule), plus the **new** `ci_check_registry_code_locus_exists.sh`. The last is a deliberate **registry-integrity meta-gate**, not a per-rule gate: it validates that every `crates/**.rs` and `ci/**.sh` path named in any registry `code_locus` exists on disk (it guards the registry itself, so it cannot be attributed to a single invariant). It is correctly absent from any rule's **CI** row by design — it should not be "bound" to a rule. **100 of 104** on-disk `ci_check_*.sh` gates are rule-bound; the 4 unbound are these (3 hygiene + 1 meta-gate). HEAD_DELTAS §5 does not yet list the meta-gate — reconcile on the next HEAD_DELTAS regeneration (its baseline should bump to `5db9aae` or beyond).

### SEAMS

Closed registries / closed grammars referenced by rule sources (mini-protocol ID registry, closed message taxonomies, closed log/event vocabularies, `LeaderCheckVerdict`, `ExpectedVrfInput`, `Mode`, `IngressSource`, and the PHASE4-N-Y closed sets — `SyncEffect` with the private `AdmitPlan`, `MithrilImportError`, `GenesisSourceError`) are catalogued in `docs/ade-SEAMS.md` §3. DC-GENESIS-SRC-01 records the deliberate *absence* of a `GenesisAnchor`/`MithrilAnchor` trait or plugin seam — a closed surface, not an extension point. No SEAMS change at `5db9aae`.

---

## Honest enforcement gaps (standing)

The four-cell contract is filled for every `enforced` rule (Code + Tests + CI all present and verified on disk at HEAD `5db9aae` via grep/ls — no build/test was run this pass). The standing gaps below are recorded rather than hidden. They are unchanged in substance from the prior pass; `5db9aae` touched only registry `code_locus` strings and added the meta-gate, so no gap opened or closed.
...
- **RO-MITHRIL-IMPORT-01 — `partial` (provenance-only).** What is enforced is *provenance binding*: a Mithril manifest's attested `{network_magic, genesis_hash, certified_point, certificate_hash}` is cross-checked against the independently `--json-seed`-minted `BootstrapAnchor` and fails closed on mismatch (CN-MITHRIL-01 / DC-MITHRIL-01). Not yet enforced: (a) seed-*bytes*-from-Mithril decode; (b) a wired production composition site with a CI gate asserting the bound anchor's `seed_point` originates from the `--json-seed` UTxO extraction and **not** the Mithril manifest — the **re-tautologization hazard** flagged in the S7 security re-review (if the two compared origins ever collapse to one, the binding compares a value to itself and the check becomes vacuous); (c) a committed reproducible Mithril fixture + release evidence. Status `partial`, `blocked_until_mithril_import_wiring_slice`.
...
- **Governance note — CE-not-law (mechanism vs. authority).** Several PHASE4-N-Y enforcement points are **cluster-mechanical-acceptance (CE) gates**, not constitutional-law (true-tier) enforcement, and the distinction is load-bearing:
...
Regenerated from the registry (`docs/ade-invariant-registry.toml`, 298 rules) as primary source; tests and CI scripts cross-checked against the working tree at HEAD `5db9aae` using **grep/ls only** — no `cargo build`/`cargo test`/`cargo check` was run (a memory-heavy test was occupying the host). Test existence is verified against the leaf function name (segment after the last `::`) over all `fn` definitions under `crates/**.rs`; CI existence is verified by basename against `ci/ci_check_*.sh`. Both the `ci_script` (comma-joined string) and `ci_scripts` (array) registry fields are read for the **CI** row. The registry is parsed with a TOML parser, so **multi-line array and triple-quoted-string fields are now read correctly** — 39 rules whose `tests = [` arrays span multiple lines previously rendered a false `_(no tests named in registry — gap)_` and now show their real (on-disk-verified) test lists (e.g. DC-CONS-21 and the snapshot/rollback, session, admission, seed-import, WAL, and orchestrator families).

`5db9aae` is a registry-hygiene commit, not a feature cluster: it repaired the `recovery.rs → recovery/mod.rs` `code_locus` drift (DC-STORE-05, T-REC-01, T-REC-02 now cite `crates/ade_runtime/src/recovery/mod.rs` + `recovery/restart.rs`; T-REC-01 also keeps `chaindb/crash_safety.rs`) and added the registry-integrity meta-gate `ci/ci_check_registry_code_locus_exists.sh`. Rule count, status breakdown, tier breakdown, and family breakdown are **unchanged** from the `3b78008` pass (298; 166 enforced / 19 partial / 113 declared / 0 deprecated).

At this HEAD: **no test drift and no CI drift** — every registry-named test (across all `tests` arrays, single- and multi-line) resolves to an on-disk `fn`, and every registry-named `ci_script`/`ci_scripts` entry resolves to an on-disk gate. The `_(… — gap)_` cells and the **Honest enforcement gaps** section are the actionable output — reconcile them in the registry or the codebase; do not delete them. Regenerate after every cluster close.
