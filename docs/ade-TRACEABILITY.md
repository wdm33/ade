...
>
> **HEAD:** `01e7e08` · **Generated:** 2026-05-29 · **Baseline:** `22eef90` (`.idd-config.json` `head_deltas_baseline`)
...
- **[not found on disk — drift]** (tests) / **[not on disk — drift]** (CI) flags a registry-named test/CI that could not be located in the working tree at HEAD — a registry↔code coherence gap to reconcile (rename the registry entry or restore the artifact). Test drift is resolved against the **leaf** function name (the segment after the last `::`).
...
Every crate named in a **Code** row is present in CODEMAP: `ade_codec`, `ade_core`, `ade_core_interop`, `ade_crypto`, `ade_ledger`, `ade_network`, `ade_node`, `ade_plutus`, `ade_runtime`, `ade_testkit`, `ade_types`. No crate-level CODEMAP mismatch detected. CODEMAP was regenerated at this HEAD (`01e7e08`) in the same N-W close pass.

### HEAD_DELTAS (`docs/ade-HEAD_DELTAS.md` §5 @ `01e7e08`)

The PHASE4-N-W close adds `ci_check_producer_praos_vrf.sh` (bound to CN-FORGE-04) — the producer-side Praos VRF gate that fails the build if the producer leader-VRF construction diverges from the Conway/Praos validator authority. This script appears as the **CI** row of CN-FORGE-04 above. Earlier-window CI scripts (N-T / N-V: `ci_check_produce_mode_uses_bootstrap_initial_state.sh`, `ci_check_node_binary_uses_single_bootstrap.sh`, `ci_check_forge_decode_round_trip.sh`) and prior windows are catalogued in the respective HEAD_DELTAS baselines.

**CI gates on disk not bound to any rule CI row** (surfaced, not fixed this pass): `ci_check_module_headers.sh`, `ci_check_no_secrets.sh`, `ci_check_pallas_quarantine.sh` appear in no registry `ci_script`/`ci_scripts` field — 3 real green gates with no covering rule entry, candidates for a future rule binding. `ci_check_producer_praos_vrf.sh` is **now bound** to CN-FORGE-04 (registry `ci_script` field, this pass) and no longer appears in the unbound list.
...
Regenerated from the registry as primary source; tests/CI cross-checked against the working tree at HEAD `01e7e08` (PHASE4-N-W close). Test drift is resolved against the leaf function name (segment after the last `::`). Both the `ci_script` (string) and `ci_scripts` (array) registry fields are read for the CI row. Regenerate after every cluster close. The **drift** flags and `_(… — gap)_` cells are the actionable output — reconcile them in the registry or the codebase, do not delete them.
