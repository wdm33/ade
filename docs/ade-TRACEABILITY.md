...
>
> **HEAD:** `4eb7610` (working tree, PHASE4-N-F-F cluster close) · **Generated:** 2026-06-01 · **Baseline:** `cd2484f` (`.idd-config.json` `head_deltas_baseline`, PHASE4-N-F-E close)
...
The canonical source is the **invariant registry** at `docs/ade-invariant-registry.toml` (`.idd-config.json` `invariant_registry`), **311 rules** at this working tree. This doc is a join: registry entries × codebase introspection. Rule IDs, families, names, sources, and the registry-recorded `tests` / `ci_script` / `ci_scripts` / `code_locus` fields come from the registry; each test name and CI script is cross-checked for existence in the working tree at `4eb7610`, and divergences are flagged inline as **drift**. (Verification this pass is grep/ls only — no build or test execution.)
...
- **[not found on disk — drift]** (tests) / **[not on disk — drift]** (CI) flags a registry-named test/CI that could not be located in the working tree at `4eb7610` — a registry↔code coherence gap to reconcile (rename the registry entry or restore the artifact). Test drift is resolved against the **leaf** function name (the segment after the last `::`).
...
By tier: constraint 4 · constraint_network 2 · derived 187 · operational 16 · release 40 · true 62.

By family: T 30 · DC 144 · CN 112 · RO 16 · OP 9.
...
### `T-BOUND-01`: Shell may observe nondeterminism but must convert to deterministic inputs before entering…
...
### `T-CAUSAL-01`: Future decisions may not leak into present validation
...
### `T-CONS-01`: Chain selection depends only on canonical observables
...
### `T-CONS-02`: Authoritative consensus decisions must not depend on wall-clock, arrival-order,…
...
### `T-CONSERV-01`: UTxO and asset conservation must hold for every accepted transition, except where…
...
| **Tests** | `all_42_blocks_round_trip_byte_identical`, `all_42_blocks_fields_match_reference`, `roundtrip_every_variant`, `roundtrip_every_variant`, `roundtrip_every_variant`, `roundtrip_every_variant`, `roundtrip_every_variant`, `roundtrip_every_variant`, `roundtrip_every_variant`, `roundtrip_every_variant`, `roundtrip_every_variant`, `roundtrip_every_variant`, `roundtrip_every_variant`, `real_capture_round_trips_byte_identical`, `real_capture_round_trips_byte_identical`, `real_capture_round_trips_byte_identical`, `real_capture_round_trips_byte_identical`, `real_capture_round_trips_byte_identical`, `real_capture_round_trips_byte_identical` |
...
### `T-INGRESS-01`: All authoritative external bytes enter the core through named canonical decode/validation…
...
### `T-KEY-01`: Signing and private key operations confined to shell
...
### `T-NOSPEND-01`: No input or equivalent spend authority may be consumed more than once in an accepted…
...
### `T-PLATFORM-01`: No host-environment property (locale, timezone, architecture, platform) may influence…
...
### `T-REC-03`: Loop-as-replay: the same recovered/bootstrapped state + the same ordered canonical block…
...
### `T-RESOURCE-01`: Untrusted inputs must not allocate unbounded authoritative resources before deterministic…
...
### `T-TRANSPORT-01`: Transport nondeterminism (socket fragmentation, mux ordering, timeouts) must not leak…
...
Derived Cardano-compatibility invariants — the observable per-block / per-tx / consensus / storage / network behaviours Ade must match against the Haskell node.

_144 rules._

### `DC-ADMIT-01`: Closed AgreementVerdict sum (GREEN evidence, not authority): exactly four variants —…
...
### `DC-ADMIT-02`: Verdict emitted exactly once per admit-attempt: every successful admit path produces…
...
### `DC-ADMIT-03`: Diverged + InputNotFound are authority-fatal at the binary boundary
...
### `DC-ADMIT-04`: Closed AdmissionLogEvent vocabulary (8 variants: admission_started, snapshot_imported,…
...
### `DC-ADMIT-05`: Per-admit WAL append: every successful admit appends exactly one WalEntry::AdmitBlock to…
...
### `DC-ADMIT-06`: Verdict reducer is pure: verdict::derive(admit_outcome, peer_tip) is a pure function over…
...
### `DC-ADMIT-07`: Admit-replay-equivalence (true-tier): for every successful WalEntry::AdmitBlock…
...
### `DC-ADMIT-08`: Lagging is evidence-state only: AgreementVerdict::Lagging means the local admitted chain…
...
### `DC-ADMIT-09`: Admission code paths do not add partial reference-script support, permissive ref-script…
...
### `DC-ADMIT-12`: Undecodable peer bytes are Diverged (or PeerSentUndecodableBytes)
...
### `DC-ANCHOR-01`: BootstrapAnchor canonical CBOR round-trip: encode + decode preserves all 6 fields…
...
### `DC-CBOR-02`: Original wire bytes preserved for hash computation on hash-critical paths (see Byte…
...
### `DC-CINPUT-01`: WARM-START VERIFICATION CAPABILITY (authority surface — NOT production restart)
...
### `DC-CINPUT-02a`: PROJECTION EQUIVALENCE
...
### `DC-CINPUT-02b`: PRODUCER CONSUMPTION (closes CE-A-4b)
...
### `DC-CONS-03`: Praos chain selection ordering: block number first, then Praos TiebreakerView (slot,…
...
### `DC-CONS-04`: Praos chain-dep state (evolving/candidate/epoch/previous_epoch/lab/last_epoch_block…
...
| **Tests** | `op_cert_upsert_rejects_regression`, `op_cert_upsert_accepts_equal_counter_as_noop`, `op_cert_upsert_accepts_monotonic_increasing`, `genesis_state_is_well_formed`, `nonce_zero_constant_is_zero_bytes`, `op_cert_counter_map_iteration_is_deterministic`, `layout_is_stable`, `roundtrip_empty_state`, `roundtrip_genesis_state`, `roundtrip_populated_state`, `roundtrip_chain_event_all_variants`, `roundtrip_chain_selection_reject_all_variants`, `decode_rejects_unknown_discriminant`, `decode_rejects_short_array`, `op_cert_counter_map_iteration_is_deterministic`, `header_contribution_rejects_non_monotonic_slot`, `header_contribution_advances_evolving_nonce_deterministically`, `header_contribution_does_not_touch_epoch_nonce`, `candidate_freeze_copies_evolving_to_candidate`, `candidate_freeze_does_not_advance_slot`, `epoch_boundary_promotes_candidate_to_epoch_nonce`, `epoch_boundary_rotates_previous_epoch_nonce`, `epoch_boundary_rejects_uninitialised_candidate`, `epoch_boundary_preserves_op_cert_counters`, `epoch_boundary_preserves_lab_nonce`, `blake2b_input_order_is_evolving_then_vrf_output_first_32`, `within_epoch_evolving_nonce_matches_corpus`, `epoch_boundary_freezes_and_rotates_correctly`, `nonce_evolution_replay_is_deterministic`, `apply_op_cert_inserts_first_observation`, `apply_op_cert_advances_existing_strictly`, `apply_op_cert_accepts_equal_counter_as_noop`, `apply_op_cert_rejects_lower_counter`, `apply_op_cert_independent_kes_periods_dont_collide`, `apply_op_cert_independent_pools_dont_collide`, `apply_op_cert_does_not_touch_nonces`, `apply_op_cert_does_not_touch_last_slot_or_block_no`, `normal_progression_records_highest_counter_per_window`, `regression_after_progression_rejected_with_typed_error`, `op_cert_replay_is_deterministic`, `pipeline_short_circuits_on_first_failure`, `nonce_contribution_uses_nonce_role_vrf_output_not_leader_role`, `valid_header_accepted_advances_state`, `header_with_slot_regression_rejected`, `header_with_block_no_regression_rejected`, `header_with_op_cert_regression_rejected`, `header_with_invalid_vrf_proof_rejected`, `header_beyond_forecast_horizon_rejected`, `validate_replay_is_deterministic` |
| **CI** | `ci/ci_check_consensus_closed_enums.sh` |

### `DC-CONS-05`: Authoritative rollback must never exceed the security parameter k measured in blocks…
...
### `DC-CONS-06`: rollback(state, depth) produces state byte-identical to truncated replay from the nearest…
...
### `DC-CONS-07`: BLUE consensus must consume the HFC schedule only as a typed EraSchedule value anchored…
...
### `DC-CONS-08`: slot_to_time(EraSchedule, SystemStart, SlotNo) is a pure function
...
### `DC-CONS-10`: A header's op-cert issue counter must be >= the highest observed counter for the same…
...
### `DC-CONS-17`: Block bytes delivered via producer-side block-fetch Block{bytes} are byte-identical to…
...
### `DC-CONS-18`: Header bytes announced via chain-sync RollForward{header,tip} are the header sub-segment…
...
### `DC-CONS-19`: Receive-side header-body sourcing coherence: when BlockDelivered {block_bytes} arrives at…
...
### `DC-CONS-20`: ChainDb-ledger-chain_dep lockstep: a successful receive-side admission updates ChainDb,…
...
### `DC-CONS-22`: Replay-forward correctness: given state_at_slot_S and the ordered block sequence…
...
| **Tests** | `higher_block_no_wins`, `equal_block_no_tiebreaker_decides`, `fork_before_immutable_tip_rejected`, `exceeded_rollback_rejected`, `tiebreaker_loss_keeps_current`, `replay_is_deterministic`, `reject_reason_bytes_are_stable`, `no_candidates_returns_error`, `tiebreaker_prefer_lower_slot_wins`, `tiebreaker_prefer_higher_op_cert_wins_on_equal_slot_and_issuer`, `tiebreaker_prefer_lower_vrf_value_wins_on_full_tie`, `rollback_within_k_succeeds`, `rollback_exceeding_k_rejected_with_typed_reason`, `rollback_before_immutable_tip_rejected`, `rollback_event_bytes_are_stable`, `rollback_is_deterministic`, `rollback_equivalent_to_truncated_replay`, `header_arrival_updates_state_and_selector`, `rollback_walks_back_via_recent_snapshots`, `rollback_to_block_older_than_snapshots_rejected`, `epoch_boundary_emits_no_event`, `synthetic_session_replays_identically`, `synthetic_session_event_kinds_match_corpus`, `synthetic_session_final_state_matches_corpus`, `op_cert_regression_header_rejected_mid_stream`, `cardano_node_session_sustained_window` |
...
| **Tests** | `query_uses_state_epoch_nonce_for_vrf_input`, `query_returns_unknown_pool_when_no_vrf_key`, `query_returns_outside_forecast_range_for_far_future`, `query_does_not_mutate_state`, `eligible_on_threshold_with_high_stake_emits_eligible_verdict`, `not_eligible_with_zero_stake_emits_not_eligible_verdict`, `corpus_returns_canonical_answer_for_known_pools`, `corpus_rejects_unknown_pool`, `corpus_rejects_out_of_forecast_horizon`, `corpus_is_leader_helper_matches_pinned_probe`, `corpus_is_deterministic_across_runs`, `view_returns_corpus_pool_stake_and_vrf_keyhash`, `view_unknown_pool_returns_none`, `view_unknown_epoch_returns_none`, `view_is_pure`, `pool_distr_view_no_hashmap` |
| **CI** | _(no CI script — gap)_ |

### `DC-CORE-01`: BLUE authoritative crates are sync-only: no async fn, .await, tokio::, async_std::,…
...
### `DC-EPOCH-01`: Conway governance timing: proposals accumulate during epoch, ratification and enactment…
...
### `DC-EPOCH-02`: Hard fork transitions triggered at deterministic slot/epoch boundaries
...
### `DC-EVIDENCE-01`: Operator-pass live evidence: the C5 live operator pass against the local docker…
...
### `DC-GENESIS-01`: Given the same canonical Shelley genesis JSON bytes + the same operator-supplied…
...
### `DC-INGRESS-01`: Block/tx/protocol message decoding enters core through named chokepoints
...
### `DC-INGRESS-02`: Storage rehydration enters core through the same canonical decode chokepoints as network…
...
### `DC-KES-HEADER-01`: unsigned_header_pre_image(slot, block_no, prev_hash, vrf_data, opcert, kes_period,…
...
### `DC-LEDGER-06`: Script context (ScriptContext/TxInfo) derived from tx + ledger state + network-wide…
...
### `DC-LEDGER-07`: Coexisting supported versions must return same validity verdict for consensus-relevant…
...
### `DC-LEDGER-08`: Conway cert-state accumulation is a closed, total, era-versioned transition: for each…
...
### `DC-LEDGER-09`: Conway governance-certificate accumulation is a closed, total, era-versioned transition…
...
### `DC-LEDGER-10`: Credential identity is faithful end-to-end: a stake/committee/DRep credential is a closed…
...
### `DC-LEDGER-11`: proposal_procedures MUST NOT remain an opaque byte field in the authoritative Conway…
...
### `DC-LEDGER-12`: Every tx in a forged block is admissible via ade_ledger::mempool::admit against the base…
...
### `DC-MEM-04`: Replaying the same ordered ingress trace against the same base ledger state produces a…
...
### `DC-NET-01`: Peer selection uses three-tier management (cold/warm/hot) with bounded admission,…
...
### `DC-NODE-01`: Per-peer session isolation: one peer session's failure (decode error, validity reject,…
...
### `DC-NODE-02`: Persistent-writer cadence fidelity: the orchestrator's persistent-snapshot writer calls…
...
### `DC-NODE-03`: Clock-injection seam + replay equivalence: the orchestrator depends on a Clock trait…
...
### `DC-NODE-04`: Authority-fatal halt + shutdown-resume identity: authoritative errors (chain_write…
...
### `DC-NODE-05`: Forge-slot discipline on the --mode node relay run-loop
...
> **Strengthened in PHASE4-N-F-F (S4).** The forge-slot discipline is now exercised with operator-material-backed keys (`relay_loop_with_operator_material_forge_reaches_fenced_path`, `relay_loop_with_operator_material_two_runs_byte_identical`, `#[cfg(test)]`-only in `node_sync.rs`) without relaxing containment — still exactly one fenced `forge_one_from_recovered` call, no durable tip advance, self-accept-only.

### `DC-OPCERT-01`: Given the same canonical envelope bytes, parse_opcert_envelope produces a byte-identical…
...
### `DC-OUTBOUND-FIFO-01`: The per-peer outbound channel preserves FIFO order: OutboundCommands enqueued for…
...
### `DC-PROD-01`: Producer-mode evidence log emits a closed `ProducerLogEvent` vocabulary: handshake_ok,…
...
| **Tests** | `idle_request_next_with_immediate_data_yields_can_await_then_roll_forward`, `idle_request_next_with_no_data_yields_must_reply_via_await`, `roll_forward_signal_carries_header_and_tip_byte_identical`, `roll_backward_signal_carries_point_and_tip_byte_identical`, `find_intersect_with_known_point_yields_intersected_signal`, `find_intersect_with_unknown_points_yields_no_intersection`, `client_done_terminates_session`, `illegal_message_in_idle_returns_error`, `wrong_agency_returns_error`, `version_gating_rejects_out_of_version_message`, `empty_find_intersect_points_returns_malformed`, `signal_trace_synthetic_divergence_corpus`, `idle_request_range_yields_busy_then_start_batch`, `busy_no_blocks_returns_to_idle_with_event`, `streaming_block_delivers_bytes_byte_identical`, `streaming_batch_done_returns_to_idle`, `multi_block_streaming_preserves_order`, `client_done_terminates_session`, `illegal_message_in_idle_returns_error`, `wrong_agency_returns_error`, `version_gating_rejects_out_of_version_message`, `inverted_range_returns_malformed`, `block_delivered_event_carries_bytes_verbatim`, `block_fetch_frame_corpus`, `init_handshake_opens_session`, `request_blocking_ids_then_reply_round_trips`, `request_non_blocking_ids_with_empty_reply_round_trips`, `request_txs_then_reply_delivers_bytes_byte_identical`, `request_txs_with_partial_reply_legal`, `blocking_reply_empty_is_malformed`, `reply_ids_overfill_is_malformed`, `request_txs_empty_is_malformed`, `reply_txs_overfill_is_malformed`, `done_terminates_session`, `illegal_message_in_init_returns_error`, `wrong_agency_returns_error`, `version_gating_rejects_out_of_version_message`, `tx_submission2_mempool_trace`, `client_ping_then_server_pong_round_trips`, `cookie_mismatch_returns_malformed`, `client_done_terminates_session`, `illegal_message_in_idle_returns_error`, `wrong_agency_returns_error`, `version_gating_rejects_out_of_version_message`, `share_request_then_full_reply_round_trips`, `share_request_then_empty_reply_is_legal`, `reply_exceeds_amount_returns_malformed`, `client_done_terminates_session`, `illegal_message_in_idle_returns_error`, `wrong_agency_returns_error`, `version_gating_rejects_out_of_version_message`, `keep_alive_event_trace`, `peer_sharing_event_trace`, `local_chain_sync_request_next_then_roll_forward`, `local_chain_sync_roll_backward_signal`, `local_chain_sync_find_intersect_known_point`, `local_chain_sync_find_intersect_unknown`, `local_chain_sync_client_done_terminates`, `local_chain_sync_wrong_agency_returns_error`, `local_chain_sync_version_gating`, `submit_tx_then_accept_round_trips`, `submit_tx_then_reject_carries_reason_bytes`, `client_done_terminates`, `wrong_agency_returns_error`, `version_gating`, `acquire_then_acquired_transitions_to_acquired`, `acquire_then_failure_returns_to_idle`, `query_then_result_round_trips`, `release_returns_to_idle`, `re_acquire_from_acquired_transitions_to_acquiring`, `client_done_from_idle_terminates`, `client_done_from_acquired_terminates`, `wrong_agency_returns_error`, `idle_acquire_then_acquired`, `acquired_release_returns_to_idle`, `acquired_await_acquire_re_acquires`, `acquired_next_tx_then_reply_with_tx`, `acquired_next_tx_then_reply_empty_mempool`, `acquired_has_tx_then_reply_present`, `acquired_has_tx_then_reply_absent`, `acquired_get_sizes_then_reply`, `acquired_get_measures_then_reply`, `get_measures_rejected_below_v2`, `client_done_terminates_from_idle`, `wrong_agency_returns_error`, `version_gating_rejects_out_of_version_message`, `local_chain_sync_event_trace`, `local_tx_submission_event_trace`, `local_state_query_event_trace`, `local_tx_monitor_event_trace` |
| **CI** | _(no CI script — gap)_ |
...
| **Code** | `crates/ade_network/src/chain_sync/state.rs`, `crates/ade_network/src/chain_sync/agency.rs`, `crates/ade_network/src/chain_sync/signal.rs`, `crates/ade_network/src/chain_sync/transition.rs`, `crates/ade_network/src/block_fetch/state.rs`, `crates/ade_network/src/block_fetch/agency.rs`, `crates/ade_network/src/block_fetch/event.rs`, `crates/ade_network/src/block_fetch/transition.rs`, `crates/ade_network/src/tx_submission/state.rs`, `crates/ade_network/src/tx_submission/agency.rs`, `crates/ade_network/src/tx_submission/event.rs`, `crates/ade_network/src/tx_submission/transition.rs` |
| **Tests** | `idle_request_next_with_immediate_data_yields_can_await_then_roll_forward`, `idle_request_next_with_no_data_yields_must_reply_via_await`, `roll_forward_signal_carries_header_and_tip_byte_identical`, `roll_backward_signal_carries_point_and_tip_byte_identical`, `find_intersect_with_known_point_yields_intersected_signal`, `find_intersect_with_unknown_points_yields_no_intersection`, `client_done_terminates_session`, `illegal_message_in_idle_returns_error`, `wrong_agency_returns_error`, `version_gating_rejects_out_of_version_message`, `empty_find_intersect_points_returns_malformed`, `signal_trace_synthetic_divergence_corpus`, `idle_request_range_yields_busy_then_start_batch`, `busy_no_blocks_returns_to_idle_with_event`, `streaming_block_delivers_bytes_byte_identical`, `streaming_batch_done_returns_to_idle`, `multi_block_streaming_preserves_order`, `client_done_terminates_session`, `illegal_message_in_idle_returns_error`, `wrong_agency_returns_error`, `version_gating_rejects_out_of_version_message`, `inverted_range_returns_malformed`, `block_delivered_event_carries_bytes_verbatim`, `block_fetch_frame_corpus`, `init_handshake_opens_session`, `request_blocking_ids_then_reply_round_trips`, `request_non_blocking_ids_with_empty_reply_round_trips`, `request_txs_then_reply_delivers_bytes_byte_identical`, `request_txs_with_partial_reply_legal`, `blocking_reply_empty_is_malformed`, `reply_ids_overfill_is_malformed`, `request_txs_empty_is_malformed`, `reply_txs_overfill_is_malformed`, `done_terminates_session`, `illegal_message_in_init_returns_error`, `wrong_agency_returns_error`, `version_gating_rejects_out_of_version_message`, `tx_submission2_mempool_trace`, `real_capture_round_trips_byte_identical`, `rollforward_header_opaque_pass_through`, `real_capture_round_trips_byte_identical`, `block_body_is_wrapped_cbor_item`, `real_capture_round_trips_byte_identical`, `cookie_echo_invariant_holds`, `real_capture_round_trips_byte_identical` |
| **CI** | _(no CI script — gap)_ |

### `DC-PROTO-03`: Full N2N mini-protocol surface: Handshake, ChainSync, BlockFetch, TxSubmission2,…
...
| **Tests** | `roundtrip_every_variant`, `roundtrip_every_variant`, `roundtrip_every_variant`, `roundtrip_every_variant`, `roundtrip_every_variant`, `local_chain_sync_request_next_then_roll_forward`, `local_chain_sync_roll_backward_signal`, `local_chain_sync_find_intersect_known_point`, `local_chain_sync_find_intersect_unknown`, `local_chain_sync_client_done_terminates`, `local_chain_sync_wrong_agency_returns_error`, `local_chain_sync_version_gating`, `submit_tx_then_accept_round_trips`, `submit_tx_then_reject_carries_reason_bytes`, `client_done_terminates`, `wrong_agency_returns_error`, `version_gating`, `acquire_then_acquired_transitions_to_acquired`, `acquire_then_failure_returns_to_idle`, `query_then_result_round_trips`, `release_returns_to_idle`, `re_acquire_from_acquired_transitions_to_acquiring`, `client_done_from_idle_terminates`, `client_done_from_acquired_terminates`, `wrong_agency_returns_error`, `idle_acquire_then_acquired`, `acquired_release_returns_to_idle`, `acquired_await_acquire_re_acquires`, `acquired_next_tx_then_reply_with_tx`, `acquired_next_tx_then_reply_empty_mempool`, `acquired_has_tx_then_reply_present`, `acquired_has_tx_then_reply_absent`, `acquired_get_sizes_then_reply`, `acquired_get_measures_then_reply`, `get_measures_rejected_below_v2`, `client_done_terminates_from_idle`, `wrong_agency_returns_error`, `version_gating_rejects_out_of_version_message`, `local_chain_sync_event_trace`, `local_tx_submission_event_trace`, `local_state_query_event_trace`, `local_tx_monitor_event_trace` |
| **CI** | _(no CI script — gap)_ |

### `DC-PROTO-05`: Version negotiation is closed: enumerated N2N/N2C versions, explicit handshake,…
...
### `DC-PROTO-08`: Once chain-sync enters a state where the server holds agency, the pure per-session…
...
### `DC-PUMP-01`: Wire pump emits AdmissionPeerEvent::{Block, TipUpdate, Disconnected} only
...
### `DC-QUERY-01`: N2C queries are era-aware, typed, and version-gated: each NodeToClientVersion gates which…
...
### `DC-REF-01`: Every claimed equivalence check must identify its reference source, extraction method,…
...
### `DC-SEED-01`: Canonical UtxoFingerprint determinism: the imported UTxOState uses BTreeMap<TxIn, TxOut>…
...
### `DC-SESS-01`: Handshake-before-traffic: no mini-protocol frame reaches the orchestrator inbox until the…
...
### `DC-SESS-02`: Closed mini-protocol id registry: the dispatch table over `MiniProtocolId` is a closed…
...
### `DC-SESS-03`: Per-mini-protocol ordering + session replay equivalence: replaying the same byte chunks…
...
### `DC-SESS-04`: Backpressure discipline: every per-peer + per-mini-protocol channel is bounded
...
### `DC-SESS-05`: Wire-layer clock injection: the session reducer + dispatch table contain no SystemTime /…
...
### `DC-SESS-06`: Replay equivalence under fragmented inbound streams: two reducer runs over the same…
...
### `DC-SNAPSHOT-01`: ServedChainHandle::push_atomic is deterministic in its argument order: the same sequence…
...
### `DC-STORE-04`: ChainDB structure: ImmutableDB (append-only, blocks immutable when k-deep), VolatileDB…
...
### `DC-STORE-05`: Recovery is snapshot + forward replay (not full genesis replay): load most recent valid…
...
### `DC-STORE-06`: VolatileDB uses ValidateAll after unclean shutdown
...
### `DC-STORE-07`: Snapshot cadence determinism: the decision to take a snapshot at slot S is a pure…
...
### `DC-STORE-08`: Snapshot encoder canonicality: encode_snapshot(s) is byte-identical across runs
...
### `DC-STORE-09`: Snapshot bytes carry a closed u32 version tag (initial == 1) and the source state's…
...
### `DC-SYNC-01`: During network forward-sync, a block's preserved wire bytes and its WAL entry MUST be…
...
### `DC-SYNC-02`: Continuous relay sync: every loop iteration preserves durable-before-advance (DC-SYNC-01)…
...
### `DC-TXV-02`: A transaction is Valid iff both phase-1 (structural + UTxO rules + witnesses) and phase-2…
...
### `DC-TXV-03`: Ade's Valid/Invalid verdict for a transaction equals the reference cardano-node verdict,…
...
### `DC-TXV-04`: A Valid transaction yields an applied LedgerState' (the mempool's accumulating view)
...
### `DC-TXV-05`: For each era, required_signers(state, tx_body) is a closed, explicit, era-versioned…
...
### `DC-TXV-06`: For each era, the certificate-deposit classification map(state, cert) is a closed, total,…
...
### `DC-TXV-07`: All deposit/refund amounts used by Conway transaction value-conservation accounting must…
...
### `DC-VAL-01`: A block's validity verdict is a pure function of (LedgerState, PraosChainDepState,…
...
### `DC-VAL-02`: A block is Valid iff both the consensus header authority (validate_and_apply_header) and…
...
### `DC-VAL-03`: The header is validated before the body
...
### `DC-VAL-04`: Ade's Valid/Invalid verdict for a block equals the reference cardano-node verdict,…
...
### `DC-VAL-05`: A Valid block yields evolved (LedgerState', PraosChainDepState')
...
### `DC-VAL-06`: Every crypto-input, field-size, and structural check on the authority path rejects…
...
### `DC-WAL-01`: WAL is append-only by type: the WalStore trait carries no method named truncate / rewrite…
...
### `DC-WAL-02`: WAL fingerprint-chain integrity: every WalEntry::AdmitBlock has prior_fp == previous…
...
| **Tests** | `file_wal_store_verify_chain_passes_then_catches_break`, `replay_from_anchor_catches_chain_break`, `wal_replay_from_anchor_rejects_chain_break` |
| **CI** | _(no CI script — gap)_ |

### `DC-WAL-03`: Anchor + WAL replay-equivalence: replaying (BootstrapAnchor + WAL entries 1..N) against…
...
Constraint-network classification-table invariants — closed enums, closed grammars, fail-closed boundaries, and the structural constraints that keep authority surfaces closed.
...
### `CN-BUILD-01`: No build profile, feature flag, cfg, or optimization mode may alter authoritative…
...
### `CN-BUILD-02`: All semantic variability must be explicit runtime protocol data, not hidden compile-time…
...
### `CN-BUILD-03`: Exactly one semantic interpretation may exist for a given protocol version and bootstrap…
...
### `CN-BUILD-04`: Operator configuration may tune transport, logging, and telemetry, but may not silently…
...
### `CN-CINPUT-01`: The seed-epoch consensus inputs (epoch, active-slots coefficient, total active stake, and…
...
### `CN-CINPUT-02`: The SeedEpochConsensusInputs sidecar MUST be populated ONLY on the verified-bootstrap…
...
### `CN-CINPUT-03`: Consume-side anti-laundering fence: on the node-lifecycle forge path the leadership view…
...
### `CN-CONS-01`: Chain selection must be deterministic for the same candidate chains and protocol…
...
### `CN-CONS-02`: Supported rollout skew must not allow a single adversarial input to induce persistent…
...
### `CN-CONS-03`: After temporary partition, honest nodes must converge using only protocol-defined…
...
| **Tests** | `pipeline_short_circuits_on_first_failure`, `nonce_contribution_uses_nonce_role_vrf_output_not_leader_role`, `valid_header_accepted_advances_state`, `header_with_slot_regression_rejected`, `header_with_block_no_regression_rejected`, `header_with_op_cert_regression_rejected`, `header_with_invalid_vrf_proof_rejected`, `header_beyond_forecast_horizon_rejected`, `validate_replay_is_deterministic`, `candidate_fragment_carries_anchor_block_no`, `conway_corpus_headers_all_pass_leader_check`, `praos_vrf_keyhash_mismatch_rejected`, `altered_body_rejected_by_hash_binding` |
| **CI** | _(no CI script — gap)_ |

### `CN-CONS-05`: Authoritative consensus decisions must not depend on wall-clock time, arrival-order…
...
### `CN-CONS-06`: Cross-impl acceptance: blocks forged by Ade are accepted by cardano-node when delivered…
...
### `CN-CONS-07`: Self-acceptance bridge: a forged block is NOT eligible for RED broadcast unless Ade's own…
...
### `CN-CONS-08`: Receive-side single admission authority: every block that lands in ChainDb via the…
...
### `CN-CRYPTO-01`: Verification belongs to the authoritative core
...
### `CN-CRYPTO-03`: All collections with consensus meaning must be ordered deterministically before hashing…
...
### `CN-CRYPTO-04`: Verification failure must fail once and deterministically
...
### `CN-EPOCH-02`: At each slot or epoch point there is exactly one authoritative committee and governance…
...
### `CN-EPOCH-03`: Stake snapshots and reward computations must be derivable solely from canonical chain…
...
### `CN-EPOCH-04`: Future decisions may not leak into present validation, and later states may not…
...
### `CN-FORGE-02`: Leader-check splits across the RED/BLUE color boundary: RED produces a VRF proof/output…
...
### `CN-FORGE-03`: Producer/validator codec symmetry: forge_block emits the era-tagged [era, block] envelope…
...
### `CN-FORGE-04`: Producer-side Praos VRF construction must match the Conway/Praos validator authority: the…
...
### `CN-KES-HEADER-01`: The KES signature in a forged block's header is over the canonical unsigned-header CBOR…
...
### `CN-LEDGER-03`: Validity decisions for transactions and blocks must match the Cardano reference oracle…
...
### `CN-LEDGER-04`: Any two supported production versions that may coexist must return the same validity…
...
### `CN-LEDGER-05`: Each feature must have one semantic processing result
...
### `CN-LEDGER-07`: UTxO and asset conservation must hold for every accepted transition, except where…
...
### `CN-LEDGER-08`: No input or equivalent spend authority may be consumed more than once in an accepted…
...
### `CN-LEDGER-09`: Witnesses must bind exactly to the intended body, certificates, withdrawals, governance…
...
### `CN-LEDGER-10`: Conway governance and certificate transitions must occur only through explicit legal…
...
### `CN-MEM-01`: Untrusted inbound work must be admitted through deterministic bounded policies before…
...
### `CN-MEM-02`: Mempool pressure and peer churn must not starve block validation, chain selection, or…
...
### `CN-MEM-03`: Under overload, work shedding must follow deterministic policy, not timing-dependent…
...
### `CN-MEM-04`: Mempool acceptance rules must never contradict block and ledger acceptance rules for the…
...
### `CN-META-03`: Every equivalence claim must be reproducible from named fixtures, oracle versions, and…
...
### `CN-NET-01`: A block producer must not accept arbitrary public peer connectivity
...
### `CN-NET-02`: Relay paths must be geographically and topologically diverse enough that isolating one…
...
### `CN-NET-03`: No single peer, ASN, region, or operator cluster may dominate the node's authoritative…
...
### `CN-NET-04`: Peer selection and promotion policies must not allow one adversary-controlled set to…
...
### `CN-NODE-01`: Single bootstrap authority: exactly one pub fn in ade_runtime::bootstrap returns the…
...
> **Strengthened in PHASE4-N-F-F (S5).** The single-bootstrap gate `ci_check_node_binary_uses_single_bootstrap.sh` (which enforces CN-NODE-01 per its banner; formally carried by DC-NODE-04’s CI row) had its stale `ReceiveState::new "≤1 per crate"` guard replaced with a `node.rs node_lifecycle.rs` owner allow-list — green again after being red since N-F-C/N-F-D added `node_lifecycle` as a second legitimate owner. CN-NODE-01’s own registry CI row (the three bootstrap-closure gates) is unchanged; N-F-F’s `--mode node` forge-on flip preserves the single recovered `BootstrapState` (no second bootstrap).

### `CN-NODE-02`: `--mode node` is the single live-run lifecycle owner
...
### `CN-NODE-03`: Operator-key ingress + forge-on flip for `--mode node`
...
> **Boundary honesty.** CN-NODE-03 is `enforced` for operator-key **ingress + activation assembly + the binary `Some`/`None` flip** — the binary is now forge-CAPABLE with a complete operator key set. It is NOT a live forge / serve / gossip / peer-acceptance / BA-02 / RO-LIVE claim: on the empty-source binary path the loop halts cleanly before any `ForgeTick`, so the forge is not observable. Observable forge against a real peer remains the **RO-LIVE-01 follow-on** (operator-gated; RO-LIVE-01 stays `partial`). No new BLUE rule; no rule weakened or deprecated.

### `CN-OPCERT-01`: The opcert envelope parser accepts a real cardano-cli `node.opcert` text envelope (closed…
...
### `CN-OPS-01`: After any partition, authoritative post-incident reconciliation must be derived solely…
...
### `CN-OPS-02`: Emergency recovery procedures must have explicit admissibility criteria, deterministic…
...
### `CN-OPS-03`: Incident evidence must be sufficient to reconstruct the canonical decision path without…
...
### `CN-PLUTUS-01`: Same script + same redeemers/datum/context + same cost model must produce identical…
...
### `CN-PLUTUS-03`: Script context must be canonically and completely derived from transaction plus ledger…
...
### `CN-PROD-01`: Producer-mode listener completes the N2N handshake (CN-SESS-02) on every accepted inbound…
...
> **Strengthened in PHASE4-N-F-F.** The RED key-custody line is extended to the `--mode node` path via `ade_node::operator_forge`: `load_operator_producer_shell` reuses `ProducerShell::init` (which enforces the KES-period-vs-opcert freshness bound) and `OperatorForgeMaterial` holds the custody shell without exposing private-key bytes — custody stays RED-confined. No KES rotation past period; no new key-custody surface.

### `CN-PROD-03`: produce_mode's forge base state is derived from bootstrap_initial_state (cold-start, fed…
...
### `CN-PROTO-01`: Each miniprotocol must be an explicit deterministic state machine with legal typed…
...
### `CN-PROTO-02`: For the same peer transcript, authoritative state and outbound transcript must be…
...
### `CN-PROTO-03`: Agency must be enforced strictly
...
### `CN-PROTO-04`: Socket fragmentation, multiplexing, arrival order, and timeout behavior must not leak…
...
### `CN-PROTO-05`: Untrusted network inputs must not allocate unbounded authoritative resources before…
...
### `CN-PROTO-06`: The producer-side session orchestrator can only construct outgoing mini-protocol messages…
...
### `CN-REL-01`: A release is not mainnet-eligible unless mixed-version topologies against supported…
...
### `CN-REL-02`: No single implementation bug should exceed the protocol's intended safety or liveness…
...
| **Tests** | `utxo_seed_parses_minimal_two_entry_fixture`, `utxo_seed_two_imports_byte_identical`, `utxo_seed_btree_order_independent_of_json_order`, `utxo_seed_rejects_unparseable_json`, `utxo_seed_rejects_bad_txin_key`, `utxo_seed_rejects_bad_address`, `utxo_seed_inline_datum_entry_round_trips`, `utxo_seed_canonical_txout_address_extracted`, `utxo_seed_accepts_plutus_v1_reference_script`, `utxo_seed_accepts_plutus_v2_reference_script`, `utxo_seed_accepts_plutus_v3_reference_script`, `utxo_seed_accepts_simple_script_reference_script`, `utxo_seed_reference_script_changes_fingerprint`, `utxo_seed_reference_script_deterministic_across_two_imports`, `utxo_seed_rejects_unknown_script_type`, `utxo_seed_rejects_non_hex_cbor_hex`, `utxo_seed_rejects_missing_cbor_hex`, `utxo_seed_canonical_script_ref_encoder_known_vector`, `parse_utxo_seed_json_reference_script_entry` |
| **CI** | `ci/ci_check_seed_import_closure.sh`, `ci/ci_check_seed_import_full_preprod_support.sh` |

### `CN-SESS-01`: Single mux frame authority: ade_network::mux::frame::{encode_frame, decode_frame} is the…
...
### `CN-SESS-02`: Single handshake authority: ade_network::handshake::n2n_transition is the SOLE pub fn…
...
### `CN-SESS-03`: Single session step authority: ade_network::session::core::step is the SOLE pub fn…
...
### `CN-SESS-04`: Session reducer per-mini-protocol payload reassembly: the GREEN session reducer maintains…
...
### `CN-SNAPSHOT-02`: A RequestRange covering a slot range that is not entirely present in ServedChainSnapshot…
...
### `CN-STORE-01`: No authoritative storage initialization may occur before bootstrap or anchor verification…
...
### `CN-STORE-02`: WAL entries, checkpoints, and recovered artifacts must be bound to exactly one anchor or…
...
### `CN-STORE-03`: Crash recovery must produce the same authoritative state as clean replay over the…
...
### `CN-STORE-06`: On-disk bytes must re-enter through the same canonical validation and decode chokepoints…
...
### `CN-STORE-07`: Single materialize authority for rolled-back state: the function that materializes…
...
### `CN-STORE-08`: Single encoder authority: encode_ledger_state + decode_ledger_state + encode_chain_dep +…
...
### `CN-TEST-01`: Consensus-relevant inputs must be fuzzed differentially across all supported versions and…
...
### `CN-TEST-02`: Every malformed or discrepant input that ever triggered a fork, preview mismatch, or…
...
### `CN-TEST-03`: Previously failed, duplicate, or boundary-case inputs must remain verdict-stable under…
...
### `CN-WAL-01`: Single WAL append authority: WalStore::append is the SOLE mutation method on any WalStore…
...
### `CN-WIRE-01`: Hash-critical original bytes must be preserved and used on all hash/signature-critical…
...
### `CN-WIRE-03`: Consensus-critical deserialization must be equivalent across all supported versions and…
...
### `CN-WIRE-04`: Malformed consensus-relevant inputs must be rejected deterministically before any…
...
| **Tests** | `roundtrip_every_variant`, `decode_rejects_unknown_tag`, `decode_rejects_truncated_input`, `decode_rejects_invalid_utf8_in_text_fields`, `roundtrip_every_variant`, `decode_rejects_unknown_tag`, `decode_rejects_truncated_input`, `decode_rejects_invalid_utf8_in_text_fields`, `roundtrip_every_variant`, `decode_rejects_unknown_tag`, `decode_rejects_truncated_input`, `roundtrip_every_variant`, `decode_rejects_unknown_tag`, `decode_rejects_truncated_input`, `roundtrip_every_variant`, `decode_rejects_unknown_tag`, `decode_rejects_truncated_input`, `roundtrip_every_variant`, `decode_rejects_unknown_tag`, `decode_rejects_truncated_input`, `roundtrip_every_variant`, `decode_rejects_unknown_tag`, `decode_rejects_truncated_input`, `roundtrip_every_variant`, `decode_rejects_unknown_tag`, `decode_rejects_truncated_input`, `roundtrip_every_variant`, `decode_rejects_unknown_tag`, `decode_rejects_truncated_input`, `roundtrip_every_variant`, `decode_rejects_unknown_tag`, `decode_rejects_truncated_input`, `roundtrip_every_variant`, `decode_rejects_unknown_tag`, `decode_rejects_truncated_input`, `decode_rejects_invalid_utf8_measure_name`, `real_capture_round_trips_byte_identical`, `real_capture_round_trips_byte_identical`, `rollforward_header_opaque_pass_through`, `real_capture_round_trips_byte_identical`, `block_body_is_wrapped_cbor_item`, `real_capture_round_trips_byte_identical`, `real_capture_round_trips_byte_identical`, `real_capture_round_trips_byte_identical`, `accept_version_decodes_to_semantic_version_range` |
| **CI** | _(no CI script — gap)_ |

### `CN-WIRE-08`: N2N tag-24 CBOR-in-CBOR payload envelopes are constructed and stripped through ONE shared…
...
Release obligations — the live-evidence and operator-pass commitments that gate a release claim (many are operator-gated by design and tracked as honest partials).
...
### `RO-GENESIS-REPLAY-01`: Ade independently replays the chain from byron genesis through the bootstrap point P,…
...
### `RO-LIVE-01`: A Haskell cardano-node peer issuing RequestRange covering an Ade-forged block receives,…
...
### `RO-LIVE-02`: A cardano-node peer's RollForward + BlockDelivered stream, consumed by the receive…
...
### `RO-LIVE-03`: Live tip-following pass: operator runs `ade_node --peer ADDR` against a private…
...
### `RO-LIVE-04`: Live wire-smoke pass: operator runs `ade_node --mode wire_only --peer ADDR --network…
...
| **Tests** | `main_wire_only_exits_zero_after_tip_read`, `main_wire_only_emits_peer_tip_read_with_responder_tip`, `main_wire_only_never_emits_agreement_verdict`, `main_without_genesis_does_not_attempt_admission`, `peer_dial_failure_exits_nonzero_with_error_event`, `admission_mode_fails_closed_with_ledger_seed_unavailable`, `jsonl_events_are_valid_one_object_per_line`, `live_log_writer_emits_one_object_per_line`, `live_log_writer_serializes_node_started_canonically`, `live_log_writer_two_runs_are_byte_identical`, `live_log_event_match_is_exhaustive` |
| **CI** | `ci/ci_check_wire_only_event_vocabulary_closed.sh`, `ci/ci_check_wire_only_no_bootstrap.sh` |

### `RO-LIVE-05`: Live admission-agreement pass: operator runs `ade_node` against a private cardano-node…
...
### `RO-MITHRIL-IMPORT-01`: Ade imports a Mithril-authenticated snapshot as an alternative to the cardano-cli JSON…
...
### `RO-REL-01`: Release not mainnet-eligible without mixed-version topology consensus equivalence on…
...
### `RO-REL-03`: No single implementation bug should exceed the protocol's intended safety or liveness…
...
### `RO-TEST-02`: Every fork/mismatch/parser disagreement that ever occurred becomes a permanent regression…
...
### `RO-TEST-03`: Failed, duplicate, and boundary-case inputs remain verdict-stable under resubmission and…
...
Operational invariants — key custody, operator-facing CLI/log vocabularies, and node-operation discipline.

_9 rules._

### `OP-MEM-01`: Mempool pressure and peer churn must not starve block validation, chain selection, or…
...
### `OP-NET-01`: Block producer connects only through trusted relay topology
...
### `OP-NET-02`: Relay paths geographically and topologically diverse
...
### `OP-OPS-02`: Emergency recovery procedures have explicit admissibility criteria, deterministic…
...
### `OP-OPS-03`: Incident evidence sufficient to reconstruct canonical decision path without relying on…
...
> **Strengthened in PHASE4-N-F-F.** Operator-supplied keys now also feed `--mode node` (not just `--mode produce`): `ade_node::operator_forge` reuses the existing cold/VRF/KES/opcert loaders to build the node-path `ForgeActivation`, with no parser reimplementation and no new custody surface (`ci_check_operator_forge_no_secret_leak.sh`).
...
### CODEMAP (`docs/ade-CODEMAP.md` @ `4eb7610`, PHASE4-N-F-F close)

CODEMAP was regenerated for the PHASE4-N-F-F close at `4eb7610`. It records **112** `ci_check_*.sh` gates (matching the on-disk count at this working tree). Every crate named in a **Code** row is present in CODEMAP: `ade_codec`, `ade_core`, `ade_core_interop`, `ade_crypto`, `ade_ledger`, `ade_network`, `ade_node`, `ade_plutus`, `ade_runtime`, `ade_testkit`, `ade_types`. No crate-level mismatch.

- **No BLUE crate changed this cluster.** PHASE4-N-F-F is operator-key ingress + the forge-on flip inside the RED binary/driver crate `ade_node` (two NEW sub-trees: GREEN-by-content `forge_intent`, RED `operator_forge`; the RED `node_lifecycle` arm rewired) on top of the N-F-E forge-tick spine — `run_relay_loop`'s body is byte-identical to N-F-E. CODEMAP confirms all CN-NODE-03 code loci (`forge_intent.rs` → `//! GREEN`, `operator_forge.rs` → `//! RED`, `node_lifecycle.rs` → `//! RED`) in the already-mapped `ade_node` crate, and the canonical-type total holds at **456** (no BLUE change). Enforcement is **mechanical at the library surface** (named tests + the three per-rule gates), not via the binary's live run path — RO-LIVE-01 is unchanged / operator-gated.
- **CI count is aligned:** CODEMAP's 112 matches the on-disk `ci/ci_check_*.sh` count at this working tree (Δ +2 vs the N-F-E close: the two new CN-NODE-03 gates; `ci_check_node_binary_uses_single_bootstrap.sh` was modified in place, no count change).

### HEAD_DELTAS (`docs/ade-HEAD_DELTAS.md` §5)

**HEAD_DELTAS is one cluster behind** — its baseline is `cd2484f` (the PHASE4-N-F-E close) and its §5 catalogues 110 → 110 gates (the N-F-E forge-tick span). It does **not yet catalogue the PHASE4-N-F-F gates**; reconcile on the next HEAD_DELTAS regeneration (bump baseline to the N-F-F close commit `4eb7610`). The N-F-F gates are all rule-bound:

- `ci_check_forge_intent_closed.sh` — **NEW (S1)**, bound to **CN-NODE-03** (intent half: closed two-variant `ForgeIntent`, no third "partial" variant, sole `classify_forge_intent` entry, partial arm bound by name, `ForgeIntentError` static flag-name strings only). Not yet in HEAD_DELTAS §5.
- `ci_check_operator_forge_no_secret_leak.sh` — **NEW (S2/S3)**, bound to **CN-NODE-03** (custody half: loader reuse, `OperatorForgeMaterial` not `Debug`/`Serialize`, no private-key byte accessor / serialization / logging; OP-OPS-04). Not yet in HEAD_DELTAS §5.
- `ci_check_node_binary_uses_single_bootstrap.sh` — **MODIFIED in place (S5)**, formally carried by **DC-NODE-04**'s CI row and enforcing **CN-NODE-01** per its banner: the stale `ReceiveState::new "≤1 per crate"` guard (red since N-F-C/N-F-D added `node_lifecycle` as a second legitimate owner) was replaced with a `ReceiveState` owner allow-list (`node.rs node_lifecycle.rs`). The gate is already in HEAD_DELTAS §5 (count unchanged).
- `ci_check_node_run_loop_containment.sh` — bound to **CN-NODE-02, DC-SYNC-02, T-REC-03, DC-NODE-05, CN-NODE-03** (N-F-D relay-loop containment, extended by N-F-E forge-tick subordination, shared by N-F-F's CN-NODE-03 run-loop containment). Already in HEAD_DELTAS §5.
- `ci_check_loop_planner_closed.sh` — bound to **CN-NODE-02, DC-NODE-05**. Already in HEAD_DELTAS §5.

No HEAD_DELTAS §5 gate is referenced by zero rules across this delta.

**CI gates on disk not bound to any rule CI row** (surfaced, not fixed this pass): **3 of 112** on-disk `ci_check_*.sh` gates are unbound — these are project-wide hygiene/meta gates that cannot be attributed to a single invariant. Full unbound list: `ci_check_module_headers.sh`, `ci_check_no_secrets.sh`, `ci_check_pallas_quarantine.sh`. The remaining **109** on-disk gates are rule-bound (the two new N-F-F gates are both bound to CN-NODE-03).

### SEAMS

Closed registries / closed grammars referenced by rule sources (mini-protocol ID registry, closed message taxonomies, closed log/event vocabularies, `LeaderCheckVerdict`, `ExpectedVrfInput`, `Mode`, `IngressSource`, the PHASE4-N-Y closed sets, the PHASE4-N-Z `MithrilBootstrapError`, the PHASE4-N-F-C BA-02 evidence surface) are catalogued in `docs/ade-SEAMS.md` §3. PHASE4-N-F-D adds the closed GREEN loop-planner lifecycle vocabulary `{ SyncOnce, Idle, HaltCleanly }`; PHASE4-N-F-E extends the planner forge input to a closed content-blind `Due | NotDue` and the planner output with a forge-tick precedence table; PHASE4-N-F-F adds the closed two-variant GREEN forge-intent classifier `ForgeIntent { On(ForgePaths), Off }` (+ `ForgeIntentError::PartialKeySet`) — no new plugin seam: the relay loop consumes closed types and never opens an extension point.
...
- **`declared` rules (112) are open enforcement gaps by design.** Each carries `_(no enforcing code located — gap)_` / `_(no tests named in registry — gap)_` / `_(no CI script — gap)_` cells (or a subset). These are named-but-not-yet-mechanically-enforced invariants; the gap is the actionable backlog, not a defect to paper over.

- **CN-NODE-03 (PHASE4-N-F-F, `enforced`) — forge-CAPABLE, not forge-OBSERVABLE.** The four-cell contract is filled (Code: `forge_intent.rs` + `operator_forge.rs` + `node_lifecycle.rs`; Tests: 11 classifier/ingress/lifecycle/replay tests, all on disk; CI: `ci_check_forge_intent_closed.sh` + `ci_check_operator_forge_no_secret_leak.sh` + `ci_check_node_run_loop_containment.sh`, all on disk). What is enforced is operator-key **ingress + activation assembly + the binary `Some`/`None` flip**. What is NOT claimed: on the empty-source binary path the loop halts cleanly before any `ForgeTick`, so the forge is not observable; there is no live forge / serve / gossip / peer-acceptance / BA-02 / durable-tip claim. Observable forge against a real peer is the **RO-LIVE-01 follow-on**.

- **RO-LIVE-01 — remains `partial` / operator-gated (unchanged this close).** The producer-side block-fetch live leg against a real Haskell peer is still an operator-action pass not yet executed. PHASE4-N-F-F makes the `--mode node` binary forge-CAPABLE with a complete operator key set, but the live unbounded-peer leg stays RO-LIVE-01 follow-on — CN-NODE-03's forge stays subordinate + self-accept-only and the N-F-E containment gate is byte-unchanged.

- **DC-NODE-05 (PHASE4-N-F-E, `enforced`; strengthened in N-F-F) — forge tick is subordinate to the sync spine.** Enforced by `ci/ci_check_loop_planner_closed.sh` + `ci/ci_check_node_run_loop_containment.sh` + 10 forge planner/guard/wiring/replay/fail-closed tests in `ade_node` (all resolve on disk). The forge tick advances no durable tip, admits/serves/gossips nothing, and single-epoch-fails-closed on an unsupported slot (cluster-scope containment, not permanent behaviour). N-F-F exercises this discipline with operator-material-backed keys (S4 `relay_loop_with_operator_material_*` tests) without relaxing containment. This is the **mechanism** enforced at the library surface; it is NOT a live forge-into-the-network claim.

- **CN-NODE-01 single-bootstrap gate (strengthened in N-F-F, S5).** The gate `ci_check_node_binary_uses_single_bootstrap.sh` — which enforces CN-NODE-01 per its banner but is formally carried by **DC-NODE-04**'s CI row in the registry — was stale/red since N-F-C/N-F-D added `node_lifecycle` as a second legitimate `ReceiveState` owner. S5 replaced its `ReceiveState::new "≤1 per crate"` guard with a `node.rs node_lifecycle.rs` owner allow-list; the gate is green again. CN-NODE-01's own registry `ci_script` field (and thus its CI row) carries the three bootstrap-closure gates (`ci_check_bootstrap_closure.sh`, `ci_check_produce_mode_uses_bootstrap_initial_state.sh`, `ci_check_node_mode_closure.sh`), unchanged.

- **Operator-gated release legs unchanged this close:** RO-LIVE-06 (BA-02 evidence — schema/correlation mechanics only), RO-SYNC-EVIDENCE-01 (vacuous-until-committed manifest), CE-Y-16 (two-Haskell-node witnessed live capture). PHASE4-N-F-F does not touch these.

---

## Generation notes

INCREMENTAL refresh for the **PHASE4-N-F-F cluster-close** at HEAD `4eb7610` (working tree). Primary source: the working-tree invariant registry (`docs/ade-invariant-registry.toml`, **311 rules**). The prior on-disk pass was the PHASE4-N-F-E close (HEAD `cd2484f`, 310 rules). This pass re-joins the N-F-F-affected rules to their Source / Code / Tests / CI from the registry and recomputes the mechanical counts; unrelated rule entries are carried forward unchanged. Every registry-named test (resolved to its leaf `fn`) and CI script cited in an edited row was cross-checked against the working tree using **grep/ls only** — no `cargo build`/`cargo test`/`cargo check` was run.

N-F-F delta (operator-key ingress + the forge-on flip; **NO BLUE crate changed** — 456 canonical types unchanged):

- **NEW rule `CN-NODE-03`** (`tier = constraint_network`, `status = enforced`): operator-key ingress + forge-on flip for `--mode node`. Code cites `crates/ade_node/src/forge_intent.rs` (GREEN classifier), `crates/ade_node/src/operator_forge.rs` (RED ingress + `build_operator_forge_material`), `crates/ade_node/src/node_lifecycle.rs` (Some/None flip + `ForgeKeyIngress`). All three exist on disk; all **11** registry-named tests resolve on disk (`forge_intent.rs`, `operator_forge.rs`, `node_lifecycle.rs`, `node_sync.rs`); all **3** registry-named CI gates exist on disk. Inserted into the CN family after `CN-NODE-02`, preserving ID order.
- **`strengthened_in += "PHASE4-N-F-F"` on four rules:** `CN-NODE-01` (single bootstrap authority — S5 `ci_check_node_binary_uses_single_bootstrap.sh` `ReceiveState` owner-allow-list precision fix), `OP-OPS-04` (operator-supplied keys — operator keys now also feed `--mode node`, not just `produce`), `CN-PROD-02` (key custody / no KES rotation past period — RED key-custody line extended to the node path via `operator_forge`), `DC-NODE-05` (forge-slot discipline — now exercised with operator-material-backed keys, S4). Each row carries a strengthening note; their registry-faithful Code/Tests/CI rows are otherwise unchanged this pass.
- **No rule weakened or deprecated; no new BLUE rule.** CN-NODE-03 is the only new entry; the forge remains subordinate + self-accept-only (N-F-E containment gate byte-unchanged).

Mechanical counts recomputed fresh from the registry at `4eb7610` (`grep -cE "^id = "` = **311**; no duplicate IDs):

- Status: **179 enforced** (178 → 179, the CN-NODE-03 flip) / **20 partial** / **112 declared** / **0 deprecated**.
- By tier: constraint 4 · constraint_network **2** (1 → 2, CN-NODE-03) · derived 187 · operational 16 · release 40 · true 62.
- By family: T 30 · DC 144 · **CN 112** (111 → 112, CN-NODE-03) · RO 16 · OP 9.

**Drift check (this pass, edited rows):** **0** registry-named tests and **0** registry-named CI scripts fail to resolve on disk at `4eb7610` across CN-NODE-03 and the four strengthened rules. Every `crates/**.rs` code locus and every `ci/**.sh` gate cited in an edited row was verified present on disk (the `ci_check_registry_code_locus_exists.sh` invariant).

**CODEMAP** (regenerated at `4eb7610`, 112 CI checks) maps every CN-NODE-03 code locus inside the already-mapped `ade_node` crate; the on-disk `ci/ci_check_*.sh` count (112) matches CODEMAP. **HEAD_DELTAS** baseline is one cluster behind (`cd2484f`, the N-F-E close) and does **not yet catalogue the N-F-F gates** (`ci_check_forge_intent_closed.sh`, `ci_check_operator_forge_no_secret_leak.sh`) — reconcile on the next HEAD_DELTAS regeneration. The `_(… — gap)_` cells and the **Honest enforcement gaps** section are the actionable output — reconcile them in the registry or the codebase; do not delete them. Regenerate after every cluster close.
