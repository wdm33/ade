...
>
> **HEAD:** `6bd60c80` (PHASE4-N-F-G-D close — S4 close commit `6bd60c80`, registry close-pass edits applied) · **Generated:** 2026-06-02 · **Prior pass:** `6f848825` (PHASE4-N-F-G-E close)
...
The canonical source is the **invariant registry** at `docs/ade-invariant-registry.toml` (`.idd-config.json` `invariant_registry`), **315 rules** at HEAD `6bd60c80` (`+1` this close — the NEW `CN-REHEARSAL-FIDELITY-01`, `tier = release`, added at `status = enforced` at the G-D close). This doc is a join: registry entries × codebase introspection. Rule IDs, families, names, sources, and the registry-recorded `tests` / `ci_script` / `code_locus` fields come from the registry; each test name and CI script is cross-checked for existence in the working tree at `6bd60c80`, and divergences are flagged inline as **drift**. (Verification this pass is grep/ls only — no build or test execution; the two new rule-bound G-D gates `ci_check_node_path_fidelity.sh` + `ci_check_rehearsal_manifest_schema.sh` were additionally executed green via `bash`, and the byte-unchanged BA-02 bounty gate `ci_check_ba02_evidence_manifest_schema.sh` was re-run green.)
...
- **[not found on disk — drift]** (tests) / **[not on disk — drift]** (CI) flags a registry-named test/CI that could not be located in the working tree at `6bd60c80` — a registry↔code coherence gap to reconcile. This pass: **0** such drifts (every cited test leaf-fn and every cited CI script resolves on disk; the 7 `CN-REHEARSAL-FIDELITY-01` tests + its 2 new CI gates all resolve).
...
By tier: constraint 4 · constraint_network 2 · derived 190 · operational 16 · release 41 · true 62.

By family: T 30 · DC 147 · CN 113 · RO 16 · OP 9.
...
The four-cell contract is filled for every `enforced` rule (Code + Tests + CI all present and verified in the working tree via grep/ls, with the two new rule-bound G-D gates `ci_check_node_path_fidelity.sh` + `ci_check_rehearsal_manifest_schema.sh` additionally executed green this pass, and the byte-unchanged BA-02 bounty gate re-run green). The standing gaps below are recorded rather than hidden.
...
- **`CN-OPERATOR-EVIDENCE-01` (PHASE4-N-S-C, `enforced`; strengthened N-F-G-C) — operator-pass manifest schema; standing Tests gap (carried).** The rule's `tests` array is `[]` in the registry; it is **gate-enforced** (the manifest-schema gates fire only when a `CE-*-LIVE_*.toml` is committed, and **0** are committed at HEAD, so they are vacuously satisfied). **G-D does not bump it** (no `strengthened_in += "PHASE4-N-F-G-D"`). Carried unchanged as a standing Tests gap.

- **`RO-LIVE-01` (PHASE4-N-G, `partial`; strengthened N-Q + N-F-G-C) — producer-side live leg, MECHANICAL half wired (carried).** Four-cell contract has Code + Tests + CI, but **status stays `partial`** (verified: registry `status = "partial"`, `strengthened_in = ["PHASE4-N-Q", "PHASE4-N-F-G-C"]` — **no G-D token**). The LIVE half is operator-gated: peer ACCEPT stays `blocked_until_operator_stake_available`. **G-D does not touch `RO-LIVE-01`** — it rehearses the same path in a private venue but does not advance the operator-witnessed accept. Carried unchanged.

- **`RO-LIVE-06` (PHASE4-N-F-C, `enforced`; strengthened N-F-G-C) — BA-02 evidence, schema + correlation mechanics ONLY (carried).** What is enforced: the closed versioned `Ba02Manifest`, the pure/total/deterministic HASH-PRIMARY `correlate` as the SOLE constructor, the allow-list parser, and the operator-pass file I/O whose write path accepts only a `correlate`-produced `Ba02Manifest`. **What is NOT claimed:** that BA-02 was achieved live. Verified: registry `status = "enforced"`, `strengthened_in = ["PHASE4-N-F-G-C"]` — **no G-D token**. **G-D does not touch `RO-LIVE-06`.** Carried unchanged.

- **`DC-NODE-06` (PHASE4-N-F-G-B, `enforced`; strengthened N-F-G-C) — self-accept → serve handoff, mechanism-only (carried).** What is enforced: only a BLUE self-accepted artifact (typed `SelfAcceptedHandoff`, sole provenance `from_self_accepted`) reaches the single `ServedChainHandle::push_atomic`, fed only by `into_accepted()`; the relay-loop body performs no serve/admit/`push_atomic`/durable-tip mutation. **G-D leaves the handoff fence byte-unchanged** and adds no `strengthened_in` token. Carried unchanged.

- **Operator-gated release legs unchanged this close (none flipped):** `RO-LIVE-01` (producer-side live leg against a real peer — MECHANICAL half wired by G-C, LIVE half operator-gated), `RO-LIVE-06` (BA-02 evidence — schema/correlation mechanics only; peer ACCEPT proven only by the peer's validation log, never by Ade's self-accept / `ForgeSucceeded` / served-block / any wire-success signal), `RO-SYNC-EVIDENCE-01` (vacuous-until-committed manifest), `CE-Y-16` (two-Haskell-node witnessed live capture). PHASE4-N-F-G-D does **not** flip any of these — it adds a path-faithful, non-promotable rehearsal HARNESS whose own live C1 execution is itself operator-gated (`blocked_until_operator_c1_net_executed`), with no bounty / preview / preprod completion claim.

---

## Generation notes

INCREMENTAL refresh — **PHASE4-N-F-G-D close** at HEAD `6bd60c80` (the S4 close commit). The rule set is **315** (`+1` vs the prior pass): the single NEW `CN-REHEARSAL-FIDELITY-01` (`tier = release`, `introduced_in = "PHASE4-N-F-G-D"`, `strengthened_in = []`), added directly at `status = enforced`. This pass joins the new row to its Source / Code / Tests / CI from the registry at `6bd60c80` (the close-pass edits already applied) and re-runs the mechanical counts + the on-disk existence checks (grep/ls), plus a green execution of the two rule-bound G-D gates and a green re-run of the byte-unchanged BA-02 bounty gate. The prior pass was the PHASE4-N-F-G-E close (HEAD `6f848825`, 314 rules, inventory 182 / 20 / 112).

The substantive delta since the prior pass (the only registry close-pass edit) — **one NEW `enforced` rule, NO existing rule weakened / re-joined / status-flipped:**

- **`CN-REHEARSAL-FIDELITY-01` — NEW, added at `enforced`.** `introduced_in = "PHASE4-N-F-G-D"`, `strengthened_in = []`, `tier = release`. **Source:** `docs/planning/phase4-n-f-g-d-invariants.md` + the cluster/slice docs under `docs/clusters/PHASE4-N-F-G-D/` (`cluster.md` + S1–S4). Two coupled clauses: (1) PATH FIDELITY — the C1 private dry-run uses the SAME `--mode node` accepted-block path as preview/preprod (no private-only flag/branch/bootstrap/from-genesis constructor); (2) EVIDENCE NON-PROMOTABILITY — any private manifest is rehearsal-marked, stored only under the rehearsal home, sha256-bound, `correlate`-produced, and flips no RO-LIVE rule. **Code:** GREEN `ade_node::rehearsal_evidence` (`PrivateRehearsalManifest`) + RED `ade_node::rehearsal_pass` (file I/O) + the reused GREEN `ba02_evidence::correlate` + the shared `ade_runtime::consensus_inputs::canonical::import_live_consensus_inputs` (the path the S1 fence pins). **Tests:** the 7 named below. **CI:** `ci/ci_check_node_path_fidelity.sh` + `ci/ci_check_rehearsal_manifest_schema.sh` (both NEW). **cross_ref:** `RO-LIVE-01`, `RO-LIVE-06`, `CN-OPERATOR-EVIDENCE-01`, `DC-NODE-06`, `DC-EPOCH-03`, `CN-NODE-02`, `CN-CINPUT-03`, `DC-CINPUT-02b` — references the bounty path it rehearses but does **NOT** advance it.

No other rule was added, removed, weakened, re-joined, or deprecated. **Crucially, G-D advances no bounty deliverable:** the literal `PHASE4-N-F-G-D` appears in **no** `strengthened_in` array in the registry — `RO-LIVE-01` stays `partial` (`["PHASE4-N-Q", "PHASE4-N-F-G-C"]`), `RO-LIVE-06` stays `enforced` (`["PHASE4-N-F-G-C"]`), `CN-OPERATOR-EVIDENCE-01` stays `enforced` (`["PHASE4-N-F-G-C"]`) — verified by grep. No BLUE crate / canonical type changed (**456** holds — `git diff --name-only da205bff..6bd60c80` over every BLUE `core_paths` entry is empty; the NEW `ade_node::rehearsal_evidence` is GREEN-by-content and the NEW `ade_node::rehearsal_pass` is RED, neither canonical-counted). Two CI scripts added on disk (119 → 121: `ci_check_node_path_fidelity.sh` + `ci_check_rehearsal_manifest_schema.sh`), both rule-bound on landing to `CN-REHEARSAL-FIDELITY-01`; the three serve/containment/memory fences (`ci_check_node_run_loop_containment.sh`, `ci_check_served_chain_handoff_fence.sh`, `ci_check_live_feed_memory_bounds.sh`) AND the BA-02 bounty gate (`ci_check_ba02_evidence_manifest_schema.sh`) are byte-unchanged.

Mechanical counts recomputed fresh from the registry at `6bd60c80` (`grep -cE '^\[\[rules\]\]'` = **315**; `grep -cE '^id = '` = **315**; no duplicate IDs; `grep -cE '^status = '` = **315**):

- Status: **183 enforced** / **20 partial** / **112 declared** / 0 deprecated. (**+1 enforced** vs the prior pass — the single NEW `CN-REHEARSAL-FIDELITY-01`. Cross-checked against the anchored registry `^status =` lines: 183 / 20 / 112; sum 315.)
- By tier: constraint 4 · constraint_network 2 · derived 190 · operational 16 · release **41** · true 62. (`release` +1 — `CN-REHEARSAL-FIDELITY-01` is `tier = release`; `derived` already includes the prior-pass `DC-LIVEMEM-01`.)
- By family: T 30 · DC 147 · CN **113** · RO 16 · OP 9. (`CN` +1 — the new rule's `CN-` ID prefix; inserted in ID order between `CN-PUMP-01` and `CN-REL-01`.)

**Drift check (this pass): 0 drifts.** All 7 `CN-REHEARSAL-FIDELITY-01` tests resolve on disk: `node_accepted_block_consensus_inputs_via_shared_import` (`crates/ade_node/tests/node_path_fidelity.rs:70`); `rehearsal_envelope_wraps_correlate_produced_payload` (`crates/ade_node/src/rehearsal_pass.rs:94`), `rehearsal_correlate_no_evidence_writes_nothing` (`:120`), `rehearsal_envelope_is_structurally_distinct_from_ba02_manifest` (`:143`); `c1_dry_run_correlate_to_rehearsal_envelope` (`crates/ade_node/tests/node_c1_dry_run_rehearsal.rs:94`), `node_c1_dry_run_rehearsal_live` (`:140`); `rehearsal_gate_fails_on_archived_home_leak` (`crates/ade_node/tests/rehearsal_gate_archived_home.rs:54`). All four code loci resolve on disk (`PrivateRehearsalManifest` + `from_correlate_outcome` + `to_canonical_toml` in `rehearsal_evidence.rs`; `correlate_peer_log_file_into_rehearsal` + `write_private_rehearsal_manifest` in `rehearsal_pass.rs`; `correlate` at `ba02_evidence.rs:290`; `import_live_consensus_inputs` at `consensus_inputs/canonical.rs:112`). The source doc `docs/planning/phase4-n-f-g-d-invariants.md` and the cluster docs `docs/clusters/PHASE4-N-F-G-D/{cluster.md,S1..S4}.md` resolve. Both CI gates resolve and executed green (exit 0): `ci/ci_check_node_path_fidelity.sh` (clause 1 — 28-flag closed allow-list + no from-genesis constructor + shared importer) and `ci/ci_check_rehearsal_manifest_schema.sh` (clause 2 — vacuous-until-committed, three non-promotability barriers incl. the S4 archived-home scan). Across the full 315 rules the prior pass recorded 0 unresolved references; this close adds only resolved references, so the **0-drift** property holds.
