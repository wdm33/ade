...
>
> **HEAD:** `f87d0056` (PHASE4-N-AF — single-producer extend-own-durable-spine: after initial peer catch-up (`DC-NODE-15`) AND an explicit RED venue-adoption certificate for Ade's first successor, an explicitly single-producer Ade may forge the next successor on its OWN durable adopted spine WITHOUT a relay echo of the adopted block — closing the Rung-1 Finding B stall where the relay never re-announces Ade's own block; new rule `DC-NODE-18` ENFORCED for the scoped successor-extension core, sibling observation rule `DC-NODE-17` retained DECLARED) · **Generated:** 2026-06-08 · **Prior pass:** `6363683e` (PHASE4-N-AE.F close, 341 rules) · **Registry:** `docs/ade-invariant-registry.toml` @ **343 rules** (210 enforced / 20 partial / 113 declared / 0 deprecated) · **CODEMAP:** `docs/ade-CODEMAP.md` @ `6363683e` (458 canonical types · 142 CI · 341 rules — **NOT yet regenerated to `f87d0056`**; this is a TRACEABILITY-only incremental refresh for the PHASE4-N-AF close) · **HEAD_DELTAS:** `6363683e` (§5 @ 142 CI; §7 @ 341 rules — **NOT yet regenerated to `f87d0056`**; the NEW gate `ci_check_single_producer_extend_own_spine.sh` is on disk but not yet listed in HEAD_DELTAS §5 — surfaced below as a cross-doc lag to reconcile at the next CODEMAP/HEAD_DELTAS refresh, not a TRACEABILITY drift)
...
By tier: constraint 4 · constraint_network 2 · derived 213 · operational 17 · release 41 · true 64.

By family: T 32 · DC 165 · CN 119 · RO 16 · OP 9.
...
_PHASE4-N-AF close delta (`6363683e` → `f87d0056`): `+2` rules since the prior refresh — `DC-NODE-18` (derived/DC, `introduced_in = "PHASE4-N-AF"`, **`enforced`** for the scoped successor-extension-after-adoption-certificate core) and `DC-NODE-17` (derived/DC, `introduced_in = "TBD"`, **`declared`**, observation-only); `0` strengthenings; `0` rules removed / deprecated / weakened. CI: `+1` NEW gate `ci_check_single_producer_extend_own_spine.sh` bound to `DC-NODE-18` (142 → **143** on disk). Registry↔TRACEABILITY join: **343 ↔ 343**. **HEADLINE (Rung-1 Finding B fix; NO `RO-*` count change):** the sole-producer stall is closed. OQ-1 (c2t4 live) DISPROVED `DC-NODE-17`-as-fix — the relay does NOT re-announce Ade's own adopted block over the follow link (Ade forged + got block 12 adopted, but `followed_peer_tip` stayed at 11; no `RollForward(12)`), so observing the echo cannot un-stick the loop. `DC-NODE-17` is therefore RETAINED as a SAFETY/OBSERVATION invariant only (no enforcing tests/code/CI — `tests = []`, `ci_script = ""`). The actual fix is `DC-NODE-18`: after initial catch-up (`DC-NODE-15`) AND an explicit RED venue-adoption certificate (matched by chain-point identity hash+block_no, slot ignored, NEVER self-admit-inferred), an EXPLICITLY single-producer Ade forges the next successor on its OWN durable adopted spine WITHOUT a relay echo, behind a fail-closed `ForgeRefused::SingleProducerFenceViolation` and a default-`Unknown`-runs-the-verbatim-`DC-NODE-15`-path venue gate. **LIVE CORE PROOF (CE-AF-6a; C2-LOCAL cardano-testnet magic 42, run c2t7, `docs/evidence/phase4-n-af-extend-own-spine.{md,jsonl}`):** Ade forged block 11 → the non-producing Haskell relay adopted it → the operator wrote the RED venue-adoption certificate (`11 327 7e67cd0d…`) → Ade promoted by chain-point identity → Ade forged block 12 from its OWN durable adopted spine → the relay adopted block 12, NO echo of 11 back to Ade. Two live-surfaced loop bugs were found + fixed en route, BOTH missed by the hermetic suite + the IDD/security reviews (the live gate's value): (1) the post-forge mode advancement fired on a not_leader tick (forged=true but nothing admitted), wrongly promoting before any block — now fenced by `forge_mode_after_admit`; (2) the cert promotion used full `TipPoint` equality incl. slot instead of chain-point identity. **BLUE UNCHANGED** (`pump_block` idempotent admission `DC-NODE-16`; ledger/chain_dep/WAL untouched; successor construction from the evolved admitted spine `DC-NODE-10`/`DC-CONS-24`; `DC-CONS-03` the sole fork-choice authority); `RO-LIVE-01`/`RO-LIVE-05` NOT re-counted (the operator-witnessed sustained pass stays gated). **DEFERRED, NOT claimed (CE-AF-6b → `DC-NODE-19`):** sustained production past k, relay ImmutableDB settlement, follow-link liveness, and forge-loop continuation after the Ade→relay follow link EOFs (the c2t7 run stopped at 2 blocks because that link EOF'd — relay idle timeout, no keep-alive). **Cross-doc lag surfaced honestly:** CODEMAP + HEAD_DELTAS have NOT been regenerated to `f87d0056` (both still record 142 CI / 341 rules); the NEW gate `ci_check_single_producer_extend_own_spine.sh` is on disk but not yet in HEAD_DELTAS §5 — to reconcile at the next CODEMAP/HEAD_DELTAS refresh; this is an incremental TRACEABILITY-only pass, not a drift._

_PHASE4-N-AE.F close delta (`a76672b9` → `6363683e`): `+1` rule — `DC-NODE-16` (derived/DC, `introduced_in = "PHASE4-N-AE"`, all four cells `enforced`) — receive idempotency at the durable-admit chokepoint; `0` strengthenings; `0` rules removed / deprecated / weakened. CI: `+1` NEW gate `ci_check_receive_idempotency.sh` (141 → **142**). Registry↔TRACEABILITY join: **341 ↔ 341**. **HEADLINE (a continuity/robustness fix, NO `RO-*` count change):** after the CE-A5 relay adopted Ade's forged block 17 (the PHASE4-N-AE close manifest), the relay RE-ANNOUNCED that same block over Ade's follow link, and the BLUE header authority correctly rejected `SlotBeforeLastApplied{last=421,attempted=421}` — terminating the run (exit 43) AFTER `AddedToCurrentChain`. Not a manifest blocker, but it ends a continuous run. Fix: `pump_block`, immediately after `decode_block`, queries `db.get_block_by_hash(&decoded.block_hash)`; if `Some(stored)` AND `stored.slot == decoded.header_input.slot` it returns `Ok(None)` (idempotent no-op) BEFORE the RollForward/BlockDelivered reducer steps — no validation, no WAL append, no tip change. The gate is HASH-keyed (`get_block_by_hash` is hash-exact), so a DIFFERENT block (different hash) at/before the last-applied slot returns `None` here, falls through to the unchanged BLUE `validate_and_apply_header`, and STAYS fail-closed (`SlotBeforeLastApplied` / `BlockNoOutOfOrder`). The no-op appends nothing to the WAL, so warm-start replay is unaffected (`T-REC-05` / `DC-WAL-02` preserved); no skip-past, no fork-choice (`DC-CONS-03` untouched). **RED chokepoint only — NO BLUE change:** `get_block_by_hash` is a DETERMINISTIC durable-store query (not nondeterminism), so the gate lives at the RED `pump_block` chokepoint with no new reducer input and the BLUE authority is untouched — a refinement of the AE.F sketch's BLUE `ReceiveOutcome::AlreadyHave`. **One honest enforcement note (carried, NOT new):** `DC-PROTO-10` still has a regression test but `ci_script = ""` (no dedicated CI gate, test-enforced); unchanged this pass._
...
_tier: derived · status: enforced_ · _introduced_in: PHASE4-N-AF_
...
### PHASE4-N-AF pass (`f87d0056`) — current-pass coherence

- **CODEMAP** (`docs/ade-CODEMAP.md`) was last regenerated to HEAD `6363683e` and its headline reads **458 canonical types / 142 CI checks / 341 rules**. This TRACEABILITY pass is at `f87d0056` with **143 CI / 343 rules**, so CODEMAP **lags by `+1` CI gate and `+2` rules** — a known lag for an incremental TRACEABILITY-only refresh, NOT a drift. The `DC-NODE-18` **Code** row maps to already-CODEMAP-mapped modules — `ade_node::node_sync` + `ade_node::node_lifecycle` + `ade_node::cli` (all RED `ade_node` lifecycle/sync entries) — so the next CODEMAP refresh adds NO new module; only the rule count (341 → 343) and CI headline (142 → 143) move. The BLUE `core_paths` trees are unchanged (the fix is a RED forge-admissibility gate + a GREEN forge-mode state machine, no new canonical type — 458 unchanged).
- **HEAD_DELTAS** (`docs/ade-HEAD_DELTAS.md`) was last regenerated to HEAD `6363683e` and still records **142 CI / 341 rules**. The NEW gate `ci_check_single_producer_extend_own_spine.sh` is on disk + executable but is **NOT yet referenced anywhere in HEAD_DELTAS** (verified by grep) — to be added to §5 (CI **142 → 143**, `+1 new`, bound to `DC-NODE-18`) and §7 (rules **341 → 343**, status 209/20/112 → **210/20/113**) at the next HEAD_DELTAS refresh. Surfaced honestly as a cross-doc lag, not a drift.
- **SEAMS** — `DC-NODE-18` introduces no new extension point: it reuses the existing RED forge-admissibility chokepoint (the `run_relay_loop_with_sched` ForgeTick arm) and the existing fenced `forge_one_from_recovered` path, adding a GREEN `ForgeMode` state machine + a typed `SingleProducerForgeDecision` + a fail-closed `ForgeRefused::SingleProducerFenceViolation`, all behind an explicit `VenueRole::SingleProducer` gate whose default (`Unknown`) runs the VERBATIM prior `DC-NODE-15` path. The closed forge-admissibility surface is unchanged (it is REFINED, not extended); the venue-adoption certificate is admissibility-only (never persisted / replay-visible). SEAMS needs no new §3 entry.

## Cross-references

### PHASE4-N-AE.F pass (`6363683e`) — current-pass coherence

- **CODEMAP** (`docs/ade-CODEMAP.md`) was regenerated to HEAD `6363683e` in the AE.F fix commit and its headline reads **458 canonical types / 142 CI checks / 341 rules** — CI count (142) and rule count (341) MATCH this doc. CODEMAP independently records the RED-only / 0-BLUE-diff / 0-new-type fact (`git diff a76672b9..6363683e` over the BLUE `core_paths` trees adds zero `^+(pub )?(struct|enum)` lines; 458 unchanged) and lists the RED `ade_runtime::forward_sync::pump` module change in its §RED table — `DC-NODE-16`'s **Code** row maps to that already-CODEMAP-mapped module. No CODEMAP drift.
- **HEAD_DELTAS** (`docs/ade-HEAD_DELTAS.md`) was regenerated to HEAD `6363683e` in the AE.F fix commit and AGREES with this pass: §5 records the CI delta **141 → 142** (`+1 new`, `0 modified`, `0 removed`) with the single NEW gate `ci_check_receive_idempotency.sh` listed and bound to `DC-NODE-16`; §7 records the rule delta **340 → 341** (`+1`, status 208/20/112 → **209/20/112**, `0` strengthenings, `0` removed/weakened). No HEAD_DELTAS drift.
- **SEAMS** — `DC-NODE-16` introduces no new extension point: it reuses the existing `ChainDb::get_block_by_hash` read primitive and the existing `pump_block` durable-admit chokepoint, adding one hash-exact already-have guard before the unchanged BLUE `forward_sync_step` reducer. The closed receive/admit surface is unchanged; SEAMS needs no new §3 entry.
...
INCREMENTAL DELTA-REFRESH — **PHASE4-N-AF — single-producer extend-own-durable-spine (Rung-1 Finding B fix)** at HEAD `f87d0056` (user-directed targeted regeneration for the cluster close; the cluster doc lives at `docs/clusters/PHASE4-N-AF/cluster.md`, slice `S1-single-producer-extend-own-spine.md`). Only the **two** changed rules were re-traced from scratch against disk — NEW `DC-NODE-18` (`introduced_in = "PHASE4-N-AF"`, `enforced` for the scoped successor-extension core) and NEW `DC-NODE-17` (`introduced_in = "TBD"`, `declared`, observation-only). Every other rule's four-cell trace row is **preserved verbatim** from the prior pass (HEAD `6363683e`). The rule set is **343** (`+2` vs the prior `341`); registry↔TRACEABILITY is **343 ↔ 343** (1:1). CI: **143** on disk (`+1` `ci_check_single_producer_extend_own_spine.sh`). grep / ls only; no build or test execution; the 341 carried rows were not re-verified this pass.

The substantive delta — **two rules added (one enforced, one declared); NO existing rule weakened / re-joined / removed / deprecated; NO strengthenings; NO `RO-*` advanced** (`DC-NODE-18` is the sole-producer-stall fix proven LIVE at the core scope but the operator-witnessed sustained pass — `RO-LIVE-01`/`RO-LIVE-05` — stays gated; the c2t7 run forged + got 2 blocks adopted then the follow link EOF'd):

- **`DC-NODE-18` — NEW, `enforced`** (for the SCOPED core only). `tier = derived`, `introduced_in = "PHASE4-N-AF"`. **Source:** `docs/planning/single-producer-extend-own-spine-invariants.md` + `docs/clusters/PHASE4-N-AF/{cluster.md,S1-single-producer-extend-own-spine.md}`. After initial peer catch-up (`DC-NODE-15`) AND an explicit RED venue-adoption certificate for Ade's first successor — matched by chain-point identity (hash + block_no), slot ignored, NEVER self-admit-inferred — an EXPLICITLY single-producer Ade forges the next successor on its OWN durable adopted spine WITHOUT a relay echo. Forge mode is an explicit 4-state enum `ForgeMode` (`InitialCatchupRequired → CaughtUpToPeerTip → FirstOwnBlockServed → SingleProducerExtendOwnDurableSpine`, no booleans); fails closed to a typed `ForgeRefused::SingleProducerFenceViolation`; the default `VenueRole::Unknown` runs the VERBATIM prior `DC-NODE-15` path. A gate-APPLICABILITY refinement, NOT a fork-choice weakening — `DC-CONS-03` stays the sole chain-selection authority and the followed-peer-tip signal still may not select/replace/reorder/prefer chains. **Disk verification (`f87d0056`):** all 6 cited tests resolve (`forge_mode_transitions_are_total_and_deterministic`, `extend_own_spine_promotion_requires_adoption_certificate`, `extend_own_spine_forges_on_durable_tip_without_followed_equality`, `single_producer_fence_fails_closed`, `extend_own_spine_two_runs_byte_identical`, `forge_mode_after_admit_only_advances_on_real_admit` — all in `crates/ade_node/src/node_sync.rs::tests`, one `fn` each); the `code_locus` resolves (`ForgeMode`@`node_sync.rs:783` + `VenueRole`/`VenueAdoptionCertificate`/`SingleProducerFenceReason`/`SingleProducerForgeDecision`/`single_producer_forge_decision`/`forge_mode_after_admit` + `ForgeRefused::SingleProducerFenceViolation`@`node_sync.rs:669`/`926`; the mode-aware ForgeTick gate + `read_adoption_cert` + `declare_single_producer_venue` in `node_lifecycle.rs`; `--single-producer-venue`/`--adoption-cert-path` in `cli.rs`); the gate `ci_check_single_producer_extend_own_spine.sh` is on disk + executable (164 lines, 5 named assertions a–e, 17 references to the load-bearing symbols — NOT a stub); the live evidence files `docs/evidence/phase4-n-af-extend-own-spine.{md,jsonl}` (run c2t7) exist and record the block-11 → cert → block-12 extend-without-echo sequence. **DEFERRED, NOT claimed (CE-AF-6b → `DC-NODE-19`):** sustained production past k, relay ImmutableDB settlement, follow-link liveness, forge-loop continuation after follow-link EOF.

- **`DC-NODE-17` — NEW, `declared` (observation-only).** `tier = derived`, `introduced_in = "TBD"`. **Source:** `docs/planning/sustained-single-producer-forge-invariants.md`. The candidate Rung-1-Finding-B fix (recognize catch-up to Ade's own adopted block via the peer's re-announcement). **OQ-1 (c2t4 live) DISPROVED it as the fix** — the relay does NOT re-announce Ade's own adopted block over the follow link, so observing the echo cannot un-stick the loop. RETAINED as a SAFETY/OBSERVATION invariant only. **Three load-bearing cells are honest gaps** (`tests = []`, `ci_script = ""`, `code_locus` is a CANDIDATE pending OQ-1, which resolved NEGATIVE) — recorded as `_(declared — gap)_`, not hidden. The actual stall fix is `DC-NODE-18`.

**Registry reconciliations: NONE — 0 open on the two re-traced rows.** The registry `tests` / `ci_script` / `code_locus` for `DC-NODE-18` are exact on disk; `DC-NODE-17` carries `tests = []` / `ci_script = ""` by design (observation-only) and is traced as a declared-rule gap.

**Cross-doc coherence at this pass — a known, honestly-surfaced lag (NOT a TRACEABILITY drift):** CODEMAP (`docs/ade-CODEMAP.md`) and HEAD_DELTAS (`docs/ade-HEAD_DELTAS.md`) were last regenerated to `6363683e` and BOTH still record **142 CI / 341 rules**. This TRACEABILITY pass records **143 CI / 343 rules** at `f87d0056`. The deltas to reconcile at the next CODEMAP/HEAD_DELTAS refresh: (1) HEAD_DELTAS §5 CI **142 → 143** with the NEW gate `ci_check_single_producer_extend_own_spine.sh` bound to `DC-NODE-18` (verified on disk: the gate is NOT yet referenced anywhere in HEAD_DELTAS); (2) HEAD_DELTAS §7 rules **341 → 343** (status 209/20/112 → **210/20/113**); (3) CODEMAP rule count **341 → 343** and its CI headline **142 → 143**. The `crates/ade_node/src/{node_sync,node_lifecycle,cli}.rs` modules that carry the `DC-NODE-18` Code row are already CODEMAP-mapped (RED `ade_node` lifecycle/sync tables) — no NEW module to add; the BLUE `core_paths` trees are unchanged (the fix is RED forge-admissibility + a GREEN forge-mode state machine, no new canonical type — 458 unchanged).

FAST DELTA-REFRESH — **PHASE4-N-AE.F — receive idempotency at the durable-admit chokepoint (post-CE-A5 echo fix)** at HEAD `6363683e` (user-directed targeted regeneration; the cluster doc lives at `docs/clusters/PHASE4-N-AE/slices/AE.F.md`). Only the **one** PHASE4-N-AE.F rule was re-traced from scratch against disk — NEW `DC-NODE-16` (`introduced_in = "PHASE4-N-AE"`, all four cells `enforced`). Every other rule's four-cell trace row is **preserved verbatim** from the prior pass (HEAD `a76672b9`). The rule set is **341** (`+1` vs the prior `340`); registry↔TRACEABILITY is **341 ↔ 341** (1:1). CI: **142** (`+1` `ci_check_receive_idempotency.sh`). grep / ls only; no build or test execution; the 340 carried rows were not re-verified this pass.

The substantive delta — **one rule introduced + enforced; NO existing rule weakened / re-joined / removed / deprecated; NO strengthenings; NO `RO-*` advanced** (`DC-NODE-16` is a RED receive-side robustness fix at the `pump_block` chokepoint, not a live-acceptance step — `RO-LIVE-01` stays operator-gated):

- **`DC-NODE-16` — NEW, `enforced`.** `tier = derived`, `introduced_in = "PHASE4-N-AE"`. **Source:** `docs/clusters/PHASE4-N-AE/slices/AE.F.md` + `docs/planning/phase4-n-ae-f-echo-idempotency-invariants.md` + `docs/evidence/phase4-n-ae-ce-a5-relay-adoption.md` (the CE-A5 run that surfaced the echo). After the relay adopted Ade's forged block 17 it re-announced that block over Ade's follow link; the unchanged BLUE header authority rejected `SlotBeforeLastApplied{last=421,attempted=421}` (a CORRECT fail-closed) and ended the run (exit 43) AFTER `AddedToCurrentChain`. Fix: `pump_block`, immediately after `decode_block`, queries `db.get_block_by_hash(&decoded.block_hash)`; on a hit whose `stored.slot == decoded.header_input.slot` it returns `Ok(None)` (idempotent no-op) BEFORE the RollForward/BlockDelivered reducer steps. The gate is HASH-keyed, so a DIFFERENT block (different hash) at/before the last-applied slot returns `None` and falls through to the unchanged BLUE `validate_and_apply_header`, staying fail-closed (CE-F2). The no-op runs no reducer step and appends nothing to the WAL → warm-start replay is unaffected (`T-REC-05` / `DC-WAL-02` preserved). 3 on-disk tests (CE-F1 `pump_block_reannounced_block_is_idempotent_noop`, CE-F2 `pump_block_different_block_at_or_before_tip_still_fails_closed`, CE-F4 `run_node_sync_survives_reannounced_block_in_feed`); gate `ci_check_receive_idempotency.sh` (NEW; fences hash-keyed + gate-before-reducer + slot-consistency). **RED chokepoint only — 0 BLUE diff:** `get_block_by_hash` is a DETERMINISTIC durable-store query (not nondeterminism), so the gate lives at the RED `pump_block` chokepoint with no new reducer input and the BLUE authority is untouched — a refinement of the AE.F sketch's BLUE `ReceiveOutcome::AlreadyHave`; the canonical-type count is unchanged (458).

**Registry reconciliations: NONE — 0 open on the one re-traced row.** The registry `tests` / `ci_script` / `code_locus` for `DC-NODE-16` are exact on disk (all 3 cited leaf-fns + the gate + the `get_block_by_hash` locus at `pump.rs:102` resolve; `0` drifts).

**Cross-doc coherence at this pass — all three grounding docs AGREE at HEAD `6363683e`:** CODEMAP (`docs/ade-CODEMAP.md`, regenerated to `6363683e` in the AE.F fix commit) reports **458 canonical types / 142 CI / 341 rules** — CI (142) and rule (341) counts match this doc; it lists the RED `ade_runtime::forward_sync::pump` module change and independently records the 0-BLUE-diff / 0-new-type fact. HEAD_DELTAS (`docs/ade-HEAD_DELTAS.md`, regenerated to `6363683e`) §5 records CI **141 → 142** with `ci_check_receive_idempotency.sh` bound to `DC-NODE-16`, and §7 records rules **340 → 341** (status 208/20/112 → 209/20/112). **0 cross-doc drift** for the PHASE4-N-AE.F span.
