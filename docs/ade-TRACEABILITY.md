...
>
> **HEAD:** `233644f7` (cluster-close for `MEM-OPT-OPS` — the RED/GREEN memory-optimization + measurement cluster). **THE DELTA IS A `+5`-RULE MEM-OPT-OPS REFRESH** over the prior pass at `0887b2ad` (slice `TXSUB2-CODEC-REALWIRE`): five new registry rules land — `DC-MEM-05`, `DC-MEM-06`, `DC-MEM-07`, `DC-MEM-08` (UTxO memory-representation / on-disk-backend / compaction invariants) and `OP-MEM-02` (owned-RSS-below-Haskell operational invariant). `DC-MEM-06` enters `partial` (allocator-neutrality mechanically gated; store-iteration-order clause still declared pending the on-disk UTxO); the other four enter `declared` (their backing clusters `MEM-OPT-UTXO-DISK` / `MEM-OPT-COMPACT` are not built — explicit gap-by-design cells). **Two additional in-span `declared → partial` flips are reflected from the registry** (they are NOT new rules): `CN-MEM-01` (bounded inbound admission, from `MEM-MEASURE-A1`) and `OP-MEM-01` (no-starvation memory transcript, from `MEM-MEASURE-A2`) — both were `declared`/gap at the `0887b2ad` baseline and flipped to `partial` on work that landed after it within this span. **`OP-MEM-02` stays `declared` as an HONEST no-flip:** all three of its enforcement cells resolve on disk, but the S3 owned measurement returned verdict `ade_heavier` (Ade active-admission owned p50 4.59 GiB > Haskell 2.57 GiB), so the gate enforces the measurement discipline, not the bound. · **This is a delta-refresh:** every rule's four-cell row was regenerated from the registry joined with on-disk introspection (`git grep` / `ls` only; no build or test execution); the registry is authoritative; every named test fn + CI script for the changed rows was verified present on disk (zero drift in the MEM rows). The 373 prior rows are carried forward verbatim except the two flips above. · **Generated:** by `/traceability` at HEAD `233644f7` · **Registry:** `docs/ade-invariant-registry.toml` @ **378 rules** (250 enforced / 1 enforced_scaffolding / 22 partial / 105 declared / 0 deprecated) · **CODEMAP:** `docs/ade-CODEMAP.md` is **current at `233644f7`** (464 canonical types · 190 CI · 378 rules) and already catalogs `DC-MEM-05/06/07/08`, `OP-MEM-02`, and the new MEM CI gates · **HEAD_DELTAS:** `docs/ade-HEAD_DELTAS.md` is **current at `233644f7`** (§0 HEAD `233644f7`; baseline `862cd2cb`; 378 rules; §5 catalogs every MEM-OPT-OPS / MEM-MEASURE CI gate referenced below).
...
The canonical source is the **invariant registry** at `docs/ade-invariant-registry.toml` (`.idd-config.json` `invariant_registry`), **378 rules** at HEAD `233644f7` (250 enforced / 1 enforced_scaffolding / 22 partial / 105 declared / 0 deprecated). TRACEABILITY is a join: every registry `[[rules]]` entry becomes one rule sub-section; **Source** / **Requirement** are lifted from the registry `source` / `statement`; **Code** / **Tests** / **CI** are populated from the registry `code_locus` / `tests` / `ci_script`(`s`) and then **verified against the working tree** (every named test function and CI script is checked for on-disk existence; misses are flagged inline as drift). The registry↔TRACEABILITY join is **378 ↔ 378** (1:1, verified by rule-heading count == registry `[[rules]]` count, family-for-family: T 33 · DC 200 · CN 119 · RO 16 · OP 10). **No MUST/SHALL candidate rules were found in the `.idd-config.json` `normative_docs`** (the gate-statement / contract docs carry no numbered normative MUST clauses; all rules live in the registry) — so there are no un-registered candidates to surface this pass.

This pass's substantive delta (`0887b2ad` → `233644f7`, cluster-close for `MEM-OPT-OPS`) — a `+5`-rule refresh:

- **`DC-MEM-05` — NEW, `declared`, `tier = derived`.** UTxO/ledger fingerprint + post-state are backend-independent (in-memory vs on-disk UTxO replay byte-identically; same WAL + checkpoint => same tail fingerprint); a memory-representation/storage change is NEVER a consensus/replay change. Backing store (`MEM-OPT-UTXO-DISK`) not yet built → **Code / Tests / CI are an explicit gap-by-design** (`_(no … — gap)_`).
- **`DC-MEM-06` — NEW, `partial`, `introduced_in = "MEM-OPT-OPS"`, `strengthened_in = ["MEM-OPT-OPS"]`, `tier = derived`.** Fingerprint computed over canonical fixed-width-BE CBOR keys (never store iteration order) AND allocator-neutral. The allocator-neutrality clause is **mechanically enforced** (`ci/ci_check_alloc_determinism_neutral.sh` asserts exactly one `#[global_allocator]` at the RED binary entry + zero allocator refs in any BLUE crate; `mimalloc` swapped in `crates/ade_node/src/main.rs`); strengthened at S2 onto the streaming seed-import path (`crates/ade_runtime/src/seed_import/importer.rs`, `ci/ci_check_mem_opt_s2_import_peak.sh`). The store-iteration-order clause stays declared pending the on-disk UTxO → `partial`. **All 5 tests + both CI scripts resolve on disk.**
- **`DC-MEM-07` — NEW, `declared`, `tier = derived`.** In-memory UTxO portion (read cache + last-k changelog) bounded by fixed closed constants; pressure cannot grow it unboundedly. Backing store not yet built → **gap-by-design cells**.
- **`DC-MEM-08` — NEW, `declared`, `tier = derived`.** Compact TxOut (canonical CBOR slice as source of truth + lazy views) preserves canonical bytes and ledger semantics. Cluster `MEM-OPT-COMPACT` not yet built → **gap-by-design cells**.
- **`OP-MEM-02` — NEW, `declared`, `tier = operational`.** Ade's owned RSS (`Private_Dirty`/`RssAnon`) stays clearly below the reference Haskell node on the same chain, without changing semantics/selection/persisted-bytes/replay. **`declared` is an HONEST no-flip, not a gap:** all three cells resolve on disk (`crates/ade_node/src/mem_measure/rss_sampler.rs`, test `owned_samplers_present_on_linux`, `ci/ci_check_mem_opt_s3_owned.sh`), but the gate enforces the **owned-measurement discipline**, not the bound — the S3 measurement returned verdict `ade_heavier` (Ade active-admission owned p50 4.59 GiB > Haskell 2.57 GiB during admission), so the bound is not met and the rule stays declared until the on-disk UTxO drives the comparison verdict to `ade_below`.
- **2 in-span `declared → partial` flips reflected from the registry (NOT new rules):** `CN-MEM-01` (bounded inbound admission, `strengthened_in += MEM-MEASURE-A1`; Code+8 tests+CI now resolve on disk) and `OP-MEM-01` (no-starvation memory transcript, `strengthened_in += MEM-MEASURE-A2`; Code+CI resolve, Tests is an explicit gap — the evidence is the committed transcript, not a unit test). Both were `declared`/gap at the `0887b2ad` baseline; the `MEM-MEASURE-A1`/`A2` commits that flipped them landed **after** that doc pass, within this span — so this refresh reconciles them to the authoritative registry.
- **`0` rules removed / deprecated / weakened.** **`0` BLUE law added by MEM-OPT-OPS** — `ade_core` 49→49, `ade_ledger` 181→181 canonical types; every MEM-OPT-OPS / MEM-MEASURE change is RED/GREEN (allocator swap, streaming import, observe-only memory-evidence seams). **`0` `RO-*` flipped.**
...
- **`_[not found on disk — drift]_`** (tests) / **`_[not on disk — drift]_`** (CI) flags a registry-named test/CI that could not be located in the working tree at `233644f7`. **This pass: exactly ONE such test-name drift, carried forward unchanged** — `DC-NODE-18` names `extend_own_spine_promotion_requires_adoption_certificate`, which still has no `fn` on disk (the adoption-certificate mechanism `DC-NODE-18` introduced was superseded by `DC-NODE-20` and the cert removed by `DC-NODE-21`; the other 5 tests in `DC-NODE-18`'s array resolve). Recorded honestly in that row, not hidden. **The `+5` MEM-OPT-OPS rows + the 2 flips added ZERO new drift** — every named test fn and CI script for `DC-MEM-06`, `OP-MEM-02`, `CN-MEM-01`, `OP-MEM-01` resolves on disk (re-verified by `git grep "fn <name>"` / `ls`). **Zero CI drift** — every CI script any rule names exists on disk (190 distinct rule-referenced gates, matching CODEMAP's 190-CI count at `233644f7`; the 5 new MEM gates `ci_check_alloc_determinism_neutral` / `ci_check_mem_opt_s2_import_peak` / `ci_check_mem_opt_s3_owned` / `ci_check_mem_measure_evidence` / `ci_check_bounded_inbound_admission` are all present and also catalogued in `HEAD_DELTAS` §5).
...
By tier: true 65 · derived 248 · operational 18 · release 41 · constraint 4 · constraint_network 2.

By family: T 33 · DC 200 · CN 119 · RO 16 · OP 10. _(registry-exact; the doc body has exactly 378 rule headings matching the registry family-for-family.)_

_Delta (`0887b2ad` → `233644f7`, cluster-close for `MEM-OPT-OPS`): `+5` rules (373 → 378). NEW: `DC-MEM-05` (declared), `DC-MEM-06` (partial), `DC-MEM-07` (declared), `DC-MEM-08` (declared), `OP-MEM-02` (declared). Status mix: `enforced` 250 → 250 (unchanged); `partial` 19 → 22 (`+3` = `DC-MEM-06` new + `CN-MEM-01` & `OP-MEM-01` flipped `declared → partial`); `declared` 103 → 105 (`+4` new declared − 2 leaving for `partial`). The two `declared → partial` flips are NOT new rules — they land on `MEM-MEASURE-A1`/`A2` work that committed after the `0887b2ad` doc pass but within this span, and are reconciled here against the authoritative registry. Registry↔TRACEABILITY join: 378 ↔ 378 (1:1). CI on disk: 185 → 190 distinct rule-referenced gates (`+5` new MEM gates). `0` rules removed / deprecated / weakened. `0` `RO-*` flipped. `0` new BLUE law (MEM-OPT-OPS is all RED/GREEN). Prior delta (`862cd2cb` → `0887b2ad`, slice `TXSUB2-CODEC-REALWIRE`): `+1` rule (372 → 373, `DC-PROTO-11`, `enforced`); `DC-PROTO-02` FLIPPED `declared → enforced` plus 9 further in-span declared→enforced flips (10 flips total per `HEAD_DELTAS` §7)._
...
_Declared at MEM-OPT scoping (2026-06-15). The load-bearing on-disk-UTxO invariant: the redb-backed UTxO (cluster `MEM-OPT-UTXO-DISK`) is a pure storage substitution behind the unchanged BLUE ledger interface, enforced by extending the `DC-WAL-03` / `replay_from_anchor` corpus to run byte-identically under both backends. Backing store not yet built → all four enforcement cells are an explicit gap by design._
...
_tier: derived · status: declared · cross_ref: CN-MEM-01, DC-MEM-05_
...
_Declared at MEM-OPT scoping (2026-06-15). Reuses the established fixed-closed-bound pattern (`MAX_SERVE_RANGE_BLOCKS` / `MAX_WIRE_PUMP_LOOKAHEAD` / the A1 admission budgets); the k-deep changelog window is bounded by the security parameter and the read cache by a fixed cap. No backing store yet → enforcement cells are an explicit gap by design._
...
_tier: operational · status: declared · cross_ref: OP-MEM-01, DC-MEM-05, DC-MEM-07_
...
| **Code** | `crates/ade_node/src/mem_measure/rss_sampler.rs` (`sample_rss_anon_kib` / `sample_private_dirty_kib`), `ci/ci_check_mem_opt_s3_owned.sh` |
| **Tests** | `owned_samplers_present_on_linux` |
| **CI** | `ci/ci_check_mem_opt_s3_owned.sh` |

_**`status = declared` is an HONEST no-flip, not a gap-by-omission.** All three enforcement cells resolve on disk (the owned-RSS sampler, its `owned_samplers_present_on_linux` test, and the `ci_check_mem_opt_s3_owned.sh` gate), but the gate enforces the **measurement discipline** (owned `RssAnon`/`Private_Dirty` sampled for both Ade and the Haskell node, paired to the A2 replay verdict), NOT the bound itself. The S3 owned measurement (2026-06-15, `docs/evidence/mem-opt-ops-s3-owned-preprod-memory.*`) returned verdict **`ade_heavier`**: Ade active-admission owned p50 4.59 GiB > Haskell windowed owned p50 2.57 GiB during admission (Ade idle/recovered owned 1.95 GiB IS below the ≤3 GB target; the import-side S1+S2 wins are real, but the dominant admission-time owned cost is the `seed_to_snapshot`/chain.db serialization). The rule therefore stays `declared` — it flips only when the committed comparison verdict becomes `ade_below` and the RSS-ceiling gate is green (driven by `MEM-OPT-UTXO-DISK`). Every lever is a representation/storage change proven replay-equivalent (`DC-MEM-05`)._
...
### `233644f7` pass (cluster-close `MEM-OPT-OPS`) — current-pass coherence
...
**This pass (`233644f7`, cluster-close `MEM-OPT-OPS`) — gap accounting:** the `+5` MEM-OPT-OPS rows split cleanly into *enforced-cells-present* and *gap-by-design*, all recorded honestly:

- **`DC-MEM-06` (NEW, `partial`)** fills Code + Tests + CI and all resolve on disk (the `ci_check_alloc_determinism_neutral.sh` gate, the 5 `streaming_*` tests in `crates/ade_runtime/src/seed_import/importer.rs`, `ci_check_mem_opt_s2_import_peak.sh`, and the `mimalloc` `#[global_allocator]` in `main.rs`). It is `partial` (not `enforced`) by design: the allocator-neutrality clause is mechanically gated but the store-iteration-order clause stays declared pending the on-disk UTxO (`MEM-OPT-UTXO-DISK`). Not a hidden gap — a scope-bounded partial.
- **`DC-MEM-05`, `DC-MEM-07`, `DC-MEM-08` (NEW, `declared`)** are explicit gap-by-design: their backing clusters (`MEM-OPT-UTXO-DISK`, `MEM-OPT-COMPACT`) are not built, so each row's Code / Tests / CI reads `_(no … — gap)_`. The gap IS the backlog item.
- **`OP-MEM-02` (NEW, `declared`) — HONEST no-flip, NOT a gap-by-omission.** All three cells resolve on disk (`rss_sampler.rs::sample_rss_anon_kib`/`sample_private_dirty_kib`, the `owned_samplers_present_on_linux` test, `ci_check_mem_opt_s3_owned.sh`), but the gate enforces the owned-measurement *discipline*, not the *bound*: the S3 measurement returned verdict `ade_heavier` (Ade active-admission owned p50 4.59 GiB > Haskell 2.57 GiB). The rule stays `declared` until the on-disk UTxO drives the committed comparison verdict to `ade_below` — surfaced as the load-bearing residual, not softened.
- **TWO `declared → partial` flips reconciled from the registry (`CN-MEM-01`, `OP-MEM-01`)** — both landed on `MEM-MEASURE-A1`/`A2` work that committed AFTER the `0887b2ad` doc pass but within this span; this refresh brings the doc rows into 1:1 agreement with the authoritative registry. `CN-MEM-01` now fills Code + 8 tests + CI (all on disk); `OP-MEM-01` fills Code + CI (on disk) with **Tests** an explicit gap (the registry names no test fn — the evidence is the committed A2 transcript validated by the CI gate, not a unit test). **The `+5` rows + 2 flips added ZERO new test-name or CI drift.**

**Prior pass (`0887b2ad`, slice `TXSUB2-CODEC-REALWIRE`) — gap accounting (carried):** the four-cell contract is filled for the new + changed rows: `DC-PROTO-11` (new, `enforced`) has Code + Tests + CI all present and disk-verified (the `ci_check_tx_submission2_real_capture.sh` gate, all 9 named tests, and the `tx_submission.rs` codec symbols resolve), and `DC-PROTO-02` (FLIPPED `declared → enforced`) has its state-machine Code row + the same CI gate, all on disk. **The headline this pass is the opposite of a gap: `DC-PROTO-02` (transcript-equivalent mini-protocol behavior with the Haskell node) FLIPPED `declared → enforced`, backed by the new `DC-PROTO-11` real-wire codec rule.** Three honest items recorded this pass, none hidden:
