...
>
> **HEAD:** working tree (uncommitted; will be committed in the PHASE4-N-F-C close) · **Generated:** 2026-05-31 · **Baseline:** `a3a0636` (`.idd-config.json` `head_deltas_baseline`)
...
The canonical source is the **invariant registry** at `docs/ade-invariant-registry.toml` (`.idd-config.json` `invariant_registry`). This doc is a join: registry entries × codebase introspection. Rule IDs, families, names, sources, and the registry-recorded `tests` / `ci_script` / `ci_scripts` / `code_locus` fields come from the registry; each test name and CI script is cross-checked for existence in the working tree (the PHASE4-N-F-C close, uncommitted), and divergences are flagged inline as **drift**. (Verification this pass is grep/ls only — no build or test execution.)
...
- **Families** are the registry ID-prefix scheme: `T` (true), `DC` (derived), `CN` (constraint-network / classification table), `RO` (release obligation), `OP` (operational). The registry `tier` field (true / derived / operational / release / constraint) is noted per rule; it does not always coincide with the ID prefix (e.g. some `CN-*` and `DC-*` entries carry `tier = "release"`).
...
- **[not found on disk — drift]** (tests) / **[not on disk — drift]** (CI) flags a registry-named test/CI that could not be located in the working tree (PHASE4-N-F-C close, uncommitted) — a registry↔code coherence gap to reconcile (rename the registry entry or restore the artifact). Test drift is resolved against the **leaf** function name (the segment after the last `::`).
...
By tier: constraint 4 · derived 185 · operational 16 · release 40 · true 61.

By family: T 29 · DC 142 · CN 110 · RO 16 · OP 9.
...
### `T-CONSERV-01`: UTxO and asset conservation must hold for every accepted transition, except where protocol rules explicitly authorize mint, burn, rewards, or treasury effects
...
_tier: true · status: enforced · strengthened: PHASE4-N-B, PHASE4-B1, PHASE4-B2, PHASE4-N-C, PHASE4-N-G, PHASE4-N-H, PHASE4-N-I, PHASE4-N-J, PHASE4-N-K, PHASE4-N-L, PHASE4-N-M-A, PHASE4-N-M-B, PHASE4-N-M-FRAG, PHASE4-N-Y_
...
### `T-INGRESS-01`: All authoritative external bytes enter the core through named canonical decode/validation chokepoints; unchecked bypasses forbidden except for explicitly whitelisted sites with CI enforcement
...
_tier: true · status: enforced · strengthened: PHASE4-N-F-A, PHASE4-N-F-C_
...
| **Tests** | `recover_from_snapshot_and_replay_forward`, `recover_from_genesis_when_no_snapshot`, `apply_failure_surfaces_with_slot`, `snapshot_decode_failure_surfaces_as_error`, `snapshot_with_no_post_blocks_is_ok`, `stress_kill_smoke`, `stress_kill_1000`, `snapshot_table_intact_after_kill_loop`, `persistent_passes_crash_safety_with_no_kill`; (N-F-C production-path) `node_sync_kill_then_warm_start_recovers_same_tip` |
| **CI** | `ci/ci_check_recovery_contract.sh`, `ci/ci_check_chaindb_crash_safety.sh` |

> **PHASE4-N-F-C strengthening.** The PRODUCTION restart path is now wired: the `--mode node` lifecycle owner (`crates/ade_node/src/node_lifecycle.rs::warm_start_recovery`) drives `replay_from_anchor → bootstrap_initial_state(RequiredFromRecoveredProvenance)` from the binary, and `node_sync_kill_then_warm_start_recovers_same_tip` proves a synced+advanced tip recovers byte-identically through it. Code/Tests/CI of the rule's own four cells are unchanged save the added cross-cluster test; the recovery contract itself is governed by the existing gates.
...
| **Tests** | `recover_from_snapshot_and_replay_forward`, `recover_from_genesis_when_no_snapshot`; (N-F-C production-path) `node_sync_kill_then_warm_start_recovers_same_tip` |
| **CI** | `ci/ci_check_recovery_contract.sh` |

> **PHASE4-N-F-C strengthening.** The `--mode node` owner replays from the persisted WAL to reconstruct the recovered surface; `node_sync_kill_then_warm_start_recovers_same_tip` proves the synced tip is fully replay-derivable through the production path. Rule cells otherwise unchanged.
...
Derived invariants — Cardano-compatibility constraints the authoritative core must satisfy (ledger rules, tx-validity, crypto vectors, epoch/HFC translation, consensus state, storage/WAL, admission, network forward-sync, genesis/Mithril provenance).
...
### `DC-ADMIT-11`: Cross-epoch silent use forbidden. If a peer sends a block whose
...
_tier: derived · status: enforced · strengthened: PHASE4-N-M-A, PHASE4-N-Y, PHASE4-N-F-A_
...
### `DC-CINPUT-01`: Warm-start verification of the recovered seed-epoch consensus inputs (authority surface + PRODUCTION restart wiring)

_tier: derived · status: enforced · introduced: PHASE4-N-F-A · strengthened: PHASE4-N-F-C_

| Aspect | Location |
|--------|----------|
| **Source** | docs/clusters/PHASE4-N-F-A/cluster.md; A3a-wal-provenance-entry.md; A3b-warm-start-restore.md |
| **Requirement** | The seed-epoch consensus-input import is a replay-reconstructable WAL fact (additive closed `WalEntry::SeedEpochConsensusInputsImported`, distinct tag, NOT in the AdmitBlock prior_fp/post_fp chain; WAL append is the commit point). Replay reconstructs exactly one typed `RecoveredBootstrapProvenance` per store/anchor (duplicate or anchor-mismatch fails closed); `bootstrap_initial_state`'s warm-start branch (`RequiredFromRecoveredProvenance`) restores the sidecar and verifies it fail-closed (present, `blake2b_256 == provenance.sidecar_hash`, A1 decode, anchor_fp + epoch_no binding, byte-identity re-encode) or halts (typed `BootstrapError`, `EXIT_AUTHORITY_FATAL_DECODE`, no bundle fallback). **PHASE4-N-F-C wired the PRODUCTION restart path**: the `--mode node` lifecycle owner (`node_lifecycle.rs::warm_start_recovery`) opens `PersistentChainDb` + `FileWalStore`, replays via `replay_from_anchor`, and calls `bootstrap_initial_state(RequiredFromRecoveredProvenance)` from the binary — the same fail-closed verify chain, now driven from the warm-start branch (`node.rs` `run_node_until_shutdown` + `recover_node_state` remain the legacy test-only surface). |
| **Code** | crates/ade_ledger/src/wal/event.rs (`SeedEpochConsensusInputsImported`, tag 3); crates/ade_ledger/src/wal/replay.rs (`ReplayOutcome` + `RecoveredBootstrapProvenance`); crates/ade_runtime/src/seed_consensus_provenance.rs (append helper); crates/ade_runtime/src/bootstrap.rs (`SeedEpochConsensusSource`, `BootstrapState`, `restore_seed_epoch_consensus_inputs`); **(N-F-C production wiring)** crates/ade_node/src/node_lifecycle.rs (`warm_start_recovery` — the binary's warm-start owner) |
| **Tests** | `wal_seed_cinput_entry_round_trips_byte_identical`, `replay_yields_bootstrap_provenance_view`, `replay_rejects_duplicate_provenance_entry`, `replay_rejects_anchor_mismatched_provenance_entry`, `admit_block_chain_unaffected_by_provenance_entry`, `warm_start_restores_seed_epoch_consensus_inputs_byte_identical`, `warm_start_fails_closed_on_missing_sidecar`, `warm_start_fails_closed_on_hash_mismatch`, `warm_start_fails_closed_on_anchor_mismatch`, `warm_start_fails_closed_on_epoch_mismatch`, `warm_start_required_provenance_rejects_malformed_sidecar`, `warm_start_never_falls_back_to_consensus_inputs_path`; **(N-F-C production path)** `warm_start_recovers_seed_epoch_consensus_inputs_byte_identical`, `warm_start_fails_closed_on_missing_wal_provenance`, `warm_start_fails_closed_on_duplicate_provenance`, `warm_start_fails_closed_on_multiple_anchor_lineages`, `warm_start_dispatch_succeeds_end_to_end`, `node_sync_kill_then_warm_start_recovers_same_tip` |
| **CI** | `ci/ci_check_lifecycle_owner_uses_bootstrap_initial_state.sh` — fences the `--mode node` lifecycle owner to obtain initial state SOLELY via `bootstrap_initial_state`, requires the `RequiredFromRecoveredProvenance` constructor in the owner, and forbids genesis/bundle/cold/`recover_node_state` fallbacks. |

> **PHASE4-N-F-C: production restart path wired (partial → enforced).** The deferred obligation from N-F-A is closed. The `--mode node` lifecycle owner (`node_lifecycle.rs::warm_start_recovery`) is the FIRST PRODUCTION caller of `replay_from_anchor → bootstrap_initial_state(RequiredFromRecoveredProvenance)`, with the new gate `ci_check_lifecycle_owner_uses_bootstrap_initial_state.sh` fencing it against any fallback. **Enforcement-surface honesty:** `node_lifecycle`/`node_sync` are tested-but-(not-yet)-binary-run-path library surfaces — the six N-F-C tests and the lifecycle-owner gate prove the wiring mechanically at the surface, not yet via a live binary run. Live BA-02 / operator-pass execution remains separate (RO-LIVE-01 partial; RO-LIVE-06 schema/correlation mechanics only).

### `DC-CINPUT-02a`: Projection equivalence — recovered seed-epoch consensus inputs project deterministically to the leadership-consumed PoolDistrView + eta0
...
| **Requirement** | The recovered `SeedEpochConsensusInputs` projects deterministically (pure BLUE field map) to the full leadership `PoolDistrView` surface (`total_active_stake`, `pool_active_stake`, `pool_vrf_keyhash`, `active_slots_coeff`; single-epoch — off-epoch queries return `None`) via `PoolDistrView::from_seed_epoch_consensus_inputs`, EQUIVALENT to the prior operator-bundle projection (`pool_distr_view_from_consensus_inputs`) for the seed epoch; and recovered eta0 (from chain_dep) drives `leader_vrf_input` identically. **This rule covers the PROJECTION only**; producer CONSUMPTION of the projected recovered surface is deferred to PHASE4-N-F-C (CE-A-4b). |
...
> **PHASE4-N-F-C strengthening.** A4 proved the PROJECTION; N-F-C's DC-CINPUT-02b binds the producer to CONSUME it (closing CE-A-4b). This rule's cells are unchanged; the consumption half is traced under DC-CINPUT-02b below.

### `DC-CINPUT-02b`: Producer consumption of the recovered surface (closes CE-A-4b) — node-lifecycle forge base built from recovered seed-epoch consensus inputs

_tier: derived · status: enforced · introduced: PHASE4-N-F-C_
...
| **CI** | `ci/ci_check_consensus_input_provenance.sh` (guard (d) — consume-side fence; shared with CN-CINPUT-03) |

> **Enforcement-surface honesty (CODEMAP-aligned).** `node_sync` is a tested-but-not-yet-binary-run-path library surface. The three tests above + guard (d) prove producer consumption of the recovered surface **mechanically at the surface**, not yet via a live binary run. No BLUE crate changed; this is GREEN/RED node wiring. CE-A-4b is closed on the **node-lifecycle forge path** only — `produce_mode` stays diagnostic (`SeedEpochConsensusSource::NotRequired`). Live BA-02 is NOT claimed (see RO-LIVE-06).

### `DC-COMPAT-01`: Cardano compatibility is proven ONLY on observable surfaces — per-block accept/reject
...
| **Requirement** | Cardano compatibility is proven ONLY on observable surfaces — per-block accept/reject |
...
### `DC-CONS-03`: Praos chain selection ordering: block number first, then Praos TiebreakerView (slot, issuer, op-cert issue number, VRF output). Density-based ordering is reserved for Genesis/catch-up and must not be used for caught-up Praos fork-choice.
...
### `DC-CONS-04`: Praos chain-dep state (evolving/candidate/epoch/previous_epoch/lab/last_epoch_block nonces, op-cert counters, last_slot) is owned by N-B consensus, not by the ledger, and evolves deterministically as a function of validated headers and epoch boundaries.
...
### `DC-CONS-05`: Authoritative rollback must never exceed the security parameter k measured in blocks (mainnet k = 2160). Rollback requests deeper than k return ExceededRollback. Forecast/stability windows may be slot-based but must accommodate at least k+1 blocks.
...
### `DC-CONS-06`: rollback(state, depth) produces state byte-identical to truncated replay from the nearest checkpoint. Rollback that would cross the immutable tip (≥ k deep) returns ForkBeforeImmutableTip and never alters state.
...
### `DC-CONS-07`: BLUE consensus must consume the HFC schedule only as a typed EraSchedule value anchored to BootstrapAnchorHash. Genesis text parsing happens in RED; BLUE never reads files, JSON, or operator config directly. The schedule is part of replay evidence.
...
### `DC-CONS-08`: slot_to_time(EraSchedule, SystemStart, SlotNo) is a pure function; no BLUE consensus path may consult the wall clock to derive a slot or UTC instant for an authoritative decision.
...
### `DC-CONS-09`: Consensus-derived queries for slots beyond the ledger-view safe zone return OutsideForecastRange, never guessed values. The bound is derived from era history + safe zone + HFC schedule, not encoded as a magic constant in caller code.
...
### `DC-CONS-10`: A header's op-cert issue counter must be >= the highest observed counter for the same (pool, kes_period). Regression yields HeaderInvalid with a typed OpCertCounterError reason; ChainDepState never accepts a regression.
...
### `DC-CONS-11`: OpCert kes_period field equals the KES period at the forged slot under an operator-supplied anchor. period_at_slot(slot, anchor) = (slot - anchor) / slots_per_kes_period (integer floor). BLUE rejects header/opcert combinations with mismatched periods at forge time. BLUE MUST NOT infer the anchor from wall-clock or filesystem state; the anchor is a pure input on the canonical ProducerTick.
...
### `DC-CONS-13`: Forge is pure given a canonical ProducerTick. forge_block has no wall-clock, no rand, no HashMap iteration, no I/O, no locale, and no ambient state. All inputs flow through the ProducerTick value (slot, ledger_state, mempool_snapshot, pparams, vrf_proof, kes_sig, opcert). Strengthens T-DET-01 for the producer authority surface.

_tier: derived · status: enforced · strengthened: PHASE4-N-H_
...
### `DC-CONS-16`: Forged header.body_hash MUST equal blake2b_256(forged_body_wire_bytes), where forged_body_wire_bytes are produced by the single Cardano-compatible canonical block-body encoder used by the validator hash path. The producer and validator hash the same bytes through the same encoder. Strengthens T-ENC-01 for the producer surface; closes any potential producer/validator encoder bifurcation.

_tier: derived · status: enforced · strengthened: PHASE4-N-G, PHASE4-N-H_
...
### `DC-CONS-17`: Block bytes delivered via producer-side block-fetch Block{bytes} are byte-identical to AcceptedBlock.as_bytes() for the AcceptedBlock that cleared self_accept. The producer-side server never re-encodes.

_tier: derived · status: enforced · strengthened: PHASE4-N-R-B, PHASE4-N-X_
...
### `DC-CONS-18`: Header bytes announced via chain-sync RollForward{header,tip} are the header sub-segment of the AcceptedBlock whose body bytes are subsequently servable via block-fetch. block_body_hash applied to the served body MUST equal the body-hash field of the announced header.

_tier: derived · status: enforced · strengthened: PHASE4-N-R-A, PHASE4-N-S-A, PHASE4-N-T, PHASE4-N-V, PHASE4-N-X_
...
_tier: derived · status: enforced · strengthened: PHASE4-N-I, PHASE4-N-J, PHASE4-N-Y_
...
### `DC-CONS-IN-02`: Canonical fingerprint: LiveConsensusInputsCanonical.fingerprint
...
### `DC-EPOCH-01`: Conway governance timing: proposals accumulate during epoch, ratification and enactment are atomic at epoch boundary, pulsing distributes DRep stake computation across epoch
...
### `DC-EPOCH-02`: Hard fork transitions triggered at deterministic slot/epoch boundaries; era translation functions mandatory; forecast horizon extends to era boundary
...
| **Requirement** | A controlled genesis enters initial state ONLY through the single closed |
...
### `DC-KES-HEADER-01`: unsigned_header_pre_image(slot, block_no, prev_hash, vrf_data, opcert, kes_period, hot_vkey, body_hash, body_size, protocol_version) is a pure BLUE function. Same canonical inputs → byte-identical UnsignedHeaderPreImage output. Replay-equivalence anchor for the pre-image recipe.
...
### `DC-LEDGER-05`: Witness binding is era-specific: Byron TxWitness, Shelley+ WitsVKey/Scripts/BootstrapWitnesses, Alonzo+ Redeemers/Datums, Conway governance witnesses
...
### `DC-LEDGER-06`: Script context (ScriptContext/TxInfo) derived from tx + ledger state + network-wide constants (EpochInfo, SystemStart); no host-environment data
...
### `DC-LEDGER-12`: Every tx in a forged block is admissible via ade_ledger::mempool::admit against the base ledger state, in the snapshot's canonical accumulating order. No tx in a forged block bypasses mempool validation. Forge MUST NOT permute, fabricate, or skip the snapshot's canonical accumulating order.
...
### `DC-MEM-03`: Tx ingress reduces to a closed IngressEvent before BLUE mempool admission; the source variant is evidence/policy/replay metadata only and MUST NOT change the validity verdict.
...
### `DC-MEM-04`: Replaying the same ordered ingress trace against the same base ledger state produces a byte-identical sequence of (MempoolState, AdmitOutcome) pairs.
...
### `DC-MITHRIL-01`: verify_mithril_binding is a pure deterministic BLUE predicate over its inputs (the
...
| **Requirement** | verify_mithril_binding is a pure deterministic BLUE predicate over its inputs (the |
...
| **Requirement** | For Mithril bootstrap, the BootstrapAnchor seed_point MUST be derived from the |
...
### `DC-OPCERT-01`: Given the same canonical envelope bytes, parse_opcert_envelope produces a byte-identical DecodedOpCertEnvelope across runs. Replay-equivalence anchor for the opcert envelope decode.
...
### `DC-OUTBOUND-FIFO-01`: The per-peer outbound channel preserves FIFO order: OutboundCommands enqueued for PeerId(p) in order O₁..Oₙ arrive at the peer's TCP socket in the same order (mpsc::Sender::send guarantees FIFO; MuxPump's session-aware encoder processes them sequentially).
...
_tier: derived · status: enforced · strengthened: PHASE4-N-W_
...
### `DC-PROTO-06`: BLUE mini-protocol transitions are pure functions of (canonical prior state, canonical input message, selected protocol version, deterministic configuration); no ambient session-glue state may alter authoritative behavior. Selected version is an explicit input, never read from RED context.
...
### `DC-PROTO-07`: Given canonical inputs (negotiated_version, peer_message_sequence, broadcast_arrival_sequence, session_event_sequence), the producer-side chain-sync / block-fetch session orchestrator emits a byte-identical sequence of outgoing mini-protocol frames across replays. The per-session reducer is a pure deterministic transition.
...
### `DC-PROTO-08`: Once chain-sync enters a state where the server holds agency, the pure per-session reducer must return exactly one of: a legal RollForward, a legal RollBackward, a legal AwaitReply, or a structured deterministic session-close/error. It must not return an ambiguous wait state unless the wait condition is an explicit replay input.
...
### `DC-SNAPSHOT-01`: ServedChainHandle::push_atomic is deterministic in its argument order: the same sequence of push_atomic(a₀), push_atomic(a₁), ..., push_atomic(aₙ) produces a byte-identical ServedChainView (fingerprint equals over BTreeMap insertion order). Replay-equivalence anchor for the broadcast → serve path.
...
_tier: derived · status: enforced · strengthened: PHASE4-N-Y_
...
### `DC-STORE-04`: ChainDB structure: ImmutableDB (append-only, blocks immutable when k-deep), VolatileDB (recent blocks within k), LedgerDB (snapshots + forward replay)
...
### `DC-SYNC-01`: During network forward-sync, a block's preserved wire bytes and its WAL entry MUST

_tier: derived · status: enforced · introduced: PHASE4-N-Y · strengthened: PHASE4-N-F-C_

| Aspect | Location |
|--------|----------|
| **Source** | docs/clusters/PHASE4-N-Y/S2-network-forward-sync.md; S6-torn-write-recovery-reconciliation.md |
| **Requirement** | During network forward-sync, a block's preserved wire bytes and its WAL entry MUST |
| **Code** | crates/ade_runtime/src/forward_sync/{reducer,pump}.rs; crates/ade_runtime/src/recovery/restart.rs (WAL-tail reconciliation); **(N-F-C production driver)** crates/ade_node/src/node_sync.rs (`run_node_sync` — first production caller of `forward_sync::pump_block`) |
| **Tests** | `forward_sync_wal_and_bytes_precede_tip_advance`, `forward_sync_replay_two_runs_byte_identical`, `forward_sync_admission_through_chokepoints`, `recovery_torn_put_block_before_wal_append_drops_orphan`; **(N-F-C)** `node_sync_pump_advances_recoverable_tip`, `node_sync_fails_closed_on_undecodable_block`, `node_sync_kill_then_warm_start_recovers_same_tip` |
| **CI** | `ci/ci_check_forward_sync_chokepoint_only.sh`, `ci/ci_check_node_sync_via_pump.sh` |

> **PHASE4-N-F-C strengthening.** The `--mode node` lifecycle is the FIRST PRODUCTION driver of `forward_sync::pump_block` — `run_node_sync` feeds peer-fetched block bytes (decoupled from the admission verdict loop) into `pump_block`, advancing the recoverable tip durable-before-tip and failing closed on an undecodable block. New gate `ci_check_node_sync_via_pump.sh` isolates the `run_node_sync` body and fences it positive on `pump_block(` while forbidding `follow`/`derive_verdict`/manual tip-advance/`run_real_forge` — so peer tip-agreement (`ade_core_interop::follow`) can never masquerade as validating sync.

### `DC-TXV-01`: tx_validity is a pure function of (LedgerState, tx_cbor). No wall-clock, arrival order, HashMap/HashSet iteration, float, or ambient state may influence a transaction's Valid/Invalid verdict.
...
### `DC-TXV-02`: A transaction is Valid iff both phase-1 (structural + UTxO rules + witnesses) and phase-2 (Plutus, when scripts are present) accept it. No path may produce a Valid verdict while skipping either phase.
...
### `DC-TXV-04`: A Valid transaction yields an applied LedgerState' (the mempool's accumulating view); an Invalid transaction leaves the input state unchanged plus a structured reason. No partial or in-place mutation occurs on the invalid path.
...
### `DC-TXV-07`: All deposit/refund amounts used by Conway transaction value-conservation accounting must be sourced from canonical ledger protocol parameters or explicit certificate fields, never from testkit defaults, shell configuration, ambient ConwayGovParams, or fallback constants.
...
### `DC-VAL-01`: A block's validity verdict is a pure function of (LedgerState, PraosChainDepState, EraSchedule, LedgerView, block_cbor). No wall-clock, arrival order, HashMap/HashSet iteration, float, or ambient state may influence it.
...
### `DC-VAL-02`: A block is Valid iff both the consensus header authority (validate_and_apply_header) and the ledger body authority (apply_block_with_verdicts) accept it. No path may produce a Valid verdict while skipping either authority.
...
### `DC-VAL-03`: The header is validated before the body; body validation never runs on a header-invalid block. The first failing authority determines the reason (fail-fast ordering).
...
### `DC-VAL-04`: Ade's Valid/Invalid verdict for a block equals the reference cardano-node verdict, including the reason class where the reference exposes it. Established over both a positive corpus (real valid blocks) and a mandatory adversarial corpus (blocks the reference rejects).
...
### `DC-VAL-05`: A Valid block yields evolved (LedgerState', PraosChainDepState'); an Invalid block yields the unchanged input states plus a structured reason. No partial or in-place mutation occurs on the invalid path.
...
### `DC-VAL-06`: Every crypto-input, field-size, and structural check on the authority path rejects (produces Invalid) on wrong size or shape and never silently skips. The pattern `if X.len() == K { check } else { skip }` is forbidden in BLUE validation; size checks go through a helper that returns an error. No defined-but-unwired check and no tautological (value-compared-to-itself) guard may stand in for a real check.
...
### `DC-VIEW-01`: LiveLedgerView determinism + epoch-window guard. The view is
...
_tier: derived · status: enforced · strengthened: PHASE4-N-M-A, PHASE4-N-Y_
...
_tier: derived · status: enforced · strengthened: PHASE4-N-M-A, PHASE4-N-M-B, PHASE4-N-Y, PHASE4-N-F-C_
...
| **Tests** | `crates/ade_runtime/tests/wal_replay_from_anchor.rs::wal_replay_from_anchor_two_runs_byte_identical`, `crates/ade_runtime/tests/wal_replay_from_anchor.rs::wal_replay_from_anchor_post_fp_matches_wal_tail`, `crates/ade_runtime/tests/wal_replay_from_anchor.rs::wal_replay_from_anchor_persists_across_reopen`, `crates/ade_ledger/src/wal/replay.rs::tests::replay_from_anchor_three_entry_chain_ok`, `crates/ade_ledger/src/wal/replay.rs::tests::replay_from_anchor_two_runs_byte_identical`; **(N-F-C cross-cluster)** `node_sync_kill_then_warm_start_recovers_same_tip` (production `replay_from_anchor` caller) |
| **CI** | _(no CI script — gap)_ |

> **PHASE4-N-F-C strengthening.** The `--mode node` lifecycle owner is the FIRST PRODUCTION caller of `replay_from_anchor` — `warm_start_recovery` replays (BootstrapAnchor + WAL) at restart, and `node_sync_kill_then_warm_start_recovers_same_tip` proves a peer-synced + `pump_block`-advanced tip is recovered byte-identically across a kill boundary. Rule's own four cells otherwise unchanged.

## CN — Classification-Table Constraint-Network Invariants (`classification_table.md`)

Constraint-network invariants from the classification table — adversarial-rationale-backed constraints on the wire, ledger, consensus, storage, session, producer, and bootstrap-provenance surfaces.
...
_tier: release · status: enforced · strengthened: PHASE4-N-M-A, PHASE4-N-Y, PHASE4-N-F-A_
...
### `CN-CINPUT-01`: SeedEpochConsensusInputs closed type + SOLE version-gated byte-canonical codec

_tier: constraint · status: enforced · introduced: PHASE4-N-F-A_

| Aspect | Location |
|--------|----------|
| **Source** | docs/clusters/PHASE4-N-F-A/cluster.md; A1-seed-epoch-consensus-inputs-type.md |
| **Requirement** | The seed-epoch consensus inputs (epoch, active-slots coefficient, total active stake, per-pool active-stake + registered VRF keyhash distribution) persisted as Ade recovered state MUST be a single closed canonical type (`SeedEpochConsensusInputs`) with a SOLE encoder/decoder pair: deterministic CBOR, BTreeMap-ordered (no HashMap), version-gated (decode rejects any version `!= SEED_CINPUT_SCHEMA_VERSION` fail-closed), and byte-canonical (a structurally valid but non-canonically-encoded buffer is rejected, never silently re-canonicalized). No second encoder/decoder for this type may exist. |
...
| **CI** | `ci/ci_check_consensus_input_provenance.sh` — fails the build if any production (test-stripped) site outside `{genesis_bootstrap.rs, mithril_bootstrap.rs, seed_consensus_provenance.rs}` calls `.put_seed_epoch_consensus_inputs(` / `merge_seed_epoch_consensus_inputs(` / `append_seed_epoch_provenance(`, or if `produce_mode.rs` names any sidecar build/put/encode token. |

> **Open obligation CLOSED by PHASE4-N-F-C.** Populate-side + forge-time fence enforced here (N-F-A A2). The CONSUME-side fence and producer consumption of the recovered surface (**CE-A-4b**) landed in PHASE4-N-F-C on the **node-lifecycle forge path** (not `produce_mode`, which stays diagnostic): the new **CN-CINPUT-03** fences the node-sync/forge driver against constructing a `SeedEpochConsensusInputs` literal or naming the forge-time bundle tokens (guard (d) of the same `ci_check_consensus_input_provenance.sh` gate), and the new **DC-CINPUT-02b** binds `forge_one_from_recovered` to project leadership only from the recovered surface. `produce_mode` continues to pass only `SeedEpochConsensusSource::NotRequired`. (Live BA-02 is still NOT claimed — see RO-LIVE-06.)

### `CN-CINPUT-03`: Consume-side anti-laundering fence — node-lifecycle forge leadership view projected only from recovered surface

_tier: constraint · status: enforced · introduced: PHASE4-N-F-C_

| Aspect | Location |
|--------|----------|
| **Source** | docs/clusters/PHASE4-N-F-C/cluster.md; L5-produce-from-recovered-state.md |
| **Requirement** | Consume-side anti-laundering fence: on the node-lifecycle forge path the leadership view MUST be projected from the recovered `SeedEpochConsensusInputs` surface (`PoolDistrView::from_seed_epoch_consensus_inputs` over the recovered `BootstrapState`) and MUST NOT be built from a forge-time operator bundle. No `SeedEpochConsensusInputs` value may be CONSTRUCTED on the production forge path (no shape-swap of an operator bundle into the recovered-surface type), and the forge-time consensus-input tokens (`import_live_consensus_inputs` / `pool_distr_view_from_consensus_inputs` / `--consensus-inputs-path`) MUST NOT appear in the node-sync/forge driver. Enforced by a data-flow-resistant containment gate (guard (d): positive recovered-projection grep + negative bundle/cold-token grep + no-literal-construction fence over the comment/test-stripped `run_node_sync`/forge body), not a bypassable RHS grep. |
| **Code** | crates/ade_node/src/node_sync.rs (`forge_one_from_recovered` — projects leadership only from the recovered `BootstrapState`) |
| **Tests** | `forge_from_recovered_uses_recovered_pool_distr`, `forge_from_recovered_fails_closed_without_recovered_inputs` |
| **CI** | `ci/ci_check_consensus_input_provenance.sh` — guard (d) consume-side fence: fails the build if the node-sync/forge body constructs a `SeedEpochConsensusInputs` literal or names a forge-time bundle token (`import_live_consensus_inputs` / `pool_distr_view_from_consensus_inputs` / `--consensus-inputs-path`), or if it does not project from `from_seed_epoch_consensus_inputs`. |

> **Attack rationale.** If the node-lifecycle forge path could construct (or be fed) a `SeedEpochConsensusInputs`-shaped object from a forge-time operator bundle, the "forge from recovered Ade state" guarantee would collapse into a relabeled operator input at the moment of block production — the laundering class CN-CINPUT-02 fences on the populate side, closed here on the consume side. **Enforcement-surface honesty:** `node_sync` is a tested-but-not-yet-binary-run-path library surface; guard (d) is a data-flow-resistant containment gate proving the fence at the surface, not via a live binary run.
...
### `CN-FORGE-01`: The producer-mode forge handler is a closed transition from CoordinatorEvent::RequestForge { slot, kes_period, ledger_snapshot_ref, chain_tip } to exactly one of three ForgeResult variants: ForgeSucceeded { slot, artifact } where artifact.bytes decodes via Ade's BLUE block decoder AND self_accept(artifact, chain_tip, ledger_snapshot) returns Accepted; ForgeNotLeader { slot, vrf_output_fingerprint }; or ForgeFailed { slot, structured_error }. No other outcome is permitted. ForgeSucceeded MUST NOT be emitted if self_accept rejects the artifact — the handler emits ForgeFailed { SelfAcceptRejected } instead. Empty-block forging is the explicit scope; mempool integration is out of scope for the rule's enforcement evidence.

_tier: derived · status: enforced · strengthened: PHASE4-N-S-A, PHASE4-N-T, PHASE4-N-W_
...
### `CN-FORGE-04`: Producer-side Praos VRF construction must match the Conway/Praos validator authority: the leader VRF proof alpha, the leader-schedule evidence, the LeaderScheduleAnswer.expected_vrf_input contract, and the self_accept header verification must all use ONE era-correct Praos construction. For Conway/Praos the producer alpha MUST equal the validator alpha (praos_vrf_input(slot, eta0) = blake2b256(slot||eta0) + vrfLeaderValue range-extension), NOT the TPraos role-tagged alpha (slot||eta0||0x4C). No verification/construction fallback may accept both TPraos and Praos VRF inputs — for a given era/protocol version there is exactly one VRF transcript authority.
...
### `CN-LEDGER-05`: Each feature must have one semantic processing result; no alternate path may disagree on whether work was already applied, failed, or remains valid
...
### `CN-LEDGER-07`: UTxO and asset conservation must hold for every accepted transition, except where protocol rules explicitly authorize mint, burn, rewards, or treasury effects
...
### `CN-MITHRIL-01`: A Mithril-sourced seed may bootstrap only after a verified binding: the Mithril

_tier: constraint · status: enforced · strengthened: PHASE4-N-Z_

| Aspect | Location |
|--------|----------|
| **Source** | docs/clusters/PHASE4-N-Y/S1-mithril-import-authority.md; S7-real-mithril-binding.md |
| **Requirement** | A Mithril-sourced seed may bootstrap only after a verified binding: the Mithril |
...
_tier: release · status: enforced · strengthened: PHASE4-N-K, PHASE4-N-M-B, PHASE4-N-T, PHASE4-N-Y, PHASE4-N-F-A, PHASE4-N-F-C_
...
| **CI** | `ci/ci_check_bootstrap_closure.sh`, `ci/ci_check_produce_mode_uses_bootstrap_initial_state.sh`, `ci/ci_check_node_mode_closure.sh` (N-F-C: now pins the closed 5-variant Mode set `{WireOnly, Admission, KeyGenKes, Produce, Node}`, no wildcard arm), `ci/ci_check_lifecycle_owner_uses_bootstrap_initial_state.sh` (N-F-C: the `--mode node` lifecycle owner obtains initial state SOLELY via `bootstrap_initial_state`) |

> **PHASE4-N-F-C strengthening.** The new `--mode node` lifecycle owner is the single owner threading first-run-vs-warm-start (a pure function of on-disk state) → `bootstrap_initial_state` → produce, with NO second bootstrap/recovery/storage-init authority. `ci_check_node_mode_closure.sh` now pins the closed 5-variant `Mode` set; the new gate `ci_check_lifecycle_owner_uses_bootstrap_initial_state.sh` fences the owner against genesis/bundle/cold/`recover_node_state` fallbacks.

### `CN-OPCERT-01`: The opcert envelope parser accepts a real cardano-cli `node.opcert` text envelope (closed type check `NodeOperationalCertificate` + CBOR array(2) shape locked by N-R-A A1 OQ4 fixtures against cardano-cli 11.0.0.0 / cardano-node 11.0.1). Element 0 is an array(4) of [hot_vkey(bytes(32)), sequence_number(uint), kes_period(uint), sigma(bytes(64))] mapping to the canonical `OperationalCert`. Element 1 is bytes(32) cold_vk. Any shape mismatch (wrong type field, malformed cborHex, wrong outer/inner arity, wrong field types or lengths) fail-closes with a structured `OpCertParseError` variant. No `String` payloads in load-bearing error variants.
...
### `CN-OPS-03`: Incident evidence must be sufficient to reconstruct the canonical decision path without relying on nondeterministic logs or local operator memory
...
_tier: derived · status: enforced · strengthened: PHASE4-N-R-B, PHASE4-N-S-B_
...
### `CN-PROTO-06`: The producer-side session orchestrator can only construct outgoing mini-protocol messages tagged with Server agency. Client-originated messages from the server-role pump are unrepresentable in the public API; misuse is a compile error (closed ServerReply<M> wrapper).
...
### `CN-REL-01`: A release is not mainnet-eligible unless mixed-version topologies against supported predecessors show consensus equivalence on malformed and boundary-case inputs
...
_tier: release · status: enforced · strengthened: PHASE4-N-M-A, PHASE4-N-M-A1.1, PHASE4-N-Y_
...
### `CN-SNAPSHOT-01`: A forged block becomes visible to peers only AFTER ServedChainHandle::push_atomic succeeds. The push_atomic call covers the full served_chain_admit call inside a watch::Sender::send_modify closure — no observer can read a torn snapshot mid-insertion. Coordinator emits BroadcastBlock; RED effect handler calls push_atomic; only on Ok(ServedTip) may per-peer reducers serve the block. Fail-closed shutdown on PushError.

_tier: derived · status: declared · strengthened: PHASE4-N-T_
...
_tier: true · status: partial · strengthened: PHASE4-N-F-C_

| Aspect | Location |
|--------|----------|
| **Source** | classification_table.md §I |
| **Requirement** | WAL entries, checkpoints, and recovered artifacts must be bound to exactly one anchor or bootstrap lineage. **PHASE4-N-F-C (L3) gives the first real enforcement of the RECOVERED-ARTIFACT clause only** — the `--mode node` warm-start discovers the seed-epoch anchor lineage and FAILS CLOSED on more than one anchor lineage, on a provenance-anchor mismatch, or on duplicate provenance. The **WAL-entry + checkpoint binding clauses are not yet comprehensively gated** by a dedicated CI check; only the recovered-artifact anchor-lineage side is mechanically proven here. |
| **Code** | crates/ade_node/src/node_lifecycle.rs (`warm_start_recovery`: anchor-lineage discovery + fail-closed on multiple/mismatched anchors) — recovered-artifact clause only |
...
| **CI** | _(no dedicated CI gate — gap; the WAL-entry + checkpoint binding clauses are unenforced. The recovered-artifact clause is exercised by the N-F-C lifecycle tests above but has no rule-bound gate.)_ |

> **PARTIAL, not enforced (honest gap).** N-F-C closes only the recovered-artifact anchor-lineage clause; the WAL-entry and checkpoint binding clauses remain ungated. Promote to `enforced` only when a dedicated CI check covers all three clauses.
...
### `CN-TEST-01`: Consensus-relevant inputs must be fuzzed differentially across all supported versions and decode or validation paths; any verdict mismatch is release-blocking
...
### `CN-TEST-02`: Every malformed or discrepant input that ever triggered a fork, preview mismatch, or parser disagreement becomes a permanent regression corpus entry
...
### `RO-LIVE-06`: BA-02 peer-acceptance evidence closure (SCHEMA + CORRELATION MECHANICS ONLY — live BA-02 NOT claimed)

_tier: release · status: enforced (for schema/correlation mechanics only) · introduced: PHASE4-N-F-C_
...
| **Code** | crates/ade_node/src/ba02_evidence.rs (`Ba02Manifest`, `BA02Outcome`, `PeerAcceptEvent`, `NoEvidenceReason`, `parse_peer_accept_events`, `correlate`) |
| **Tests** | `ba02_manifest_schema_round_trips`, `ba02_correlate_served_block_yields_manifest`, `ba02_correlate_chain_tip_only_yields_manifest`, `ba02_correlate_both_signals_agree_records_served_primary`, `ba02_correlate_served_block_without_slot_yields_manifest`, `ba02_correlate_conflicting_signals_is_no_evidence`, `ba02_correlate_wrong_hash_is_no_evidence`, `ba02_correlate_chain_point_mismatch_is_no_evidence`, `ba02_correlate_no_slot_wrong_hash_is_no_evidence`, `ba02_correlate_stale_log_is_no_evidence`, `ba02_correlate_empty_peer_log_is_no_evidence`, `ba02_self_accept_is_not_evidence`, `ba02_forge_succeeded_alone_is_not_evidence`, `ba02_block_received_alone_is_not_evidence`, `ba02_agreement_verdict_is_not_evidence`, `ba02_parser_accepts_well_formed_served_block`, `ba02_parser_drops_unknown_and_malformed_lines`, `ba02_correlate_two_runs_byte_identical` (18 `ba02_*` tests — all resolve on disk) |
| **CI** | `ci/ci_check_ba02_evidence_closed.sh` — fails the build if `correlate` is not the sole `Ba02Manifest` constructor, if the parser is not allow-list only, or if the hash-primary correlation discipline is violated. |

> **MECHANICS-ONLY — live BA-02 IS NOT CLAIMED.** This rule is `enforced` strictly for the manifest **schema + correlation mechanics**. A real BA-02 result requires (1) an Ade-forged block hash, (2) real Haskell-peer accept evidence (a served-block or chain-tip signal from a genuine cardano-node peer) naming that exact hash, and (3) a reviewed evidence manifest produced by `correlate` over that captured log. Synthetic fixtures prove the mechanics only and CANNOT satisfy BA-02. The live capture is **operator-gated and distinct from RO-LIVE-01** (which remains `partial` / operator-gated). **Enforcement-surface honesty:** `ba02_evidence` is a tested-but-not-yet-binary-run-path library surface — the gate + 18 tests bind the closed surface mechanically, not via a live binary run.

### `RO-MITHRIL-IMPORT-01`: Ade imports a Mithril-authenticated snapshot as an alternative to

_tier: release · status: partial · strengthened: PHASE4-N-Y, PHASE4-N-Z_
...
### `RO-SYNC-EVIDENCE-01`: A committed snapshot->tip sync-evidence manifest carries the closed schema (oracle
...
|--------|----------|
| **Source** | docs/clusters/PHASE4-N-Y/S5-compatibility-evidence.md |
| **Requirement** | A committed snapshot->tip sync-evidence manifest carries the closed schema (oracle |
...
### `OP-OPS-05`: Slot-deadline forging SLA. Forge + self-accept + N2N hand-off must complete within the slot's deadline (1s on mainnet, smaller on testnets). Operational, not constitutional: missing the deadline costs a slot but does not violate a hash-critical invariant.
...
### CODEMAP (`docs/ade-CODEMAP.md` @ working tree, PHASE4-N-F-C close)

CODEMAP was regenerated for the PHASE4-N-F-C close (working tree, uncommitted; baseline `a3a0636`). It records **108** `ci_check_*.sh` gates (105 at the N-F-A close + 3 new: `ci_check_ba02_evidence_closed.sh`, `ci_check_node_sync_via_pump.sh`, `ci_check_lifecycle_owner_uses_bootstrap_initial_state.sh`). Every crate named in a **Code** row is present in CODEMAP: `ade_codec`, `ade_core`, `ade_core_interop`, `ade_crypto`, `ade_ledger`, `ade_network`, `ade_node`, `ade_plutus`, `ade_runtime`, `ade_testkit`, `ade_types`. No crate-level mismatch.

- **No BLUE crate changed this cluster.** PHASE4-N-F-C is GREEN/RED node-lifecycle wiring. The new code loci `crates/ade_node/src/node_sync.rs` and `crates/ade_node/src/ba02_evidence.rs` are in the already-mapped `ade_node` crate. CODEMAP notes these as **tested-but-(not-yet)-binary-run-path library surfaces**; this doc traces RO-LIVE-06, DC-CINPUT-02b, and CN-CINPUT-03 the same way — enforcement is **mechanical at the surface** (the named tests + the per-rule gate), **not via the binary's run path**. This is stated in each of those rule blocks so the distinction is not lost.
- **CI count is aligned:** CODEMAP's 108 matches the on-disk `ci/ci_check_*.sh` count at this working tree.

### HEAD_DELTAS (`docs/ade-HEAD_DELTAS.md` §5)

HEAD_DELTAS' baseline is `a3a0636` (the PHASE4-N-F-A close) and was last narrated one cluster behind this close. The three new PHASE4-N-F-C CI gates are **rule-bound** but **not yet catalogued in HEAD_DELTAS §5** until its baseline is bumped to the N-F-C close commit:

- `ci_check_consensus_input_provenance.sh` — now bound to **CN-CINPUT-02, CN-CINPUT-03, DC-CINPUT-02b** (guard (d) consume-side fence is the N-F-C addition). Its prior N-F-A binding (CN-CINPUT-02) carried into HEAD_DELTAS' reconcile-list at the last close.
- `ci_check_node_sync_via_pump.sh` (new) — DC-SYNC-01.
- `ci_check_lifecycle_owner_uses_bootstrap_initial_state.sh` (new) — CN-NODE-01, DC-CINPUT-01.
- `ci_check_ba02_evidence_closed.sh` (new) — RO-LIVE-06.
- `ci_check_node_mode_closure.sh` (extended to the closed 5-variant `Mode` set) — CN-NODE-01.

Reconcile on the next HEAD_DELTAS regeneration (bump baseline to the N-F-C close commit). No HEAD_DELTAS §5 gate is referenced by zero rules across this delta.

**CI gates on disk not bound to any rule CI row** (surfaced, not fixed this pass): `ci_check_module_headers.sh`, `ci_check_no_secrets.sh`, `ci_check_pallas_quarantine.sh` (pre-existing, project-wide hygiene gates with no single covering rule), plus `ci_check_registry_code_locus_exists.sh` (the deliberate registry-integrity meta-gate — it validates that every `code_locus` path exists on disk, so it cannot be attributed to a single invariant; correctly absent from any rule's **CI** row). **104 of 108** on-disk `ci_check_*.sh` gates are rule-bound; the 4 unbound are these (3 hygiene + 1 meta-gate).

### SEAMS

Closed registries / closed grammars referenced by rule sources (mini-protocol ID registry, closed message taxonomies, closed log/event vocabularies, `LeaderCheckVerdict`, `ExpectedVrfInput`, `Mode`, `IngressSource`, the PHASE4-N-Y closed sets — `SyncEffect` with the private `AdmitPlan`, `MithrilImportError`, `GenesisSourceError`, the PHASE4-N-Z `MithrilBootstrapError`) are catalogued in `docs/ade-SEAMS.md` §3. PHASE4-N-F-C adds the closed BA-02 evidence surface — `Ba02Manifest` / `BA02Outcome` / `PeerAcceptEvent` / `NoEvidenceReason` with `correlate` as the SOLE constructor and an allow-list-only log parser (RO-LIVE-06) — and extends the closed `Mode` set to its fifth variant (`Node`). No new extension point: the recovered-surface forge path consumes a closed type, it does not open a plugin seam.

---

## Honest enforcement gaps (standing)

The four-cell contract is filled for every `enforced` rule (Code + Tests + CI all present and verified in the working tree via grep/ls — no build/test was run this pass). The standing gaps below are recorded rather than hidden.

- **`declared` rules (112) are open enforcement gaps by design.** Each carries `_(no enforcing code located — gap)_` / `_(no tests named in registry — gap)_` / `_(no CI script — gap)_` cells. These are named-but-not-yet-mechanically-enforced invariants; the gap is the actionable backlog, not a defect to paper over.

- **CN-STORE-02 — `declared` → `partial` (first real anchor-lineage enforcement; WAL/checkpoint clauses still ungated).** PHASE4-N-F-C (L3) gives the first mechanical enforcement of the **recovered-artifact** clause only: `warm_start_recovery` discovers the seed-epoch anchor lineage and fails closed on multiple lineages, an anchor mismatch, or duplicate provenance (3 lifecycle tests). The rule's **WAL-entry + checkpoint binding clauses remain unenforced** — there is **no dedicated CI gate** for any of CN-STORE-02's three clauses, and the recovered-artifact side is exercised only by the N-F-C lifecycle tests. Traced as `partial`, not `enforced`. Promote only when a dedicated CI check covers all three clauses.

- **RO-LIVE-06 — `enforced` for SCHEMA + CORRELATION MECHANICS ONLY (live BA-02 NOT claimed).** The `Ba02Manifest` schema + the pure hash-primary `correlate` + the allow-list-only parser are enforced by `ci_check_ba02_evidence_closed.sh` + 18 `ba02_*` tests (all resolve on disk). This is **NOT** a claim that BA-02 was achieved live — a real BA-02 result requires an Ade-forged block hash, real Haskell-peer accept evidence naming that exact hash, and a reviewed `correlate` manifest over the captured log. Synthetic fixtures prove the mechanics only. The live capture is **operator-gated and distinct from RO-LIVE-01** (which remains `partial` / operator-gated).

- **RO-LIVE-01 — remains `partial` / operator-gated (unchanged this close).** The producer-side block-fetch live leg against a real Haskell peer is still an operator-action pass not yet executed. N-F-C does not touch RO-LIVE-01; it is cross-referenced by RO-LIVE-06 only to make the boundary explicit (evidence-manifest mechanics ≠ live acceptance).

- **DC-CINPUT-01 — `partial` → `enforced` (production restart path wired by N-F-C).** The N-F-A deferred obligation is closed: the `--mode node` lifecycle owner (`node_lifecycle.rs::warm_start_recovery`) is the first PRODUCTION caller of `replay_from_anchor → bootstrap_initial_state(RequiredFromRecoveredProvenance)`, now CI-fenced by `ci_check_lifecycle_owner_uses_bootstrap_initial_state.sh`. **Surface honesty:** the production wiring is proven mechanically at the `node_lifecycle`/`node_sync` library surface (6 N-F-C tests + the lifecycle-owner gate), not via a live binary run.

- **CE-A-4b (producer consumption of the recovered surface) — CLOSED by N-F-C on the node-lifecycle forge path.** DC-CINPUT-02b binds `forge_one_from_recovered` to project leadership only from the recovered `SeedEpochConsensusInputs`, and CN-CINPUT-03 fences that path against constructing the type or naming forge-time bundle tokens (guard (d) of `ci_check_consensus_input_provenance.sh`). `produce_mode` stays diagnostic (`SeedEpochConsensusSource::NotRequired`). CE-A-4b is closed on the **node-lifecycle** path; it is **not** a claim of live BA-02 (RO-LIVE-06 is mechanics-only).

- **RO-SYNC-EVIDENCE-01 — `partial` (vacuous-until-committed).** The schema + sha256 gate (`ci_check_sync_evidence_manifest_schema.sh`) is enforced but **vacuously satisfied** until an operator commits a manifest. The two-Haskell-node snapshot→tip live capture is **operator-witnessed**: `blocked_until_operator_pass_executed` (CE-Y-16). Unchanged this close.

- **CE-Y-16 (operator-witnessed live leg).** The snapshot→tip differential sync against two real Haskell nodes is the live evidence backing DC-COMPAT-01 and RO-SYNC-EVIDENCE-01. DC-COMPAT-01 is `enforced` for the *mechanism*, but the *witnessed live capture* is an operator pass not yet executed — the same class as the RO-LIVE-01 / RO-LIVE-06 operator-gated legs. Unchanged this close.

- **Governance note — CE-not-law (mechanism vs. authority), N-F-C additions.** PHASE4-N-F-C's node-lifecycle wiring is enforced by **CI text/containment gates over library surfaces**, not by type-level impossibility or a live binary run:
  - `ci_check_lifecycle_owner_uses_bootstrap_initial_state.sh` (CN-NODE-01 / DC-CINPUT-01) and `ci_check_node_sync_via_pump.sh` (DC-SYNC-01) are grep-class structural gates over the `--mode node` owner / `run_node_sync` bodies. Breadth across future call sites is the gates' responsibility, not a type-level guarantee.
  - `ci_check_consensus_input_provenance.sh` guard (d) (CN-CINPUT-03 / DC-CINPUT-02b) is a data-flow-resistant containment gate (positive recovered-projection grep + negative bundle/cold-token grep + no-literal-construction fence over the comment/test-stripped forge body).
  - `ci_check_ba02_evidence_closed.sh` (RO-LIVE-06) binds a closed schema + a sole-constructor correlator; it proves the **mechanics**, not a live BA-02.
  - **PHASE4-N-F-C proves the Ade node lifecycle mechanics through evidence closure. It does not claim live BA-02. RO-LIVE-01 remains partial/operator-gated. RO-LIVE-06 is only schema/correlation mechanics.**

  None of these are regressions — they are the honest boundary between *mechanism enforced now* and *authority claimed*, recorded so the next cluster can decide whether to promote any from CE-gate to true-tier law (e.g. CN-STORE-02's two ungated clauses, or a live RO-LIVE-06 capture).

---

## Generation notes

INCREMENTAL refresh for the **PHASE4-N-F-C cluster-close** (working tree, uncommitted; the 3 new registry rules + this doc are committed together in the close commit). Primary source: the working-tree invariant registry (`docs/ade-invariant-registry.toml`, **306 rules**). Tests and CI scripts for the N-F-C delta were cross-checked against the working tree using **grep/ls only** — no `cargo build`/`cargo test`/`cargo check` was run. Rows for rules this cluster did not touch were preserved verbatim from the N-F-A pass; only the N-F-C delta (3 new rules + 9 strengthened/re-statused rows) was re-verified and rewritten.

Baseline for this incremental pass: **`a3a0636`** (the PHASE4-N-F-A close). Delta: **+3 rules** (303 → 306):

- **CN-CINPUT-03** (`enforced`, `constraint`) — consume-side anti-laundering fence on the node-lifecycle forge path. Code `crates/ade_node/src/node_sync.rs` (`forge_one_from_recovered`); CI `ci/ci_check_consensus_input_provenance.sh` guard (d); 2 tests (`forge_from_recovered_uses_recovered_pool_distr`, `forge_from_recovered_fails_closed_without_recovered_inputs`) — both resolve on disk.
- **DC-CINPUT-02b** (`enforced`, `derived`) — producer consumption of the recovered surface (closes CE-A-4b). Code `crates/ade_node/src/node_sync.rs`; CI `ci/ci_check_consensus_input_provenance.sh`; 3 tests (the 2 above + `forge_from_recovered_is_deterministic_across_two_runs`) — all resolve on disk.
- **RO-LIVE-06** (`enforced` for SCHEMA/CORRELATION MECHANICS ONLY, `release`) — BA-02 evidence-manifest closure. Code `crates/ade_node/src/ba02_evidence.rs`; CI `ci/ci_check_ba02_evidence_closed.sh`; 18 `ba02_*` tests — all 18 resolve on disk and match the registry list exactly. `open_obligation` states live BA-02 is NOT claimed (operator-gated; distinct from RO-LIVE-01).

**9 strengthenings** (`strengthened_in += PHASE4-N-F-C`):

- **DC-CINPUT-01** — `partial` → **`enforced`** (production warm-start wired). Added tests incl. `warm_start_recovers_seed_epoch_consensus_inputs_byte_identical`, `node_sync_kill_then_warm_start_recovers_same_tip` (all resolve); new CI `ci/ci_check_lifecycle_owner_uses_bootstrap_initial_state.sh`.
- **CN-NODE-01** — `enforced` (unchanged status); +2 gates: `ci_check_node_mode_closure.sh` now pins the closed 5-variant `Mode` set, and the new `ci_check_lifecycle_owner_uses_bootstrap_initial_state.sh`.
- **DC-SYNC-01** — `enforced`; +gate `ci/ci_check_node_sync_via_pump.sh`; +3 N-F-C tests (`node_sync_pump_advances_recoverable_tip`, `node_sync_fails_closed_on_undecodable_block`, `node_sync_kill_then_warm_start_recovers_same_tip`).
- **DC-FORGE-01** — `enforced`; +test `forge_from_recovered_is_deterministic_across_two_runs` + node-sync code locus.
- **CN-CINPUT-02** — `enforced`; `open_obligation` now CLOSED (consume-side fence + producer consumption landed via CN-CINPUT-03 / DC-CINPUT-02b).
- **DC-CINPUT-02a** — `enforced`; note only (projection proved at A4; consumption now bound by DC-CINPUT-02b).
- **T-REC-01, T-REC-02** — `enforced`; the production restart path is now wired; cross-cluster test `node_sync_kill_then_warm_start_recovers_same_tip` added; own cells otherwise unchanged.
- **DC-WAL-03** — `enforced`; cross-cluster note (the `--mode node` owner is the first production `replay_from_anchor` caller); own cells otherwise unchanged.

**Re-statused (not a simple strengthening):** **CN-STORE-02** — `declared` → **`partial`**. First real enforcement of the recovered-artifact anchor-lineage clause only; the WAL-entry + checkpoint binding clauses remain ungated (stated as a gap in the row and in Honest enforcement gaps). **CN-PROD-03 — UNCHANGED** (no N-F-C strengthening; deliberately not touched).

Status breakdown moves from 170/20/113 to **174 enforced / 20 partial / 112 declared / 0 deprecated** (the 3 new rules are all enforced → +3 enforced; DC-CINPUT-01 flips partial → enforced (+1 enforced, −1 partial) and CN-STORE-02 flips declared → partial (−1 declared, +1 partial), so partial nets to 20, enforced is 170+3+1=174, declared is 113−1=112). Three new CI gates on disk raise the `ci_check_*.sh` count to **108** (matching CODEMAP). **CODEMAP** maps every new code locus inside the already-mapped `ade_node` crate; **HEAD_DELTAS** is one cluster behind (baseline `a3a0636`) and does not yet catalogue the 3 new gates — reconcile on the next HEAD_DELTAS regeneration.

**Drift check across the N-F-C delta:** every registry-named test for the 3 new rules + 9 strengthenings resolves to an on-disk `fn`, and all 3 new `ci_script`s resolve to on-disk gates — **no test drift and no CI drift across this delta**. The 18 `ba02_*` test names match the RO-LIVE-06 registry list exactly. The `_(… — gap)_` cells and the **Honest enforcement gaps** section are the actionable output — reconcile them in the registry or the codebase; do not delete them. Regenerate after every cluster close.

**Boundary (preserved verbatim):** PHASE4-N-F-C proves the Ade node lifecycle mechanics through evidence closure. It does not claim live BA-02. RO-LIVE-01 remains partial/operator-gated. RO-LIVE-06 is only schema/correlation mechanics.
