...
>
> **HEAD:** `80dac1f7` (working tree, PHASE4-N-F-G-A cluster close) · **Generated:** 2026-06-01 · **Prior pass:** `4eb7610` (PHASE4-N-F-F close)
...
The canonical source is the **invariant registry** at `docs/ade-invariant-registry.toml` (`.idd-config.json` `invariant_registry`), **313 rules** at this working tree. This doc is a join: registry entries × codebase introspection. Rule IDs, families, names, sources, and the registry-recorded `tests` / `ci_script` / `ci_scripts` / `code_locus` fields come from the registry; each test name and CI script is cross-checked for existence in the working tree at `80dac1f7`, and divergences are flagged inline as **drift**. (Verification this pass is grep/ls only — no build or test execution.)
...
- **[not found on disk — drift]** (tests) / **[not on disk — drift]** (CI) flags a registry-named test/CI that could not be located in the working tree at `80dac1f7` — a registry↔code coherence gap to reconcile. This pass: **0** such drifts (every cited test leaf-fn and every cited CI script resolves on disk).
...
| **CI** | `ci/ci_check_loop_planner_closed.sh`, `ci/ci_check_node_run_loop_containment.sh` |
...
- `ci_check_node_forge_single_epoch_fail_closed.sh` (CE-G-A-4) — **rule-bound to `DC-EPOCH-03`** (the only G-A gate carried in a registry `ci_script` field). Off-epoch forge fails closed before leadership / KES signing; the forge path drives no `NonceInput::EpochBoundary` / `CandidateFreeze` nonce promotion.
- `ci_check_node_forge_real_cli_ingress.sh` (CE-G-A-2) — **NOT bound to any registry `ci_script` field.** Enforces that the `--mode node` operator-forge ingress uses the REAL `parse_opcert_envelope` + `parse_shelley_genesis` (retiring the `parse_simple_*` stubs on the node path). The rules it conceptually backs (`CN-OPCERT-01`, `CN-GENESIS-01`) received `strengthened_in += "PHASE4-N-F-G-A"` tokens but **no `ci_script` addition** — see the gap list below.
- `ci_check_recovered_ledger_pparams_sourced.sh` (CE-G-A-2a) — **NOT bound to any registry `ci_script` field.** Enforces that the recovered ledger's `protocol_params` are sourced from the operator consensus-inputs bundle's oracle preimage, never `ProtocolParameters::default()`. The rule it conceptually backs (`DC-CINPUT-02b`) was strengthened but **no `ci_script` addition** — see the gap list.
- `ci_check_genesis_consistency_fixture_present.sh` (CE-G-A-1) — **NOT bound to any registry `ci_script` field.** Proves the S1 private-net Ade-as-leader reference fixture + pinning harness are committed, well-formed, and leak no secret key material. The rules it conceptually backs (`CN-GENESIS-01`, `DC-NODE-05`) were strengthened but **no `ci_script` addition** — see the gap list.

**CI gates on disk not bound to any rule CI row (6 of 116):** the four-listed includes **three of the four new G-A gates** (`ci_check_genesis_consistency_fixture_present.sh`, `ci_check_recovered_ledger_pparams_sourced.sh`, `ci_check_node_forge_real_cli_ingress.sh`) plus the three standing project-wide hygiene/meta gates that cannot be attributed to a single invariant (`ci_check_module_headers.sh`, `ci_check_no_secrets.sh`, `ci_check_pallas_quarantine.sh`). The hygiene trio is expected-unbound; the three G-A gates are a **traceability gap** (the build enforces them but no registry rule cites them) — surfaced, not fixed this pass. The remaining **110** on-disk gates are rule-bound.
...
- **Three new G-A CI gates enforce the build but are bound to no registry rule (traceability gap).** `ci_check_node_forge_real_cli_ingress.sh` (CE-G-A-2), `ci_check_recovered_ledger_pparams_sourced.sh` (CE-G-A-2a), and `ci_check_genesis_consistency_fixture_present.sh` (CE-G-A-1) all exist on disk and fail the build on violation, but none appears in any registry `ci_script` / `ci_scripts` field. The rules they conceptually back (`CN-OPCERT-01`, `CN-GENESIS-01`, `DC-CINPUT-02b`, `DC-NODE-05`) received `strengthened_in += "PHASE4-N-F-G-A"` tokens this close but **no corresponding `ci_script` addition**. Per the registry's append-only `ci_scripts` discipline, the right reconciliation is to append each gate to the `ci_script` field of the rule it enforces (e.g. `ci_check_node_forge_real_cli_ingress.sh` → `CN-OPCERT-01` + `CN-GENESIS-01`; `ci_check_recovered_ledger_pparams_sourced.sh` → `DC-CINPUT-02b`; `ci_check_genesis_consistency_fixture_present.sh` → `CN-GENESIS-01` + `DC-NODE-05`). Until then, the enforcement is real but **untraceable from the registry**.
...
INCREMENTAL refresh for the **PHASE4-N-F-G-A cluster-close** at HEAD `80dac1f7` (working tree, close-pass registry edits applied). Primary source: the working-tree invariant registry (`docs/ade-invariant-registry.toml`, **313 rules**). The prior on-disk pass was the PHASE4-N-F-F close (HEAD `4eb7610`, 311 rules). This pass re-joins every rule to its Source / Code / Tests / CI from the registry and recomputes the mechanical counts. Every registry-named test (resolved to its leaf `fn`) and CI script cited in any row was cross-checked against the working tree using **grep/ls only** — no `cargo build`/`cargo test`/`cargo check` was run.

N-F-G-A delta (forge fidelity on the `--mode node` spine; **NO BLUE crate changed** — 456 canonical types unchanged):

- **NEW rule `DC-EPOCH-03`** (`tier = derived`, `status = enforced`, `introduced_in = PHASE4-N-F-G-A`): single-epoch forge fail-closed on the `--mode node` spine. Code cites `crates/ade_node/src/node_sync.rs` (`forge_epoch_admission` + closed `ForgeEpochAdmission`), `crates/ade_node/src/node_lifecycle.rs`, `crates/ade_node/src/run_loop_planner.rs`, `crates/ade_core/src/consensus/nonce.rs`. All on disk; all **6** registry-named tests resolve on disk (in `node_sync.rs`); its **1** registry-named CI gate (`ci_check_node_forge_single_epoch_fail_closed.sh`) exists on disk. Inserted into the DC family after `DC-EPOCH-02`, preserving ID order.
- **NEW rule `DC-NODE-06`** (`tier = derived`, `status = declared`, `introduced_in = PHASE4-N-F-G-B`): self-accept → serve handoff (shape B) — a **forward sketch for the next sub-cluster, NOT enforced by G-A.** Empty `tests` / `ci_script`; rendered with `declared` status and gap Tests/CI cells. Inserted into the DC family after `DC-NODE-05`.
- **`strengthened_in += "PHASE4-N-F-G-A"` on seven rules:** `CN-OPCERT-01` + `CN-GENESIS-01` (S2 — real cardano-cli text-envelope ingress on the node path), `DC-LEDGER-10` (credential fidelity carried through the recovered current ledger view), `CN-NODE-01` + `DC-CINPUT-02b` (S2a — oracle-current `ProtocolParameters` installed into the recovered ledger, not `::default()`), `DC-NODE-05` (S1/S4 — genesis-consistency-proven + epoch-hardened), `DC-NODE-03` (S3 — clock seam over the real genesis anchor via checked `checked_millis_to_slot`). Each carries a strengthening note; their registry-faithful Code/Tests/CI rows are otherwise unchanged this pass.
- **No rule weakened or deprecated.** DC-EPOCH-03 + DC-NODE-06 are the only new entries.

Mechanical counts recomputed fresh from the registry at `80dac1f7` (`grep -c '^\[\[rules\]\]'` = **313**; no duplicate IDs):

- Status: **180 enforced** (179 → 180, the DC-EPOCH-03 flip) / **20 partial** / **113 declared** (112 → 113, DC-NODE-06).
- By tier: constraint 4 · constraint_network 2 · derived **189** (187 → 189, DC-EPOCH-03 + DC-NODE-06) · operational 16 · release 40 · true 62.
- By family: T 30 · DC **146** (144 → 146, DC-EPOCH-03 + DC-NODE-06) · CN 112 · RO 16 · OP 9.

**Drift check (this pass, all rows):** across all 313 rules, **0** of the 1591 registry-named test references and **0** of the registry-named CI scripts fail to resolve on disk at `80dac1f7`. Every `crates/**.rs` code locus and every `ci/**.sh` gate cited in any row was verified present on disk. The registry's enforcement pointers are clean.

**CODEMAP** (regenerated at `80dac1f7`, 116 CI checks) maps every G-A code locus inside the already-mapped `ade_node` / `ade_runtime` / `ade_testkit` crates; the on-disk `ci/ci_check_*.sh` count (116) matches CODEMAP and the four new G-A gates. **HEAD_DELTAS** is at the same HEAD (baseline `4eb7610`) and its §5 catalogues the four new gates; its registry-state note anticipates exactly the close-pass edits this pass reads. The `_(… — gap)_` cells, the three unbound-G-A-gate traceability gap, and the **Honest enforcement gaps** section are the actionable output — reconcile them in the registry or the codebase; do not delete them. Regenerate after every cluster close.
