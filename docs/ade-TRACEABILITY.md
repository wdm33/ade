...
>
> **HEAD:** `b0365df0` (PHASE4-N-AA close — bounded peer-driven serve range, pre-RO-LIVE hardening item 1) · **Generated:** 2026-06-06 · **Prior pass:** `999199f8` (post-N-U-close gate-hygiene span, 333 rules) · **Registry:** `docs/ade-invariant-registry.toml` @ 334 rules · **CODEMAP:** `docs/ade-CODEMAP.md` @ `5c9f6cf6` (458 canonical types · 136 CI · 334 rules)
...
The canonical source is the **invariant registry** at `docs/ade-invariant-registry.toml` (`.idd-config.json` `invariant_registry`), **334 rules** at HEAD `b0365df0` (202 enforced / 20 partial / 112 declared / 0 deprecated). **This pass is the PHASE4-N-AA close delta-refresh (`999199f8` → `b0365df0`):** `+1` NEW rule — `DC-SERVEMEM-01` (derived/DC, introduced `enforced`, S2 close) — bounded peer-driven serve range; `+2` `strengthened_in += "PHASE4-N-AA"` appends — `DC-NODE-13` (the serve-as-projection is now bounded by the S1 capped read primitives) and `DC-LIVEMEM-01` (its symmetric serve-side analog now exists); `0` rules removed or deprecated. CI: `+1` NEW gate `ci/ci_check_serve_range_bounded.sh` → 135 → **136** (aligned with CODEMAP at `5c9f6cf6`). The registry↔TRACEABILITY join is **334 ↔ 334** (1:1). This doc is a join: registry entries × codebase introspection. Rule IDs, families, names, sources, and the registry-recorded `tests` / `ci_script` / `code_locus` fields come from the registry; each cited test name and CI script is cross-checked for existence in the working tree at `b0365df0`, and divergences are flagged inline as **drift**. This is a **fast delta-refresh** — only the three PHASE4-N-AA rules (one new + two strengthened) were re-traced from scratch against disk; every other rule's four-cell trace row is preserved verbatim from the prior pass (HEAD `999199f8`). Verification this pass is grep / ls only — no build or test execution. **DC-SERVEMEM-01 disk-verification:** all **12** cited test leaf-fns resolve (`fn <name>` present) — the 6 S1 contract bodies in `crates/ade_runtime/src/chaindb/contract.rs` (run against BOTH `InMemoryChainDb` + `PersistentChainDb` via `run_contract_tests`), the 4 S2 serve bodies in `served_chain_projection.rs`, and the 2 ade_node integration tests in `node_spine_serve_loopback.rs`; the trait methods `range_bytes_capped` / `last_block_bytes` are declared in `chaindb/mod.rs` (NOT `contract.rs` as the registry `code_locus` states) with `CappedSlotRange` in `chaindb/types.rs`; the gate `ci/ci_check_serve_range_bounded.sh` is present. Two append-only registry reconciliations recorded (NOT enforcement gaps): the registry `tests` array lists 10 of the 12 on-disk tests (the two inverted-range MEDIUM-fix tests are not yet appended), and the `code_locus` cites `contract.rs` for the trait primitives (the trait lives in `mod.rs`).
...
By tier: constraint 4 · constraint_network 2 · derived 206 · operational 17 · release 41 · true 64.

By family: T 32 · DC 159 · CN 118 · RO 16 · OP 9.
...
_tier: derived · status: enforced_ · _introduced_in: PHASE4-N-AA_
...
_drift (registry↔disk): the registry `DC-SERVEMEM-01.tests` array lists 10; the working tree at `b0365df0` carries 12 — the two inverted-range tests (`range_bytes_capped_inverted_range_is_empty` (S1) + `serve_range_inverted_range_fails_closed` (S2), the close MEDIUM-fix additions) are on disk but not yet appended to the registry array. Also: the registry `code_locus` names `chaindb/contract.rs` for the trait primitives, but on disk the `range_bytes_capped` / `last_block_bytes` **trait methods** are declared in `chaindb/mod.rs` and `CappedSlotRange` in `chaindb/types.rs`; `contract.rs` holds the generic contract-test bodies. Both are append-only registry reconciliations (test array + code_locus precision), not enforcement gaps — every cited leaf resolves on disk._
...
- **`DC-SERVEMEM-01` (PHASE4-N-AA, `enforced`) — bounded peer-driven serve range carries NO four-cell gap.** Introduced `enforced` at the S2 close with Code + Tests + CI all on disk (Code: the `ChainDb` capped read primitives in `chaindb/{mod,types,persistent,in_memory}.rs` + the `ChainDbServedSource` cap + closed `ServeRangeOutcome` fail-closed in `served_chain_projection.rs`; Tests: **12 leaf-fns on disk**, the registry array names 10; CI: `ci_check_serve_range_bounded.sh`, non-vacuous). Two **append-only registry reconciliations** (NOT enforcement gaps, every cited leaf resolves on disk): append the two inverted-range MEDIUM-fix tests to the `tests` array, and correct the `code_locus` trait-primitive path (`chaindb/contract.rs` → `chaindb/mod.rs` + `chaindb/types.rs`). Strengthens `DC-NODE-13` + `DC-LIVEMEM-01` (`strengthened_in += "PHASE4-N-AA"`); flips no `RO-*` (a defensive read-path bound, not a live-acceptance step).
...
**Registry reconciliations (append-only; surfaced for the next registry edit — NOT enforcement gaps, every cited leaf resolves on disk):** (1) `DC-SERVEMEM-01.tests` lists 10 of the 12 on-disk tests — append `range_bytes_capped_inverted_range_is_empty` (S1) + `serve_range_inverted_range_fails_closed` (S2), the close MEDIUM-fix additions; (2) `DC-SERVEMEM-01.code_locus` cites `chaindb/contract.rs` for the trait primitives — the `range_bytes_capped` / `last_block_bytes` trait methods are declared in `chaindb/mod.rs` and `CappedSlotRange` in `chaindb/types.rs` (`contract.rs` holds the generic contract-test bodies). Both are the registry trailing the close MEDIUM-fix, not a code gap.
