...
>
> **HEAD:** `c6e7fafe` (PHASE4-N-AB close — outbound mux segmentation, pre-RO-LIVE hardening item 2) · **Generated:** 2026-06-06 · **Prior pass:** `b0365df0` (PHASE4-N-AA close, 334 rules) · **Registry:** `docs/ade-invariant-registry.toml` @ 335 rules · **CODEMAP:** `docs/ade-CODEMAP.md` @ `c6e7fafe` (458 canonical types · 137 CI · 335 rules)
...
The canonical source is the **invariant registry** at `docs/ade-invariant-registry.toml` (`.idd-config.json` `invariant_registry`), **335 rules** at HEAD `c6e7fafe` (203 enforced / 20 partial / 112 declared / 0 deprecated). **This pass is the PHASE4-N-AB close delta-refresh (`b0365df0` → `c6e7fafe`):** `+1` NEW rule — `CN-SESS-05` (derived/CN, `introduced_in = "PHASE4-N-AB"`, declared at scoping → `enforced` at the S1 close) — outbound mux segmentation, the outbound inverse of `CN-SESS-04` inbound reassembly; `+2` `strengthened_in += "PHASE4-N-AB"` appends — `CN-SESS-04` (the inbound reassembly now has its outbound inverse — receive+send symmetry) and `DC-SERVEMEM-01` (the bounded serve can now actually transmit a large in-range block, not just read it); `0` rules removed or deprecated. CI: `+1` NEW gate `ci/ci_check_outbound_segmentation.sh` → 136 → **137** (aligned with CODEMAP at `c6e7fafe`). The registry↔TRACEABILITY join is **335 ↔ 335** (1:1). This doc is a join: registry entries × codebase introspection. Rule IDs, families, names, sources, and the registry-recorded `tests` / `ci_script` / `code_locus` fields come from the registry; each cited test name and CI script is cross-checked for existence in the working tree at `c6e7fafe`, and divergences are flagged inline as **drift**. This is a **fast delta-refresh** — only the three PHASE4-N-AB rules (one new + two strengthened) were re-traced from scratch against disk; every other rule's four-cell trace row is preserved verbatim from the prior pass (HEAD `b0365df0`). Verification this pass is grep / ls only — no build or test execution. **CN-SESS-05 disk-verification:** all **7** registry-cited test leaf-fns resolve (`fn <name>` present, exactly one each) in `crates/ade_network/src/session/core.rs::tests::` (`outbound_payload_at_max_payload_is_one_frame`, `outbound_payload_over_max_payload_segments_into_two`, `outbound_segment_order_preserved`, `outbound_segments_keep_same_mini_protocol_id_and_mode`, `outbound_large_payload_reassembles_byte_identical_via_inbound`, `outbound_payload_at_upper_bound_is_allowed`, `outbound_payload_over_upper_bound_fails_closed`); the Code loci resolve — `handle_outbound` (owns segmentation, with the `MAX_OUTBOUND_PAYLOAD_BYTES` = 16 MiB fail-closed guard) + `encode_inner_frame` (the single-frame encoder authority, `MAX_PAYLOAD` = 65535 per-frame guard) in `session/core.rs`, `MAX_PAYLOAD` declared in `mux/frame.rs`; the gate `ci/ci_check_outbound_segmentation.sh` is present (executable). **0 drifts; 0 registry reconciliations** — the registry `tests`/`ci_script`/`code_locus` for `CN-SESS-05` are all accurate on disk.
...
By tier: constraint 4 · constraint_network 2 · derived 207 · operational 17 · release 41 · true 64.

By family: T 32 · DC 159 · CN 119 · RO 16 · OP 9.
