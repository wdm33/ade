...
>
> **HEAD:** `6363683e` (PHASE4-N-AE.F — receive idempotency at the durable-admit chokepoint: a peer-re-announced block already durably present byte-identically (same slot, same hash in `ChainDb`) is an idempotent no-op in `pump_block`, closing the post-CE-A5 echo that ended a continuous run with `SlotBeforeLastApplied` AFTER `AddedToCurrentChain`; new rule `DC-NODE-16`) · **Generated:** 2026-06-07 · **Prior pass:** `a76672b9` (PHASE4-N-AE close, 340 rules) · **Registry:** `docs/ade-invariant-registry.toml` @ 341 rules · **CODEMAP:** `docs/ade-CODEMAP.md` @ `a76672b9` (458 canonical types · 141 CI · 340 rules — regenerated to `6363683e` in the AE.F fix commit) · **HEAD_DELTAS:** `6363683e` (§5 records CI 141 → **142**, the new gate `ci_check_receive_idempotency.sh` bound to `DC-NODE-16`; §7 records rules 340 → **341**) — all three grounding docs now AGREE at 142 CI / 341 rules
