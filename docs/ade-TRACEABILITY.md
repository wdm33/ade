...
>
> **HEAD:** `5858288e` (PHASE4-N-AH — local selected durable chain forge-base authority + cert evidence-only + single-producer warm-start re-entry: in a declared rung-1 single-producer venue, after Ade self-admits a valid forged block through `pump_block` onto its LOCAL durable ChainDB spine, the next forge base is Ade's LOCAL SELECTED DURABLE TIP (`ChainDb::tip`) — NOT `followed_peer_tip` and NOT an operator adoption certificate; the cert is evidence-only and fully REMOVED from `ade_node`; warm-start re-enters the extend state on the recovered own-spine tip without a fresh catch-up — `DC-NODE-20` / `DC-NODE-21` / `DC-NODE-22` all ENFORCED; the deferred PHASE4-N-AF / N-AG CODEMAP debt is paid) · **Generated:** 2026-06-08 (re-run: prior-pass `DC-NODE-20`/`21` registry test-name drift RECONCILED → 0 drift) · **Prior pass:** `f87d0056` (PHASE4-N-AF close, 343 rules) · **Registry:** `docs/ade-invariant-registry.toml` @ **347 rules** (213 enforced / 20 partial / 114 declared / 0 deprecated) · **CODEMAP:** `docs/ade-CODEMAP.md` @ `5858288e` (458 canonical types · **148 CI** · **347 rules** — REGENERATED to this HEAD; counts MATCH this pass) · **HEAD_DELTAS:** `f87d0056` (§5 still @ 143 CI; §7 still @ 343 rules — **NOT yet regenerated to `5858288e`**; the three N-AH gates `ci_check_local_durable_forge_base.sh` / `ci_check_cert_evidence_only.sh` / `ci_check_warm_start_re_entry.sh` + `ci_check_single_producer_loop_continuation.sh` + `ci_check_live_transcript_forge_base.sh` are on disk but not yet in HEAD_DELTAS §5 — surfaced below as a cross-doc lag to reconcile at the next HEAD_DELTAS refresh, not a TRACEABILITY drift)
...
The canonical source is the **invariant registry** at `docs/ade-invariant-registry.toml` (`.idd-config.json` `invariant_registry`), **347 rules** at HEAD `5858288e` (213 enforced / 20 partial / 114 declared / 0 deprecated). **This pass is the PHASE4-N-AH cluster-close refresh; it folds the whole `f87d0056` → `5858288e` span — the PHASE4-N-AF close (already traced last pass), the PHASE4-N-AG cluster (`DC-NODE-19`, declared/partial), and the PHASE4-N-AH cluster (`DC-NODE-20` / `DC-NODE-21` / `DC-NODE-22`, enforced) — and re-traces the rules that changed.** `+4` NEW rules since the prior pass — `DC-NODE-19` (derived/DC, `introduced_in = "TBD"`, **`declared`**; single-producer forge-loop continuation after follow-link EOF — `tests = []`, `ci_script = ""`; its live CE was re-homed to PHASE4-N-AH CE-AH-6 on the `DC-NODE-20` path, so it remains declared, not overclaimed), `DC-NODE-20` (derived/DC, `introduced_in = "PHASE4-N-AH"`, **`enforced`**; local selected durable chain forge-base authority), `DC-NODE-21` (derived/DC, **`enforced`**; adoption certificate is rung-1 evidence-only, never forge authority — the cert parser is fully DELETED from `ade_node`), and `DC-NODE-22` (derived/DC, **`enforced`**; single-producer warm-start re-entry derives forge mode from the recovered local durable spine). `9` strengthenings (`strengthened_in += "PHASE4-N-AH"`): `DC-NODE-05`, `DC-NODE-12`, `DC-NODE-15`, `DC-NODE-18`, `DC-NODE-19`, `T-REC-03`, `T-REC-05`, `CN-NODE-02`, `CN-NODE-04`. `0` rules removed / deprecated / weakened. **`DC-CONS-03` is EXPLICITLY untouched** (rung-2 fork-choice successor — a competing candidate fails closed, never resolved). CI: `+6` NEW gates since the prior on-disk CODEMAP base (142 → **148** on disk): `ci_check_single_producer_extend_own_spine.sh` (N-AF, `DC-NODE-18`), `ci_check_single_producer_loop_continuation.sh` (N-AG, `DC-NODE-19`), `ci_check_local_durable_forge_base.sh` (N-AH, `DC-NODE-20`), `ci_check_cert_evidence_only.sh` (N-AH, `DC-NODE-21`), `ci_check_warm_start_re_entry.sh` (N-AH, `DC-NODE-22`), `ci_check_live_transcript_forge_base.sh` (N-AH S4a, `CN-NODE-04` / `DC-NODE-20` evidence); `ci_check_forge_followed_tip_admission.sh` was **phase-split / repaired in place** for the `DC-NODE-20` call chain (`run_relay_loop_with_sched → dc_node_15_refusal → forge_followed_tip_admission` — the `DC-NODE-15` INITIAL catch-up half; the POST-self-admit forge base asserted as the local `ChainDb::tip`), and `ci_check_node_path_fidelity.sh` was edited in place (the S2 cert-removal `28 → 29` reconcile). The registry↔TRACEABILITY join is **347 ↔ 347** (1:1). The four NEW rows plus the nine strengthened rows were re-traced from scratch against disk at `5858288e` (`fn` / `ls` / grep); every other rule's four-cell trace row is preserved verbatim from the prior pass (HEAD `f87d0056`). Verification this pass is `git grep` / `ls` only — no build or test execution.

**PHASE4-N-AH disk-verification (re-run — registry drift RECONCILED).** A prior pass flagged one registry test-name drift on `DC-NODE-20` + `DC-NODE-21` (both named `post_self_admit_forges_on_local_tip_durable_ne_followed_no_cert`, which had no `fn` on disk). **That drift is now RECONCILED in the registry:** both rules name `forge_base_selected_transcript_witnesses_local_tip` instead — a real `#[tokio::test]` at `crates/ade_node/src/node_sync.rs:5811` that witnesses `forge_base_source = local_chaindb_tip` + `cert_path_present = false`. The stale name has **0** occurrences anywhere in the registry. All `7` distinct registry-cited test leaf-fns across the four NEW rows now resolve as `fn <name>` in `crates/ade_node/src/node_sync.rs::tests` (one each): `caughtup_self_admit_enters_extend_directly_no_cert`, `forge_base_selected_transcript_witnesses_local_tip`, `local_spine_sustains_two_successors_no_cert`, `local_spine_two_runs_byte_identical`, `local_spine_cert_file_absent_from_replay_surface`, `warm_start_reentry_requires_tip_above_recovered_anchor`, `warm_start_single_producer_re_enters_extend_and_forges` — **0 test-name drift on the N-AH rows.** The `DC-NODE-20`/`22` `code_locus` resolves (`single_producer_forge_decision` ×1, `warm_start_forge_mode` ×1, `forge_mode_after_admit`, `enum ForgeMode`, `SingleProducerExtendOwnDurableSpine`, `VenueRole`, `SingleProducerFenceReason`, `ForgeRefused::SingleProducerFenceViolation`, `dc_node_15_refusal`, `forge_followed_tip_admission`, `replayed_anchor_block_no`, `ForgeBaseSelected`, `plan_loop_step` all present). The `DC-NODE-21` cert-removal claim is verified on disk: `read_adoption_cert` and `--adoption-cert-path` are GONE from `ade_node` (0 hits each); the only residual `VenueAdoptionCertificate` token is a single comment at `node_sync.rs:765` recording the removal; `parse_hex32` survives only as an unrelated C1-rehearsal test helper, not the cert parser. All `6` NEW CI gates are present + executable on disk (`ci_check_local_durable_forge_base.sh` 110 lines, `ci_check_cert_evidence_only.sh` 47, `ci_check_warm_start_re_entry.sh` 96, `ci_check_single_producer_loop_continuation.sh` 157, `ci_check_single_producer_extend_own_spine.sh` 177, `ci_check_live_transcript_forge_base.sh` 92; `ci_check_forge_followed_tip_admission.sh` 179 after the phase-split). The CE-AH-6 live evidence files exist at `docs/evidence/phase4-n-ah-ce-ah-6-close.{md,jsonl}` (run-4, full 8+3 bar; **C2-LOCAL cardano-testnet magic 42, NOT preprod**). **0 drifts** on the nine strengthened rows; **0 drift** on the four NEW rows (reconciled this re-run). The prior PHASE4-N-AF close pass narrative is preserved below verbatim.
...
- **[not found on disk — drift]** (tests) / **[not on disk — drift]** (CI) flags a registry-named test/CI that could not be located in the working tree at `5858288e` — a registry↔code coherence gap to reconcile. **This re-run: ZERO such drift** — the prior-pass drift (the `DC-NODE-20` + `DC-NODE-21` registry `tests` arrays naming `post_self_admit_forges_on_local_tip_durable_ne_followed_no_cert`) is RECONCILED: both rows now name `forge_base_selected_transcript_witnesses_local_tip` (a real `#[tokio::test]` at `node_sync.rs:5811`); the stale name has 0 occurrences in the registry; all 7 distinct N-AH test fns + all 6 NEW CI gates resolve on disk. Rows outside the re-traced PHASE4-N-AF/AG/AH set were not re-verified this delta-refresh — their drift status is carried from the prior pass.
- **Honest status notes (NOT gaps):** `DC-NODE-19` is `declared`/partial by design (its live CE was re-homed to the `DC-NODE-20` path; the loop-continuation slice landed the gate `ci_check_single_producer_loop_continuation.sh` but the registry keeps it `declared` pending the full slice contract — not overclaimed); `DC-NODE-17` is `declared` (safety/observation-only, no gate — OQ-1 disproved it as the stall fix). These are intentional, recorded, and distinct from the four-cell gaps.
...
By tier: constraint 4 · constraint_network 2 · derived 219 · operational 17 · release 41 · true 64.

By family: T 32 · DC 171 · CN 119 · RO 16 · OP 9.

_PHASE4-N-AH close delta (`f87d0056` → `5858288e`): `+4` rules — `DC-NODE-19` (derived/DC, `declared`/partial; single-producer forge-loop continuation after follow-link EOF, gate landed in N-AG but kept `declared` pending the full slice contract), `DC-NODE-20` + `DC-NODE-21` + `DC-NODE-22` (all derived/DC, all introduced `enforced` at the N-AH close). `+9` `strengthened_in += "PHASE4-N-AH"` appends (`DC-NODE-05`, `DC-NODE-12`, `DC-NODE-15`, `DC-NODE-18`, `DC-NODE-19`, `T-REC-03`, `T-REC-05`, `CN-NODE-02`, `CN-NODE-04`). CI: `+6` NEW gates since the prior on-disk CODEMAP base (142 → 148): `ci_check_single_producer_extend_own_spine.sh` (N-AF), `ci_check_single_producer_loop_continuation.sh` (N-AG), `ci_check_local_durable_forge_base.sh`, `ci_check_cert_evidence_only.sh`, `ci_check_warm_start_re_entry.sh`, `ci_check_live_transcript_forge_base.sh` (N-AH); `ci_check_forge_followed_tip_admission.sh` phase-split + `ci_check_node_path_fidelity.sh` reconcile in place. Registry↔TRACEABILITY join: 347 ↔ 347. `0` rules removed / deprecated / weakened; `DC-CONS-03` EXPLICITLY untouched (rung-2 fork-choice); NO `RO-*` flipped (`RO-LIVE-01` stays operator-gated — the run-4 C2-LOCAL live transcript where a real Haskell relay adopted Ade's cert-free local-tip-forged blocks sustained > k across a follow-link EOF + a restart is the rung-1 MECHANISM, not operator-witnessed bounty acceptance on preprod)._
...
_tier: true · status: enforced_ · _introduced_in: PHASE4-N-F-D · strengthened_in: PHASE4-N-F-E, PHASE4-N-AH_
...
_tier: true · status: enforced_ · _introduced_in: PHASE4-N-U · strengthened_in: PHASE4-N-AD, PHASE4-N-AE, PHASE4-N-AH_
...
_tier: derived · status: enforced_ · _introduced_in: PHASE4-N-F-E · strengthened_in: PHASE4-N-F-F, PHASE4-N-F-G-A, PHASE4-N-AE, PHASE4-N-AH_
...
_tier: derived · status: enforced_ · _introduced_in: PHASE4-N-U · strengthened_in: PHASE4-N-AH_
...
### PHASE4-N-AH pass (`5858288e`) — current-pass coherence
...
The four-cell contract is filled for every `enforced` rule re-traced this pass (the three NEW enforced rules `DC-NODE-20` / `DC-NODE-21` / `DC-NODE-22`: Code + Tests + CI all present and verified on disk via grep/ls — all six NEW CI gates resolve, and ALL registry-cited test fns across the four NEW rows resolve, one `fn` each in `node_sync.rs::tests`, the prior-pass drift reconciled). The standing gaps and honest-status notes below are recorded rather than hidden.
