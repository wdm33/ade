...
>
> **HEAD:** `6f848825` (working tree, PHASE4-N-F-G-E close — registry close-pass edits applied) · **Generated:** 2026-06-02 · **Prior pass:** `351d46bc` (PHASE4-N-F-G-C close)
...
The canonical source is the **invariant registry** at `docs/ade-invariant-registry.toml` (`.idd-config.json` `invariant_registry`), **314 rules** at this working tree (`+1` this close — the NEW `DC-LIVEMEM-01`, added + flipped to `enforced` in one slice). This doc is a join: registry entries × codebase introspection. Rule IDs, families, names, sources, and the registry-recorded `tests` / `ci_script` / `ci_scripts` / `code_locus` fields come from the registry; each test name and CI script is cross-checked for existence in the working tree at `6f848825`, and divergences are flagged inline as **drift**. (Verification this pass is grep/ls only — no build or test execution; the one new rule-bound G-E gate `ci_check_live_feed_memory_bounds.sh` was additionally executed green via `bash`.)
...
- **[not found on disk — drift]** (tests) / **[not on disk — drift]** (CI) flags a registry-named test/CI that could not be located in the working tree at `6f848825` — a registry↔code coherence gap to reconcile. This pass: **0** such drifts (every cited test leaf-fn and every cited CI script resolves on disk; the 4 `DC-LIVEMEM-01` tests + the 1 new CI gate all resolve).
...
By tier: constraint 4 · constraint_network 2 · derived 190 · operational 16 · release 40 · true 62.

By family: T 30 · DC 147 · CN 112 · RO 16 · OP 9.
...
### CODEMAP (`docs/ade-CODEMAP.md` @ `6f848825`, PHASE4-N-F-G-E close)

CODEMAP was regenerated for the PHASE4-N-F-G-E close at **`6f848825`** (working tree, alongside this doc). It records **119** `ci_check_*.sh` gates, **456** canonical types, **2234** tests, **314** rules — the CI count matches the on-disk count at this working tree (`ls ci/ci_check_*.sh | wc -l` = 119). Every crate named in a **Code** row is present in CODEMAP: `ade_codec`, `ade_core`, `ade_core_interop`, `ade_crypto`, `ade_ledger`, `ade_network`, `ade_node`, `ade_plutus`, `ade_runtime`, `ade_testkit`, `ade_types`. No crate-level mismatch. Every `crates/**.rs` path cited in any `code_locus` resolves on disk (the `ci_check_registry_code_locus_exists.sh` invariant) — including the THREE NEW `DC-LIVEMEM-01` loci `crates/ade_network/src/session/core.rs`, `crates/ade_node/src/node_sync.rs`, `crates/ade_runtime/src/network/mux_pump.rs` (all pre-existing files; the cap consts + the additive enum variant are edits in place, not new modules).

- **No BLUE crate changed this cluster.** PHASE4-N-F-G-E is a single-slice operational-hardening cluster: it adds two closed memory caps to existing modules, each fail-closed **before** the BLUE `ade_codec` decode path. CODEMAP confirms the changed loci — **GREEN** `ade_network::session::core` gains `MAX_REASSEMBLY_TAIL_BYTES = 16 MiB` + the additive closed-enum variant `SessionError::ReassemblyBufferOverflow` (its sole exhaustive consumer `ade_runtime::network::mux_pump::session_err_to_halt` maps it → `PeerHaltReason::ChainSyncDecodeError`); **RED** `ade_node::node_sync` gains `MAX_WIRE_PUMP_LOOKAHEAD = 256` (back-pressuring the existing bounded `mpsc`, cap 64); **RED** `ade_runtime::network::mux_pump` maps the new variant to drop the peer. `session/` is **NOT** one of the 9 BLUE `ade_network` submodule `core_paths`, so the new `SessionError` variant is **GREEN-by-content and NOT canonical-counted** — the canonical-type total holds at **456** (CODEMAP confirms every BLUE crate's struct/enum count is byte-unchanged: `ade_ledger` 177, `ade_core` 49, `ade_codec` 11, `ade_types` 81, `ade_crypto` 21, `ade_plutus` 8, `ade_network` BLUE submodules 109).
- **CI count is aligned:** CODEMAP's 119 matches the on-disk `ci/ci_check_*.sh` count (Δ +1 vs the N-F-G-C close's 118: the one new G-E gate `ci_check_live_feed_memory_bounds.sh`). The two containment fences `ci_check_node_run_loop_containment.sh` + `ci_check_served_chain_handoff_fence.sh` are **byte-unchanged** between `90791691` (G-C slice-span HEAD) and `6f848825` (verified: `git diff --stat 90791691 6f848825 --` over both paths is empty).

### HEAD_DELTAS (`docs/ade-HEAD_DELTAS.md` §5)

HEAD_DELTAS is **two clusters behind this pass** — it is still at the **G-B** close (HEAD `339cccb1`, baseline `80dac1f7`) and renders `DC-NODE-06` as `declared` / **117** CI gates. It does **not yet narrate** either the G-C span (`339cccb1 → 351d46bc`) or the G-E span (`351d46bc → 6f848825`), and does not catalogue the two new gates (`ci_check_ba02_evidence_manifest_schema.sh` from G-C, `ci_check_live_feed_memory_bounds.sh` from G-E). **This is a recorded coherence note, not soften'd:** the registry, CODEMAP, and *this* TRACEABILITY all agree at `6f848825` (119 gates, 314 rules, `DC-LIVEMEM-01` enforced); HEAD_DELTAS lags at `339cccb1` and is the one of the four docs to refresh next (`/head-deltas 339cccb1`).

The G-E gate and its rule-binding status (this will appear in HEAD_DELTAS §5 once it is refreshed):

- `ci_check_live_feed_memory_bounds.sh` (CE-G-E, S1) — **NEW gate, rule-bound to `DC-LIVEMEM-01`** (appears on that rule's **CI** row above; resolves on disk; exit 0). It strips line comments first (so the doc-comments naming "CLI / env / config" do not self-trip), then verifies both bounds are **CLOSED LITERAL constants** (`MAX_REASSEMBLY_TAIL_BYTES = 16 * 1024 * 1024` in `session::core`, `MAX_WIRE_PUMP_LOOKAHEAD = 256` in `node_sync`) AND that neither is wired to a CLI / env / config source — the no-escape-hatch guard. Executed this pass: `OK (live-feed memory bounds): MAX_REASSEMBLY_TAIL_BYTES (16 MiB) + MAX_WIRE_PUMP_LOOKAHEAD (256) are closed literal constants, not wired to CLI/env/config (DC-LIVEMEM-01).` (exit 0).
- `ci_check_node_run_loop_containment.sh` + `ci_check_served_chain_handoff_fence.sh` (pre-existing serve/containment fences, co-bound to `DC-NODE-06` / `CN-NODE-02` / `DC-SYNC-02` / `DC-NODE-05`) — **byte-identical** between `90791691` and `6f848825` (verified `git diff --stat` empty). G-E adds a bound *in front of* the live feed; it does not touch the serve or relay-loop containment surfaces.

**CI gates on disk not bound to any rule CI row (3 of 119):** unchanged from the prior pass — the project-wide hygiene/meta gates `ci_check_module_headers.sh`, `ci_check_no_secrets.sh`, `ci_check_pallas_quarantine.sh` (expected-unbound; no single invariant to attribute them to; verified 0 registry hits each). The one new G-E gate `ci_check_live_feed_memory_bounds.sh` **is rule-bound** (`DC-LIVEMEM-01`; 2 registry hits) — so it is **NOT a new unbound gate**; the unbound set stays **3**, not 4. The other **116** on-disk gates are rule-bound.

### SEAMS

Closed registries / closed grammars referenced by rule sources (mini-protocol ID registry, closed message taxonomies, closed log/event vocabularies, `LeaderCheckVerdict`, `ExpectedVrfInput`, `Mode`, `IngressSource`, the PHASE4-N-Y closed sets, the PHASE4-N-Z `MithrilBootstrapError`, the PHASE4-N-F-C BA-02 evidence surface, the N-F-D loop-planner lifecycle vocabulary, the N-F-E forge-tick `Due | NotDue` planner input, the N-F-F two-variant `ForgeIntent { On(ForgePaths), Off }`, the N-F-G-A closed `SlotAlignmentError` + `ForgeEpochAdmission`, the N-F-G-B constructor-fenced `SelfAcceptedHandoff`) are catalogued in `docs/ade-SEAMS.md` §3. PHASE4-N-F-G-E adds **one additive closed-enum variant and no new plugin seam**: `SessionError::ReassemblyBufferOverflow { protocol, len, cap }` extends the closed `SessionError` enum (no wildcard; one exhaustive consumer `mux_pump::session_err_to_halt`). It introduces **no new error *vocabulary* type, no new `NodeBlockSource` variant, no new `CoordinatorEvent` variant**; the two new bounds are closed literal constants with no runtime escape hatch. G-E **closes** SEAMS §7 candidate #6 (the G-C-review-flagged unbounded peer-driven memory surfaces).

---

## Honest enforcement gaps (standing)

The four-cell contract is filled for every `enforced` rule (Code + Tests + CI all present and verified in the working tree via grep/ls, with the one new rule-bound G-E gate `ci_check_live_feed_memory_bounds.sh` additionally executed green this pass). The standing gaps below are recorded rather than hidden.

- **Honest scope of this close (load-bearing — do NOT soften / do NOT broaden).** PHASE4-N-F-G-E bounds **peer-driven memory on the live `--mode node` feed BEFORE authoritative decode/apply** — and nothing more. The claim is **NARROW**: it is **NOT** full network DoS resistance, **NOT** peer resource fairness, **NOT** BA-02 / live-evidence readiness. **Precision (close-review — record, do not overclaim):** the reassembly cap is checked *post-extend*, so a single buffer's transient peak is `cap + one ≤64 KiB mux frame` (~16.06 MiB), not an absolute 16 MiB; `ProtoBuffers` holds up to ~10 INDEPENDENT per-protocol buffers, each capped separately, so the per-connection aggregate ceiling is ~10× the single-buffer cap — still O(constant) per connection (the invariant "peer-driven memory is bounded before decode/apply" holds). **Per-connection-COUNT limits / peer resource fairness are a SEPARATE, out-of-scope surface** (a future hardening slice). There is **NO BLUE change**; the serve/forge/containment fences (`ci_check_node_run_loop_containment.sh` + `ci_check_served_chain_handoff_fence.sh`) are **byte-unchanged**; **NO live-evidence / BA-02 / RO-LIVE flip or claim.**

- **`DC-LIVEMEM-01` (PHASE4-N-F-G-E, `enforced`) — live-feed bounded memory, operational-hardening.** Four-cell contract filled and verified (Code: the GREEN `session::core` reassembly-tail cap + additive `SessionError::ReassemblyBufferOverflow`, the RED `node_sync` WirePump lookahead cap, the RED `mux_pump` overflow→drop-peer mapping; Tests: 4, all on disk — `session_reassembly_tail_over_cap_fails_closed`, `session_reassembly_tail_under_cap_still_drains_complete_item`, `wirepump_lookahead_stops_at_cap`, `wirepump_lookahead_cap_preserves_relay_behavior_under_normal_feed`; CI: `ci_check_live_feed_memory_bounds.sh`, exit 0). `tier = derived` — explicitly **NOT BLUE consensus law** (the rule text says so); it bounds shell/glue memory, never a chain fact. No gap on this rule. Its `cross_ref` (`CN-SESS-04`, `DC-SESS-06`, `DC-SYNC-01`, `DC-SYNC-02`, `DC-NODE-06`, `CN-NODE-02`) is carried unchanged — the cap is an additive bound on the reassembly contract; no existing rule weakened or re-joined.

- **`declared` rules (112) are open enforcement gaps by design.** Each carries `_(no enforcing code located — gap)_` / `_(no tests named in registry — gap)_` / `_(no CI script — gap)_` cells (or a subset). These are named-but-not-yet-mechanically-enforced invariants; the gap is the actionable backlog, not a defect to paper over. (Unchanged at 112 — no status flip this close beyond the single NEW `DC-LIVEMEM-01`, which was added directly at `enforced`.)
...
- **`CN-OPERATOR-EVIDENCE-01` (PHASE4-N-S-C, `enforced`; strengthened N-F-G-C) — operator-pass manifest schema; standing Tests gap (carried).** The rule's `tests` array is `[]` in the registry; it is **gate-enforced** (the two manifest-schema gates fire only when a `CE-*-LIVE_*.toml` is committed, and **0** are committed at HEAD, so both are vacuously satisfied). Carried unchanged as a standing Tests gap.

- **`RO-LIVE-01` (PHASE4-N-G, `partial`; strengthened N-Q + N-F-G-C) — producer-side live leg, MECHANICAL half wired (carried).** Four-cell contract has Code + Tests + CI, but **status stays `partial`**. The LIVE half is operator-gated: peer ACCEPT stays `blocked_until_operator_stake_available`, proven only by the operator-captured peer log through `ba02_evidence::correlate`. **G-E does not touch `RO-LIVE-01`** — bounding live-feed memory is upstream of, and orthogonal to, the operator-witnessed accept. Carried unchanged.

- **`RO-LIVE-06` (PHASE4-N-F-C, `enforced`; strengthened N-F-G-C) — BA-02 evidence, schema + correlation mechanics ONLY (carried).** What is enforced: the closed versioned `Ba02Manifest`, the pure/total/deterministic HASH-PRIMARY `correlate` as the SOLE constructor, the allow-list parser, and the operator-pass file I/O whose write path accepts only a `correlate`-produced `Ba02Manifest`. **What is NOT claimed:** that BA-02 was achieved live. **G-E does not touch `RO-LIVE-06`.** Carried unchanged.

- **`DC-NODE-06` (PHASE4-N-F-G-B, `enforced`; strengthened N-F-G-C) — self-accept → serve handoff, mechanism-only (carried).** What is enforced: only a BLUE self-accepted artifact (typed `SelfAcceptedHandoff`, sole provenance `from_self_accepted`) reaches the single `ServedChainHandle::push_atomic`, fed only by `into_accepted()`; the relay-loop body performs no serve/admit/`push_atomic`/durable-tip mutation (containment byte-unchanged). **G-E leaves both fences byte-unchanged** and adds `DC-LIVEMEM-01` to its own `cross_ref` (the live feed that DC-NODE-06's serve mechanism does not gate is the surface G-E bounds upstream) — recorded as a `cross_ref` token on `DC-LIVEMEM-01`, **not** a four-cell change to `DC-NODE-06`. Carried unchanged.

- **Operator-gated release legs unchanged this close (none flipped):** `RO-LIVE-01` (producer-side live leg against a real peer — MECHANICAL half wired by G-C, LIVE half operator-gated), `RO-LIVE-06` (BA-02 evidence — schema/correlation mechanics only; peer ACCEPT proven only by the peer's validation log, never by Ade's self-accept / `ForgeSucceeded` / served-block / any wire-success signal), `RO-SYNC-EVIDENCE-01` (vacuous-until-committed manifest), `CE-Y-16` (two-Haskell-node witnessed live capture). PHASE4-N-F-G-E does **not** flip any of these — it adds a memory bound upstream of authoritative decode, with no live-evidence claim.

---

## Generation notes

INCREMENTAL refresh — **PHASE4-N-F-G-E close** at HEAD `6f848825` (working tree). The rule set is **314** (`+1` vs the prior pass): the single NEW `DC-LIVEMEM-01` (`tier = derived`, operational-hardening NOT BLUE consensus law, `introduced_in = "PHASE4-N-F-G-E"`), added directly at `status = enforced`. This pass joins the new row to its Source / Code / Tests / CI from the working-tree registry (the close-pass edits already applied) and re-runs the mechanical counts + the on-disk existence checks (grep/ls), plus a green execution of the one rule-bound G-E gate. The prior pass was the PHASE4-N-F-G-C close (HEAD `351d46bc`, 313 rules, inventory 181 / 20 / 112).
...
- **`DC-LIVEMEM-01` — NEW, added at `enforced`.** `introduced_in = "PHASE4-N-F-G-E"`, `strengthened_in = []`, `tier = derived`. **Source:** `docs/planning/phase4-n-f-g-e-invariants.md`. **Code** (`code_locus`): `crates/ade_network/src/session/core.rs` (GREEN `MAX_REASSEMBLY_TAIL_BYTES` cap + `SessionError::ReassemblyBufferOverflow`), `crates/ade_node/src/node_sync.rs` (RED `MAX_WIRE_PUMP_LOOKAHEAD` cap), `crates/ade_runtime/src/network/mux_pump.rs` (overflow variant → `PeerHaltReason::ChainSyncDecodeError`). **Tests:** the 4 named below. **CI:** `ci/ci_check_live_feed_memory_bounds.sh`. **cross_ref:** `CN-SESS-04`, `DC-SESS-06`, `DC-SYNC-01`, `DC-SYNC-02`, `DC-NODE-06`, `CN-NODE-02` (all carried unchanged — the cap is an additive bound on the reassembly contract; no existing rule was edited).

No other rule was added, removed, weakened, re-joined, or deprecated. No BLUE crate / canonical type changed (**456** holds — `session/` is not a BLUE `ade_network` submodule `core_path`, so the additive `SessionError` variant is GREEN-by-content and not canonical-counted). One CI script added on disk (118 → 119: `ci_check_live_feed_memory_bounds.sh`), rule-bound on landing to `DC-LIVEMEM-01`; the two containment fences (`ci_check_node_run_loop_containment.sh`, `ci_check_served_chain_handoff_fence.sh`) are byte-unchanged.

Mechanical counts recomputed fresh from the registry at `6f848825` (`grep -cE '^\[\[rules\]\]'` = **314**; `grep -c '^id = '` = **314**; no duplicate IDs):

- Status: **182 enforced** / **20 partial** / **112 declared** / 0 deprecated. (**+1 enforced** vs the prior pass — the single NEW `DC-LIVEMEM-01`. Cross-checked against the rendered registry `status =` lines: 182 / 20 / 112.)
- By tier: constraint 4 · constraint_network 2 · derived **190** · operational 16 · release 40 · true 62. (`derived` +1 — `DC-LIVEMEM-01` is `tier = derived`.)
- By family: T 30 · DC **147** · CN 112 · RO 16 · OP 9. (`DC` +1 — `DC-LIVEMEM-01`.)

**Drift check (this pass): 0 drifts.** All four `DC-LIVEMEM-01` tests resolve on disk: `session_reassembly_tail_over_cap_fails_closed` (`crates/ade_network/src/session/core.rs:464`), `session_reassembly_tail_under_cap_still_drains_complete_item` (`crates/ade_network/src/session/core.rs:502`), `wirepump_lookahead_stops_at_cap` (`crates/ade_node/src/node_sync.rs:2133`), `wirepump_lookahead_cap_preserves_relay_behavior_under_normal_feed` (`crates/ade_node/src/node_sync.rs:2162`). All three code loci resolve on disk (`MAX_REASSEMBLY_TAIL_BYTES` + `ReassemblyBufferOverflow` in `session/core.rs`; `MAX_WIRE_PUMP_LOOKAHEAD` in `node_sync.rs`; `SessionError::ReassemblyBufferOverflow { .. } => …` in `mux_pump.rs:302`). The source doc `docs/planning/phase4-n-f-g-e-invariants.md` resolves. The CI gate `ci/ci_check_live_feed_memory_bounds.sh` resolves and executed green (exit 0, the no-escape-hatch guard). Across the full 314 rules the prior pass recorded 0 unresolved references; this close adds only resolved references, so the **0-drift** property holds.

**CODEMAP** (regenerated this close at `6f848825`, **119** CI checks, **456** canonical types, **2234** tests, **314** rules) is coherent with this pass: it records 119 gates (matching on-disk `ls ci/ci_check_*.sh | wc -l` = 119), no BLUE change (456 canonical types unchanged; the additive `SessionError` variant is GREEN-by-content), and the `DC-LIVEMEM-01` loci (the GREEN `session::core` reassembly cap + additive variant, the RED `node_sync` lookahead cap, the RED `mux_pump` overflow→drop mapping). **HEAD_DELTAS is two clusters behind** — it is still at the **G-B** close (baseline `80dac1f7`, HEAD `339cccb1`, **117** CI, `DC-NODE-06` rendered `declared`) and narrates neither the `339cccb1 → 351d46bc` G-C span nor the `351d46bc → 6f848825` G-E span. This is recorded as a coherence note (not soften'd): registry + CODEMAP + this TRACEABILITY agree at `6f848825`; HEAD_DELTAS lags and is the doc to refresh next (`/head-deltas 339cccb1`). The remaining `_(… — gap)_` cells (the 112 `declared` rules; the standing `CN-PROD-04` CI gap; the `CN-OPERATOR-EVIDENCE-01` standing Tests gap; the project-wide hygiene/meta unbound gates — 3 of 119) and the **Honest enforcement gaps** section are the actionable output; reconcile them in the registry or the codebase, do not delete them. Regenerate after every cluster close.
