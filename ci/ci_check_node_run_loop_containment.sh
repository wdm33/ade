...
#
# The `--mode node` relay loop (`node_lifecycle::run_relay_loop_with_sched`) composes the
...
if ! echo "$LOOP_FN" | grep -qE 'run_node_sync\('; then
    print_fail "run_relay_loop_with_sched must call run_node_sync( — the sole block-consumption / tip-advance path (DC-SYNC-02)"
fi
...
for tok in 'pump_block\(' '\.put_block\(' 'AdvanceTip' 'rollback_to_slot\('; do
    if echo "$LOOP_FN" | grep -qE "$tok"; then
        print_fail "run_relay_loop_with_sched reaches a tip-mutation token directly: $tok — the tip advances ONLY through run_node_sync (CN-NODE-02 / DC-SYNC-02)"
    fi
done
...
if (( FAILED == 0 )); then
    echo "OK (node run-loop containment): run_relay_loop_with_sched drives sync only via run_node_sync, forges via exactly one fenced forge_one_from_recovered (self-accept-only, no tip/serve/admit), no direct run_real_forge/evidence, no verdict/follower, no second bootstrap (CN-NODE-02 / DC-SYNC-02 / CE-E-4)"
fi
exit $FAILED
