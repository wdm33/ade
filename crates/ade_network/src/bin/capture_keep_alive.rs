...
// RED — Imperative Shell capture binary for N2N KeepAlive (protocol 8).
//
// Flow:
//   1. Handshake (protocol 0).
//   2. Send MsgKeepAlive with a chosen cookie. Read MsgResponseKeepAlive.
//   3. Repeat for the requested number of rounds (default 3) with
//      distinct cookies to prove the server echoes the *sent* cookie.
//   4. Send MsgDone for polite close.
//
// Captured corpus: each sent and received frame saved as a separate
// <scenario>_msg_NN_<dir>_<kind>.cbor file plus a meta TOML.
...
async fn main() -> io::Result<()> {
    ...
    let out_dir = arg_value(&args, "--out")
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("corpus/network/n2n/keep_alive"));
    let scenario =
        arg_value(&args, "--scenario").unwrap_or_else(|| "local_preprod_pings".into());
    ...
    eprintln!("[ka] connecting to {peer} (magic={network_magic})");
    ...
    eprintln!("[ka] connected in {:.2}s", start.elapsed().as_secs_f64());
    ...
    stream
        ...
        .await?;
    eprintln!("[ka] sent ProposeVersions");
    ...
    eprintln!("[ka] handshake accepted at v{negotiated_version}");

    // ---- KEEP-ALIVE rounds ----
    let mut msg_idx = 0;
    for round in 0..rounds {
        let cookie = KeepAliveCookie(0x4A00 ^ (round as u16));
        let send_bytes = encode_keep_alive_message(&KeepAliveMessage::KeepAlive(cookie));
        let path = out_dir.join(format!("{scenario}_msg_{msg_idx:02}_send_keep_alive.cbor"));
        fs::write(&path, &send_bytes)?;
        eprintln!(
            "[ka] round {round}: sent KeepAlive cookie=0x{:04x} ({} bytes -> {})",
            cookie.0,
            send_bytes.len(),
            path.display()
        );
        stream
            .write_all(&wrap_in_mux_frame(send_bytes, KEEP_ALIVE_PROTOCOL_ID, MuxMode::Initiator))
            .await?;
        msg_idx += 1;

        let recv_bytes =
            read_for_protocol(&mut stream, &mut buffers, KEEP_ALIVE_PROTOCOL_ID, |buf| {
            ...
            .await?;
        let reply = decode_keep_alive_message(&recv_bytes)
            .map_err(|e| io::Error::other(format!("keep-alive decode: {e:?}")))?;
        let path = out_dir.join(format!("{scenario}_msg_{msg_idx:02}_recv_response.cbor"));
        fs::write(&path, &recv_bytes)?;
        eprintln!(
            "[ka] round {round}: recv {reply:?} ({} bytes -> {})",
            recv_bytes.len(),
            path.display()
        );
        match reply {
            KeepAliveMessage::ResponseKeepAlive(echoed) => {
                if echoed.0 != cookie.0 {
                    return Err(io::Error::other(format!(
                        "cookie mismatch: sent 0x{:04x}, got 0x{:04x}",
                        cookie.0, echoed.0
                    )));
                }
            }
            other => {
                return Err(io::Error::other(format!(
                    "expected ResponseKeepAlive, got {other:?}"
                )))
            }
        }
        msg_idx += 1;
    ...
    stream
        .write_all(&wrap_in_mux_frame(
            encode_keep_alive_message(&KeepAliveMessage::Done),
            KEEP_ALIVE_PROTOCOL_ID,
            MuxMode::Initiator,
        ))
        .await?;
    eprintln!("[ka] sent Done");

    let meta_path = out_dir.join(format!("{scenario}_meta.toml"));
    let meta = format!(
        r#"# Captured keep-alive corpus (S-A9 real-capture portion for CE-N-A-5).
peer = "{peer}"
...
"#,
        ...
        rounds = rounds,
        utc = chrono_like_utc(),
        protocol_id = KEEP_ALIVE_PROTOCOL_ID,
    );
