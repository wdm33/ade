...
pub struct BundleProfile {
    ...
    /// LIVE-FORGE-HARDENING S2 (DC-EPOCH-16): the network security parameter k, exported into the
    /// consensus-inputs JSON so the durable sidecar carries the candidate-freeze-window authority.
...
pub fn build_bundle(
    ...
    let raw = RawConsensusInputs {
        ...
        security_param: Some(profile.security_param),
        epoch_nonce_hex,
        pool_distribution,
        pool_vrf_keyhashes,
