            ...
            Ok((new_state, _)) => {
                if new_state.epoch_state.epoch.0 != prev_epoch {
                    let summary = EpochBoundarySummary {
                        ...
                        delegation_count: new_state.cert_state.as_authoritative().expect("authoritative cert state in test").delegation.delegations.len(),
                        pool_count: new_state.cert_state.as_authoritative().expect("authoritative cert state in test").pool.pools.len(),
                        retiring_count: new_state.cert_state.as_authoritative().expect("authoritative cert state in test").pool.retiring.len(),
                        retired_count: 0,
                        mark_delegation_count: new_state.epoch_state.snapshots.as_authoritative().unwrap().mark.0.delegations.len(),
                        set_delegation_count: new_state.epoch_state.snapshots.as_authoritative().unwrap().set.0.delegations.len(),
                        go_delegation_count: new_state.epoch_state.snapshots.as_authoritative().unwrap().go.0.delegations.len(),
